...
SELECT is((SELECT trial_days FROM public.platform_subscription_offers WHERE offer_code='paige-solo-beta-monthly-v1'),0,'offer has no trial');
...
SELECT ok(NOT has_function_privilege('authenticated','public.solo_beta_fulfill_checkout(text,uuid,integer,uuid,text,text,text,text,text,boolean,integer,text,text,integer,text,timestamptz,timestamptz,boolean)','EXECUTE'),'browser cannot fulfill an entitlement');
SELECT ok(has_function_privilege('service_role','public.solo_beta_fulfill_checkout(text,uuid,integer,uuid,text,text,text,text,text,boolean,integer,text,text,integer,text,timestamptz,timestamptz,boolean)','EXECUTE'),'service role retains atomic fulfillment');
SELECT throws_ok(
