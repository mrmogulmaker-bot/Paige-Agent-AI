-- Consent is per workflow, so revocation is per workflow.
--
-- THE DEFECT, IN TWO PLACES
--
-- `n8n_discovery_pin` is ONE hash over the ENTIRE workflow inventory, including each row's
-- updatedAt. Two separate things compared it, and both were wrong to:
--
--   1. `probe` -- which runs on EVERY verify, discover and preview, i.e. every time the
--      Integrations page loads -- wiped `n8n_approved_workflow_ids` WHOLESALE whenever that
--      hash moved. Editing a workflow the owner never approved, or adding an unrelated new
--      one, silently revoked every approval they had. And `probe` does not update the stored
--      pin, so it kept wiping on every subsequent load until they re-approved.
--
--   2. The RUN-TIME gate in tenant-n8n-oauth compared the same whole-inventory hash before
--      letting Paige preview an approved workflow. So any change anywhere made her refuse
--      EVERY approved workflow as "not approved" -- before anything had even been cleared.
--
-- Revoking when THE APPROVED THING changed is correct. Revoking everything when ANYTHING
-- changed is the bug.
--
-- THE RULE
--
-- Each approval carries its own workflow's mark: which workflow it is, and the version of it
-- the person was looking at when they approved it. Editing workflow C revokes nothing.
-- Editing workflow A revokes A alone, and the safety property is unchanged -- a workflow
-- rewritten under an approval still has to be approved again.
--
-- WHAT DELIBERATELY DOES NOT CHANGE
--
-- The inventory pin stays where it is CORRECT: the `approve` step. Approving is a decision
-- made against a listing the person was shown, so if that listing shifted between showing
-- and saving, refusing (N8N_DISCOVERY_CHANGED) is right -- they would otherwise be approving
-- something other than what they looked at.
--
-- WHY THE WHOLE FUNCTION IS REPRODUCED
--
-- CREATE OR REPLACE takes an entire body, so this is 20261202000100's definition with the
-- acquire, probe and approve branches changed. Diffing the two files shows exactly that.

-- workflow id -> the mark that was true when it was approved. ADDITIVE: the ids column is
-- untouched (§58), so an approval made before this migration simply has no mark yet.
ALTER TABLE public.tenant_mcp_connections
  ADD COLUMN IF NOT EXISTS n8n_approved_workflow_marks jsonb NOT NULL DEFAULT '{}'::jsonb;

COMMENT ON COLUMN public.tenant_mcp_connections.n8n_approved_workflow_marks IS
  'Per-approved-workflow identity mark (id -> hash of id + the updatedAt the approver saw). '
  'Revocation is judged per workflow against this, never against one hash of the whole inventory.';
...
BEGIN
 ...
 ELSIF _operation='acquire' THEN
  ...
  RETURN jsonb_build_object('oauth_scopes',c.oauth_scopes,'lease',lease,'generation',c.n8n_generation,'approved_ids',c.n8n_approved_workflow_ids,'discovery_pin',c.n8n_discovery_pin,
   'approved_marks',COALESCE(c.n8n_approved_workflow_marks,'{}'::jsonb),
 ...
 ELSIF _operation IN ('check','rotate','probe','snapshot','read_snapshot','approve','release') THEN
  ...
  ELSIF _operation='probe' THEN
   ...
   UPDATE public.tenant_mcp_connections SET status=CASE WHEN _input->>'state'='connected' THEN 'connected' ELSE 'error' END,
    ...
    -- Keep every approval whose OWN workflow still looks exactly as it did when it was
    -- approved. Previously this compared ONE hash of the whole inventory, so editing any
    -- workflow -- including one the owner never approved, or a brand new unrelated one --
    -- silently revoked every approval on the next page load.
    --
  ...
  ELSIF _operation IN ('read_snapshot','approve') THEN
   ...
   -- The inventory pin STAYS here on purpose: approving is a decision made against a
   -- listing the person was shown, so if that listing moved between showing and saving,
   -- refusing is right (checked above as N8N_DISCOVERY_CHANGED). What is added is each
   -- approved workflow's OWN mark, which is what revocation is judged against later.
   UPDATE public.tenant_mcp_connections SET n8n_approved_workflow_ids=ARRAY(SELECT DISTINCT jsonb_array_elements_text(_input->'workflow_ids')),
    n8n_approved_workflow_marks=COALESCE((SELECT jsonb_object_agg(w.id, _input->'marks'->>w.id)
      FROM (SELECT DISTINCT jsonb_array_elements_text(_input->'workflow_ids') AS id) w
     WHERE _input->'marks'->>w.id IS NOT NULL),'{}'::jsonb),
