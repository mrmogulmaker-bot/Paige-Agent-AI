-- Roadmap #2 remediation (verifier + compliance findings).
--
-- The consumer-invite flow shipped with the token carrying no binding to the
-- specific contact/email an admin invited. That produced three defects:
--   • BLOCKING — accept_tenant_invite re-pointed an existing client row's
--     tenant_id, silently moving a customer (and their child data) from one
--     tenant to another across the §9 seam.
--   • BLOCKING — accept linked purely on the accepting user's email, so it could
--     attach to the wrong contact, or create a duplicate + orphan the invited one.
--   • MAJOR — send-portal-invite (public) accepted any recipient address, an open
--     tenant-branded email relay.
--
-- Fix: bind the token to a contact_id + email at mint (admin-gated), accept the
-- *bound* contact, and refuse any cross-tenant move. send-portal-invite (edge)
-- is updated separately to send only to the token's bound address.

-- 1. Bind columns. contact_id is the exact clients row the admin invited; email
--    is the address the invite may be sent to (locks the relay to one recipient).
...
-- 2. Mint with an optional contact/email binding. Replacing the signature, so
--    drop the old 5-arg overload first (else named-arg calls become ambiguous).
DROP FUNCTION IF EXISTS public.create_tenant_invite_token(uuid, text, tenant_role, integer, integer);
...
GRANT EXECUTE ON FUNCTION public.create_tenant_invite_token(uuid, text, tenant_role, integer, integer, uuid, text) TO authenticated;

-- 3. Accept: prefer the bound contact, and NEVER move a client row across tenants.
