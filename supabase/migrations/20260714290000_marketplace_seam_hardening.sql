...
--
-- The adversarial-security + compliance crew (verified) surfaced real defects in
-- the install seam. This migration closes them before ship:
--
-- 1. §9 platform/tenant seam (HIGH): a tenant could self-provision a vendor and
--    INSERT a marketplace_items row badged origin='first_party', scope='public',
--    default_for_new_tenants=true, or tamper install_count/rating. Add a WITH
--    CHECK that constrains non-owner writes, plus a trigger freezing operator-only
--    columns against non-privileged callers.
-- 2. §2 finance-default guard (HIGH/MED, verified bypass): the guard scanned only
--    cosmetic display text. A generically-named item carrying a funding skill_flag
--    or lender/tradeline KB could still be set default_for_new_tenants. Make the
--    block STRUCTURAL (an is_finance flag) AND scan the effective version manifest,
--    and broaden the vocabulary.
-- 3. §13 concurrency/honesty (HIGH, verified): double-submit / retry of a kb_pack
--    install orphaned KB docs and mis-reported the seeded set. Serialize the
--    finalize with a per-(tenant,item) advisory lock and reconcile stray
--    marketplace-tagged docs to the winning seed set; fix the install_count
--    double-count; surface a loud warning if a kb_pack is installed via the RPC
--    without embedding (the edge function is the correct path).
-- 4. §17 audit immutability: make marketplace_install_ledger append-only via a
--    trigger, not merely by absence of a write policy.
-- 5. §2 over-narrowing copy: broaden the seeded catalog copy off "coach"-only.
--
-- Deferred to Phase 2 (logged): enforcing the payload_class='code' approval rail
-- in a publish RPC (no non-owner version-write path is enabled yet — default-deny
-- holds), and promoting item.category to an enum.
-- ============================================================================

BEGIN;

-- ── 1. Structural finance flag ──────────────────────────────────────────────
...
COMMENT ON COLUMN public.marketplace_items.is_finance IS
  'Structural §2 marker: a finance/credit item. Such an item may be LISTED as an opt-in but can NEVER be default_for_new_tenants, regardless of its display copy.';

-- The one finance item we ship is funding — mark it so the guard is authoritative.
UPDATE public.marketplace_items SET is_finance = true WHERE slug = 'funding';

-- ── 2. Guard: structural flag + effective-manifest scan + broader vocab ──────
  ...
  IF NEW.default_for_new_tenants THEN
    ...
    IF NEW.is_finance THEN
      RAISE EXCEPTION 'A finance/credit marketplace item cannot be a default for new tenants (section 2). It may be listed as an opt-in item only.'
        USING ERRCODE = '22023';
    END IF;
    -- Secondary net: scan the EFFECTIVE payload — display copy PLUS the current
    -- published version manifest (skill slugs, kb_pack doc titles/content/tags) —
    -- so a generically-named item that actually ships finance content is caught.
...
-- ── 3. §9 seam: constrain non-owner writes to marketplace_items ──────────────
    ...
    OR (
      EXISTS (SELECT 1 FROM public.marketplace_vendors v
               WHERE v.id = marketplace_items.vendor_id
                 AND v.owner_tenant_id IS NOT NULL
                 AND public.is_tenant_admin(v.owner_tenant_id))
      -- A tenant vendor can never badge as first-party, publish platform-wide,
      -- or seed itself as a default. Those require platform review (Phase 2).
      AND origin = 'vendor'
...
-- Freeze operator-only columns against non-privileged callers. The install RPCs
-- run SECURITY DEFINER as the migration owner (current_user = a privileged role),
-- so their install_count updates pass; a direct authenticated UPDATE cannot forge
-- install_count/rating/origin/default/scope (defense in depth beyond WITH CHECK).
...
BEGIN
  IF current_user IN ('postgres','supabase_admin','service_role') OR public.is_platform_owner() THEN
    RETURN NEW;  -- privileged path (the definer RPCs, service role, operator)
  END IF;
...
-- ── 5. §2 over-narrowing copy on the seeded catalog ─────────────────────────
...
-- ── 6. install RPC: advisory lock + orphan reconcile + count fix + honesty ───
...
BEGIN
  ...
  -- Serialize the whole install of one (tenant,item) so concurrent / retried
  -- invocations can't double-seed KB or double-count (§13).
  ...
  -- Honesty: a kb_pack installed via this RPC without embedded doc IDs seeds no
  -- knowledge (only the marketplace-install edge function can embed). Don't lie —
  -- flag it loudly so Paige/UI route through the edge function (§10/§13).
  ...
  -- Reconcile: under the advisory lock, remove any stray marketplace-tagged docs
  -- for this item that aren't in the winning seed set (cleans up a crashed/raced
  -- prior run so we never leave orphaned, untracked KB — §13).
  ...
  -- Count only a genuinely-new activation: no prior row, or the prior row was
  -- uninstalled (a 'disabled' row was already counted and never decremented).
