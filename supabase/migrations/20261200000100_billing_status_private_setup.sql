-- CLI scaffold: 20260903224642_billing_status_private_setup.sql. Ordered after the existing
-- future-dated migration head 20261200000000 for deterministic sequential replay.
-- Forward-only P0 privacy repair. No provider, contact, plan or entitlement writes.
BEGIN;
...
DECLARE
  ...
  _mapped_cust text; _mapped_pm text;
...
BEGIN
  ...
  SELECT a.stripe_customer_id, a.payment_method_id
    INTO _mapped_cust, _mapped_pm
...
COMMENT ON FUNCTION public.get_workspace_billing_status() IS
  'Owner-only Solo billing status. Connected boolean only; provider and card details remain server-confined. Promotional access and contact selection are independent of setup.';
COMMIT;
