-- Add Paige's n8n automation tools to the autonomy catalog so the operator can
-- set each to Ask-first / Autopilot / Off in Settings → Paige Autonomy, exactly
-- like every other mutating tool. Read-only n8n tools (list/executions) are not
-- gated and don't appear here. CREATE OR REPLACE re-declares list_tool_autonomy
-- with an added "Automations" category — the resolve/set functions are unchanged.
