-- Keep the booking-preset lifecycle mutations visible in the autonomy catalogue (E5).
-- This re-declares the COMPLETE latest catalogue (from 20270127000100) and adds only the
-- seven governed booking-preset mutations, so an operator can see them and set per-tool
-- autonomy. It exposes nothing new and makes nothing autonomous: action-risk.ts classifies
-- publish/revise/archive as HIGH (never bypassable by a stored mode) and create/pause/
-- duplicate/restore as ordinary; the default mode stays 'confirm'. `booking_preset_list` is
-- a read and is deliberately NOT catalogued (only mutating tools carry an autonomy mode).
