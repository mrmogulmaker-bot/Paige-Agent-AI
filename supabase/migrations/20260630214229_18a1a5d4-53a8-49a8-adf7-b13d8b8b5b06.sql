...
-- Allow linked clients to advance their own onboarding via a SECURITY DEFINER
-- RPC. Direct UPDATE on public.clients is blocked for linked clients because
-- there's no permissive UPDATE policy for them, so the "Begin onboarding"
-- button was silently no-op'ing. This RPC validates the caller owns the row,
-- enforces forward-only stage transitions, writes the change, and emits an
-- audit row so admins can see the progression in real time.
...
BEGIN
  ...
  -- Forward-only progression; ignore no-op or backward attempts silently.
  IF v_to_idx > v_from_idx THEN
    ...
    INSERT INTO public.paige_audit_log (actor_user_id, action, entity_type, entity_id, metadata)
    VALUES (
      v_user_id,
