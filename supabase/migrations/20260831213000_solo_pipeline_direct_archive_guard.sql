-- Include platform operators in caller-preserving deal visibility and enforce
-- occupied-stage archive protection for every table update path.

create or replace function public.prevent_occupied_stage_archive()
returns trigger language plpgsql security definer set search_path=public as $$
begin
  if old.archived_at is null and new.archived_at is not null
    and exists(select 1 from public.deals d where d.stage_id=old.id)
  then
    raise exception 'PIPELINE_STAGE_OCCUPIED: move its work before archiving' using errcode='23503';
  end if;
  return new;
end$$;
revoke all on function public.prevent_occupied_stage_archive() from public,anon,authenticated;
grant execute on function public.prevent_occupied_stage_archive() to service_role;

drop trigger if exists trg_prevent_occupied_stage_archive on public.pipeline_stages;
create trigger trg_prevent_occupied_stage_archive
before update of archived_at on public.pipeline_stages
for each row execute function public.prevent_occupied_stage_archive();
