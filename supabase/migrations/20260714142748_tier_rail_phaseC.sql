...
DECLARE _t text;
BEGIN
  IF _actor IS NULL THEN RETURN public.account_tier(_origin); END IF;
  _t := public.get_actor_access(_actor) ->> 'tier';
  IF _t IS NULL OR _t = 'none' THEN RETURN public.account_tier(_origin); END IF;
  RETURN _t;
END;
$function$;
