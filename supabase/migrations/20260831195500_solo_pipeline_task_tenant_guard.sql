-- Pipeline tenant isolation follow-up: order validation after tenant stamping,
-- bind task/deal references, and keep privileged next-action reads tenant-safe.
...
create or replace function public.enforce_task_deal_tenant_link()
returns trigger language plpgsql security definer set search_path=public as $$
begin
  if new.deal_id is not null and (
    new.tenant_id is null or not exists(
      select 1 from public.deals d where d.id=new.deal_id and d.tenant_id=new.tenant_id
    )
  ) then
    raise exception 'TASK_DEAL_TENANT_MISMATCH' using errcode='23514';
  end if;
  return new;
end$$;

revoke all on function public.enforce_task_deal_tenant_link() from public,anon,authenticated;
grant execute on function public.enforce_task_deal_tenant_link() to service_role;

drop trigger if exists trg_validate_task_deal_tenant_link on public.tasks;
create trigger trg_validate_task_deal_tenant_link
before insert or update of tenant_id,deal_id on public.tasks
for each row execute function public.enforce_task_deal_tenant_link();
