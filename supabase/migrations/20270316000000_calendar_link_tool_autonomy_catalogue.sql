...
-- contact by email/SMS). It exposes nothing new and makes nothing autonomous: action-risk.ts
-- classifies calendar_link_send as HIGH (never bypassable by a stored mode) and the default mode
-- stays 'confirm'. `calendar_link_prepare` and `calendar_link_social_copy` are READS and are
-- deliberately NOT catalogued (only mutating tools carry an autonomy mode).
