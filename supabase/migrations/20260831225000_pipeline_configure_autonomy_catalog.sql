-- Expose PAIGE's governed pipeline.configure capability to tenant autonomy controls.
...
begin
  if _caller is not null then
    ...
    if _tenant_id is not null and _tenant_id<>_tenant and not public.is_platform_owner() then raise exception 'AUTONOMY_FORBIDDEN: tenant mismatch' using errcode='42501'; end if;
    if public.is_platform_owner() and _tenant_id is not null then _tenant:=_tenant_id; end if;
  else
    _tenant:=_tenant_id;
  end if;
  return query
  WITH catalog(tool_key,label,category) AS (VALUES
  ...
  SELECT c.tool_key,c.label,c.category,coalesce(t.mode,'confirm'),t.mode is null,t.updated_at
  from catalog c left join public.tenant_tool_autonomy t on t.tool_key=c.tool_key and t.tenant_id=_tenant
