-- Carry the COMPLETE autonomy catalogue forward, plus `agreement_send`.
--
-- WHY A WHOLE-CATALOGUE MIGRATION FOR ONE ROW. `list_tool_autonomy` holds its catalogue as a
-- literal VALUES block inside the function body, so every addition restates all of it; the guard
-- reads only the LATEST migration declaring the function (scripts/ci/tool-catalogue-lint.mjs).
-- This body is 20270318000000's, unchanged, with one row appended. Nothing else moves.
--
-- WHY THE ROW IS REQUIRED, NOT OPTIONAL. `tool-catalogue-lint` holds at zero: its
-- `KNOWN_UNGOVERNED` list is empty by design, with its own note saying not to re-populate it to
-- get a tool past the guard. A tool classified in action-risk.ts but absent here is one the
-- operator is governed by and cannot see or switch off — which is the §67 complaint exactly, an
-- autonomy decision the person it belongs to cannot reach. Measured before writing this: with the
-- risk entry and no row, the guard reports `1 newly governed tool(s) the operator cannot see or
-- turn off: agreement_draft` and exits 1.
--
-- VISIBILITY ONLY, AND THE ONE CONTROL THAT MOST NEEDS TO BE VISIBLE. `agreement_send` is
-- classified `high`: it emails a real person a signable link, and it cannot be recalled. With no
-- stored row it reads `confirm` / `is_default = true`, and the risk class means the operator cannot
-- lift it to `auto` — `clampLaneByRisk` forces any stored `auto` back to `confirm` for a `high`
-- action. What this row buys is that the operator can SEE it, and can switch it OFF.
...
BEGIN
  ...
  WITH catalog(tool_key, label, category) AS (
    VALUES
      ...
      -- Agreements (INT-178, the first WRITE). Drafting only: it never sends, never mints a
      -- signing link, and produces nothing anyone outside the workspace can see. The outward-facing
      -- keys — send, resend, void — are deliberately absent until each earns its own slice.
      ('agreement_draft',                    'Draft an agreement', 'Sales'),
      ('agreement_send',                     'Send an agreement for signature', 'Sales')
  )
  SELECT
    c.tool_key,
    c.label,
