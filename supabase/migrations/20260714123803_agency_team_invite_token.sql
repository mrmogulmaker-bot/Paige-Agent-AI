-- Branded agency-team invite token (Task #213 follow-up — close the invite gate).
--
-- Before this, agency-invite-member (the shipped agency teammate flow) emailed a
-- Supabase RECOVERY link → /reset-password, the UNBRANDED platform page. That
-- breaks the §6/§9 promise: an agency teammate should land on a page wearing the
-- AGENCY's brand with inline account creation, exactly like the client-portal /join
-- and the sub-account-owner invite already do.
--
-- This adds a FOURTH first-class invite kind, `agency_team`, to the same
-- mint → /join → accept plumbing:
--
--   • The token is minted on the AGENCY tenant (tenant_id = the agency). Because
--     peek_tenant_invite already resolves the token-tenant's brand up the parent
--     chain via resolve_tenant_brand(), the /join card already wears the agency's
--     logo + color for this kind with NO change to peek — the agency IS the token
--     tenant, so its own brand resolves directly.
--   • The agency role (agency_admin / _manager / _biller / _specialist / _viewer)
--     rides on a NEW nullable `agency_role` column on the token row. Agency authority
--     is a DISTINCT taxonomy (agency_team_members), never tenant_role, so it cannot
--     be stuffed into default_role — hence its own column. default_role stays a
--     harmless 'member' placeholder for this kind.
--   • On accept: upsert the agency_team_members row (the shipped table) with that
--     agency_role, ACTIVE. Least-privilege (§13): agency_owner can NEVER be granted
--     via an invite, a NULL/invalid role is refused, and a bound-email token can
--     only be redeemed by that address.
--
-- The agency-invite-member edge function authorizes the caller from THEIR JWT
-- (agency_team_can_manage) and derives the agency id from the server, then mints
-- this token with the service role (direct INSERT — auth.uid() is null under the
-- service role, so it deliberately does NOT go through create_tenant_invite_token's
-- auth gate; authorization already happened one layer up). max_uses=1 + a TTL keep
-- the token single-use and expiring, matching every other invite kind.

BEGIN;

-- 1. Carry the agency role on the token. Nullable (only agency_team uses it), CHECK
--    admits the assignable agency roles only — NEVER agency_owner (ownership is not
--    an invitable role) and never junk. Guarded so re-runs are a no-op.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conrelid = 'public.tenant_invite_tokens'::regclass
      AND conname = 'tenant_invite_tokens_agency_role_chk'
  ) THEN
    ALTER TABLE public.tenant_invite_tokens
      ADD CONSTRAINT tenant_invite_tokens_agency_role_chk
      CHECK (agency_role IS NULL OR agency_role IN
        ('agency_admin','agency_manager','agency_biller','agency_specialist','agency_viewer'));
  END IF;
END $$;

COMMENT ON COLUMN public.tenant_invite_tokens.agency_role IS
  'Agency-tier role granted on accept for kind=''agency_team'' (agency_team_members taxonomy, §9). NULL for every other kind. Never agency_owner.';

-- 2. Widen the mint RPC to recognize the new kind. Signature unchanged, so this
--    CREATE OR REPLACE preserves the existing `authenticated` grant (§12: extend,
--    never rebuild). NOTE: this RPC does not set agency_role (no param), so the
--    canonical agency_team mint path is the agency-invite-member edge function's
--    direct insert; a bare RPC-minted agency_team token carries a NULL agency_role
--    and is refused at accept below (least-privilege), by design.
...
BEGIN
  ...
  -- subaccount_owner / agency_team invites never carry a contact.
...
-- 3. Accept: add the agency_team branch. Body is the 20260713060000 definition with
--    a new ELSIF for agency_team between subaccount_owner and the team/staff fallback.
...
BEGIN
  ...
  ELSIF _tok.kind = 'agency_team' THEN
    -- AGENCY TEAM path: grant an agency_team_members row on the AGENCY tenant
    -- (tenant_id = the agency). This is agency-operator staff (§9) — a DISTINCT
    -- taxonomy from tenant_members; no clients row, no tenant_role, contact ignored.
    --
    -- §13 least-privilege + anti-relay:
    --   • The role comes from the token's agency_role column, NOT default_role, and
    --     must be one of the assignable agency roles. agency_owner is never grantable
    --     via an invite; a NULL/invalid role is refused outright (a bare RPC-minted
    --     token with no agency_role fails here on purpose).
    --   • A bound-email token can only be redeemed by that exact address (forwarded
    --     links can't be hijacked into agency authority). Pairs with max_uses=1.
    SELECT email INTO _email FROM auth.users WHERE id = _uid;
    IF _tok.email IS NOT NULL AND lower(_tok.email) <> lower(COALESCE(_email, '')) THEN
      RAISE EXCEPTION 'This invite was sent to a different email address. Accept it while signed in as %', _tok.email;
    END IF;
    ...
    -- Clear any stale pending-by-email placeholder for this address (user_id NULL),
    -- then upsert the real (agency, user) row. A manual update-then-insert avoids the
    -- partial-unique-index ON CONFLICT target and PRESERVES any scoped_subaccounts the
    -- invite edge function already set (specialist scope is not carried on the token).
  ...
  ELSE
    -- TEAM / staff invite — idempotent membership upsert (unchanged). Grants exactly
    -- the invite's default_role; inline signup cannot escalate beyond it (§13).
    INSERT INTO public.tenant_members (tenant_id, user_id, role, status, joined_at)
