-- ============================================================================
-- INT-163 — the agreements engine joins the autonomy catalogue.
--
-- WHY THIS MIGRATION EXISTS AT ALL. `agreement_send`, `agreement_resend` and `agreement_void` are
-- classified HIGH in `_shared/action-risk.ts`, which means they always require the rendered approval
-- card. But a tool that is gated at runtime and absent from `list_tool_autonomy` is governed
-- INVISIBLY: the operator cannot see it in their Trust Compass and — the part that matters — cannot
-- turn it OFF. `lint:tool-catalogue` refuses that state, correctly, and this is the fix rather than a
-- baseline entry to get past the guard.
--
-- This changes VISIBILITY only. It adds no executable action and grants no autonomy: every row
-- defaults to 'confirm', and the action-class clamp means `auto` on a HIGH tool still resolves to
-- `confirm` wherever it is read.
--
-- The body below is 20270318000000's, reproduced with four rows added — copied programmatically
-- rather than retyped, because a hand-transcribed CREATE OR REPLACE is how an operator silently
-- loses controls they already had.
--
-- ROLLBACK (forward-only production procedure): re-apply the previous body from
-- 20270318000000_governed_crm_tool_autonomy_catalogue.sql in a forward migration.
-- ============================================================================
...
BEGIN
  ...
  WITH catalog(tool_key, label, category) AS (
    VALUES
      ...
      -- ── added by INT-163: the agreements engine. Three of the four are HIGH in action-risk.ts
      -- and can never be bypassed by a stored mode; they are listed here so the operator can SEE
      -- them and switch them OFF. `agreement_draft` is ordinary (a draft nobody has seen).
