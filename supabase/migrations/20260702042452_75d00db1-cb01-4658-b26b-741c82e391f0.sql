...
BEGIN
  ...
  INSERT INTO public.paige_admin_notifications (
    title, body, severity, source_workflow_key, scope, assigned_role
  ) VALUES (
...
BEGIN
  ...
  IF v_escalated > 0 THEN
    INSERT INTO public.paige_admin_notifications (title, body, severity, source_workflow_key, scope, assigned_role)
    VALUES (
      'Metering dead-letter: manual review required',
      v_escalated::text || ' metering event(s) exceeded 10 retry attempts and now require manual reconciliation. [doctrine §205]',
      'critical', 'billing_metering', 'platform', 'admin'
    );
  END IF;
