-- Campaign-brief reach (Slice 2): keep the two governed campaign-brief write tools visible in the
-- existing Trust Compass autonomy catalogue. This is NOT a second approval system. The function body
-- below preserves the complete latest catalogue from 20261226000000 verbatim and adds exactly two
-- Campaigns entries — `campaign_brief_create` and `campaign_brief_revise` — so an operator can SEE
-- them and set their lane (auto | confirm | off). They default to `confirm`, and because they are
-- classified `ordinary` (reversible in-tenant PLANNING records, action-risk.ts), an owner/admin may
-- grant them `auto` for standing-delegated authority; confirm stays the escalation lane. The read
-- tool `campaign_brief_list` is not a governed mutation and needs no catalogue row.
