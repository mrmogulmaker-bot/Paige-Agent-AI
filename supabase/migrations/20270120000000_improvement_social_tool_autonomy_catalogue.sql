-- Keep the three newly-classified write tools visible in the existing Trust Compass autonomy
-- catalogue (action-risk.ts repair, 2026-09-12). This is NOT a second approval system. The function
-- body below preserves the complete latest catalogue from 20261229000000 VERBATIM and adds exactly
-- three entries so an operator can SEE them and set their lane (auto | confirm | off):
--   · improvement_propose — the evaluation loop's PROPOSE leg. Classified `ordinary` (stages a
--     proposal; applies nothing), so an owner/admin may grant it `auto`; confirm stays the escalation.
--   · improvement_decide  — the evaluation loop's DECIDE leg, the governed sign-off. Classified
--     `high`: the clamp forces any stored `auto` back to `confirm` at execution (action-risk.ts), so
--     a lane set here can never let it run without the rendered approval card. It records the
--     decision only and never auto-applies.
--   · social_post — publishes to the workspace's public social accounts. Classified `high` for the
--     same reason (public, client-visible); the clamp keeps it card-gated whatever lane is set.
-- Without these rows the three tools are governed but INVISIBLE — permanently `confirm`, with no way
-- for the tenant to see or set them — which is exactly the gap `capabilities.v3.test.tsx` asserts
-- against ("every executable mutation names the entity it touches" / the gate ⊆ catalogue invariant).
-- The read tools (improvement_list, social_analytics, social_accounts) are not governed mutations and
-- need no catalogue row.
