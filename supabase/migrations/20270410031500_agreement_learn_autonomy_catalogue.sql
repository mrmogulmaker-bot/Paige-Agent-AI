-- INT-163 — list the agreement-learning control, so a workspace can switch it off.
--
-- `agreement_learn_on_complete` governs ONE derived, tenant-internal write: when an agreement
-- completes, a summary of it (counterparty, title, dates, commercial terms) is filed into the
-- tenant's own knowledge base so Paige can answer what a client agreed to. It carries no signer
-- evidence — no IP, no user agent, no signature image — and no legal body.
--
-- It has its own key precisely so it can be turned off without touching anything else.
--
-- VISIBILITY ONLY. This adds no executable action and grants no autonomy. Every previously listed
...
BEGIN
  ...
  WITH catalog(tool_key, label, category) AS (
    VALUES
      -- ── INT-163 ──
      ('agreement_learn_on_complete', 'Remember completed agreements', 'Agreements'),
