-- Official U.S. Census Bureau 2022 NAICS six-digit code and title reference.
-- Source: https://www.census.gov/naics/2022NAICS/6-digit_2022_Codes.xlsx
-- Imported verbatim except surrounding whitespace; no finance risk metadata is used.
begin;
create table public.naics_2022_official_reference (
  code text primary key check (code ~ '^[0-9]{6}$'),
  title text not null check (char_length(btrim(title)) between 1 and 500),
  source_year integer not null default 2022 check (source_year=2022),
  source_authority text not null default 'U.S. Census Bureau'
);
alter table public.naics_2022_official_reference enable row level security;
revoke all on table public.naics_2022_official_reference from public,anon,authenticated;
insert into public.naics_2022_official_reference(code,title) values
...
;
comment on table public.naics_2022_official_reference is 'Official U.S. Census Bureau 2022 NAICS six-digit code/title reference used by the tenant-pinned Solo Setup search RPC.';
commit;
