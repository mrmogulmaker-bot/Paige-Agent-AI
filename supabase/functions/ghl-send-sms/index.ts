// Send SMS through GHL Conversations API.
...
Deno.serve(async (req) => {
  ...
  if (!body?.contactId || !body?.message) {
    return jsonResponse({ error: "missing_fields", required: ["contactId", "message"] }, 400);
  }
  ...
  const res = await fetch(`${GHL_BASE}/conversations/messages`, {
    ...
    body: JSON.stringify({ type: "SMS", contactId: body.contactId, message: body.message }),
  });
