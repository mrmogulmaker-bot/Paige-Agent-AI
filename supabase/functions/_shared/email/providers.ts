...
// interface. The PLATFORM DEFAULT is Resend (the master/owner tenant's
// account). Any tenant may later "bring their own" sending system — SMTP,
// Mailgun, Mailchimp/Mandrill, Postmark, etc. — by storing a provider config
// on their tenant row; getProviderForTenant() resolves the right adapter.
//
// Doctrine §200: nothing tenant-specific is hardcoded here. The master tenant
// is just the config that falls back to the platform RESEND_API_KEY. New
// providers are added by implementing EmailProvider and registering a case in
// buildProvider() — no caller changes required.
...
export type TenantEmailProviderConfig = {
  ...
  // default credential is used (master tenant = platform RESEND_API_KEY).
...
class ResendProvider implements EmailProvider {
  ...
  async send(msg: OutboundEmail): Promise<SendResult> {
    ...
    try {
      ...
      return { ok: true, provider: this.key, id: (body as any)?.id ?? null };
    } catch (e) {
...
// --- exists and adding one is a localized change. Each throws until built so
// --- a misconfigured tenant fails loud rather than silently dropping mail.
 ...
 * platform key (master tenant). A tenant that has stored its own provider +
