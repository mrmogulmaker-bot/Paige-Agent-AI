      ...
      body: JSON.stringify({
        ...
        tools: [
          ...
          },
          {
            ...
            function: {
              name: "list_subagents",
              description: "Discover Paige's specialized sub-agents (Fundability Diagnostician, Legal & Compliance Reviewer, Business Credit Strategist, Funding Path Architect, Data Consistency Auditor, Market Research, Financial Research, Content Drafter, Intake Concierge, Sales Pipeline, Coach Copilot, Problem Reverse-Engineer). Use this FIRST when the user asks for deep analysis, audits, research, or anything beyond simple data lookups — then call delegate_to_subagent with the matching slug. AUTO-DELEGATION: when the user describes a problem, blocker, or 'why isn't X working' situation, immediately delegate_to_subagent with slug='problem_reverse_engineer' and pass {problem_statement: <verbatim user problem>, contact_id?: <if known>}. When you receive a root_cause_analysis result, embed the raw JSON inside a fenced code block tagged ```root-cause-analysis on its own line so the UI can render the Root-Cause card, then add a 2-3 sentence plainspoken summary underneath.",
              parameters: {
                type: "object",
                properties: {
