...
// Hard allowlist — the ONLY secrets this function can ever touch. Widening scope
...
Deno.serve(async (req) => {
  ...
  // Strict gate: is_super_admin() via caller-scoped client (mirrors admin-drop-bucket).
  ...
  if (!value) {
    log(reqId, "secret_not_configured"); // name deliberately not echoed to the client
    return json({ error: "secret not configured" }, 500, reqId);
  }

  // One audit row per invocation — the secret VALUE is never recorded. Fail closed:
  // no secret leaves the project without a paper trail, even on failure.
