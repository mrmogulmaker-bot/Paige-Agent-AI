...
Close every Standard-or-deeper task by capturing what it taught — see `references/delivery.md`, "Close out: capture what the work taught". A capability, a lesson from a defect, a config name, a dated decision, or a reusable pattern belongs in the repository's knowledge home in the same change, together with a sweep for the claims that change now falsifies.
...
6. Knowledge capture — what the work taught is recorded in the repository's durable knowledge home in the same change, and a sweep has found the claims the change falsifies.
