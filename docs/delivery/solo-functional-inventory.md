...
<!-- solo-inventory:totals total=85 LIVE=49 PARTIAL=19 UNAVAILABLE=7 NOT_CONNECTED=2 PROOF_OWED=2 BLOCKED=3 NOT_APPLICABLE=3 P0=0 P1=2 P2=10 P3=12 orphans=23 -->
...
- **49 LIVE** — the load-bearing product (conversation + confirm gate, governed CRM executor, action bus + approvals, business missions, calendar engine, billing reads, team, vault, comms stack, media/image lane, knowledge, evidence analytics) is built on canonical seams with receipts. These are the **DO-NOT-REBUILD** surfaces (§5).
...
> Resolved by PR-C: `settings.security-data` (the former P1 — copy-only placeholder tab while the real panel and deletion machinery existed unmounted) is **closed**: the canonical `AccountSecurityPanel` is mounted unmodified, the user-scoped personal-data export and the deletion-request control ride the existing seams with exact-scope labeling and requestId readback, and workspace deletion is honestly labeled unavailable (no capability exists on main).
