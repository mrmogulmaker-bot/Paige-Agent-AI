...
## Owner consent callback repair — 2026-09-03

The first real owner consent reached n8n, then failed before state consumption and returned to an absent /settings/integrations route. Safe production attempt metadata showed launched with browser binding, not exchanging. A harmless OPTIONS probe returned only three booleans: runtime public origin match=false, public path match=false, exact /tenant-n8n-oauth path match=true. No callback/code/cookie/provider values were read or emitted.

The handler now accepts the exact gateway function path as well as the exact public path, while retaining the fixed public redirect URI for registration, stored payload validation and token exchange. All cookie, state, PKCE, owner/session/workspace and commit guards remain intact. Unbound failures return to the existing /choose-account route. 31 handler tests and independent review PASS. This corrects the prior deployment-readiness claim: the original callback had not been proven through the hosted gateway. Production owner completion remains Proof Owed pending the repair deployment and fresh consent.
