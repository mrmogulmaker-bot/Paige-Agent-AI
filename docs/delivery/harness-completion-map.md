...
7. **Next phase:** fix #1154 and wire `social_post` to the `paige_social_posts` pipeline (draft→schedule→publish→receipt) with the `social.post_*` Spine caps actually registered — turning a classified-but-broken proxy into the real governed publish lane the migration already designed. **The operator-vs-tenant `paige_social_posts` schema conflict is the gating product decision here — filed as #1161** (the live operator table has no `tenant_id`; two Solo surfaces assume the never-applied tenant schema). P1 (#1155) shipped only a **compatibility bridge** that unbroke fresh replay (owner-approved 2026-09-12); it establishes no tenant-safe Social capability and is expected to be replaced by the complete Social Foundation migration. **Server-side containment shipped (2026-09-12, Task 1):** #1166 made the capability-truth manifest say `social.publish` → "planned", and the Social-containment change made the executable seam AGREE — `paige-ai-chat`'s `social_post`/`social_analytics`/`social_accounts` executors no longer proxy the platform-wide Upload-Post credential (`paige-social`); they return a truthful governed `unavailable` via `_shared/social-publish-containment.ts`. So no tenant can publish through the shared platform credential (§9/§38) and the tool no longer contradicts the truth (§13/§36). The data layer was already operator-only (live `paige_social_posts` RLS `is_platform_owner()`; #1159's bridge skips the tenant policies). This is containment, NOT the tenant pipeline — tenant-safe Social publishing stays deferred to #1161.
