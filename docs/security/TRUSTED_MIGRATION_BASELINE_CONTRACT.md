...
- `manifest`: contract version, generation/expiration timestamps, reviewed `main` commit, migration-tree object ID, approved PostgreSQL engine pin, sanitizer-policy digest, public-safety assertions, schema size, and schema SHA-256;
...
4. prove source ancestry and an unchanged migration tree against the PR's base `main`;
