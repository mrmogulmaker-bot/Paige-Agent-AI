...
| C-7 | **RESOLVED in this PR.** Every inbound SMS row was written with no `tenant_id`, and the live RESTRICTIVE policy `tenant_isolation` admitted `tenant_id IS NULL` — so an untenanted row was readable AND writable (the admin policy is `FOR ALL`) by the admin of every other tenant. Reproduced live on the preview branch and closed: writers stamp the tenant, a contact-derived trigger backs them up, the NULL escape is removed, and the admin policy gains a tenant clause. Two things this row previously got WRONG, corrected here: the coach policy does **not** leak (`can_access_contact` is itself tenant-scoped and false for a NULL contact), and a tenant-stamping trigger **did** already exist (`trg_stamp_tenant_id`) — it is merely inert for service-role because it keys on `auth.uid()`. | Done. | — |
