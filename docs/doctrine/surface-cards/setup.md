...
## Public Presence child — released (2026-09-06)
...
**Proof status:** PR #1021 passed CI, Security Audit, and UI-delivery-evidence, then squash-merged as `d4392b9786e68ce44b72905f3ffe6771aea319ab`. Both production version endpoints returned `d4392b9786e68ce44b72905f3ffe6771aea319ab-mtqao86w`. The protected production route redirected an unauthenticated browser to the encoded sign-in return path. Focused tests, changed-file lint, repository ratchets, build, binding-ledger validation, and deterministic responsive geometry are also proven. Signed-in Solo-owner interaction, workspace switching, and every provider path remain `PROOF OWED`. The mandatory Integration Capability Registry has no Public Presence entry on refreshed `origin/main`; do not create a competing registry. Preserve the Registry Steward handoff and require the canonical capability/scopes/authority/M1/outcome/Rail-Mind-Memory/proof/limitations entry before one complete governed provider lane is designed or enabled.
...
**Provider-preparation MVP candidate (2026-09-06):** Public Presence opens the one dedicated Paige workspace through a tenant-stamped action pointer; the server re-resolves the tenant and allowlisted owner-confirmed public facts through `get_public_presence_paige_context()`. Client/Public Presence focus is exclusive and clears on account switch. Secrets, private material, raw reviews, and uploads are excluded. Google Search Console and Google Business Profile stay `UNAVAILABLE`; their typed contracts and registry records are readiness definitions, not live connections. Search Console is first, Business Profile second, and later providers remain ordered/deferred.

**Candidate proof:** 114 real-shell rendered states prove no redundant Setup banner or decorative Public Presence eyebrow, correct sticky clearance and opacity, one vertical Settings scroll owner, no horizontal overflow, both themes, all required viewports, 390×844 mobile actions at 44px, and dedicated Paige focus. Authenticated runtime, real provider paths, Rail outcomes, Mind/Memory, merge SHA, and deployment remain `PROOF_OWED` until closeout.
