...
| Item | Where | Status |
|---|---|---|
| Switch control | `WorkspaceExitControl.tsx` | **CANONICAL** (revised 2026-09-07) — an EXIT, not a picker: it selects nothing and navigates to `/choose-account`, where the choice is made. Replaces `MemberAccountSwitcher.tsx`, now deleted. Shown to anyone holding more than one enterable workspace and to Platform staff, who always have Platform as a distinct deliberate choice; no tier condition |
| Platform choice | `ChooseAccount.tsx` | **CANONICAL** (owner ruling 2026-09-07) — Platform staff always pause after sign-in and explicitly select Platform or a directly authorized Paige workspace. Platform exits tenant scope before `/operator/fleet`; no `/admin` fallback exists |
