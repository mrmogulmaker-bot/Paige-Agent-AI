...
`lint:governed-execution` (+ `--self-test`, 48 cases — `grep -c '^  check('`, not a count of output lines, which included the trailing summary line and is how 43 was published as 44). Each rule was mutation-tested by introducing
