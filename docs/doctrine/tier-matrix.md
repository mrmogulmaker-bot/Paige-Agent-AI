...
| Agreement list read (`paige_agreement_overview`, widened) | ✓ act-as | **refused at the database** | ✓ | ✓ | — | 403 | **code MERGED to branch, migration `20270410031600` (renumbered from `20270408000000`, which collided with another lane's migration of the same version) REPLAYS from zero in the `database-contract` CI job.** Emits no storage key: sealedness and file-presence are booleans, and the three signer facts aggregate server-side. **NOT YET BOUND — the rebinding is OWED.** This row previously claimed the per-signer array "stops reaching the browser"; that was false on the day it was written and is corrected here rather than quietly dropped (§13). Nothing in the application calls `paige_agreement_overview` yet — `src/solo/useSoloAgreementSignings.ts:708` still embeds `paige_agreement_signers(full_name,decline_reason,first_viewed_at,signing_order)` and aggregates those rows client-side. The widened read is the prerequisite for retiring that parallel query; retiring it is its own slice. Found by the Codex review on PR #1382, not by the author |
