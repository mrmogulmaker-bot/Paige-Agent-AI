...
| Uploaded document actually SENT (`agreement-send` reads `document_path` from `tenant-agreements`) | ✓ act-as | — | ✓ | ✓ | — | — | **code MERGED to branch, NOT deployed, NOT proven** — fixes a LIVE defect in which an upload was frozen and emailed as a document byte-identical to an empty one. The service-role read is §9-scoped: the caller-supplied `document_path` must sit under the server-derived tenant's folder, because `tenant-agreements` is protected by folder-based RLS that the admin client bypasses and `save_paige_agreement` checks only that the path is non-empty. The bytes are also opened with the seal's own parser before the send freezes them, so a corrupt or password-protected upload is refused while refusing is still free rather than stranding a signed agreement that can never complete. Both found by the Codex review on PR #1382 |
