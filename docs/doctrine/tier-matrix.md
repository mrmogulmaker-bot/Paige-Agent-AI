...
| Segment | What it is | State | Operator | Agency | Solo | Sub-account | Client |
|---|---|---|---|---|---|---|---|
| `integrations` → **Paige's tools** (MCP gateway) | The registry-native gateway section on the Integrations leaf: the tenant's own MCP tools listed with an honest status, an **Add a tool** path that opens the one catalogue (79 listed vendors plus an always-present *Any MCP server* entry), and a per-tool drawer to re-key or disconnect. Additive — the shipped provider card grid, the Social drawer and the legacy n8n/Zapier drawers are untouched, and the catalogue's n8n/Zapier/Social tiles route **into** those existing drawers rather than reimplementing them (§58) | **PARTIAL, and deliberately narrower than the catalogue it shows.** Wired today: list, add (generic remote MCP — bearer/header/url/none — and n8n by API key), re-key, soft-disconnect and hard-delete, all through the G1a-1 `SECURITY DEFINER` RPCs with server-derived tenant scope. **Listed is never connected:** a vendor tile whose sign-in is not wired renders an honest stop, never a Connect that cannot connect, and a new or re-keyed row reads **Not checked yet** because the probe that would promote it is a later slice. OAuth start/callback, per-tool discovery and single-tool revoke are out of scope. Proof: 32 DOM-driven flow tests plus the hook's 7 tenant tests; **the authenticated live drive and the four Solo viewports are UNVERIFIED and owed to a browser-capable session** (§32.c) | — | — | ✓ | ✓ | — |
