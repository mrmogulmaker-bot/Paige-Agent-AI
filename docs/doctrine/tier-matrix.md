...
**2026-09-03 P0 correction — repair RELEASED / owner retest pending; payment setup remains FAIL.** The authenticated owner attempt failed before hosted setup. `STRIPE_SECRET_KEY` is present; missing-secret speculation is superseded. PR #898 merged as `df38d4c7`; its production deployment is READY and serving the repaired bundle. Both migrations are persisted and both Edge handlers match the release source. The released repair removes card fields from the Solo response, binds delayed client work to workspace/actor, requests cards explicitly and uses atomic retryable webhook persistence without selecting an invoice default. The #899 follow-up adds deterministic same-second confirmation ordering through an additive migration. Prior paragraphs below describe the historical release, not verified provider completion. [Current repair and separated proof ledger](../delivery/billing-payment-setup-p0.md).
