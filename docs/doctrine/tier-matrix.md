...
| Agreement list read (`paige_agreement_overview`, widened) | ✓ act-as | **refused at the database** | ✓ | ✓ | — | 403 | **code MERGED to branch, migration `20270410031600` (renumbered from `20270408000000`, which collided with another lane's migration of the same version) REPLAYS from zero in the `database-contract` CI job.** Emits no storage key: sealedness and file-presence are booleans. Signer facts aggregate server-side, so the per-signer array stops reaching the browser |
...
| Completed agreement remembered in tenant knowledge (`agreement_learn_on_complete`) | — (no client book) | — | ✓ | ✓ | — | — | **code MERGED to branch, catalogue migration `20270410031500` NOT proven applied.** Summary only — counterparty, title, dates, terms. Carries NO signer evidence: no IP, no user agent, no signature image, no legal body. Only an explicit `off` blocks |
