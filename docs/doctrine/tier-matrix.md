...
Proof (§13/§32/§66): server seam (both migrations) proven by a 34-group local-Postgres replay (95 PASS/0 fail, `docs/evidence/proofs/booking-preset-lifecycle/`) AND prod persisted-apply verified by the `deploy-migrations` pipeline's §32 verify step (`db-live` = `4fb76496`, zero migration drift). The authenticated owner live-drive is the owner's own validation pass on the deployed surface (§32.c/§70). The Paige chat capability that drives the SAME RPCs is authored (`domains/calendar_preset.ts`, all 8 verbs) but **NOT yet registered/wired** — that is the **E5 slice** (branch `claude/calendar-paige-e5`, separate PR), per `docs/architecture/booking-preset-capability-adoption.md`.
