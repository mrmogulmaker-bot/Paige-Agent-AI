...
| Agreement list read (`paige_agreement_overview`, widened) | ✓ act-as | **refused at the database** | ✓ | ✓ | — | 403 | **code MERGED to branch, migration `20270410000000` (renumbered from `20270408000000`, which collided with another lane's migration of the same version) REPLAYS from zero in the `database-contract` CI job.** Emits no storage key: sealedness and file-presence are booleans. Signer facts aggregate server-side, so the per-signer array stops reaching the browser |
