...
**Governed Calendar-link SHARING — E7 (RELEASE-READY DRAFT, HELD 2026-09-13 — branch `claude/calendar-e7-link-sharing`; NOT yet live, owner reserved a separate release decision).** A NEW capability class, not a change to the tiers/gating above: Paige prepares a PUBLISHED calendar's public `/book/{slug}` link and, after the confirm gate, SENDS it to a tenant contact by EMAIL or SMS — a comms-governed send of a read-only link (distinct from the booking-preset lifecycle and the FU-2 booking write path). Availability follows the booking-preset envelope — Solo / Sub-account / Enterprise, bounded by the canonical RPCs' §59 authority (`calendar_link_shareable` → `_assert_can_manage_preset`) + `send-message`'s admin/coach JWT gate, NOT a chat-tier feature gate; Agency-as-tenant on its own book qualifies, operators no-op (no tenant), Client/Anonymous refused by scope. Reuses `send-message` (email/SMS, caller-JWT forwarded), `runPreSend` (consent/DND/suppression), and `send-message`'s own `comms.outbound` Rail — no second consent model/queue/social adapter/provider call. `calendar_link_send` = `high` in `action-risk.ts`, confirm-gated, catalogued in `list_tool_autonomy` (`20270316000000`); the `calendar_link_prepare`/`calendar_link_social_copy` reads are `read_only`. Social is COPY-READY only (no governed social-post executor exists; owner ruling 2026-09-13). Honesty: success reported ONLY on `send-message outcome==="sent"` (queued/blocked/failed/`needs_config` reported as themselves). Proof: pgTAP `calendar_link_shareable.sql` 24/24 + adapter smoke 39/39, wired into `calendar-preset-seam.yml`. This row records a HELD draft — it flips to LIVE at the owner-authorized release closeout (§66; master §4.0 stays N/A until merge). OWED (§32.c/§70): `deno check`, authenticated live-drive, `/book` origin confirm, prod-SQL persisted-confirm. Package: `docs/evidence/proofs/calendar-link-sharing-e7/`.
