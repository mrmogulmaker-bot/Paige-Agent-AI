...
| `docs/delivery/sales-agreements-source-contract.md` | **What `tenant_client_agreements` may and may not expose to Spine, Rail or Mind** — the column-by-column may-cross / may-never-cross classification, the "a count of one is an identity" rule, and the two authorization traps a resolver inherits (a denied read returns `200`/`[]`/no error, and a workspace's own clients are authenticated users of that workspace) | Before registering any Spine capability, Rail projection or Mind evidence over Solo Sales agreements — the placement decision itself is Spine item 4's (issue #890) |
...
| `docs/handoff/platform-billing-spine-source-contract.md` | PROPOSED/UNMERGED source contract for Spine readers: the safe billing-status fields, the notice event vocabulary, what never leaves Billing, and unavailable behaviour while no sender exists | Any Spine/Rail work that wants to show billing state or billing activity |
