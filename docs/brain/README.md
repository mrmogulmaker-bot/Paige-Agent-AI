...
| `.claude/skills/` (`flow-by-flow` + `flow-prototype`, vendored) | The §69 delivery skill itself, versioned in-repo so a fresh container has its `references/` and not just a `SKILL.md`. `PROVENANCE.md` records upstream, the MIT notice caveat, and the one local modification (the Gate 6 knowledge-capture close-out) | Any software task here — §69 makes it mandatory · before taking an upstream skill update |
