...
| `../doctrine/paige-attention-register.md` | **Where a platform finding lives so it is never lost** — the five records and what each is for, the register's nine fields and six views, intake and closeout rules, and the honest record that the GitHub Project does not exist yet | **Whenever a finding is discovered outside the current assignment's scope**, or before closing out any workstream |
...
| `docs/delivery/solo-sales-usability-r1.md` | Solo Sales usability repair — commercial terms, working drawer exits/discard, canonical source returns, source privacy boundary and separate evidence ledger; implementation pending release | Current Sales capability and proof status; PR #895 date fixes do not close UX or legal agreements. Read before Release 2 prototype work, which requires rendered owner approval |
...
Sales usability is **PARTIAL**, with R1 implementation pending release and authenticated production proof owed; see `docs/delivery/solo-sales-usability-r1.md`. Legal documents, signatures and sends remain outside this commercial-terms record and require the Release 2 rendered-prototype approval.
