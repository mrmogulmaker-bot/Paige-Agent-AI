...
| `docs/integration-registry/README.md` + `integration-capability-registry.json` | **The Integration Capability Registry** — the one authoritative, living catalogue + taxonomy of every third-party provider/API/connector/Marketplace service/external-execution platform Paige may use, and how each is governed (taxonomy · tier eligibility · authority lane · M1 dependency · canonical provider receipt · Rail/Mind/Memory boundary · six-word status LIVE/PARTIAL/PROPOSED/UNAVAILABLE/DEFERRED/PROOF OWED). Cardinal rule: listed ≠ connected/available/autonomous. Marketplace entries = global metadata only. CI-enforced (`lint:integration-registry`). Distinct from `config-registry.md` (wiring names/IDs), `provider-result-contract.md` (runtime per-tenant state), and the Spine/`tenant_mcp_connections` code registries (§18) | **MANDATORY before proposing/implementing/opening a PR for/changing ANY provider integration; update the entry before merge (delivery rule)** |
...
| `docs/audits/paige-secure-browser-audit-2026-09-06.md` | Paige Secure Browser / Twin Phase-0 audit — current-state matrix (research LIVE/PARTIAL, credentialed modes UNAVAILABLE, no browser provider wired), collision packet vs the owner-locked five-slice plan, provider evaluation (Twin not install-ready; Browserbase/Anchor/Steel evidence), three-mode security architecture, Phase 0–4 roadmap, and the owner credential-custody/provider decision owed | Any Secure Browser / Twin / connected-account-via-browser / owner-assisted-login / bounded-crawl work; "do we have a browser capability?" |
