...
- [ ] **3a. EVIDENCE / CONTEXT (Layer A) — does she need to KNOW it unprompted?** The domain owner
      publishes the safe evidence contract. If it must join a central context composer, the
      Mind/shared-Spine owner wires the resolver from a precise Spine Change Request, using a real
      scoped read with an honest "not available" fallback.
