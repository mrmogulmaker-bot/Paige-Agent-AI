...
- **The Rail seam is real and well-shaped.** `record_capability_run` (migration
  ...
  into `failed`. Membership-validated **on the argument subject, not the caller**, `SECURITY DEFINER`, service-role-only — on the 6-argument signature. The 10-argument overload added by `20270107000000` carried no ACL until `20270411000000` (in the repo; prod apply owed).
