...
- **PAIGE Spine Integration Standard — OWNER-APPROVED ARCHITECTURE POLICY (2026-09-01; docs only)** — `paige-spine-integration-standard.md` is the one platform-wide contract for domain event -> safe tenant/client-scoped evidence -> Mind/PAIGE Chat -> governed action when separately authorized -> attributable Rail outcome. Rail is one component, not the whole integration architecture. Feature/domain workstreams own their UI, data, server contract, safe evidence manifest, domain action, and outcome shape; only the designated PAIGE Chat workstream completes bounded Chat read/write behavior, while designated shared owners control Mind central resolution, Trust Compass, Rail core, Systems Check core, and shared execution. Every feature assignment must declare evidence/action/outcome/no-obligation plus domain/Chat/shared owners. Missing shared interfaces are Spine Change Requests, never private rewrites. Trust may restrict but never elevate permission; job titles/responsibilities remain descriptive. **Implementation maturity is PARTIAL platform-wide; this policy does not claim universal adapters or authenticated coverage.**
