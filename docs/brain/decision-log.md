...
- **Layer C · C1 — the safe governed event→act engine foundation (2026-09-13, owner "Orchestration Completion Roadmap", Gate A, draft PR #1173).** Owner-confirmed boundary: Claude Code owns Layer C (event producers, rules/conditions, automation/act lifecycle, the connector-neutral adapter protocol, correlation, signed inbound readback, canonical-domain-update routing, exact per-act outcomes) — NOT the execution kernel/Capability Gateway/Registry Steward/specialist runtime/scheduler/Chat/domain provider adapters. **C1 landed** the safe foundation: the ONE per-act outcome ledger `public.paige_act_executions` (migration `20270125000000`, renumbered from a `20270123000000` collision with `main`'s `media_credit_ledger`) + the atomic MONOTONIC transition RPC `paige_record_act_execution` (a final outcome is never overwritten/resurrected; only accepted/retrying/ambiguous advance; retries keep the same idempotency/correlation ref; service-role only, `auth.uid() IS NULL` guard in-body §59); the pure decision core (`decide.ts`), the connector-neutral `ActionAdapter` contract+registry (`adapters.ts`, n8n first, all else fail-closed unsupported), the governed engine (`engine.ts`, person-attributed auto grant), and the dispatcher wiring. **Owner corrections all addressed:** #1 rebased onto fresh main + PR desc; #2 INDEPENDENT event-integrity (`subject-tenant.ts` re-derives tenant from the canonical subject and asserts == claimed, fail-closed, NO decision on mismatch/unknown); #3 fail-closed persistence (every ledger write CHECKED; failure → retry, never a false completion); #4 monotonic transitions; #6 three distinct levels (event-level `no_subscriber` · subscriber delivery · per-act outcome; `acts_executed` legacy metadata only). #5 (real Gateway capability/connection/consent/budget/availability resolution before dispatch) is a **C2 prerequisite** — C1 performs NO external send, stops at `accepted_for_execution`, and uses `availability:"unknown"` (the seam's declared non-adoption no-op, never a claim). **Reviews (real pushed diff):** §5 compliance ITERATE + §39 adversarial verifier ITERATE, no BLOCK, boundary clean, tenant-isolation consistent end-to-end. **Findings folded:** the read paths (`loadActs` / `resolve_automation_autonomy` / `resolveAuthorizingPerson`) now distinguish an INFRA error (→ retryable `subscriber_retry`, records nothing) from a genuine decision — a swallowed error no longer bakes a false terminal `held_by_lane`/`refused_authority`/silent "0 acts done" (verifier F2 / compliance #1-2); the verifier's CI-breaking F1 (a source-assertion test in `contact-created-native-event.test.ts` asserted removed strings) fixed by rewriting it to the new behavior — **lesson: a source-string test is a §37 consumer; run the FULL suite when a shared source file changes, not just the new files**; `refusalToOutcome` comment made honest + a totality test over the real `GOVERNED_REFUSAL_CODES` added (compliance #3); the mismatch reason no longer leaks the foreign tenant id into the claimed tenant's readable `last_error` (verifier NIT); touch-fn `search_path` pinned. **Carry-forward (verifier F3, C2/C3, NOT C1 defects):** `approval_pending`∈`_final` blocks the future approval→executed transition — C3's approval-redemption needs its own advanceable state; C2 should assert monotonic RANK (not a flat final set) once `retrying` exists; C2 wires the Gateway availability resolution before the first real dispatch. **Proof (§32):** 53/53 targeted tests + full suite 4367 passed / the same 18 PRE-EXISTING Solo-frontend failures in 3 untouched files (SoloGamePlanWorkspace, resend-receipt-handler, settings.connections-actions); `ci:tsc` adds ZERO errors from these files; `deno check` clean; the monotonic RPC proven headless against real PG16 (advance · terminal-protection · settled-refusal-protection · service-only-guard 42501); `lint:migration-versions/definer-fns/governed-execution/action-risk/write-targets` pass. Migration persisted-apply is **PROOF OWED** on merge (deploy-migrations). **Inherited `main` reds on the non-required `verify` job, NOT this PR:** `chat-tool-registry` (5 Chat tools), `lint:views` (`paige_unclassified_inbound`), `ci:tsc` (`paige-capability-gateway.test.ts` EXPECTED Record missing `proof_owed`/`no_applicable_capability` — a Harness-owned Capability Gateway bug, filed as a Harness follow-up; `baseline-guard` correctly forbids absorbing it). **Still WIP:** C2 (bounded native `crm_advance_journey_stage`→`set_journey_stage` auto-execute vertical with signed re-read + receipt/Rail), C3 (approval-required n8n vertical reusing `paige-n8n` run/execution_get), C4 (generic adapter/callback contract), C5 (reusable automation lifecycle) follow as subsequent Gate-A slices. `paige_act_executions` is the ONE per-act home (no `paige_event_act_runs`, confirmed repo-wide).
