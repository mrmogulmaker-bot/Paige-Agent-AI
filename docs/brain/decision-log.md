...
- **PAIGE Spine Integration Standard — OWNER-APPROVED ARCHITECTURE POLICY (2026-09-01; docs only; owner correction folded)** — `paige-spine-integration-standard.md` is the one platform-wide contract for domain event -> safe tenant/client-scoped evidence -> Mind/PAIGE Chat -> governed action when separately authorized -> attributable Rail outcome. Rail is one component, not the whole architecture. **Current authority is the server action-risk policy plus confirmation/approval gate; Compass is non-authoritative/in-memory or derived display today and does not evaluate actions or permissions.** Feature/domain workstreams own their domain contract; the PAIGE Chat workstream completes the final bounded binding. Normal domains self-register within the stable schema; only a new shared primitive/schema requires a Spine Change Request. The registry must become code-enforced with CI rejecting direct/hard-wired Chat tools/actions and competing approval channels. Exact-source audit: current Chat uses `MUTATING_TOOLS` + `resolve_tool_autonomy`; `lint:tool-catalogue` reports 52 runtime-gated / 29 catalogued / 23 governed-but-invisible and is a ratchet, not the final wall. Adoption is incremental: new capabilities comply now; legacy direct couplings migrate behind existing seams one at a time without pausing unrelated work. Job titles/responsibilities remain descriptive. **Implementation maturity is PARTIAL; deployed persistence/authenticated coverage was not re-proven here.**
