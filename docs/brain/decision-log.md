...
- **Impeccable is MANDATORY for all UI/UX work + owner UI principle (performance over wording) — RULED & RECORDED (2026-09-22, owner ruling, PR #1324, docs-only).** Owner ruled Impeccable a mandatory skill for every UI/UX design, redesign, visual-polish, or interaction task: before design or implementation, agents re-open/read and cite `https://github.com/pbakaus/impeccable/blob/main/.claude/skills/impeccable/SKILL.md` and report which of its checks were applied and their results. Owner UI principle recorded alongside: performance and interaction quality over wording — no redundant banner/intro copy (don't repeat context the user already has from where they are); usable space for the real work over decorative headers; purposeful, performant motion (`transform`/`opacity` only, reduced-motion safe). Landed in `AGENTS.md` — the canonical cross-agent task-start file (its header binds "Codex, Claude, and every other implementation agent") — as **Mandatory routing #4** (the routing mandate, beside Flow-by-Flow and paige-ui-design) plus an **Interface standard** paragraph (the owner UI principle); this binds every build agent, not only Claude. Origin: owner approval of the G1b Connections prototype ("the Impeccable pass was exactly right") after the Impeccable finish-review cut the chrome, recolored gold→violet for §11, and made the disconnect controls keyboard-operable. Docs-only; no code or behavior change.
