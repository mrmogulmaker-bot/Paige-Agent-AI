...
- **INT-102 per-thread PAIGE composer drafts — IMPLEMENTED / DRAFT PR #1308, merge gated under INT-083 (2026-09-20; exact code head `7c2f8e8d0c2d95eec693625ca06bc9bd15787af8`; base `6d46ff6671785c646bcf0739340896a1baa56ede`; conflict-free sync main `0f3c994cb54168099e9a102bdb07fd10524c9219`; classification patch; visible flow, no visual/layout change).** Owner chose option (a): unsent typed and dictated text belongs to its conversation. A new session-memory-only store keys every draft by resolved tenant + effective user + real thread id (or a stable, surface/client/mission-qualified `new-chat` slot), moves the New-chat entry when `ensureThread` returns the persisted id, stages rather than destroys text during send, clears only after a terminal stream `[DONE]`, and retains it on auth/transport/non-2xx/truncated-2xx/cancel/stale-scope failure. Thread, account, tenant, user, surface, and signed-out contexts never render another key; sign-out makes drafts inaccessible while the same user can recover within the still-running module-memory session, and reload loss remains explicit/out of scope. #1298's dictation teardown and late-delivery fence are unchanged; delivered words stay in the origin draft. Initial failing-first 10/10 red → restored 10/10 green; requested-review regressions 3 red/10 green → final focused 13/13 and focused+remount 14/14; load-bearing thread-key mutation failed 5/6 Solo assertions; tsc ratchet 12→12, production build green, ESLint zero errors. Eight-file regression is 66/67: the only red is a pre-existing `SoloPaigeWorkspace.contract` text assertion against untouched `SoloApp.tsx`, reproduced from base; no INT-102 diff in either file. The one requested Codex review on `f65a792d` found two valid P2s — cross-surface New-chat aliasing and missing-`[DONE]` false clears — both fixed failing-first in the single permitted round. `ConversationsRichComposer` is controlled, but its `ClientsConversations` parent still owns one cleared-on-selection body and needs a separate owner-approved repair; untouched here. No `_shared`, Edge, provider, database, migration, attachment, Draft-with-Paige follow-up, or capability change. Ready-triggered exact-head auto review, final CI, merge, Vercel identity, and authenticated runtime remain owed.
