...
- **business-verifier authorization bypass closed — governed authority adoption (2026-09-13, Gate A; branch `claude/paige-os-integration-82ywpq`, PR pending).** Slice 2 of the A/B governed-adoption program (item 2, bypass #2). `business-verifier` was invocation-gated (`verify_jwt=true`) but NEVER used the caller identity: it read `business_id` + `triggered_by` from the BODY, used a SERVICE-ROLE client with NO tenant filter, then contacted external registries + auto-activated PAID adapters (D&B/LexisNexis/TransUnion/Array = real budget) — so any authenticated user of any tenant could verify any business (§9 IDOR) and spoof the actor via body `triggered_by`. FIX (mirrors the write-back authority adapter): authenticate the caller — a verified user JWT → `person` (actor = `auth.uid()`), else the bearer == service-role key → trusted `system` (skill-runner/paige-mcp/DB trigger), else 401 (closes the anon-key-passes-verify_jwt hole); authorize a person against the BUSINESS'S OWN tenant (`is_platform_owner` OR `is_tenant_admin(businesses.tenant_id)` OR `agency_can_manage_child`), owner/admin only (a coach is NOT admitted — a business is tenant-level with no `coach_clients` bond, and `business_verify` is `high`/paid, so admitting coaches would be broader than write-back's assignment-scoped path for a spend act); derive the actor from the JWT, never the body; authorize BEFORE any run insert or provider contact (a denied person burns ZERO budget); governed audit receipt for every decision. New pure adapter `_shared/business-verifier/governed-adapter.ts` (§18, reuses canonical key `business_verify`). §67 DEFERRED, not skipped: `business_verify` is `high`, so `decideGovernedExecution` on the default lane would clamp auto→confirm→propose, breaking this synchronous caller-INITIATED flow (all 3 producers) — so this door adopts the authority re-resolution + audit but executes on a passed verdict; the autonomy clamp is for Paige-AUTONOMOUS invocation, a separate §67 concern, recorded (`risk:"high"` on every audit row) not hidden. §37 (4 producers, none broken): card (person JWT admin), paige-mcp (system, unchanged), skill-runner (system, unchanged, already tenant-scopes before calling), `trg_business_auto_verify` DB trigger (system). §58: only the IDOR + actor-spoof closed; verification stays available to admin/owner + service producers; a coach is newly gated (a tightening of a previously-UNGOVERNED endpoint, not removal of a scoped capability). FOLLOW-UPS (noted, not fixed — scope): (a) paige-mcp `verify_business` does NOT tenant-scope `business_id` before its service-role call → separate governed-adoption target; (b) `trg_business_auto_verify` (mig 20260630013855) hardcodes a STALE `_project_url` (bfmyebsjyuoecmjskqhs ≠ prod xygzykjyynhzqytbqnzu) + swallows errors → pre-existing infra question; (c) MATERIAL FINDING `export-clients-csv` gated only on global `user_roles` admin → exports every tenant's profiles+FICO+auth-emails platform-wide (§9 cross-tenant READ exfiltration) — surfaced to owner, candidate future slice. PROOF: 14/14 vitest on the pure adapter (full person/system + tenant matrix + receipt shape) + `deno check` clean on both edge files. PROOF OWED (§32.c): authenticated runtime live-drive.
