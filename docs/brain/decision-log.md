...
- **The five unresolved review findings on the Spine PR, repaired — and the repair's own peer-gate found a sixth (#729, branch `claude/paige-spine-pr728-hotfix-p8mhmr`, NOT MERGED, RELEASE-BLOCKED by #746, 2026-09-02)** — #728 merged with five open P1/P2 threads. **(1)** `useRailEvents` guarded its in-flight history with a component-lifetime ref, which cannot express "the scope that asked for this is no longer the scope being looked at": React clears it in the old effect's cleanup and sets it again in the new effect's body BEFORE the previous scope's read resolves, so one tenant's activity merged into another's feed. Now an effect-local flag — and, after review, the STATE is also cleared during render rather than in an effect, because a passive effect runs after commit and was painting one frame of the previous scope. **(2)** `paige-apply-extraction` branched on `Response.ok`, but `sync-credit-report-data` answers HTTP 200 + `success:true` and records per-group outcomes in its BODY, so an approval could be stamped terminally `applied` with part of it never written. **(3)** a `fetch` that REJECTED never reached the release, so the row stayed `applied` and every later attempt answered `already_applied`. **(4)** `useSoloPendingActions` re-ran only on a manual refresh counter while Trust Compass survives an account switch, so the previous tenant's drafts and rationales stayed on screen. **(5)** Skip set `skipped` synchronously and discarded the promise, so a failed request or expired session left the row `awaiting_review` while the card said otherwise. **THE SIXTH, WHICH THE §39 PEER-GATE FOUND AND WHICH MATTERED MOST:** repair (4)'s own comment asserted that passing no `tenant_id` was safe because "the live policy derives the tenant from the session". The policy is `(tenant_id = current_user_tenant_id() AND has_any_role(...)) OR is_platform_owner()` — for super_admin it SHORT-CIRCUITS TO TRUE and the read returned EVERY tenant's filed actions into a modal naming one account. The select did not even fetch `tenant_id`. The sibling rail hook in the SAME COMMIT had reached the opposite conclusion and filtered explicitly. Now narrowed to the active account, with `tenant_id` selected so a later reader can see a row's scope; and a resolved session with NO active account reads nothing and says so, rather than rendering a cross-tenant union or an empty list that reads as "nothing is waiting" (§56, decided not defaulted). **Two §13 corrections on the repair itself:** the failure sentence said "Nothing was changed" on the one branch where that is provably false (a partial failure means some groups HAD written), and it interpolated `failed_groups` so a person read `negative_items, hard_inquiries`; and the docstring claimed every approved group's outcome was checked when the callee discards the errors it would need to report — see `lessons-learned.md`. **Two false greens closed in the tests themselves:** the double answered `maybeSingle()` with an array, making `!![]` true and the "release matched no row" branch unreachable; and the "clears IMMEDIATELY" assertion passed against an effect-phase clear until it was rewritten to observe COMMITS via `useLayoutEffect`. **KNOWN OPEN, not fixed here:** after a release that itself fails, the row stays `applied` and the next attempt returns 200 `already_applied`, which the card renders as "Done — I've saved that." Closing it means changing how the card treats an `already_applied` it did not cause — a change to the approval path, outside these five repairs. The answer no longer invites a retry it knows cannot work. **RELEASE-BLOCKED, and by the repair's own follow-through:** repair (1) is correct in source and cannot execute in production. `authenticated` holds no `SELECT` privilege on `public.paige_client_events` — `has_table_privilege('authenticated','public.paige_client_events','SELECT')` is `false` against prod, where the sibling `paige_actions` is `true`. The grant was added by `20260712190000` and revoked by `20260712200000` with no re-grant. A table privilege is checked BEFORE RLS, so the read is refused outright, and both consumers destructure only `{ events, connected }`, so the refusal renders as "Nothing yet" rather than as an error. Filed as #746 and NOT fixed here: the fix needs a grant or a safe reader, it is a different workstream, and it collides with PR #644. #729 must not merge or deploy while #746 is open. **THE DISTINCTION THIS COST US, now the standing vocabulary:** *repaired in code* (the source no longer contains the defect), *preview-proven* (a build renders and the flow drives in a browser), and *production-executable* (a real session, against real privileges and policies, can actually reach it). Repair (1) is the first two and provably not the third. A code-level guard is not evidence of production reachability. **PROVEN at this head:** 1865 vitest across 154 files, 85 checks in the apply-extraction driver, tsc ratchet unchanged, build green, **23 of the 23 distinct lint gates `ci.yml` invokes** PASS (enumerated from the workflow and run, after a second review caught that an earlier draft of this line said "15 of 15" — 15 was the number I happened to run, never a denominator I had counted, written into the very paragraph that says state what you ran), and a 13/13 real-Chromium drive of the scope switch and of Skip failing then retrying — each new assertion proven to fail against the pre-fix source. **The changed edge function's Deno type-check is CI's, not this session's:** `deno` is not installed in this sandbox, and the `verify` job's deno edge ratchet compares base against head on the affected functions. An earlier version of this entry said "`deno check` clean" as though it had been run here. **§13 CORRECTION, and then a correction OF the correction — both recorded, because the second is the more instructive.** This entry originally recorded "1871 vitest, 75 checks". Re-measured, commit `2febcda3` runs **1862** vitest tests, and the count only ever grew across this branch, so 1871 is a number no commit here produced. The first correction went further and impugned the driver figure too — without measuring it. Independent review then measured it: `2febcda3` runs **75 checks, exactly as originally recorded.** So a correction written to condemn unmeasured numbers was itself an unmeasured number, and it defamed an accurate one. The rule survives its own author: state what you ran, and correcting a claim you did not verify is the same error wearing a virtuous face. **UNVERIFIED, and owed:** authenticated production — the branch is not deployed, this session holds no credentials, and a preview deployment is not a production one. #729 is not shipped, not live, and not production-verified.
