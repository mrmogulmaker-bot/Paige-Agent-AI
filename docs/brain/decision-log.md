...
- **PAIGE Spine Integration Standard — OWNER-APPROVED ARCHITECTURE POLICY (2026-09-01; docs only; owner correction + Chat collision folded)** — `paige-spine-integration-standard.md` is the one platform-wide contract for domain event -> safe tenant/client-scoped evidence -> Mind/PAIGE Chat -> governed action when separately authorized -> attributable Rail outcome. Rail is one component, not the whole architecture. **Current authority is the server action-risk policy plus canonical approval gate; Compass is non-authoritative/in-memory or derived display today and does not evaluate actions or permissions.** Feature/domain workstreams own their domain contract; the PAIGE Chat workstream completes the final bounded binding and exclusively owns how approval is proven in `docs/doctrine/one-approval-gate.md`. Normal domains self-register within the stable schema; only a new shared primitive/schema requires a Spine Change Request. The registry must become code-enforced with CI rejecting direct/hard-wired Chat tools/actions and competing approval channels. Current `main` still has 52 runtime-gated / 29 catalogued / 23 governed-but-invisible; unmerged PR #675 exact head `6cf23866…` adds no-new-inline and one-gate CI ratchets but explicitly does not claim a full Spine registry. Adoption is incremental: new capabilities comply now; legacy direct couplings migrate behind existing seams one at a time without pausing unrelated work. **Merge dependency:** #675 first, then rebase/reverify this docs branch. **Implementation maturity is PARTIAL; deployed persistence/authenticated coverage was not re-proven here.**
