...
- **§61 Default tier-placement rule** (2026-08-11, owner-ruled) — the standing DEFAULT answer to "which tier gets this?", so it stops going to the owner per-feature. Owner: *"yet another time you've asked me where things should be placed when we should already have this understanding… lock this in as a complete doctrine, in our brain and the master project."* The rule keys on ONE question: **God gets everything** (§57); a **RESELLABLE capability** (skill/pack/tool/knowledge — an agency can resell it) → **Solo + Agency + Sub-account + Enterprise + God** (agencies included because they resell); an **OPERATIONAL "doing" surface** (own client/campaign book) → all-but-Agency + God (the existing `customer_portal_invite`/`growth`/`studio` pattern, generalized); **operator tooling** → God only. Authoritative home `docs/doctrine/tier-matrix.md` §61; CLAUDE.md §61 (PROPOSED, §-number provisional). **Anchoring application:** the Skills Wave `skills` feature = resellable → **all tier keys** (solo·agency·sub·enterprise·god); Marketplace "skills" → renamed "capability packs" so "skill" = only the executable recipe (§58 slice). Docs-only PR.
