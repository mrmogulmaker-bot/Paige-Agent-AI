...
- **The §39 peer gate broke two of the four fixes that had just been made to satisfy a review bot (2026-09-23, branch `claude/optimistic-meitner-95a0gl`, PR #1382).** An adversarial verifier was dispatched on the real diff and found the §9 guard and the PDF check both still permitted the exact irreversible outcome their own comments claimed to prevent. **(1) The tenant guard was bypassable by path traversal, and the escape left the BUCKET.** `path.split("/", 1)[0] === tenantId` passes for `<own>/../<victim>/source/x.pdf`, because `@supabase/storage-js` does not encode the object key on download — `_getFinalPath` interpolates it bare (`encodeStoragePath` is used only by `purgeCache`, L1226) and WHATWG dot-segment normalisation resolves it before the request leaves. Reproduced independently: the guard sees `<own>` while the request reaches `tenant-agreements/<victim>/source/theirs.pdf`, or `paige-agreements/<victim>/sealed.pdf` — **another tenant's SEALED EXECUTED agreements** — or `/storage/v1/bucket`, the whole project's bucket list as service_role. And because the send then hashes, freezes and emails those bytes to a signer address the caller chose, it was exfiltration rather than a read. Replaced with structural allow-list validation in its own module, `_shared/agreements/storage-path.ts`, with the eight measured escapes as tests CI runs. **(2) `getPageCount()` is not the question `addPage()` asks.** `getPageCount` counts real page leaves by traversal; `addPage` gates on the page tree's DECLARED `/Count` (pdf-lib `core/structures/PDFPageTree.js`). Reproduced: a pdf-lib-built 2-page file patched length-preservingly to `/Count 1` still reports 2 real pages and throws `targetIndex=2 must be less than Count=1` at the seal — after the freeze and after the signature, both one-way. A wrong or absent `/Count` is a common real defect from naive merge tools and scanner firmware. The check now PERFORMS the seal on a throwaway copy (`load` → `addPage` → `save`) instead of approximating it. **(3) The 8s race left an orphan.** `kb-ingest-core` COMMITS the `tenant_knowledge_docs` row before the embedding call and deletes the orphan only at the end, so abandoning mid-ingest left a durable row advertising a `chunk_count` it never produced — in the knowledge base, unretrievable, lying about its size. The ingest is now handed to `EdgeRuntime.waitUntil` where available (the `paige-tts`/`generate-image` convention) so it finishes and cleans up off the signer's request; the bounded race survives only as the fallback, and its timer is cleared. **(4) The new fail-closed `catch` was unreachable** — supabase-js resolves `{data: null, error}` rather than throwing, so a missing function, a permission denial and a dead network all read as "not opted in". It now reads `error`. **(5)** The catalogue migration header still documented the reversed governance and was corrected. **The lesson, and it is the §39 lesson exactly: a fix written in response to a finding is not itself reviewed by that finding.** All four had passed a green agreements Deno suite, a clean `deno check` and `ci:tsc` at baseline. Writing the tests is what caught two more gaps the verifier had not raised — `PDFDocument.load` accepting a bare `%PDF-1.7` header, and `deno check` rejecting `number` for a `setTimeout` handle. Exit codes after: agreements Deno suite 0 (60 cases across four files) · `deno check` 0 on all three functions · `ci:tsc` 0 (baseline 12, current 12) · nine repo lints 0. `storage-path.test.ts` was added to the agreements CI step by name, because that workflow's own comment records that an edge-function test not named there "executes NOWHERE".

- **Agreements follow-up — a review bot found three P1s in the fix itself, and one of them was the defect the brief warned about twice (2026-09-23, branch `claude/optimistic-meitner-95a0gl`, PR #1382).** The Codex review on `db86356c` returned 5 findings, 3×P1 / 2×P2, and **every one was verified real** rather than argued with. (1) **§9 cross-tenant read.** `tenant-agreements` is protected by FOLDER-based RLS — every policy tests `(storage.foldername(name))[1]` against the caller's `tenant_members` rows — `save_paige_agreement` validates `_document_path` only for non-emptiness, and the new send read it with the SERVICE-ROLE client, which bypasses all of it. A tenant admin could have persisted another workspace's object key and had those bytes copied, hashed, frozen and emailed under their own agreement: a caller-supplied identifier resolved with elevated credentials and no tenant filter, which is precisely the `docusign-send-envelope` pattern the brief said twice not to reproduce. The path's first folder must now equal the server-derived tenant. (2) **A `%PDF-` sniff is not a parse.** `sealAgreementPdf` calls `PDFDocument.load`, so a corrupt, truncated or password-protected upload passed send, froze under `pa_sent_is_frozen_ck`, got signed, and then could never complete — signature and freeze are both one-way. The send now opens the bytes with the seal's own parser. **Writing the test changed the fix:** `PDFDocument.load` does NOT throw on a file that is only the eight bytes `%PDF-1.7`; it parses to a zero-page document that would seal into a record whose only page is the signing record — a counterparty who provably signed nothing, the blank-document defect by another door. The check therefore counts pages too. Asserted in `document.test.ts`, which CI runs, rather than claimed in a comment. (3) **An autonomy lane read as a grant.** The ingest blocked only an explicit `off`, and `resolve_tool_autonomy` returns `confirm` for a key with no row while `trust_effective_rung` clamps `auto` down to `confirm` — so allowed-unless-`off` gave every workspace that had never heard of the key an automatic write of its agreement summaries into chat-retrievable knowledge (§16/§67). Now opt-in: only an exact `auto` proceeds, and a resolver that will not answer withholds rather than grants (§68 — absence of proof is not proof). The prior reasoning ("`confirm` nobody can answer means never") is recorded in the module rather than deleted, because it is the argument a future session will re-make. (4) **An unbounded embed on the signer's own request path** — `ingestDoc` reaches Voyage with no timeout, awaited after the agreement was already completed, sealed and emailed, so a hung provider would have shown the signer a failed signing for an agreement that succeeded. Bounded at 8s. (5) **A false claim in this repo's own ledger (§13).** The tier-matrix row asserted the per-signer array "stops reaching the browser"; nothing calls `paige_agreement_overview` at all — `useSoloAgreementSignings.ts:708` still embeds `paige_agreement_signers(...)` and aggregates client-side. Corrected in place, with the rebinding marked OWED. **The lesson worth keeping: the §32 proof and the §39 peer gate answer different questions, and here the peer was a bot.** Every one of these passed a green `database-contract`, a green agreements Deno suite and a clean `deno check`, because those assert what their author thought to assert. Exit codes after the fixes: agreements Deno suite 0 (53 cases) · `deno check` 0 on all three affected functions · `ci:tsc` 0 (baseline 12, current 12).

- **Agreements reconciliation — the send was mailing an EMPTY document (INT-163 follow-up, 2026-09-23, branch `claude/optimistic-meitner-95a0gl`, head `57dc6397`; PR unopenable, GitHub 403 on the integration).** `save_paige_agreement` stores NULL in `body_markdown` for `tenant_upload` because the wording lives in the file; `agreement-send` passed that NULL to the renderer and never read `document_path` or the `tenant-agreements` bucket at all — so it froze, hashed and emailed a document byte-identical to an empty one, and `pa_sent_is_frozen_ck` made it uncorrectable. Measured, not inferred: the NULL-body and empty-body renders share a digest (904 bytes, `0c46f85f…`) while real bodies scale. Became reachable when the UI lane wired the send in `8d1b7319`; found by re-verifying rather than rebuilding what they had already built. FIXED: an upload is read and hashed from its real bytes, and refused loudly when the path is absent, the object unreadable, the bytes empty or the file not a PDF; an empty generated body is refused too. ALSO: `paige_agreement_overview` widened so the parallel read off `paige_agreements` can retire — three signer facts aggregated server-side, each on its own terms (counterparty at lowest `signing_order`, `first_viewed_at` as a minimum across ALL signers, `decline_reason` from the signer who actually declined), sealedness and file-presence as booleans and no storage key; `agreement.overview` registered as a Spine READ capability (a read needs no chat tool, so INT-003's deadlock does not reach it, and the registry comment claiming DRAFT/VOID/STATUS registered was false and is corrected); `add_agreement_signer` given its first producer, with `AgreementClient` finally carrying the `email` its mapper had been discarding, so a client with no address is no longer a dead end; and a completed agreement now writes a summary into tenant knowledge through `kb-ingest-core`, authored by the actor who SENT it, gated by its own `agreement_learn_on_complete` key, carrying no signer evidence. A VERSION COLLISION caught by CI, not by review: `20270408000000` was free when chosen and another lane landed its own `20270408000000_unclassified_inbound_security_invoker.sql` before this merged. `supabase_migrations.schema_migrations` keys on the numeric prefix and not the filename, so `db reset` aborted with a duplicate-key on the PK — a timestamp is not a reservation, and the only check that sees it is a full replay. Renumbered to `20270410031600`. OWED: neither new migration (`20270410031600`, `20270410031500`) is proven applied on PROD — MCP, local postgres and docker were all attempted and none was reachable — and nothing after the original defect has authenticated runtime proof.
