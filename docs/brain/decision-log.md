...
- **Mind · Memory · Tenant-Knowledge · Second-Brain vertical re-grounded + Mind Workspace design gate (2026-09-13, NOTHING SHIPPED).** A five-scout crew re-grounded the whole vertical on fresh `main` (@ `af249ef`, zero diff). Produced `docs/architecture/mind-memory-knowledge-brain-capability-map.md` — the consolidated 5-state status map (LIVE / SOURCE-BUILT / PARTIAL / UNAVAILABLE / PROOF OWED) across Mind/context-assembly, Memory (client=LIVE; owner governed seam=SOURCE-BUILT, no UI; view/correct/forget=UNAVAILABLE), Tenant Knowledge (read LIVE-via-source; ingest/delete/share SOURCE-BUILT; metadata-edit/folders UNAVAILABLE; `source_url` stored-not-surfaced, no `last_verified`), Business Vault (PARTIAL, binary intake UNAVAILABLE by design), chat retrieval/citations (in-prose only, no structured contract), and every Solo/other-tier UI surface + screen→seam + per-tier grid; internal-vs-tenant boundary = SAFE (docs/brain, raw prompts, CoT provably never served). **§13 correction:** Spine registry = **35** capabilities (all `maturity: PARTIAL`), not "1" (mind-matrix) or "~17" (spine-and-rail-state); only 1 (`pipeline.deal_stage_evidence`) is in the C2/C3/C4 Mind envelope contract. Also produced a throwaway Mind Workspace prototype + `-GATE.md` (`docs/design-references/prototypes/solo-mind-workspace.*`): orb-anchored, provenance-first, truth-state on every item, Memory honestly "not available yet", all 9 states; built in the pack `--pg-*` language; verified `impeccable detect` 0 anti-patterns + Playwright 21/21, 0 console errors, frames captured. Global caveat: Supabase MCP denied → authenticated-prod re-confirm PROOF OWED. Mind-matrix banner-corrected; brain README + master §10 updated same PR. **No capability shipped, no product code, no Chat/CRM/Calendar/Harness change; §4 unchanged.** §66/§57/§13 tier-matrix CORRECTION same PR: the Mind ledger falsely showed Sub-account + Enterprise Mind as ✓; code-verified false (only `SoloMindWorkspace` mounts the orb; sub-account/agency/enterprise route via `AgencyApp`, no `mind` subtab — `tierBranches.ts:401,648`; §11c/§60 is the sequenced target) — corrected with a dated §58-preserving banner (no new tier visibility shipped). Next: owner reviews at the gate (no production impl until approved). **[post-merge note, 2026-09-14, §13/§58:** base-syncing `main` into this branch brought in E7's `calendar_link` Spine domain (+2 `maturity: PARTIAL` calendar-read capabilities, both `mindBinding: UNAVAILABLE`; +1 domain module), so the LIVE registry is now **37 capabilities across 13 domain modules** (distribution 17 PARTIAL / 20 UNAVAILABLE / 0 LIVE); **35 across 12 modules remains accurate as of the af249ef grounding**; the Mind-envelope contract is unchanged (still only `pipeline.deal_stage_evidence`).]**
