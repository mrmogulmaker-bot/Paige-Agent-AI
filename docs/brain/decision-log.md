...
- **Live Conversation was a single-account product wearing a multi-tenant shell (2026-09-24, migration `20270419000000`, branch `claude/serene-hypatia-rlp8cj`).** `paige_live_pilot_authorized_internal` read a **singleton** readiness row and demanded `pilot_actor_user_id = _actor_user_id` AND `pilot_authorized_by = _actor_user_id` AND `is_platform_owner(...)` — so the speaker had to BE the platform operator, and every other Solo account was structurally incapable of Live no matter how correct its standing. The rollout restriction had been written into the identity model instead of into configuration. **Fix:** admission moves to `public.paige_live_pilot_subjects`, a platform-owned service-role-only allowlist with per-subject acceptance (`CHECK (acceptance_actor_user_id = user_id)`) and an `expires_at` lifetime; the predicate asks a configuration question and stops re-resolving identity, which would have narrowed the agency-managed and operator standing `current_user_tenant_id()` grants. **§37 inventory complete** — both signatures preserved by `CREATE OR REPLACE` at the identical argument list, so all four deployed consumers are untouched and nothing redeploys; an additive 6th parameter was rejected because it leaves two candidates for the existing 5-key PostgREST body. **Proof:** pgTAP 39 → 72, including the negative the old suite never had (a *different* active member of the *same* enabled workspace is refused), a self-forged `paige_audit_log` inspection receipt admitting nobody, lapse, per-subject withdrawal, and disable-withdraws-everyone; plus 24/24 behavioural assertions run against the actual migration file on a local PostgreSQL 16. **Unchanged:** the provider gate stays shut, zero retention remains `UNAVAILABLE`, speaker identity is unenforced (#1417), the allowlist ships empty, nothing was activated.

- **Paige's Rail receipt could not be filed, because there were TWO of the function that files it (2026-09-23, migration `20270411090000`, branch `claude/laughing-goldberg-rdgvwj`).** `public.record_capability_run` existed **twice** on prod — `(uuid,uuid,text,text,uuid,text)` and `(uuid,uuid,text,text,uuid,text,text,uuid,text,jsonb)`. `20270107000000` added four optional trailing parameters and called that *"backward compatible — existing grants and callers unchanged"*, but `create or replace function` cannot replace across a differing argument list: it created a SECOND function, and no `DROP` was ever issued (four spellings searched, zero hits). Every parameter past the fifth defaults on both, so a five-argument call satisfies both. **Measured on production** with a `PREPARE` (parses and plans, never executes, writes nothing): `ERROR 42725: function public.record_capability_run(_tenant_id => uuid, _actor_id => uuid, _capability_key => text, _outcome => text, _run_id => uuid) is not unique`. All three five-named-argument service-role callers were refused — `mcp-gateway/rail-receipt.ts:66`, `mcp-outcome.ts:851` (the shipped Zapier lane), `n8n-management.ts:145` — and each swallows the failure into a `console.error`, because a Rail write is best-effort and must never turn a completed action into a reported failure. Correct instinct, and exactly what kept a total filing failure invisible. **Nothing was lost (§13):** prod has zero `mcp_connection_run`/`zapier_run_action` Rail rows and **also zero `mcp_connection_receipts`**, so no MCP or Zapier run has ever executed — the defect is LATENT, and Slice ④ is the change that makes the first real run reachable. The two in-database callers (`execute_crm_command`, `execute_crm_command_reversible`, enumerated from prod `pg_proc.prosrc`) pass TEN positional arguments, resolve fine, and have live rows — which is why the Rail looked healthy. **Fix:** drop the six-argument overload. §37 inventory complete — **no caller anywhere passes exactly six**, the only arity a drop could strand; grants identical on both; the ten-arg body is a strict superset. **§32 proof on prod inside `BEGIN…ROLLBACK`, confirmed reverted:** BEFORE 5-named `REFUSED 42725` → AFTER 5-named `RESOLVED`, 10-positional `RESOLVED`, 6-positional `RESOLVED`. Guarded by `supabase/tests/record_capability_run_single_overload.sql` in `database-contract`, bite-proven both ways. **Also corrected here:** the prior entry called this *"flagged, not measured — production SQL is permission-denied to this session."* It was not; the Supabase MCP tool reaches prod. Verify a blocker before recording one.

- **PAIGE can SEE agreements — the reach half of the e-signature engine (INT-178, 2026-09-23, branch `claude/relaxed-cori-q9pt7f`).** INT-163 shipped the engine and withheld its chat tools, and `paige-spine/registry.ts` was left carrying a comment saying *"DRAFT, VOID and STATUS register here"* while `PAIGE_SPINE_CAPABILITIES` held **zero** agreement entries — intent that reads to the next session exactly like a fact. Two READS now register (`agreement.list` / `agreement.status`, chat tools `agreement_list` / `agreement_status`, tool family matched to the five names this log already recorded), both executing the one governed seam `public.paige_agreement_overview` through `_shared/agreements/chat-read.ts`, dispatched from Chat with the **caller's** JWT client. **Why the caller's client and not the admin one:** that RPC is SECURITY DEFINER and re-proves tenant + membership from `auth.uid()` in its own body (§59) — under the service role `auth.uid()` is NULL, so the admin client would read NOTHING, not more. The admin client is used for exactly two things: the Rail receipt (`record_capability_run` is service_role-only) and the empty-result tier resolution. **Signer evidence containment** is two independent controls: the RPC emits none of it, and `project()` is an explicit ALLOWLIST so the accepted-but-unlanded RPC widening (`docs/delivery/int162-attachment-map.md`, backend lane's) cannot leak by default. Both SHA-256 hashes are withheld too; `sealed` is a boolean. The receipt carries **no `detail`** — `redactDetail` scrubs on key NAME only, so it is not a safety net for this shape. **Still NOT registered, each with a stated reason:** send/resend execute the `agreement-send` EDGE function, which the Spine validator rejects (same boundary `calendar_link_send` documents); draft/void have clean `public.*` executors but are mutations, held back so the read half lands first with a real client waiting on the send path. **§39 peer-gate found four real defects in the pushed diff, all fixed with a regression test each:** (1) the receipt read `r.reason` off a type that had no such field — a compile error that would also have written every outage to the Rail as `capability_refused`; (2) `contactId` was never validated as a UUID, so a model-invented id reached Postgres and the owner was told the agreements engine needed attention; (3) `agreement_status` silently widened to the WHOLE workspace book when `contactId` was absent, because coercing to NULL makes the RPC's filter a no-op — a tool promising one client answering with all of them; (4) an Agency got `{success:true,count:0}` and PAIGE would say *"you have no agreements"* when the truth is the account type holds no client book — the same confident false negative the module already refuses for an unknown status filter. **The one shared-contract line touched, flagged rather than buried:** two `direct-tool-definition` entries in `scripts/ci/capability-kit-bypass-baseline.json`. INT-003 resolved the deadlock for MUTATIONS only — `defineCapability()` requires `actionRiskKey: null` for a read and only a non-null key enters `declaredToolNames`, so **a read tool schema can never clear that rule through the kit**. Measured, not assumed (2 findings, exit 1). **CORRECTED after the base merge, and the correction matters more than the original claim:** this was first recorded as an unresolved contradiction for the platform to settle — the lint's comment calling a baseline entry the deliberate consequence for reads, its failure message saying never expand the baseline. Re-reading `capability-kit-lint.mjs:531-534` against PR #1383 settles it the other way: the read case IS decided, and the stated reason is that *"widening the escape to reads reopened a destructive-write hole"*. So a baseline entry per READ capability is the INTENDED cost, not a deadlock — these two entries are the correct approach rather than a workaround, and a future read capability in this lane should add its entry without treating it as debt. What genuinely survives is far smaller and is ROUTED, not fixed: the failure message at `capability-kit-lint.mjs:863` carries no read carve-out, so the next session to hit it is told the opposite of the rule 330 lines above. The tier-matrix row claiming *"nothing in the guard stops them"* was corrected because it is false for reads. **Evidence (exit codes):** 30 vitest assertions EXIT 0, each proven to BITE by restoring its defect (un-register the capabilities → 3 fail; make the projection a delete-list → the signing IP leaks; echo the database error → the refusal test fails; restored → 30 pass). `tsc -p tsconfig.app.json` — **12 errors before, 12 after, the identical set: zero introduced** (the root `npx tsc --noEmit` checks nothing and exits 0; that command is void here). `capability-kit-lint` 308/309 (the file holds 309 entries and 308 resolve; the one stale entry is inherited from `main`'s 306/307, not introduced here) · `chat-tool-registry-lint` 98, none added · `paige-spine-registry-lint` PASS 69→71 · `action-risk-lint` 0 unclassified writes. §37: `paige_agreement_overview` is unchanged and had exactly one non-code consumer (a psql proof) — zero producers can break. **OWED (§32.c/§70), not claimed:** the authenticated live drive — this session has no browser tool, so nobody has yet confirmed the owner can ask PAIGE "where do my agreements stand" on the live platform and get a true answer. Also owed: prod persistence of `20270402000000` is unverified (no database access). **ROUTED, not fixed:** the missing read carve-out in `capability-kit-lint.mjs:863`'s failure message (NOT the read-tool gap itself, which is decided — see the correction above); no lint enforces `chatBinding:"LIVE"` for a READ capability (a read can declare LIVE with zero dispatch and every gate stays green — structurally what let the stale INT-163 comment survive); `docs/doctrine/tier-matrix.md` cites migration `20270403000000`, which does not exist in the tree. **NEXT: the SEND path**, gated on confirming the Resend connector's reported-degraded state first.
- **Paige can SEE her MCP connections — the connection READ path moves onto the gateway registry (2026-09-23, branch `claude/laughing-goldberg-rdgvwj`; migration `20270410214500`, NO edge change).** Asked *"what am I connected to?"*, Paige answers from the `integrations_list` chat tool → `public.list_integration_surface()`, which read **only** `public.channel_connectors` (`20270116000000:35`). It touched no MCP table at all, so every MCP connection a tenant owned — Zapier, n8n, any remote MCP server — was **invisible** to her. Not stale: absent. **THE FIX IS A UNION, NOT A REPOINT** — the adversarial verifier's own words, and independently the shape that was built: repointing the executor would have *replaced* the tenant's email/SMS connector visibility and dropped `from_address`/`inbound_address` (a §58 capability removal, since the two populations are disjoint). The `channel_connectors` half is byte-for-byte unchanged; two MCP halves are appended. **§18 ONE HOME:** it does not re-implement the connection read — `get_mcp_connections_v2` already owns the `owner_only` visibility gate, the host-only URL projection and the `configured` predicate, so this **calls** it rather than forking logic that would drift. **§13 — WHY IT ALSO READS LEGACY:** the gateway registry was filled by a ONE-TIME backfill (`20270319000000:469-573`, helper dropped at `:576` so it cannot re-run) with **no** trigger keeping it current, while the legacy writers remain the sole live write path (the repo says so itself at `20270319000000:596`). Reading the gateway alone would hand Paige a list with a hole in it — and a hole is indistinguishable from an absence, turning honest silence into a confident false *"no, you aren't connected."* So the MCP half is **LINEAGE-PARTITIONED into three disjoint reads**: the gateway's NATIVE rows only (`legacy_source IS NULL`), every live legacy row, and the n8n API-key facet. **The §39 peer-gate killed the first draft, which preferred the gateway wherever a provider STRING matched** — a projected row is a frozen backfill snapshot, so that rule let a stale copy outrank the live record it came FROM, reporting a connection the owner had just disconnected as still live. Precedence keys on the lineage columns the backfill writes (`20270319000000:129-131`), never a provider name. Neither registry's internal name reaches the row. **§9:** both halves resolve through the SAME `current_user_tenant_id()`, no tenant value is ever accepted from the wire, and `owner_only` stays hidden from an ordinary member — proven, not reasoned: `auth.uid()` survives a nested SECURITY DEFINER call (measured in Postgres 16.13; `current_user` becomes `postgres`, `session_user` and `auth.uid()` stay the real caller). **§32:** each MCP half is separately exception-guarded and **RAISES A WARNING** with the real SQLSTATE when it degrades — a degrade path that logs nothing turns every fault into the same invisible symptom. Behaviourally executed against real Postgres: all six health/status branches, a live legacy row beating its own stale projection and listed exactly once, a native gateway row surviving that suppression, the unprojected legacy connection visible, a URL-auth row reading `healthy` rather than permanently `unconfigured` (and that exemption not leaking to a `bearer` row), every reader failing independently with the channel half surviving, zero credential fields. Regression test `supabase/tests/integrations_surface_sees_mcp.sql`, wired into `paige-spine-contract.yml` `database-contract`. **NOT DONE, and named rather than skipped: TOOL INVOCATION is NOT repointed.** `call-zapier-action/index.ts:98` still resolves via `get_tenant_mcp_secret` → legacy. Four independent blockers, each sufficient: (1) `MCP_GATEWAY_EXECUTE_ENABLED` is default OFF — an owner cloud-side flag; (2) the gateway's connection loader **does not refresh OAuth tokens** and says so in its own comment (`_shared/mcp-gateway/connection.ts:112-113`), while `call-zapier-action` DOES refresh and stores via `rotate_tenant_mcp_tokens` → **legacy table only**, so a projected row's expiry is frozen at backfill time and an access token goes stale in ~an hour → `connection_unusable` → 404 → non-2xx → `throw zapErr` **inside Paige's chat turn**; (3) no gateway discovery action exists, so `zapier_list_actions` has no counterpart at all and would degrade a real tool-name list to a number; (4) the gateway's `{outcome,code,run_id,recorded}` shares no field with what `projectOutcomeForModel` reads, so a **successful** run would project to the model as a failure. **§13 CORRECTION to this repo's own earlier claim:** PR #1381's body states `get_mcp_connections_v2` "returns only aggregate counts." That is WRONG and is corrected here — it returns a full per-connection row (`provider_key`, `label`, `auth_kind`, `configured`, `enabled`, `status`, `health`, `last_checked_at`, `granted_scopes`, `visibility`, `server_url_host`) plus `tool_count`/`approved_count` as aggregates. The aggregate-only fact was true of the **tool** list, not the **connection** list. **`record_capability_run` needs a `pg_proc` check before anything is built on it (UNVERIFIED):** it has a 6-arg (`20261220000000:220`) and a 10-arg (`20270107000000:94`) overload with no `DROP`, and `rail-receipt.ts:66-72` calls it with five named args that BOTH satisfy → possible `42725 function is not unique` / `PGRST203`.
- **MCP Gateway Slice ④ — the Solo surface is bound to the gateway edge, and sign-in + check become real (PR #1381, 2026-09-23, branch `claude/laughing-goldberg-rdgvwj`, head `0e74cd9`; frontend-only, NO migration, NO edge change, so `edge-live` and `db-live` both UNMOVED).** `useMcpGateway` now calls the one `mcp-gateway` door for `create`/`verify`/`oauth_begin`/`approve` (§18). **`rekey`/`disconnect`/reads deliberately STAY on their RPCs** — the gateway has no door for them, and routing shipped capability through a door that does not exist would delete it (§58); the hook header carries that map so a later session does not read it as a half-finished rebind. **OWNER-FACING UNLOCK:** every catalogue "connect" tile was an honest stop reading *"Sign-in coming soon"* because Slice ②'s OAuth spine had no browser caller; it now runs as create-then-`oauth_begin`, where the row is created `auth_kind:"none"` because `get_mcp_connection_secret` (20270326000000) counts endpoint + `none`/`url` as configured — and creating it as `oauth` is impossible, that bundle requires the very token sign-in produces. **"Check now"** spends `verify`; **"Sign in again"** re-runs a lapsed grant. **ERROR MAP:** the edge answers in TWO vocabularies (its own lowercase + the writers' uppercase `MCP_*` via `mapWriterError`) and both now land in one map, read off the non-2xx body via `readFunctionErrorBody` so `"Edge Function returned a non-2xx status code"` can never reach an owner; `MCP_CREDENTIAL_TOO_SHORT` was MISSING, so the commonest paste mistake fell through to a generic line. **INT-153 client guard mirrors the server EXACTLY — bearer/header only**, NOT `oauth` (provider-minted, unscanned) and NOT `api_key` (never reaches the bundle helper); gating either would invent a rule the backend does not have. **APPROVAL-LIFETIME FLOOR (the parked product decision) = 15 min floor / 24 h default / 30 days max / NO "until I revoke it".** The backend enforces only `> clock_timestamp()`, which is NOT sufficient: the value is minted on the browser clock and judged on the Postgres clock, the checkers grant no skew grace (`"exact <=, no skew/grace"` at 20270330000000:173, 20270331000000:196, 20270332000000:501) where the platform allows 60s elsewhere, five round trips separate the edge check from the trigger, and consent is verified only AFTER the runner opens an external session — worst case ~2 min, so a short lifetime can be stored, answered `approved: true`, and be dead at or before first use. 24h = `set_trust_session_posture`'s own default; 30 days = §68's longest acting window; the null option is refused because §68 forbids permanent authority AND **there is no per-tool revoke to name** (every approval DELETE is connection-scoped, so `expires_at` is the only per-tool withdrawal that exists — offering "no expiry" makes the sole withdrawal mechanism opt-out, §70.2). **It is a UI-to-backend PRODUCT contract, NOT server-enforced** — moving it into the trigger narrows the writer's accepted contract and needs a §37 producer inventory first. **§39 peer-gate caught a FAIL-OPEN in the author's own first draft:** the expiry helper returned `null` below the floor and the write site omits `expires_at` when null, so the backend stored NO EXPIRY — a one-minute request would have produced a PERMANENT approval, maximum permissiveness for the most suspicious input; the three outcomes are now distinct in the TYPE so a rejection cannot be passed where an instant is expected. **NOT DONE, AND NAMED (§13/§58):** the three legacy edge functions (`tenant-mcp-connect`, `tenant-n8n-oauth`, `tenant-n8n-api-connect`) are **NOT deleted** and must not be until backend doors land — (1) NO tool-list read door exists anywhere (`mcp_connection_tools` is `is_platform_owner()`-only RLS, no RPC reads it, `verify` returns a COUNT; `tenant-mcp-connect`'s `discover` is the only door returning tool names, so deleting it makes the gateway's own `approve` undrivable), (2) the gateway NEVER revokes at the provider (`disconnect_mcp_connection` is pure SQL; the legacy path does revoke — swapping them leaves a live token nobody can withdraw), (3) `tenant-n8n-oauth` IS an external redirect target registered per-tenant by Dynamic Client Registration, so deleting it 404s every existing grant. Also: the legacy→`mcp_connections` backfill is a ONE-TIME SNAPSHOT with no sync trigger, so anything written the legacy way since 2026-09-20 is absent or stale, and projected rows are permanently read-only to every gateway writer. **The per-tool approve screen is NOT built**, blocked on (1); `approveTool` is wired and tested but cannot be driven from a list of real tool names. Evidence: tsc 0, build 0, lint 0, gateway suites 69/69, full suite 5268 passed / 20 failed in 5 files vs main's #1372 baseline of 5248 / same 20 in the same 5 files (none touched by this diff) = **+20 passing, nothing added**. All seven previously-red lint gates PASS after merging `origin/main` (branch was one commit behind #1367). **AUTHENTICATED BROWSER PROOF IS OWED (§32.c)** — headless session, no browser tool; the eight Solo renders were NOT captured and the sign-in was NOT driven against a live provider.

**§13 CORRECTION (PR #1381, same day, caught by the §39 peer-gate and then by a regression test).** Three claims in the Slice ④ entry above and in its commit message were wrong and are corrected here rather than quietly edited away: **(1)** the phrase `"exact <=, no skew/grace"` was quoted from `20270330000000:173` / `20270331000000:196` / `20270332000000:501` as though it described the APPROVAL expiry checks. It does not — all three are the OAuth **access-token** expiry check on `_access_token_expires_at`. The no-grace conclusion is still true of the approval path (the trigger at `20270334000000:45` and the execute check at `20270323000000:228` are both a bare `<=` against the server clock with no grace term), but it is true by reading those checks, not by borrowing a sentence written about a different one. The quotation is withdrawn; the floor is unchanged. **(2)** the 24-hour default precedent was attributed to `set_trust_session_posture`, which exists nowhere in the repo; the function is **`set_trust_posture`** (`20260929000000:103`). **(3)** two owner-facing strings named controls that do not exist — the sign-in-failure copy told the owner to "add a key instead" when the row it had just created carries no credential and Re-key offers no key field for one (nor any way to change a tool's sign-in type), and an existing OAuth tool is not re-keyable at all; and the detail drawer claimed "approvals are all-or-nothing for this tool" when the approve door is per-tool-name and no all-or-nothing approval exists anywhere. Both are §70.1 violations — a rendered instruction with nothing behind it — and both are fixed. Worth recording precisely: the first fix was applied to the CALLER's fallback string and the defect still shipped, because the error MAP is what actually renders; the regression test written for that fix is what caught the miss. A fix is not verified because it was written.

- **The capability kit's verb precondition REMOVED — the canonical policy is the one gate on what may be declared (PR #1383, 2026-09-23, branch `claude/capability-kit-verb-gate`).** The #1367 entry below is **partially superseded and kept for the trail (§11: mark reversed, add the new entry)** — three of its facts are now false and a reader would act on them. **(1) There were THREE locks, not two.** `defineCapability()` ALSO required `MUTATION_VERB.test(actionRiskKey)` on top of `classifyAction()`, so 32 of the 156 curated keys — 15 of them `high`, `agreement_resend` and `agreement_void` among them — were classified and still not declarable, and a declaration `capability-kit-lint` cleared threw the moment its module was imported (**Codex P1 on #1367**; its self-test used `widget_send`, whose verb happens to pass, which is what masked it). Measured on the real tree: `crm_merge_contacts`, `booking_preset_revise`, `crm_close_deal` each THREW; `crm_create_contact` constructed. Removed — policy membership is strictly stronger than a spelling proxy for the exact thing the veto's own message claimed to check (§18), and `MUTATION_VERB` keeps its three fail-safe floors over keys nobody classified (`unclassifiedWriteReason` consults it only AFTER `RISK_BY_TOOL` misses; the MCP `server_name_floor` RAISES approval on an unclassified provider name; `action-risk-lint` demands a class for a write-shaped name). One veto replaced it: a `mutation`/`external_effect` capability may never declare `read_only` — defensive today (`ActionRisk` cannot even express it; zero entries carry it) and asserted as a RULE rather than left to the table's composition. **(2) The census was wrong: 156 entries / 82 `high` / 71 `ordinary` / 3 `owner_only`, not 150 / 75-72-3** — #1367's own commit carried 157 tuples for 156 keys. **(3) The baseline is 306/307, not "unchanged at 307"** — a legitimate shrink from deleting a duplicate `crm_update_task` tuple, not tampering. **A SECOND Codex P1 on #1367, also fixed here:** the declared-key exemption skipped duplicate `RISK` tuples, and because both policy maps were last-wins a duplicate could silently DOWNGRADE a class — which defeats `clampLaneByRisk` (it forces `confirm` on an `auto` grant only for `high`/`owner_only`), leaving a high-risk action running UNATTENDED (§67/§68). The array is deduplicated and the fold now keeps the MOST RESTRICTIVE class for a repeated key, fail-safe rather than a load-time throw because this module bundles into every governed edge function (§37). **OPEN and disclosed rather than implied (§13):** lint clearance is "a declaration mentions this key", not the constructor's whole predicate — it reads no `governance.risk`, so a declaration whose class contradicts the tuple passes CI and throws on import. Pre-existing on `main`, narrowed by this change, recorded in-tree with its falsifying input at `capability-kit-lint.mjs`; **not filed as a tracked task** (the task tool was unavailable). Exit codes: capability-kit lint 0 (306/307) · its self-test 0 (26 cases) · runtime suite 0 (28 cases, "156 classified action keys proven declarable") · action-risk-lint 0 · `ci:tsc` 0 (baseline 12, current 12). §39 peer-gate SHIP · §5 compliance BLOCK on record fidelity, which is what produced this entry.
- **MCP Gateway Slice ③ — `approve` + `execute` are LIVE (PR #1373, 2026-09-23, MERGED squash `0eee0d9a`; `edge-live` moved afe05e28→0eee0d9a; `db-live` UNMOVED f9c0a0d9, no migration; zero edge drift).** The final two actions on the one gateway door (§18) after verify/create/oauth_begin. **`approve`** (`_shared/mcp-gateway/approve.ts` `runApprove`+`readApproveInput`, MANAGE action) records a durable, endpoint-bound approval via the already-live `set_mcp_connection_approval` writer (migration `20270323000000`), mirroring runVerify/runOauthBegin's §9 chain and calling the writer through the caller's `userClient` so its in-body admin+tenant gate + `FOR UPDATE` reviewed-endpoint guard is the authority (§59). Since `get_mcp_connections_v2` exposes neither the tool pin nor the endpoint hash, the edge reads BOTH server-side (`mcp_connection_tools.pin` + `get_mcp_connection_secret().endpoint_hash`); a client-supplied `expected_endpoint_hash` is a Slice ④ forward seam the writer can only fail against. **`execute`** (`execute.ts` `runExecute`+`readExecuteInput`, USE action) composes the hardened runner `runConnectionCapability` with its production deps (`makeRpcConnectionLoader`/`makeRpcApprovalVerifier`/`makeRpcCapabilityResolver`/`makeCanonicalRailReceipt`) — the runner is the authority (foreign_tenant §9 / owner_only INT-082 / durable consent). New `_shared/mcp-gateway/writer-errors.ts` = §18 one home for the SECURITY-DEFINER coded-error map (create reuses it; `MCP_ENDPOINT_CHANGED`→409 distinct). **THE "OWNER GO" LIVE GATE (§32/§33):** `mode:"execute"` dispatch is INERT behind operator env flag **`MCP_GATEWAY_EXECUTE_ENABLED` (default OFF)** — refuses `execute_not_enabled` before any provider contact; `mode:"prepare"` always allowed; flipping the flag is cloud-side config, no deploy (§64). Nothing executes until the owner sets the flag AND has a verified connection + approved tool (pre-launch §4 blast radius ~zero). **§39 peer-gate #1 (§9) fixed:** execute was a cross-tenant STATE oracle (a guessed foreign UUID's refusal code distinguished nonexistent/disabled/unusable/foreign, reachable via `mode:"prepare"`) — now every "cannot prove you own this connection" refusal, incl. `owner_only_forbidden` (mirroring v2's hiding), collapses to a uniform `404 not_found`; oracle-closure proven in the smoke. **Proof (exit codes):** `deno check` EXIT 0 (5 modules; CI Deno ratchet 66 + gateway-smoke 94 green in `verify`; CI `contract`+`database-contract` GREEN); `smoke:mcp-gateway` 355 assertions EXIT 0 (measured request bodies through the REAL handlers + real runner over a socket). **§32.a (proven):** deploy-edge-functions run `35827601880` SUCCESS; edge-live→`0eee0d9a`, zero drift. Crew: **§5 compliance SHIP**, **§39 peer-gate ITERATE/no-blocker** (its §9 oracle + two honesty nits — approve's disabled-connection code + the "never throws" docstrings — all fixed). `verify`/`github-advanced-security` red = the accepted inherited baseline (stood down, twice). **PROOF OWED (§32.c):** the authenticated live-execute drive is UNAVAILABLE until the owner enables the flag; the whole owner-facing entry point is Slice ④. **NEXT: Slice ④ — rebind the Solo Integrations UI to the gateway (verify/create/oauth/approve/execute) + retire the legacy `tenant-mcp-connect` stack + the `useMcpGateway.ts` direct-RPC calls (the §18 dual-producer seam), folding in the deferred frontend consumer fix (map `MCP_CREDENTIAL_TOO_SHORT` + the edge codes; add a ≥12 client length guard) — a UI-capable session drives the 8-viewport `Validate UI delivery evidence` gate.**
- **INT-003 capability-kit deadlock RESOLVED, and the five checks red on main cleared (PR #1367, 2026-09-23, branch `claude/adoring-cerf-c1o2vc`).** The lint relaxes: a `RISK` entry is the capability kit's PRECONDITION, not a bypass of it — `defineCapability()` calls `classifyAction(actionRiskKey)` and throws when the key is absent from that same array, so `direct-risk-entry` forbade the thing its own remedy requires. **The deadlock had TWO locks, and this is the part worth knowing before anyone re-derives it:** `direct-tool-definition` also fires on `{name, description, parameters}` — the shape of every Paige tool — so fixing only the risk rule left the tool schema unlandable. Both rules now skip a key carrying a governed `defineCapability()` declaration and both still fire without one. Contract for a new mutating tool: classify it, declare it, ship its schema, in one change. Measured end to end (exit codes): RISK key undeclared **1** · + declaration **0** · + tool schema undeclared **1** · complete tool **0** · destructive tool mislabelled as a read **1** · clean tree **0**. **Three holes were introduced building this and closed before shipping, each reproduced first:** filtering on `MUTATION_VERB` would have blinded the guard to 32 genuinely mutating actions whose verbs that regex does not list (`booking_preset_revise`, `crm_merge_contacts`, `crm_close_deal`, `delegate_to_subagent`, …) — `RISK` holds only actions (75 `high`, 72 `ordinary`, 3 `owner_only`, zero `read_only`), so no verb filter is applied and all 307 baseline entries stay; clearing a tool schema via `identity.id` let a destructive tool declare itself a READ capability and pass while its verb kept it out of every runtime gate, so only `governance.actionRiskKey` clears it; and the strict completeness rule did not unwrap casts while the collector did, so `defineCapability({...} as any)` escaped `incomplete-capability` AND minted clearance — both readers now share one unwrap helper (that `as any` escape pre-dated the change; the collector is what made it grant something). **The five checks, each proven to still bite by restoring its defect:** `lint:views` was a REAL §9 leak — `public.paige_unclassified_inbound` ran as its owner over RLS-protected `public.messages`, exposing inbound subject and body_text across every tenant; forward migration `20270408000000` sets `security_invoker=true` (only consumer is `paige-inbox-triage` on the service role, unaffected). `lint:pack-lineage` + `lint:operator-reach` were ONE cause — the §58 unmount of `FleetTeamPulseSurface` moved it into pack-lineage's orphan branch, which has no exemption by design; the retention note cited `OperatorLegacyApp` as precedent and `src/operator/legacy/` does not exist, so the surface was stripped (§30) and the real `list_platform_staff()` read kept in `EXEMPT`. `lint:chat-tool-registry` was adjacent to INT-003, **not** the same cause (capability-kit baselines all five tools and exits 0 on the same tree) — the baseline was simply never updated; recorded with a dated reason, and the `improvement_*` tools run under tenant/role-scoped RLS on the caller's JWT, so they are architecture debt, not an auth hole. `smoke:secure-browser-security` was a STALE ASSERTION, not an ordering defect — it compared text position across function boundaries (`runDraftAndEmailDocument` is declared at L354 but invoked at L679, after authority at L644); now handler-scoped and failing closed, where a renamed call previously returned `-1` and passed silently. **Corrections to measurements made during this work:** the local `origin/main` ref was stale at `9f54833` while real main was `64c568b`, so "capability-kit-lint does not exist on main" and the branch-divergence counts were wrong; and `ci:tsc` is GREEN on real main — the "sixth red check" was that same stale-ref artifact. `npx tsc --noEmit` exits **0 while checking nothing** here (root config is solution-style, `"files": []`), so any prior "tsc 0" in this area is void — the correct command is `npx tsc --noEmit -p tsconfig.app.json`. **Parked, not fixed, with evidence:** `npm run test` EXIT=1 (20 failures across 5 files, pre-existing on clean main, zero overlap with this diff) is the genuine sixth red; plus the `MUTATION_VERB` vocabulary gap, `action-risk-lint`'s destructive-verb pattern gap, `direct-risk-entry`'s path pin (relocating the array evades it), the unverified live `relacl` on the view, the `improvement_*` RPC/Spine debt, and the `inbox_list`/`integrations_list` domain-spread migration.
- **MCP Gateway Slice ② create action — the gateway's create door (PR #1363, 2026-09-23, branch `claude/laughing-carson-iw91ey`).** Action `create` on the `mcp-gateway` edge function — the second action on the one gateway door after Slice ①'s `verify` (§18). A THIN JWT-scoped pass-through (`_shared/mcp-gateway/create.ts` `runCreate` + `readCreateInput`) onto the already-live G1a-1 writers `create_mcp_connection` (MCP facet) / `create_mcp_rest_connection` (n8n REST facet), routed through the caller's `userClient` so the writers' SECURITY DEFINER in-body §9 authority (`mcp.connections.manage`, no service-role bypass) + all credential/SSRF/INT-153-length validation stay the authority — the edge only narrows it, duplicates none of it (§18). **Design pinned by an independent §18/§9 review BEFORE code:** (§59/MAJOR-1) no service-role client in create's scope — `runCreate` takes `{ userClient }` only and `index.ts` builds the service-role `admin` lazily INSIDE the verify branch; (MAJOR-3) `_tenant_id` never forwarded (the writer derives tenant from `auth.uid()`; the edge's `expected_tenant_id` 409 guards the switch race); (MAJOR-4) explicit closed `facet` discriminator (mcp|rest), an `api_key` on the MCP facet left to the writer's own `MCP_AUTH_KIND_NOT_EXECUTABLE` reject, never rerouted; (MAJOR-2) coded-error mapping DUAL-KEYED on SQLSTATE **or** the `MCP_*` token (`42501` → uniform 403 `MCP_FORBIDDEN` with the message suffix stripped — no which-gate/cross-tenant oracle; `22023` → 400 with the specific `MCP_*` code; uncoded → 500 `create_failed`, never the raw PG message, §13). **§18 seam:** the frontend `useMcpGateway.ts` already calls these RPCs directly, so the edge is a SECOND producer — the transient overlap resolves in Slice ④ (rebind UI to the gateway, retire legacy); no consumer of the edge create action exists yet, so nothing breaks (§37). **Proof (exit codes):** `deno check` EXIT 0; `smoke:mcp-gateway` 274 assertions EXIT 0 (facet gate, no-tenant/switch-race, uniform-403 suffix-stripped, 22023 token map, INT-153 surface, no api_key reroute, no `_tenant_id` forwarded, no secret leak, a §59 throwing-admin proof); CI `contract` (gateway smoke) + `database-contract` GREEN. **§39 peer-gate: NO DEFECT** — an independent adversarial read re-ran both proofs and traced the #1 risk (the smoke fakes the PostgREST error shape) against the writer migrations + the shipped frontend: all 12 forbidden raises are `42501`, all 30 validation raises embed their `MCP_*` token, `MCP_DUPLICATE_LABEL` is `22023`+token (not 23505), and the token fallback maps every case correctly even if supabase-js does not surface the SQLSTATE. **§32 CI hardening added:** `mcp-gateway/index.ts` added to CI's `deno check` (was local-only; now CI-enforced, retroactively covering `verify` — following the INT-163 agreements precedent). **Follow-up (P3, → Slice ④):** `MCP_CREDENTIAL_TOO_SHORT` + the edge codes (`unsupported_facet`/`no_tenant`/`tenant_mismatch`) are not in `useMcpGateway.ts`'s `ERR` copy map, so they degrade to the generic honest sentence until the UI rebinds — folds into the already-deferred frontend consumer fix. `verify`/`github-advanced-security` red = the coordinator-accepted INT-158 baseline (stood down). Next: #4 oauth_begin/oauth_callback (#1355-gated).
- **MCP Gateway Slice ② foundation — OAuth-state store + INT-152 + INT-153 (migration `20270332000000`, 2026-09-23, branch `claude/laughing-carson-iw91ey`).** The DB foundation the Connected MCP Gateway's create + OAuth actions build on. Three things ride one migration: **(A) OAuth-state store** — `public.mcp_connection_oauth_state` (connection_id-keyed twin of the legacy `tenant_mcp_oauth_state`; NO provider CHECK; PKCE verifier encrypted via `platform_encrypt`, 10-min TTL, single-use) + `begin_mcp_oauth`/`consume_mcp_oauth_state` (service_role-only; §9/§59 the begin RPC re-verifies in-body the connection is in the passed tenant; consume is atomic single-use). References NO storage bucket. **(B) INT-152** — a monotonic `config_generation` on `mcp_connections`, maintained by ONE BEFORE-UPDATE trigger `trg_mcp_bump_config_generation` (bumps on endpoint/any-credential/auth_kind/header/oauth-*/scopes/expiry change or a status reset to pending_verification/unconfigured; NEVER on a probe result write or a label/enabled edit — so no writer body was reproduced, §18). `get_mcp_connection_secret` now returns `config_generation` (byte-for-byte the live 20270328 body incl. `visibility` + `endpoint_hash` — the §39 peer-gate caught a first draft based on 20270327 that would have dropped `visibility`). `mcp_connection_probe` DROP-and-recreated 5-arg→6-arg (`_expected_generation bigint DEFAULT NULL`) and void→jsonb `{applied, reason?, config_generation}`: reads the row's generation `FOR UPDATE`, and when the caller carries the loaded generation it writes ONLY if it still matches — a stale probe (endpoint/credential rotated mid-verify) is `applied:false, reason:stale_generation`, NEVER clobbering the fresh config or catalog. Edge wired: `connection.ts` surfaces `configGeneration` (absence → null → legacy unconditional, never a security downgrade), `verify.ts` includes `_expected_generation` ONLY when a numeric generation was loaded (deploy-race safety — pre-migration the old 5-arg probe accepts the key-less call; §37) and reports a mid-verify re-key honestly as `409 config_changed_during_verify`. **(C) INT-153** — a bearer/header writer min-length floor (`>= 12` == verify.ts `MIN_SECRET_SCAN_LEN`) appended AFTER the existing checks in the one home `_mcp_assert_credential_bundle` (oauth exempt — provider-minted, unscanned; url/none carry no token). **§58 behavior change flagged:** INT-153 makes a <12-char bearer/header token un-storable via the writers, so the endpoint-setter round-6 last4 test's <12 cases were updated from accept+null-last4 to **reject** (`MCP_CREDENTIAL_TOO_SHORT`); the last4 redaction survives as defense-in-depth for legacy/directly-inserted rows (before-hint block unchanged). §37 sweep: the two pgTAP files were the only producers seeding <12 bearer/header accept tokens (all fixed; remaining short tokens are reject cases whose earlier check fires first). **Proof (§13/§32, EXIT CODES not pass-counts):** MCP `execute_sql` is permission-denied for this session, so the rollback-proof ran on a local ad-hoc Postgres 16 against the REAL migration file — substrate + migration + focused assertions = `OVERALL_EXIT=0` (trigger bump semantics, probe compare-and-write no-clobber, secret keys, INT-153 floor, OAuth store encrypted/single-use/expiry/§9). `deno check` EXIT 0; `smoke:mcp-gateway` 252 assertions EXIT 0 (incl. the new INT-152 generation-passed + stale→409). The FAITHFUL full-`db reset` integration run is CI's `database-contract` job (new proof file `mcp_gateway_oauth_state_and_hardenings.sql` wired into `paige-spine-contract.yml`). **MERGED + PERSISTED (§32.a — proven, not owed):** squash-merged to `main` as `2c6852ec` (PR #1358). `deploy-migrations` run `35809040141` SUCCESS — its `Verify prod is caught up (PERSISTED, not just ran — §32)` step printed the migration list with `20270332000000` in BOTH local and remote columns and `✓ prod schema_migrations is caught up with the repo`, and `db-live` moved `d08769bf → 2c6852ec` (zero drift). `deploy-edge-functions` run `35809040169` SUCCESS (§24 — mcp-gateway edge live). Independent **Codex** review completed on `bb6a635` with no findings. Tier-matrix: not owed (backend only, no surface/tier-visibility change). **CI + peer-gate (post-open):** `database-contract` first went red on a jsonb-cast precedence bug in the new proof file (`'…'||…||'"}]'::jsonb` cast only the trailing literal) — fixed by parenthesizing, and this time verified by running the ACTUAL committed file end-to-end locally (§13, not a separate harness). `verify`/`github-advanced-security` red is the coordinator-accepted pre-existing baseline (INT-158): `verify`'s `lint:views` flags a Jan `paige_unclassified_inbound` view + `typecheck` fails in `src/solo/useSoloAgreementSignings.ts` (INT-163), both untouched by this diff; GHAS is a GitHub-side Copilot-model infra crash (commented). §39 peer-gate returned **no P1**, one folded-in fix (**P3** — added `SET search_path TO 'public'` to `_mcp_bump_config_generation`, harmless convention/advisor) and one **DEFERRED**: **P2 (deferred to a UI-capable session / Slice ④)** — the shipped Solo Integrations gateway form is a §37 CONSUMER miss (it never maps INT-153's new `MCP_CREDENTIAL_TOO_SHORT` code and has no client length guard, so a short token shows a generic — still honest — error). The fix (map the code in `useMcpGateway.ts`'s `ERR` set + a ≥12 client guard on both the create/re-key forms) touches `src/solo/**`, which fires the mandatory `Validate UI delivery evidence` gate requiring 8 authenticated Solo viewport renders that CANNOT be produced honestly in this headless/auth-gated session (§13/§32 — faking them is the exact violation the gate exists to stop). So the two frontend files (`useMcpGateway.ts`, `settings-integrations-gateway.tsx`) were REVERTED to keep PR #1358 backend-only and green; the backend still ships the `MCP_CREDENTIAL_TOO_SHORT` code, and the shipped form degrades safely to a generic honest error until Slice ④'s Integrations UI rebind — which a browser-capable session drives with the 8-viewport evidence (tracked as a follow-up task). Next: Slice ② create action + oauth_begin/oauth_callback (#1355-gated), then Slice ③/④.
- **MCP Gateway Slice ① — `verify` is LIVE (PR #1345, 2026-09-22, MERGED squash `7bc02453`; `edge-live` moved f5921fba→7bc02453).** First deployed runtime of the `mcp-gateway` edge function; action `verify` is the probe's first live caller (INT-107). Path: JWT-tenant + admin manage-gate + `get_mcp_connections_v2` ownership/visibility (§9, cross-tenant id → not_found) → `makeRpcConnectionLoader` (service-role decrypted secret) → `runReadOnlyIntake` (SSRF-guarded handshake + tools/list, never tools/call) → `mcp_connection_probe` (→ connected/healthy + replaces `mcp_connection_tools`; failed probe passes `_tools=null` so a good catalog is never wiped). §59 service-role discipline + §13 honest degrade. Codex review added a credential-reflection guard (`intakeReflectsCredential`: collision-floor `MIN_SECRET_SCAN_LEN=12` + percent-decode of URL segments; a hostile server can't get its reflected token persisted into the plaintext catalog) and a loader-fail `probe_write_failed`. DEFERRED to Slice ② (both writer/probe migrations, outside this slice's pre-approved edge set): probe TOCTOU config-generation compare-and-write = **INT-152**; writer-side minimum credential length = **INT-153**. Proof: smoke `smoke:mcp-gateway` 249 assertions (CI verify step 92 green); `deno check` clean (Deno ratchet step 66 green); `ci/verify` red only on the inherited baseline `{9,18,28,29,39,74}` (== main, zero new debt). **PROOF OWED (§32.c):** authenticated live drive (owner adds a connection → connected) at the end of the 4-slice arc — `verify` has no UI entry point until Slice ④. #1336 `integrations-catalogue.md` (docs research, subordinate to the canonical Integration Capability Registry) folded into this post-merge closeout.
- **PAIGE-native e-signature — the Agreements engine (INT-163, PR #1352, 2026-09-22, in review).** PAIGE's own signing lifecycle; no third-party signing vendor is called and the DocuSign functions are untouched and unextended. **§18 — four tables already carried the word "agreement" and none was this:** `paige_signed_agreements` (onboarding click-through, no `tenant_id`/status/signer/send), `tenant_client_agreements` (commercial terms — its own migration says "recording terms is not signing a document"; we LINK to it), `agreement_templates`/`tenant_agreement_versions` (a body library that has never produced a signature; we READ it), `legal_acceptances` (tenant→Paige, opposite direction). New: `paige_agreements` / `paige_agreement_signers` / `paige_agreement_events` + private PDF-only bucket `paige-agreements`. **Integrity is TRIGGERS, not policies** — RLS does not bind `service_role` and every writer is a service-role edge function, so the one-way status machines, freeze-at-send, write-once seal and append-only trail are BEFORE triggers; the audit table additionally withholds UPDATE/DELETE from `service_role`. **Signer access is never RLS:** an external counterparty has no account, so the control is a 256-bit token stored only as its SHA-256, scoped to one signer on one agreement, expiring and revocable, with tenant/agreement/signer read FROM THE TOKEN ROW — the deliberate inverse of the live `docusign-send-envelope` §9 defect (body `contact_id` → service-role query, no tenant filter, behind tenant-blind `has_role()`), which is NAMED and left unfixed as out of scope. Token leaves the URL into an HttpOnly/SameSite=Strict cookie on first load; the signer is streamed bytes, never a Supabase signed URL (its path is `${tenant_id}/${agreement_id}/` and it survives a void). Paige tools via the Spine domain spread: `agreement_draft` (ordinary) · `agreement_send`/`agreement_resend`/`agreement_void` (**high** → approval card) · `agreement_status` (read); all four mutations added to `list_tool_autonomy` so the operator can see and disable them. **No `agreement_sign` tool at any class — Paige never signs on anyone's behalf.** Tier: `agreement_signing` = Solo + Sub-account + Enterprise (**§61 EXCEPTION**, shaped like `customer_portal_invite` — a capability that acts on a direct client book, which a pure Agency and God do not have). **Three §13 corrections worth keeping:** (1) the design analysis claimed pdf-lib stamps a varying `/CreationDate` so renders would differ — measured false on this path across three processes; (2) a test asserted a non-Latin signer name "did not crash" the seal and passed, while the certificate recorded `?????? ??????` — now a refusal at send, because a record quietly wrong about who signed is worse than one that fails loudly; (3) the slice-1 tests executed NOWHERE in CI (vitest globs `src/**`; CI names two deno files by hand) — now explicit CI steps. Evidence: 36 Deno tests exit 0; a self-standing 19-negative database integrity proof incl. service-role probes (`scripts/agreements/run-integrity-proof.sh`) exit 0. **OWED:** prod migration persistence and prod `service_role` behaviour (no database access this session), and the §70 authenticated owner-usability drive. **Pre-existing, not introduced:** `lint:chat-tool-registry` is red on `origin/main`'s own files.
  - **SECOND REVIEW ROUND, 2026-09-22 — the engine was complete and UNREACHABLE, and the description above was written before anyone noticed.** The independent adversarial read of the pushed diff found that **nothing in the repository ever inserted a row into `paige_agreement_signers`**: `save_paige_agreement` created the agreement alone, and both the send path and the link-minter refused with "add a signer" on every agreement the product could produce. Every control listed above was therefore unexercisable. The proofs missed it because they inserted their own fixture signers — *a proof that stands up its own preconditions proves the rules, never the reachability.* **Now:** `trg_agreement_seed_counterparty` derives the counterparty from the client on every create path (no email on the client ⇒ no signer, never an invented address), `add_agreement_signer` (RPC + `agreement_add_signer` chat tool, `ordinary`) covers multi-party and repair, and `N1` in `signer-seam-proof.sql` drives the product's own create path and counts what comes out.
  - **The architecture changed under the coordinator's ruling.** The HTML signing page is GONE: the signer's surface is the owner-approved React route `/sign/:token`, so `agreement-sign` and `_shared/agreements/signing-page.ts` are deleted and replaced by `sign-agreement` (the token-gated signing ACT, no markup) plus the page's own callable contract — `create_agreement_signing` · `add_agreement_signer` · `issue_agreement_signing_link` · `peek_agreement_signing` · `decline_agreement_signing` · `void_agreement_signing`, all with ONE indistinguishable refusal. **The cookie is gone with the page**, which also dissolved a real defect: it was read token-first-from-cookie, bound to nothing, so on a shared browser one signer could be served — and sign — another party's document. A new `agreement-document` endpoint serves the sealed record by its two different doors: the signer's 365-day retrieval token, and the workspace's own admin session. Both carry `verify_jwt = false` in `config.toml`, which was never set before.
  - **Corrections to things this log previously recorded as true:** the `agreement_signing` tier flag had **zero `hasFeature` call sites** (declared, ledgered as live, read by no code — an Agency could in fact draft/send/void; enforcement is now `trg_agreement_tier` in the database and the tier-matrix rows are reconciled, §66); the column-level `REVOKE` on `token_hash` was a **no-op** under a table-level `GRANT`; `idempotencyKey` was accepted by `send-transactional-email` and never read while four call sites promised it deduplicated (now implemented, best-effort, additive column); and the service-role proof probed a role that does not exist, in a file without `ON_ERROR_STOP`, so a stated precondition silently never ran.
  - **THE PAIGE CHAT TOOLS ARE WITHHELD FROM THIS PR, and the reason is a platform-wide blocker worth knowing before anyone else hits it.** INT-003's `capability-kit-lint` contradicts itself for any NEW mutating tool: it reports `direct-risk-entry` for a symbol added to the `RISK` array in `_shared/action-risk.ts` (the baseline is shrink-only and its own message forbids expanding it), and its printed remedy — declare the capability through `defineCapability()` — *requires* that entry, because `defineCapability` calls `classifyAction(actionRiskKey)` and throws **"Mutation action-risk keys must exist in the canonical action-risk policy"** when it is absent. Probed directly rather than read: `classifyAction('agreement_send') = high` (only because it is in the array), `classifyAction('agreement_send_unlisted') = unclassified`, declaration REFUSED. The lint is green on `origin/main` and was red only on this branch, so the failure was genuinely this PR's — and the fix is not this PR's to make: widening a shared governance guard to unblock one feature is how a guard stops meaning anything. So `AGREEMENT_TOOLS`, the five `RISK` entries, the `list_tool_autonomy` rows (migration `20270403000000`, deleted) and the Spine domain registration all come out, and the engine ships its RPC seams — which is what §10 actually requires. **Nothing that shipped was removed** (§58): none of this had ever merged.
  - **Evidence, exit codes not pass counts:** `deno test` 0 (48) · `deno check` 0 (3 functions) · `run-integrity-proof.sh` 0 (now including the seam and tier controls) · `vitest src/lib/tier` 0 · `lint:tool-catalogue` 0 · `lint:definer-fns` 0 · `tsc` 0. **STILL OWED, unchanged:** prod migration persistence, prod `service_role` behaviour, and the §70 authenticated owner-usability drive.

- **MCP gateway-native connection WRITERS (G1a-1, PR #1322, 2026-09-22, in review).** Part of the "gateway-native, legacy-deleted" MCP rebuild (owner ruling 2026-09-21: clean rebuild, owner reconnects fresh, delete legacy in G3, no double code). Makes `public.mcp_connections` the sole WRITE authority for NATIVE rows (`legacy_source IS NULL`) across all four facets: adds `create_mcp_connection` (generic MCP / Zapier-OAuth / n8n-OAuth), `create_mcp_rest_connection` + `set_mcp_rest_connection_endpoint` (n8n REST api_key — a DISJOINT lane; the loader keeps refusing api_key as `connection_unusable`, connection.ts:100-104, untouched), and `disconnect_mcp_connection` (soft disable + hard delete). **Correction 1 (single home):** the per-kind credential-bundle validation is extracted to ONE home `_mcp_assert_credential_bundle`, and `set_mcp_connection_endpoint` is CREATE-OR-REPLACEd to call it — no duplicate; the existing setter pgTAP + loader-parity smoke pass UNCHANGED. Adds capability `mcp.connections.delete` (owner/tenant-admin only, platform owner EXCLUDED; hard delete gates on it, disable/create/re-key on `manage`). **Correction 2 (history-preserving delete):** `mcp_connection_receipts.connection_id` FK CASCADE→SET NULL + nullable, so receipts survive a hard delete unlinked; approvals/tools stay CASCADE (live state); the deleted-audit row is written FIRST. New closed code `MCP_BAD_LABEL`. Touches NO legacy code/tables, NO `_shared/*` TS, NO execution wiring/probe (all §9 authority-first, secrets encrypted, write-only returns). §39 peer-gate: **no P1/no P2** (5 P3s); §5 compliance: fix-first 2 MEDIUM (IMMUTABLE→VOLATILE on the helper; contract-doc error-code drift) then SHIP. Backend contract published for the G1b Connections UI (Mind agent) at `docs/mcp-gateway/g1a1-connections-contract.md`. **Follow-ups:** (a) P3-1 — a symmetric REST→MCP lane guard on `set_mcp_connection_endpoint` (recommended; deferred as a setter-behavior change beyond the authorized helper-extraction, owner call); (b) P3-3 — enforce provider `auth_kinds[]` in the create writers (carried-forward gap from the setter). Master-doc §4 SHIPPED entry DEFERRED until user-reachable (G1b UI + G2 execution). Tier-matrix: not owed (no surface/tier-visibility change).

- **SOLO-SHELL standing rule (2026-09-22, owner ruling, PR #1324, docs-only).** All product is built for the one **Solo shell**, identical per tenant: no tenant-specific code/config/migrations/UI, and no tenant IDs, names, or one-account special cases in committed artifacts or shared prototypes. Owner-account setup (e.g. reconnecting his own connections) is product usage, not code. Core chat is identical for every account; optional capabilities (e.g. business funding) are **opt-in Marketplace add-ons, never core**. Apply the §200 platform-independence checklist on every PR; when an example/fixture tenant is needed use a `test-tenant-*` row or a neutral placeholder, never a real account. Recorded in `AGENTS.md` ("Solo-shell product — one build for every tenant") beside §9/§2/§200/§63. §13 correction applied same day (Section 10): the G1b Connections prototype's demo shell chip hard-coded a real owner sub-account name — a §63/§200 slip in a shared design artifact — swapped to a neutral coaching-generic placeholder ("Bright Path Coaching").

- **Impeccable is MANDATORY for all UI/UX work + owner UI principle (performance over wording) — RULED & RECORDED (2026-09-22, owner ruling, PR #1324, docs-only).** Owner ruled Impeccable a mandatory skill for every UI/UX design, redesign, visual-polish, or interaction task: before design or implementation, agents read the **installed** Impeccable skill and cite it (canonical upstream source `https://github.com/pbakaus/impeccable/blob/main/.claude/skills/impeccable/SKILL.md`) and report which of its checks were applied and their results. Owner UI principle recorded alongside: performance and interaction quality over wording — no redundant banner/intro copy (don't repeat context the user already has from where they are); usable space for the real work over decorative headers; purposeful, performant motion (`transform`/`opacity` only, reduced-motion safe). Landed in `AGENTS.md` — the canonical cross-agent task-start file (its header binds "Codex, Claude, and every other implementation agent") — as **Mandatory routing #4** (the routing mandate, beside Flow-by-Flow and paige-ui-design) plus an **Interface standard** paragraph (the owner UI principle); this binds every build agent, not only Claude. **Exact-head Codex review P1 (accepted, fixed):** the mandate first depended only on a mutable external `main` URL while the skill is not vendored, so it was unsatisfiable offline — reworded #4 to "read the **installed** Impeccable skill" with the URL as canonical upstream source **plus an explicit availability path: if the skill is unavailable, STOP and report it — never silently skip; the finish-review is still required before delivery.** "Mandatory" is unchanged. Vendoring a pinned in-repo copy is a SEPARATE later slice (license/attribution check on `pbakaus/impeccable` first). Origin: owner approval of the G1b Connections prototype ("the Impeccable pass was exactly right"). Docs-only; no code or behavior change.

- **Command Center → Mind: orb-hero + right-rail layout + callout contrast + legend reposition — SHIPPED (2026-09-21, PR #1314, owner design authority + INT-083).** Owner-directed refinement of the R1a orb surface: `.mind-panel` → 2-column grid (orb stage-hero left, new `.mind-rail` right holding filter + record list), collapsing stacked at container ≤900px. Owner APPROVED the comps ("this is the one I like"). Two additions, style/position only: (1) callout title AA contrast — `.mind-callout` (a `<button>`) inherited the UA dark button text on the dark stage; fixed with `.mind-callout{color:var(--mind-label)}` (also fixes the `⌦` glyph); (2) legend moved bottom-right→middle-right, out of the `co-br` overlap. Codex exact-head review: one **P1** (short-wide `@media` `grid-template-rows` override clashed with the ≤900 stacked 3-area layout at 1366×768 PAIGE-open, where the shell squeezes Mind to ~830px → 3 areas but 2 row-sizes → unbounded implicit rail track → clip) — FIXED in the one allowed fix round (`a3c2dbe9`) by scoping the short-wide `.mind-panel` height+row overrides to the 2-column state via a nested `@container solo-mind (min-width:901px)`; re-review returned one **P2** (residual clip at container 781–900px AND viewport height <~636px, e.g. 1300×600 — no named Solo viewport hits it), taken as a follow-up per the gate. Squash-merge `1cdc36528121c98a186a463d6d92bbda49a709b2`; reviewed head = merged head `70dad685`; Vercel prod `dpl_BfGqhyNvoAfjPk1eXKT4VkVUbTRk` READY on `paigeagent.ai`. Frontend-only (zero `supabase/**`; edge-live/db-live unmoved). Mind vitest 64/64. §13 correction: the first comps mocked PAIGE at 410px with no nav rail so they mislabeled 1366-open as ~956px 2-column and missed the P1; the harness now uses real shell geometry (PAIGE `minmax(440,34vw)` + 72px rail). Evidence `docs/evidence/ui-delivery/mind-layout-orb-hero-rail.md`. **Follow-ups:** (a) the P2 sub-636px stacked clip; (b) pre-existing §11 gold-as-background baseline at `src/components/dashboard/BusinessCreditDashboard.tsx:271` (reproduces on `origin/main`, outside this slice — CI is diff-scoped so it never trips unless that file is touched).**

- **MCP Plan-B: C-0 DROPPED (api_key is the n8n REST facet), C-1 backfill drift check built, C-2b + C-4-gating recorded (2026-09-21, coordinator-ruled).** INT-110 (#1313, squash `47291b60`) fixed `ci/verify` 19.8 by registering `nav_pull_business_credit`/`smartcredit_pull_snapshot` in `WRITE_TARGET` (coverage-only; edge-live `19c179e0`→`47291b60`). **C-0 (add `api_key` to `MCP_EXECUTABLE_AUTH_KINDS`) DROPPED as WRONG:** the code proves `auth_kind='api_key'` is the n8n **REST** facet (migration `20270319000000` §6b backfills it from `tenant_n8n_connections`; provider seed "OAuth (MCP) or bearer/header/API-key (REST)"), and `authFromSecret` maps api_key→bearer — so a naive add would point the MCP JSON-RPC runner (`runConnectionCapability`→`mcp-client.ts`) at REST endpoints; a genuine MCP-server-with-api-key is already `auth_kind='header'`. **Ruling:** C-2 (plan-first) cuts over ONLY the MCP JSON-RPC facets (n8n-OAuth + Zapier) to `runConnectionCapability`; the n8n REST (api_key) facet stays on legacy `paige-n8n` and gets **C-2b** (a dedicated REST-adapter lane — REST transport + `X-N8N-API-KEY` auth + the same consent/tenant/endpoint-hash binding as the MCP runner — plan-first, NOT authorized). **C-4 may NOT drop `paige-n8n`/the legacy n8n REST path until C-2b is live and parity-proven** (owner rule: the legacy path is deleted, never left behind — it survives only until its replacement ships). **C-1 (this PR):** `scripts/sql/mcp-backfill-drift.sql` — a READ-ONLY drift report (no migration, no prod write) proving the `mcp_connections` backfill is 1:1 AND current with `tenant_mcp_connections`+`tenant_n8n_connections` before any cutover; four kinds (missing / orphan / duplicate projection + `credential_drift` on the verbatim-copied `*_ct` columns). §39 adversarial verifier caught a ship-blocking MISS — `a_cred` omitted `refresh_token_ct`/`oauth_client_secret_ct`, the LIVE Zapier OAuth credential (Zapier's `auth_token_ct` is NULL), so a Zapier token rotation read "current" falsely — fixed, plus a §6b duplicate detector added. Two further Codex exact-head reviews each extended `a_cred` to the COMPLETE verbatim-copied, loader-consumed bundle: **P2** added the header name + OAuth identity/expiry (`auth_header_name`/`access_token_expires_at`/`oauth_issuer`/`oauth_client_id`/`oauth_scopes`); **P1** added the `enabled` gate — §6a copies `enabled` verbatim and the loader refuses `enabled<>true` as `connection_disabled`, so a stale projection would EXECUTE a connection legacy disabled (§6b DERIVES enabled from status ⇒ deferred to the cutover parity proof — the deliberate a_cred/b_cred asymmetry). Falsifying proof `scripts/mcp-backfill-drift-proof.sh` PASSES 11/11 (both clean pairs clean). Prod read-only run OWED (headless Supabase MCP prod SQL permission-denied this session — §13).

- **Command Center → Mind: the Synapse orb (R1a) — ROUND 4 (absolute final) fix applied to the three exact-head Codex re-review P2s; back through the re-review gate (2026-09-21, coordinator-authorized).** The re-review of `aaa4f5c0` returned 3 P2 (no P1); two on STOP axes (data-honesty, accessibility) + one performance regression I introduced in round 3, so round 3 STOPPED and returned; the coordinator ruled the STOP correct and authorized ROUND 4 (absolute final) to fix all three in files I own only. First synced `main` (`25dc1ca8`) — the ONLY conflict was the recurring `docs/brain/decision-log.md` prepend collision (my R1a entries vs main's MCP-gateway INT-099 entry), resolved by keeping both (conflict-RESOLVED sync → a new head that IS re-reviewed here). Fixes: (1) **frame-cadence hidden-frame reset** (regression from round 3) — the render loop's hidden/offscreen early-return skipped `render()` without clearing `lastFrameAt`/`lastAnimated`, so a hide→show within 250 ms recorded the whole hidden gap as one displayed-frame sample and could irreversibly trip the one-time density step-down; the early-return now sets `lastAnimated=false` + `lastFrameAt=-1` so the first resumed frame is a fresh start, and `orbInteraction.test.ts` adds a hide→show-within-250 ms scenario modelling the engine's cadence state machine; (2) **orb aria tiers** (accessibility + data-honesty) — `SoloMindWorkspace` `orbLabel` told screen-reader users nodes are "coloured by their canonical source state" (the 6-state model) while the orb + visible legend use the 3 collapsed tiers, so it now reads "coloured by evidence tier: grounded, partial (held, not yet confirmed), or unavailable" mirroring the owner-approved on-screen legend EXACTLY (§00: aria correctness matching what is drawn, not new design language); (3) **evidence honesty** — `AUTOMATED_EVIDENCE` was stale at "vitest 55/55 … orbInteraction (11)"; corrected to the real post-round-4 counts (Mind set 64/64: `src/solo/mind-orb` 49 incl. orbInteraction 20, + SoloMindWorkspace 15) and now names the 30/60-FPS, idle-wake, and hide→show cadence coverage, with an explicit note the old line was stale. Evidence `INTERNAL_BUILD_IDENTITY` re-pointed to the round-4 code commit `a3089014` with `deployment=PROOF_OWED` (unchanged honesty resolution; production id owed post-merge, INT-105). Local gates GREEN on the fix head: Mind vitest 64/64, tsc-ratchet 12→12, `lint:mind-contract` + `lint:solo-parity` PASS, changed-file eslint clean, `gold-discipline` clean on `src/solo/mind-orb` + `SoloMindWorkspace.tsx`, `ui-delivery-evidence` PASS. Scope: `src/solo/mind-orb/{engine,orbInteraction.test}` + `src/solo/SoloMindWorkspace.tsx` + the evidence record + decision-log — **zero `supabase/**`, zero `_shared`, zero `paige-ai-chat` (INT-105)**. HARD-FINAL gate: push → exact-head Codex re-review; clean or P2-or-lower non-regression → merge under INT-083 (reviewed head = merged head; conflict-free sync excepted, reported); any P1 or accessibility/reduced-motion/data-honesty/tenant-isolation/regression → STOP + return with an accessibility-and-honesty audit memo (NO round 5). R1b / #1251 reissue / PR-A3 remain gated.
  - **Mind R1a follow-up list** (owner-accepted residuals, NON-blocking): (1) **Cadence reset misses the tab-background path** — browsers suspend rAF while a tab is backgrounded, so a rapid <250 ms tab-flip within the first ~60 animated frames can sample the pre-hide gap as a displayed-frame interval and trip the one-time density step-down early (~160k→~105k particles, still above the A1 readable floor). Benign — cosmetic only, fps is internal, no user-facing dishonesty. Fix requires a `visibilitychange` handler + `setVisible(false)` cadence reset. Owner accepted the residual on merge of #1303 (a3089014 code / 2b9e8e71 reviewed head). (2) Owner live-look items still owed per §32.c (authenticated Solo drive, four Solo viewports, real-device fps, A3 Mineral pick).
  - **R1a SHIPPED/MERGED (2026-09-21):** the owner ruled Option 1 (ship as-is) after four review rounds; #1303 MERGED to `main` under INT-083 via merge commit `777041731`, reviewed head = merged head `2b9e8e71`, round-4 code `a3089014`. Vercel PRODUCTION `dpl_G52jaWFeC4LmiECZMpiUReQCgy31` READY. Edge/DB NO-OP: zero `supabase/**` → `edge-live` unmoved `9c2d33dc`, `db-live` unmoved `25dc1ca8` (neither the merge commit). §4.0 Shipped Delivery Log row added. MERGED/DEPLOYED, NOT ACCEPTED until the owner's live look.
- **Command Center → Mind: the Synapse orb (R1a) — ROUND 3 (final) fix applied to the two exact-head Codex re-review P2s; back through the re-review gate (2026-09-21, coordinator-authorized).** The re-review of `fcf8f496` returned 2 P2s (no P1), both on STOP axes, so round 2 STOPPED and returned; the coordinator authorized ROUND 3 (final) to fix both in files I own only. First synced `main` (`0f3c994c`) into the branch — the ONLY conflict was a prepend collision in `docs/brain/decision-log.md` (my R1a entries vs main's INT-086 dictation entry), resolved by keeping both (a conflict-RESOLVED sync → a new head that IS re-reviewed, so the gate's "reviewed head = merged head" is satisfied by this same re-review, not bypassed). Fixes: (1) **frame cadence** — the adaptive density step-down keyed on `now() - nowT` measured render()'s CPU prep time, not the DISPLAYED-frame interval, so a vsync-pinned/throttled 30 FPS device reported ~60 and never stepped down; the render loop now records the wall-clock gap between consecutive rendered animation frames in a dedicated `S.lastFrameAt` (decoupled from the animation-timing `S.last`), skips idle→wake gaps and >250 ms hitches, and the p95/fps/step-down arithmetic is extracted to pure `orbInteraction.ts` helpers (`percentileMs`, `fpsFromIntervals`, `shouldStepDown`, `recordsFrameInterval`) that both `render()` and `measure()` call (§18); (2) **evidence build identity** — the stale `114c0bad` SHA + stale preview `dpl_` id are corrected to the frame-cadence fix commit SHA `566eec8a` with `deployment=PROOF_OWED` (a commit cannot contain its own preview id; the fix-head preview id and the post-merge production id are reported to the coordinator once resolved — the honest resolution the coordinator authorized, not a placeholder). §32: proven headless by `orbInteraction.test.ts` (now 19 tests — a simulated 30 FPS device reports ~30 and steps down, a 60 FPS device reports 60 and does not, the old CPU-time measurement would have hidden the 30 FPS device, reduced-motion/idle-wake frames are never recorded). Local gates GREEN on the fix head: mind-orb vitest 48/48, tsc-ratchet 12→12, `lint:mind-contract` + `lint:solo-parity` PASS, changed-file eslint clean, `gold-discipline` clean on `src/solo/mind-orb`, `ui-delivery-evidence` PASS. **Verify baseline PROVEN inherited, not my diff:** the 20 failing tests across 5 unrelated Solo files (`SoloGamePlanWorkspace`, `settings.connections-actions`, `SoloPaigeWorkspace.contract`, `settings.rendered-copy`, `resend-receipt-handler`) fail IDENTICALLY (20 fail / 68 pass) on a clean `origin/main` worktree — ZERO failures in `src/solo/mind-orb`. Scope: `src/solo/mind-orb/{engine,orbInteraction,orbInteraction.test}` + the evidence record + docs — **zero `supabase/**`, zero `_shared`, zero `paige-ai-chat` (INT-105)**. Gate: push → exact-head Codex re-review; clean → merge (reviewed head = merged head); P2-or-lower non-regression → Mind R1a follow-up list then merge under INT-083; any P1 or accessibility/reduced-motion/data-honesty/tenant-isolation → STOP and return. R1b / #1251 reissue / PR-A3 remain gated.
  - **Mind R1a follow-up list** (deferred, NON-regression, P2-or-lower): _none from round 3 yet — populated at merge only if the re-review returns a deferrable finding._
- **Command Center → Mind: the Synapse orb (R1a) — ONE-ROUND FIX applied to the three exact-head Codex P2s; going back through the re-review gate (2026-09-21, coordinator-authorized).** The INT-096 review of `fcb9ad02` returned 3 P2s (no P1); the coordinator accepted the STOP and authorized a one-round fix in files I own only. Fixes: (1) `pickAt` + `pickFront` now compute each node's EFFECTIVE world position with the SAME focus scale the vertex shader applies (extracted to the pure `src/solo/mind-orb/orbInteraction.ts` `focusScale`; `uFocus`/`uFocusAmt`/`aMeta.y`, 0.82/1.12), so a pointer click or keyboard Enter under focus lands on the RENDERED node; (2) `fireFeed` uses `orbInteraction.feedVisual` — reduced-motion fires NO flash — and `setReduced(true)` clears any in-flight `uFlash` so the loop idles (fixes the reduced-motion animated flash that had falsely passed REDUCED_MOTION — the prior PASS is corrected in the evidence record); (3) `MindOrbCanvas` holds a `feedSignal` that arrives during the async engine import (`reconcileFeedSignal`) and flushes it on mount instead of marking it seen and dropping it. §18: the engine calls the pure `orbInteraction` seam so rendered ≠ pickable can't recur; §32: proven headless by `orbInteraction.test.ts` (11 tests — the focus-scaled pointer + keyboard pick select the rendered node and MISS at the pre-focus position, reduced fires no flash, a held feed is not dropped). Local gates GREEN: vitest 55/55 (Mind set), tsc-ratchet 12→12, `lint:mind-contract` + `lint:solo-parity` PASS, changed-file eslint clean, `gold-discipline` clean on `src/solo`, `ui-delivery-evidence` PASS. Scope: `src/solo/mind-orb/{engine,MindOrbCanvas,orbInteraction}` + `orbInteraction.test.ts` + docs — **zero `supabase/**`, zero `paige-ai-chat` (INT-105)**. Gate: push → new head → exact-head Codex re-review; merge only if reviewed head = merged head; P2-or-lower non-regressions → Mind R1a follow-up list then merge, any P1 or accessibility/reduced-motion/data-honesty/tenant-isolation → STOP again. Honesty (coordinator Q): the prior "Merged under the owner's live-review decision" line in master §4.0 was a premature MERGED claim (not yet merged) — corrected to "Merge AUTHORIZED … not yet merged" this round. Issue #1306 (the `ci/verify` 19.8 credit-tool baseline) is coordinator-logged INT-110 and stays out of scope. R1b / #1251 reissue / PR-A3 remain gated.
  - **Mind R1a follow-up list** (deferred, NON-regression, P2-or-lower, from the INT-096 exact-head Codex review): _none — all three findings were fixed this round, not deferred._
- **Command Center → Mind: the Synapse orb (R1a) — MERGE AUTHORIZED under the owner's live-review decision; shipping to `main` under the coordinator gate (2026-09-21; supersedes the "gated on owner sign-off" status of the entry below).** The owner WAIVED the pre-merge preview sign-off (2026-09-20/21 decision) and reviews the Synapse orb LIVE ON MAIN (§4 pre-launch: go all the way to main). Gate executed: shipped "well" as the Mineral default (true-light implemented, NOT default — the owner picks live, switch is a follow-up); the UI-delivery evidence record now records AUTHENTICATED runtime, real-device fps, and the four Solo viewports as **OWED — owner live review on production (2026-09-20/21 owner decision)**, nothing marked verified that isn't. Sequence: mark Ready → wait for the exact-head Codex review (INT-096) → clean = merge; P2-or-lower NON-regression findings → reply `deferred — Mind R1a follow-up list` on each thread, record them under that heading, resolve the threads, then merge; any P1 or any behaviour worse than main (accessibility, WebGL-unavailable fallback, reduced-motion, performance collapse, data honesty, tenant data) → STOP and return; a conflict-free sync needs no re-review, a conflict-resolved sync is a new head and does. **Revert tripwire PRE-AUTHORIZED (INT-083):** an unacceptable live orb, or a Mind-tab production failure (blank canvas, crash, fallback loop) → immediate revert of the #1303 merge commit, no further approval. Post-merge report to the coordinator: the three SHAs, the Vercel PRODUCTION deployment id, and `edge-live`/`db-live` (expected unmoved, src-only — confirmed against function metadata, not the workflow summary, per INT-105). State after merge = MERGED / DEPLOYED, NOT "accepted" until the owner says so. R1b, the #1251 reissue, and PR-A3 remain gated.
  - **Mind R1a follow-up list** (deferred, NON-regression, P2-or-lower, from the INT-096 exact-head Codex review): _none recorded yet — populated at merge if the review returns any deferrable finding._
- **Command Center → Mind: the Synapse particle-field orb (R1a) — BUILT to DRAFT PR #1303, gated on owner sign-off (2026-09-21; branch `claude/mind-synapse-orb-r1a` — coordinator ruled a NEW branch so the gated #1251 stays untouched; built under coordinator authorization + amendments A1–A7; NOT merged).** The owner-approved "Synapse" direction (reference `docs/prototypes/command-center-mind-synapse.*`) is ported into the real Mind orb stack, REPLACING the glass-globe orb wholesale (§30 strip-then-rebuild; §28 unfrozen by the owner approving Synapse). The orb is a GPU `THREE.Points` additive particle field on the Synapse form (two lobes, gyri ridge, flattened base, stem); NO composer/bloom (the fragment shader glows). Structural DUST (~160k desktop / 48k small) IS the glowing FORM and never scales with record count (A1 form floor — the mind reads fully at 0/13/1,284); bright NODES are exactly one per governed record, in the record's domain region, coloured by truth tier (grounded teal / partial gold / unavailable grey — the owner-approved 6→3 orb legend; the finer 6-state provenance stays in the drawer + list, §58). Headline counts GROUNDED (LIVE SOURCE) only; the total is "held", never "grounded". Empty = the formed mind with no nodes (A2); scatter is reserved for loading/error; the incoming-knowledge stream fires ONLY on a genuinely new governed record (§13 — motion never implies activity). Light theme ships as A3 option "well" (contained dark stage) with the true-light alpha option rendered for the owner's pick at sign-off. Scope is FRONTEND-only: `src/solo/mind-orb/{engine,MindOrbCanvas,mindDomains,synapseForm}` + `SoloMindWorkspace` + `solo-mind-workspace.css` + tests — **zero `supabase/_shared/` (INT-105), zero `paige-ai-chat`, `lint:mind-contract` green (A6).** The §39 peer-gate caught and this build FIXED two §58 interaction regressions before sign-off: drag-to-rotate was dead while the orbit was paused/reduced-motion (markDirty set no flag the render loop read), and wheel/scroll-to-zoom (advertised in the aria-label) was missing — both restored, with zoom now incremental and preserved across resize. Proof: focused vitest (synapseForm §32 smoke + the A1 dust-floor `dustCap` lock + mindDomains data layer + workspace behaviour incl. ruling #2 headline) + tsc-ratchet 12→12 (no new errors; the 12 baseline errors are all in unrelated files) + changed-file eslint clean + `lint:gold` clean + a real-engine harness (esbuild-bundled `engine.ts` + `three`) rendered across real-13 / mature-1284 / empty / loading / error / light-well / light-alpha / reduced. Harness runs on headless swiftshader (structure/nodes/states/colours proven; device-brightness parity vs the reference is OWED to the owner's authenticated Vercel-preview live-drive on a real GPU, per §32.c). #1251 reissue + PR-A3 (AST guard) remain gated.
- **MCP Gateway endpoint setter (INT-099, MCP PR-2) — BUILT, pending merge under INT-083 (2026-09-20; PR [#1300](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1300); branch `claude/laughing-carson-iw91ey`; code head `73b5f82c` — round 6).** New migration `20270330000000_mcp_gateway_endpoint_setter.sql` adds `set_mcp_connection_endpoint(...)`: writes a connection's endpoint + full credential bundle in ONE atomic UPDATE sourcing every credential column from the arguments (never carrying old ciphertext forward), so a changed endpoint can NEVER inherit the old secret; the derived `endpoint_hash` follows and the existing `20270322000000` AFTER-UPDATE trigger revokes endpoint-bound consent on a real change (A→B→A cannot resurrect). REFUSES a legacy-projected row (**D1** — no cutover; `tenant_mcp_connections`/`tenant_n8n_connections` remain the sole live write path; the native-connection CREATE path is a separate, future coordinator-numbered PR, so the setter has no operable prod row today). Authority is the NEW capability `mcp.connections.manage` (scope: CREATE/CONFIGURE/ROTATE — NOT delete, **D2**) added to the single mapping `_mcp_caller_capabilities` — owner/tenant-admin of THIS tenant ONLY; a **platform owner is EXCLUDED from `manage`** (keeps `use_restricted` only, **A2**); server-derived actor `auth.uid()`, fail-closed, no service-role bypass (INT-089). `_mcp_resolve_tenant(_tenant,false)` only SCOPES the tenant (it lets a platform owner target any `_tenant_id`) — the capability gate is the authority that refuses the platform owner. **A1**: hashes/enums/booleans/counts audit (`old`/`new` endpoint_hash, `endpoint_changed`, `credential_changed` [boolean from an in-definer decrypt — plaintext NEVER logged], `auth_kind` + `auth_token_last4` before/after, `approvals_revoked`, `tools_cleared`) into the existing `paige_audit_log` (established config-change home; direct in-transaction INSERT) in the SAME transaction, with audit-fail⇒UPDATE-does-not-commit (proven in pgTAP). **A3**: write-only return (connection_id/status/endpoint_hash/auth_token_last4 — no secret, no URL). **A4**: static https/SSRF URL validation (`_mcp_endpoint_write_safe` rejects userinfo, localhost, `*.local`/`*.internal`/`*.localhost`, trailing-dot, private/loopback/link-local IPv4+IPv6, `::ffff:`, decimal/octal/hex IPv4, and — round 2 (P2b) — non-DNS-syntax hosts: whitespace/control/percent-encoding/over-length rejected before the destructive reset) — hostname→private-IP resolution is NOT covered at write time and remains `mcp-client.ts`'s job at dispatch. **A5**: EXECUTE to `authenticated` only. **D8**: INT-082 pgTAP kept EXACT-equality against the new full set (owner/admin/other-own = `[use_restricted, manage]`; platform owner = `[use_restricted]` only). New pgTAP `mcp_gateway_endpoint_setter.sql` wired into `paige-spine-contract.yml` (path filters + run step; `ci.yml` untouched — deconflicts with #1295 / zCod PR-P2 which append to `ci.yml`). Still library-only, zero deployed importer; deployment-live but functionally inert. Does NOT touch `mcp-client.ts`, `get_mcp_connection_secret`, or `verify_mcp_connection_approval`. **D4 HARD GATE (owner-ruled, recorded here):** durable persistence of the runner's EXPLICIT system-authority REASON on the canonical Rail (its own PR, NOT yet authorized) MUST land BEFORE any system/headless authority path is wired into the gateway; this setter has NO system path (**D5**), so no system reason arises in it. §39/§5/§37 pre-Ready crew (all non-blocking) applied while draft (`884e471a`→`e3aba6fc`): SSRF validator made VALUE-based (IP literals parsed to `inet` + range-checked via new `_mcp_inet_is_public` — catches expanded IPv6/shorthand IPv4/full private+reserved set, proven on real Postgres 6 accept/34 reject); setter resolves authority BEFORE reading the connection (§9 — uniform `MCP_FORBIDDEN`, no cross-tenant existence/legacy oracle; `IS DISTINCT FROM` NULL-safe); re-point resets `provider_state` + deletes the stale `mcp_connection_tools` catalog; pgTAP refusal reasons pinned per case. §39's is_staff-INSERT-policy concern was a stale-policy read — current `paige_audit_log` INSERT policy is actor-only (`20261027000000`), so the actor=auth.uid() write satisfies RLS (belt-and-suspenders atop the DEFINER-owner bypass). **INT-096 auto-review (Codex, exact head `fa073204`) round 1 — 4 findings dispositioned:** FIXED F1/P2 (rebind now also resets `last_checked_at` — no stale old-endpoint observation time on an unverified connection), F3/P2 (`_mcp_endpoint_write_safe` now validates the port 1–65535 for bracketed + unbracketed hosts, so a malformed/out-of-range port is refused before the destructive reset), F2/P1 (removed the pre-merge §4.0 Shipped-Delivery-Log row — the verified row is added post-merge per the AGENTS.md contract; decision-log pre-merge entry retained). REASONED-DECLINE F4/P2 → **forward-constraint:** `get_mcp_connection_secret`/`makeRpcConnectionLoader` do not gate on `status`, so a `pending_verification`+`enabled` connection is loadable/executable before `mcp_connection_probe` verifies it — PRE-EXISTING (any pending+enabled row, incl. legacy-backfilled, behaves this way), and gating it is an INT-078/082 connection/consent-contract change out of the endpoint-setter scope (setting `enabled=false` would strand the row since the probe writes status/health, not `enabled`); mitigated (unwired, approvals revoked on rebind, admin-chosen endpoint). **Round 2: the coordinator ACCEPTED F4 as a HARD forward-constraint on the wiring lane** — a `pending_verification` connection MUST NOT be dispatchable, to be enforced when the runner is wired (an INT-078/082 connection/consent-contract change), NOT in this endpoint-setter. Round-2 item 1 (unconditional approval revocation on EVERY successful rebind, incl. a same-URL credential rotation) makes the current inert state safe today: a rebind that has not been re-verified carries no surviving consent. **INT-096 auto-review (Codex, exact head `4f33024e`) round 2 — 5 findings/items dispositioned (all live, JWT-invokable SQL — no deferral):** FIXED item-1/P1 (approvals now revoked UNCONDITIONALLY in-function before the UPDATE, so a same-URL credential rotation drops consent that the URL-change trigger would miss; the trigger then no-ops and `approvals_revoked` is exact); item-2 (credential-bundle validation per `auth_kind` before any write — header→header-name+token, bearer/api_key→token, oauth→issuer+client_id+(token OR refresh), url/none→no credential material; closed code `MCP_BAD_CREDENTIAL_BUNDLE`, never echoes a value); item-3/P2b (hostname DNS-syntax gate — labels `[a-z0-9-]` 1–63, no leading/trailing hyphen, ≥1 dot, ≤253 total — rejects whitespace/control/percent-encoding before the destructive reset); item-4 (audit fidelity — `endpoint_changed`/`credential_changed` booleans + `auth_token_last4` before/after + `approvals_revoked`/`tools_cleared` counts, no plaintext). F4/item-5 ACCEPTED as above. pgTAP extended (per-kind bundle reject+accept incl. oauth refresh-only; same-URL rotation revokes consent; rejected rebind deletes NOTHING; new audit keys/counts; DNS-syntax rejects). Pure validator + bundle-branch smoke re-proven on ephemeral Postgres (9 accept / 43 reject; bundle OK). **INT-096 auto-review (Codex, exact head `7a61184f`) round 2 — 4 P2 findings, ALL VERIFIED real against `_shared/mcp-client.ts` and returned to the coordinator (no deferral rule; STOPPED per ruling).** **Coordinator round-3 ruling — all four fixed + the class closed (code head `01775c7e`):** PRINCIPLE recorded in the migration header — *a bundle the setter accepts must be one the runtime loads as usable; the setter never destructively replaces a working connection with an unusable one.* **F1** — new IMMUTABLE `_mcp_header_name_usable(text)` mirrors the runtime header-name gate EXACTLY (RFC 9110 token grammar `HEADER_NAME_RE` @ mcp-client.ts:77 + case-insensitive `RESERVED_HEADERS` @ :63-72, cited for drift), and the header branch requires a presentable name. **F2** — cross-scheme stray-field rejection for EVERY auth_kind (header rejects refresh/oauth-*; bearer/api_key reject header-name/refresh/oauth-*; oauth rejects a header name; url/none reject all). **F3 (coordinator spec correction)** — oauth now REQUIRES `_auth_token` (+issuer+client_id; refresh optional-additional); the refresh-only accept path + its accept-test are DROPPED and a refresh-only reject-test added, because the runtime's `authFromSecret` has no refresh step and would load a refresh-only bundle as `connection_unusable`. **FORWARD-CONSTRAINT (F3):** refresh-only OAuth is acceptable only once the gateway grows a refresh step that mints a token before first use — a wiring-lane item (INT-083), not this setter. **F4** — `credential_changed` derived from ALL credential-bearing fields (auth_kind, header name, token, refresh, oauth issuer/client_id/client_secret, scopes, expiry), compared on DECRYPTED values in-definer, plaintext never logged. **Class-closer (item 5)** — a runtime-parity proof added to `scripts/mcp-gateway-smoke.mjs` (READ-ONLY import of mcp-client.ts, never edited; no `supabase/functions/_shared/` file changed, so INT-105 does not apply): every `accept_*` case in the pgTAP bundle matrix is proven runtime-USABLE by `authFromSecret`/`authUsable`, every `reject_*` case runtime-UNUSABLE, under IDENTICAL case names on both sides. **Housekeeping:** `validator_smoke.sql`/`bundle_smoke.sql` are LOCAL SCRATCH (ephemeral-Postgres tmp), never tracked and not in the repo tree (confirmed via `git ls-files`) — not in the PR. Round-3 proven locally: header-name + bundle branch on ephemeral Postgres (all accept/reject); `node scripts/mcp-gateway-smoke.mjs` → 180 assertions incl. 11 new parity; three offline lints green. Full pgTAP runs in CI `database-contract`. **INT-096 exact-head reviews:** round-3 code `cd3f3775` came back CLEAN, but the post-sync re-review on `be5dac71` (same code) surfaced 3 P2 findings — the round-3 parity oracle (`authFromSecret`/`authUsable`) was a SUBSET of the true gate `makeRpcConnectionLoader`. **Coordinator round-4 ruling — oracle corrected to the LOADER; all 3 fixed + the class closed by enumeration (code head `73548230`):** **(1)** bracketed IPv4 — `_mcp_endpoint_write_safe` bracketed branch now requires `family(_ip)=6` (a `[8.8.8.8]` authority is invalid per WHATWG `new URL()`); tests for `[8.8.8.8]`, `[8.8.8.8]:443`, a malformed bracketed literal, + the valid-IPv6 accept. **(2)** `api_key` REJECTED with the distinct closed code `MCP_AUTH_KIND_NOT_EXECUTABLE`; the setter's accepted set now EQUALS the loader's `MCP_EXECUTABLE_AUTH_KINDS` (connection.ts:58 = {oauth,bearer,header,url,none}), guarded by a behavioral divergence assertion in the smoke that fails CI if the two ever diverge. **(3)** expired OAuth REJECTED with `MCP_OAUTH_TOKEN_EXPIRED`, mirroring the loader's `oauthExpired` (connection.ts:118) EXACTLY — `auth_kind='oauth' AND access_token_expires_at <= now()` (exact `<=`, no skew/grace; a NULL/absent expiry is live). **CLASS-CLOSER v2 (loader enumeration, connection.ts):** L1 RPC-error→no_connection **(b)** runtime read; L2 configured!=true **(b)** setter always writes an endpoint; L3 enabled!=true **(b)** setter never writes `enabled`; L4 auth===null **(a)** token required (reject_bearer/oauth_no_token); L5 empty server_url **(a)** MCP_BAD_ENDPOINT (A4); L6 connection_id / L7 tenant_id non-string **(b)** structural; L8 endpoint_hash not-64hex **(b)** RPC-derived from the written endpoint; L9 transport∉{http} **(b)** setter never writes `transport`; L10 auth_kind∉executable **(a)** reject_api_key (MCP_AUTH_KIND_NOT_EXECUTABLE); L11 !authUsable (reserved/invalid header name) **(a)** reject_header_reserved/bad_grammar; L12 oauthExpired **(a)** reject_oauth_expired; L13 header-row→bearer (no name) **(a)** reject_header_no_name; L14 visibility owner_only **(b)** authority facet, runner-enforced; D1 dispatch `new URL()` (bracketed IPv4) **(a)** _mcp_endpoint_write_safe family=6; D2 consent facet **(b)** setter revokes on rebind, not a load gate. The smoke parity proof is re-pointed at the REAL loader (`loaderFor(row)` / makeRpcConnectionLoader), case names identical to the pgTAP; READ-ONLY use of connection.ts/mcp-client.ts, no `supabase/functions/_shared/` file edited (INT-105 n/a). **Housekeeping:** `r3_smoke.sql` (and the earlier `validator_smoke.sql`/`bundle_smoke.sql`) are LOCAL SCRATCH (ephemeral-Postgres tmp), never tracked, not in the repo tree (confirmed via `git ls-files`). Round-4 proven locally: SQL logic (bracketed IPv4, api_key gate, expired-oauth mirror) on ephemeral Postgres; `node scripts/mcp-gateway-smoke.mjs` → 187 assertions incl. the loader-parity matrix + the 6-kind divergence guard; three offline lints green. Full pgTAP runs in CI `database-contract`. **Round 5 (FINAL round, coordinator-authorized) — the 4 round-4 findings dispositioned:** **F2** — the oauth-expiry check now uses `clock_timestamp()` (live wall clock), NOT `now()`/`transaction_timestamp()` (frozen at txn start), so it faithfully mirrors the loader's `Date.now()` (connection.ts:118) inside a transaction; exact `<=`, no skew/grace. **F3** — `_mcp_inet_is_public` brought to FULL PARITY with `_shared/ssrfGuard.ts` ipUnsafe (:116-143): added `fec0::/10` site-local (:133), the top-64-zero blanket (`::/64`, :129 — covers mapped/compatible/translated), and EMBEDDED-IPv4 inspection for 6to4 (`2002::/16`, :140) and NAT64 (`64:ff9b::/32`, :138) so a public embedded v4 is public and a private one blocked. The ONE IPv4 range set is factored into new helper `_mcp_v4_is_private` (mirror of ssrfGuard `ipv4Private` :42-60), used by BOTH the family-4 path and the NAT64 embedded check (single source; 6to4 is that set mechanically translated to `2002:<v4>::/(16+bits)`). Class closed by ONE shared, named IP-literal list (`supabase/tests/mcp-ip-literal-cases.json`, 36 cases) asserted on BOTH sides under IDENTICAL names: pgTAP `_mcp_inet_is_public(lit::inet)=NOT unsafe` (the `F3-SHARED-IP-LIST` block) AND the smoke vs ssrfGuard.ts `assertPublicHttpUrl` (READ-ONLY import), plus a smoke cross-check that the pgTAP rows EQUAL the JSON — neither half alone proves parity, the pair does. Hostname→IP resolution stays runtime-owned (mcp-client.ts), documented out of scope. **F4** — the accept-set divergence guard's SQL half MOVED into pgTAP: it enumerates every auth_kind in the `mcp_connections` CHECK set THROUGH THE REAL setter and asserts accepted == the documented executable set `{oauth,bearer,header,url,none}`; the smoke now asserts the loader's `MCP_EXECUTABLE_AUTH_KINDS` (:58, module-private → parsed read-only, INT-105 forbids exporting) == that SAME documented set. Two halves meet in the middle; neither alone proves setter==loader. **F1 → INT-107 (HARD FORWARD-CONSTRAINT, coordinator-issued, plan-first, NOT authorized):** the probe TOCTOU is OUT of this PR. The future endpoint PROBE/verify step MUST carry and verify the `endpoint_hash`/generation it inspected, and re-confirm it, BEFORE any wiring acts on the probe result — so a probe of endpoint A can never mark a since-repointed endpoint B verified (the load↔verify TOCTOU class INT-078 closed for dispatch). This setter already resets `status=pending_verification` + clears `provider_state`/tools on every rebind, so a repoint leaves no stale "verified"; the constraint binds the probe lane, which does not exist yet. **INT-105:** read-only use of `ssrfGuard.ts`/`connection.ts`; no `supabase/functions/_shared/` file edited. **Housekeeping:** `r4_smoke.sql` (like r3/validator/bundle before it) is LOCAL SCRATCH (ephemeral-Postgres tmp), never tracked, not in the repo tree (confirmed via `git ls-files` + `git status`). **Round-5 proven locally:** the SQL classifier and the REAL ssrfGuard.ts agree on all 36 shared literals (ephemeral Postgres for `_mcp_inet_is_public`; Node for `assertPublicHttpUrl`); the F3+F4 pgTAP DO-blocks parse and pass with a stub setter on ephemeral Postgres; `node scripts/mcp-gateway-smoke.mjs` → 220 assertions incl. the F3 ssrfGuard drive + JSON↔pgTAP cross-check + the F4 loader-constant assertion; migration-version / definer-fn / integration-registry lints green. Full pgTAP + §32 proof run in CI `database-contract`. **Round-5 exact-head Codex review (review_id 5262498459, head `b9343825`, Completed 2026-09-21):** CI on `b9343825` — `database-contract` (§32 pgTAP incl. F3/F4) + `contract` + `lint` + `audit` + `web-fetch-hardening-smoke` + Supabase/Vercel previews all GREEN; step #91 (connected MCP gateway smoke) GREEN; `verify` overall failure is the pre-existing inherited baseline `{9,18,28,29,39,74,80}` (view-guard, Secure-Browser contract, pack-lineage, operator-reachability, chat-handler, Test, client-memory) — byte-identical set, confirmed at step level, none this PR's; `github-advanced-security` = the known non-required Copilot-Autofix agent infra crash (log shows the agent's own app.js agentic-loop throwing, exit 1 in ~31s), not a code-scan finding on this diff. **ONE new finding, deferred per the coordinator's terminal rule (strand-class P2, test quality; touches none of caller-authority/tenant-isolation/secret-URL-exposure/consent-bypass/audit-integrity; not P1).** ● **DEFERRED — MUST CLEAR BEFORE GATEWAY WIRING (INT-083 gate):** *"Enumerate auth kinds from the real CHECK constraint"* (Codex P2, `supabase/tests/mcp_gateway_endpoint_setter.sql` F4 accept-set block, [#discussion_r4058774081]). The F4 divergence guard hard-codes the six `auth_kind` values (`oauth,bearer,header,api_key,url,none`) rather than reading them from the live `pg_constraint` CHECK; if a FUTURE migration adds a CHECK-allowed kind that the setter accepts but the loader does not, the loop never exercises it, its accepted set still equals `_expected`, and the loader-constant smoke also still passes — so the pair would not catch that specific future divergence. Correct fix (wiring lane): read the allowed set from `pg_constraint`, or independently assert the fixture equals the live constraint, before presenting the pair as a divergence guard. Why safe to defer now: the live CHECK set is exactly the six enumerated on this head (the setter's shape gate and the table CHECK share that literal), so the guard fully covers today's constraint; the gateway is library-only (no deployed importer, no native rows), so a future setter/loader divergence cannot reach a live connection before wiring, which is owner-first and gated on THIS list being empty. Related standing forward-constraints (loader/probe lane, previously recorded): INT-107 probe TOCTOU (bind+verify endpoint_hash/generation before wiring; threads #discussion_r4058637991 / F1); and "a `pending_verification`+`enabled` row is loader-executable" (gate the loader/`get_mcp_connection_secret` or the probe's `enabled` semantics so a pending connection is non-executable until probed; thread #discussion_r4058263620) — both pre-existing to this setter, both MUST CLEAR BEFORE GATEWAY WIRING. **Round-5→6 post-sync re-review (the conflict→new-head→exact-head re-review path):** origin/main advanced (`c5d396eb`→`6d46ff66`; #1304 PAIGE voice budgets, #1298 dictation, #1305 stale-closeout), forcing a sync at head `26d85151`. Two merge-integration blockers were resolved as part of the sync (neither a Codex finding): (a) **migration version collision** — #1304 shipped `20270329000000_paige_voice_budget_control.sql` at the SAME version as this setter's `20270329000000_mcp_gateway_endpoint_setter.sql` (a duplicate version is silently SKIPPED, never applied, on `db push`), so this setter's migration was **RENAMED to the next free later version `20270330000000_mcp_gateway_endpoint_setter.sql`** and every reference to it updated (pgTAP header, smoke comment, workflow comment, decision-log, integration-registry); main's voice-budget refs left untouched; `lint:migration-versions` clean; (b) conflicts in `paige-spine-contract.yml` (kept BOTH the endpoint-setter pgTAP step and main's voice-budget steps) and `decision-log.md` (kept both entries). The post-sync exact-head Codex re-review of `26d85151` (review_id 5262562824) surfaced a **NEW verified secret-exposure / audit-integrity finding** → **STOP + return to coordinator per the terminal rule** (NOT self-fixed or deferred): `right(_auth_token,4)` returns the WHOLE token when it is ≤4 chars (and 4 of 5 for a 5-char token), and that value flowed to the row `auth_token_last4`, `paige_audit_log` (`auth_token_last4_before/after`), AND the RPC return — so a short custom-server token leaked in plaintext despite the "last4 is non-secret" audit contract. **Round 6 (FINAL, narrow — coordinator-authorized; the STOP was ruled CORRECT):** the fix emits a last4 hint ONLY for a token of length **≥ 12** (trailing 4 chars a negligible non-secret suffix) and **NULL below that** — stronger than the ≤4 cutoff first proposed (a ≤4 cutoff still leaks 4 of 5). The ONE `_new_last4` value (migration `20270330000000_mcp_gateway_endpoint_setter.sql:693`; confirmed no second last4 code path) feeds the row column, the audit after-hint, and the return, so redacting it there redacts it everywhere; both header comments (A1 + the audit-write note) corrected to state the real contract. pgTAP: tokens of length {1,4,5,11} → last4 NULL in the row + audit payload + return AND the full short token appears NOWHERE in the audit payload for that write; {12,40} → `right(token,4)` — proven on ephemeral Postgres (the old rule would have leaked `QRST`/`RSTU`). Did NOT reject short tokens, did NOT build a shared helper, did NOT touch other last4 call sites. ● **INT-111 (coordinator-issued, SEPARATE — NOT this PR):** the last4-hint convention (`right(token,4)` with no length floor) is repo-wide at OTHER call sites; auditing/fixing those is tracked as INT-111, out of INT-099's scope. Round-6 proven locally: the last4 rule + the round-6 pgTAP block on a faithful stub (ephemeral Postgres); `node scripts/mcp-gateway-smoke.mjs` → 220 assertions; migration-version / definer-fn / integration-registry lints green; INT-105 clean (no `_shared`/`mcp-client.ts` edited). Full pgTAP + §32 proof run in CI `database-contract`; gate = exact-head Codex review of the pushed head, merge only if reviewed head == merged head.
- **DICTATION FOLLOW-UPS — INT-086 / PR #1298 terminal exact-head review (2026-09-20; Codex-reviewed head `042c3e65a319be53d337eea192b36b20174d0504`).** Deferred — INT-086 follow-up list: Conversations' existing **Draft with Paige** body-replacement action remains enabled while dictation is active/finalizing, so an in-flight trailing final can be overwritten by the generated draft or appended after it (Codex P2). This is **not a regression versus current main**: main already carries #1290's original interaction, and #1298 is a strict improvement that fences cross-context delivery, preserves resource teardown, surfaces stop/error reasons, holds Send and dashboard alternate actions, and blocks Live Retry teardown. Per the coordinator's terminal rule there is no fourth repair round in #1298. A later single-invariant follow-up should hold every Conversations body-replacing toolbar action until dictation is finalized (or explicitly finalize and await the run) and prove the trailing segment is neither discarded nor mixed into the generated draft. `AUTHENTICATED_RUNTIME` remains UNVERIFIED pending owner production acceptance.
- **Capability Kit PR-1 — `defineCapability()` contract + shrink-only anti-bypass guard (INT-003 / Completion Control Seq 3) — SHIPPED / MERGED under INT-083 (2026-09-20; PR [#1295](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1295), merge commit [`cf2f606c`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/cf2f606cc9eb7be0ab111fe54fe4024bb8059328), last Codex-reviewed head = merged head [`9de3c979`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/9de3c9797d53cd25e80ef87eec7b45b86e374680), conflict-free pre-review rebase onto `63b9a946`; no post-review sync needed; release channel **production** because live Edge artifacts redeployed; classification **internal-only foundation**).** One constructor-issued, immutable contract now defines identity, provider-safe object-root inputs, canonical action-risk/approval, opaque owner-grantable permission key, server-derived tenant/actor seams, canonical per-capability availability, provider binding, idempotency, Rail receipt, and separate six-state execution / three-state evidence outcomes. It creates no grant store, registry, executor, Marketplace authority, Spine projection, or runtime adopter. Caller declarations and schema definitions are copied from own enumerable data into fresh plain snapshots; accessors, Proxies, functions, symbols, non-plain prototypes, cycles, unknown schema keywords, invalid keyword values/ranges, and root combinators are rejected before branding; caller objects are never returned or frozen. Availability is one-home: the kit aliases `PER_CAPABILITY_AVAILABILITY_STATES` from `paige-capability-status`, excluding request-only `no_applicable_capability`, with runtime identity/set equality and compile-time union equality. The TypeScript-AST guard resolves aliases, re-exports (including `NamespaceExport`), namespace imports, object/assignment/parameter destructuring, and preserves occurrence counts; threat model remains accidental bypass in our own edits, with computed/bracket access, returned values, dynamic import, and eval documented as limits. Baseline regenerated against final main: **307/307**, zero delta. Exact-head CI: Capability Kit step **#99**, GREEN; failing steps exactly inherited `{9,18,28,29,39,74,80}`. Local proof: focused kit **24/24**, AST self-test **21/21**, capability-status resolver/render **18/18**, changed-file ESLint clean; local Deno execution **UNVERIFIED** because `deno` is absent (the ratchet correctly failed closed on spawn `ENOENT`). Final auto review on `9de3c979` had no P1 and three P2s; terminal rule applied, each thread replied `deferred — Capability Kit adoption blocker list` and resolved with no further code round. **MUST CLEAR BEFORE PR-2 ADOPTION:** (1) reject sparse arrays so holes cannot evade per-index validation; (2) runtime-validate `identity.humanSurface` as `string | null`; (3) make shrink-baseline debt retirement monotonic so removed allowances cannot return; (4) HARD forward constraint from the declined PR-1 Spine finding — after PR-2's SCR-approved Spine projection exists, no capability ID may be presented as governed or live unless projected into the Spine. PR-2 is plan-first, requires an SCR, and cannot adopt the kit until all four are cleared; PR-2 and PR-3 are NOT started or authorized. **Deployment truth:** the design has zero runtime consumers, no migration, and no intentional consumer behavior change, but editing the canonical shared availability resolver caused `deploy-edge-functions` run [35543138417](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/actions/runs/35543138417) to redeploy **8** existing importers (`crm-command`, `execute-approval`, `paige-ai-chat`, `paige-mcp`, `paige-native-event-dispatch`, `paige-social`, `paige-write-back`, `skill-runner`) and advance `edge-live` `c7e386a3`→`cf2f606c`; therefore the expected zero-function / unmoved-edge claim is FALSE and is not reported as a no-op. `db-live` is truthfully unmoved at `6757e487` (no migration). No customer-facing release.
- **The Solo parity static CI guard (PR-P2, INT-071 census) — SHIPPED / MERGED under owner standing authority INT-083 (2026-09-20; PR [#1299](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1299) MERGED to `main` as [`60e03b72`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/60e03b72593da79e813d66ee91ff0bda8c3604c3); merged branch head [`c04c74bd`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/c04c74bd83d3bade4956b1a2653709530c503618); release channel development (deployment-inert — zero src/supabase diff; the Vercel production build fired on merge but carries no runtime change); verify step #98 after Mind #97, success, failing set EXACTLY the inherited baseline {9,18,28,29,39,74,80}; Vercel production 6558208002 success, build-only).** SP1–SP5 make the census parity facts CI-invariants: exact /solo/* route chain; Solo branch of RequireSetupComplete returns children ONLY (#1277); PAIGE + CommandHub mount unconditionally; no identity branches in solo tree + routing seams (string AND numeric account literals); registry-derived snapshot pins settings/nav/tier-floor with the doctrine required to name EXACTLY the registry destinations. owner_user_id predicate rule DELIBERATELY ABSENT (plan-first access-scope PR). Review under the standing merge gate: auto review 3 P2s fixed (03268bd6) → repeat review 3 more same-class P2s → STOPPED for ruling → ruled COMMENT-ONLY round 2: HEURISTIC LIMITS header records the accidental-drift threat model, the three ruled evasion classes (conditional mounts; aliased navigation; qualified/reversed comparisons), and the two fail-closed fragilities (one-line route pin; SUB-ACCOUNT marker pin). Round-2 review CLEAN; self-test 8/8. **AST follow-up PLANNED, NOT authorized — one SHARED AST helper sequenced by the coordinator for SDK #1295 + Mind PR-A3 + this guard.**
- **Mind three-state evidence contract guard — heuristic limits documented, two residuals fixed (PR-A2) — SHIPPED/MERGED under INT-083 (2026-09-20; PR [#1294](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1294) MERGED to `main` via merge commit [`1190dbd3`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/1190dbd302f26cc61579819964557cbd133757b3); merged branch head `f4a878a6`).** Deployment-inert follow-up to PR-A (#1287) on `scripts/ci/mind-contract-lint.mjs`. FIXED two of #1287's post-merge Codex residuals — ③ `isMindProjectionCandidate` now permits `export [async] function project*MindEvidence` (async second projections are discovered → MC4); ④ `exportedCallableBodies` marks expression-bodied arrows and MC5 accepts a direct `render*MindEvidence(…)` delegate. The three further residuals (①②⑤) are NOT chased here — per owner ruling on this PR they are recorded PLAINLY in the guard header as **KNOWN HEURISTIC LIMITS of a text-based scanner**: ① nested-helper returns can mask a re-deriving exported entry point (MC5 reads the entry body's own returns, not a full top-level-return dataflow); ② generic / nested-paren / brace-bearing signatures may not be extracted; ⑤ string/template-literal braces can unbalance the block slicer. **Threat model** stated in the header: the guard defends against ACCIDENTAL drift from our own edits, NOT adversarial evasion, and claims NO completeness. The **root fix is PR-A3** — an AST-based guard using the repo's `typescript` devDependency (top-level-return walk, balanced-signature + literal-aware parsing); **PLANNED, NOT authorized; must land before R5 (Mind projection generalization).** Merge gate honored: marked Ready → the auto-triggered exact-head Codex review of `f4a878a6` COMPLETED with NO new findings → all 5 original review threads dispositioned on-thread + resolved (③④ fixed; ①②⑤ = known limit → PR-A3) → merged. `ci/verify` failing set EXACTLY the inherited baseline `{9,18,28,29,39,74,80}` with the Mind-guard step GREEN on `f4a878a6`; **zero `supabase/` diff → edge NO-OP, `edge-live` unmoved at `c7e386a3`, no migration**. Local gates: guard + self-test PASS (13 cases), vitest `mind-contract.test.ts` 5/5, tsc ratchet 12→12. Also records the new coordinator MERGE-GATE standing rule (after Ready, wait for the auto exact-head review; disposition every finding; "one review" = one requested + one auto; never merge ahead of either), added to `AGENTS.md` in this same closeout. §4.0 Shipped Delivery Log row added.
- **Connected MCP Gateway · MCP PR-1B-c — INT-082/INT-089: owner_only visibility is caller-authority enforced — SHIPPED / MERGED under owner standing authority INT-083 (2026-09-20; third and last of the three sequential single-invariant PR-1B slices; PR [#1293](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1293) MERGED to `main` via a merge commit as [`6757e487849a1c356b4deed85274d708cdd96a4c`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/6757e487849a1c356b4deed85274d708cdd96a4c); merged branch head [`fd227bf8630244f2a7e06830664382d06a7c3407`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/fd227bf8630244f2a7e06830664382d06a7c3407); Codex-clean code head `cac8e0f7` carried BYTE-IDENTICAL through the base-merge `fd227bf8`; branched off `7747a755`).** Closes the owner_only visibility bypass (#1278 Codex INT-082): `mcp_connections.visibility` ('tenant'|'owner_only') lets an owner mark a connection restricted, and `get_mcp_connections_v2` HIDES it from an ordinary member's list (`_full := auth.uid() IS NULL OR is_tenant_admin(_tenant) OR is_platform_owner()`) — but the runner resolves BY ID and carried no caller role, so a member who learned the id could prepare/execute it. Migration `20270328000000`: (1) `get_mcp_connection_secret` ALSO returns `visibility` — additive, every existing key byte-identical, and it CARRIES BOTH prior changes (INT-079 url exemption `auth_kind NOT IN ('none','url')` + INT-078 `endpoint_hash`) since it is CREATE OR REPLACE'd a 4th time (the url + load-binding pgTAPs re-run against this body and stay green as the proof, plus this PR's pgTAP re-asserts `endpoint_hash` directly); (2) NEW `_mcp_caller_capabilities(_tenant_id, _actor_user_id) → text[]` — the ONE server-side mapping (§18), granting `mcp.connections.use_restricted` to owner + tenant-admin + platform owner today (mirrors `get_mcp_connections_v2` `_full`, MINUS the null-actor bypass — INT-089: no silent service-role grant), service_role-only, EXPLICIT actor (never `auth.uid()`/a request body, §588/§59); a delegated grant is added HERE alone, re-pointing the mapping without touching the runner. Runner (TS, `authority.ts` NEW §18 home for caller authority): `RunnerRequest` gains `callerAuthority` modeled as a CAPABILITY, never a role literal — `{kind:'capabilities', tenantId, capabilities}` (allowed iff it holds `mcp.connections.use_restricted` AND `tenantId` names the loaded connection's tenant — the P2-1 tenant-bind) or `{kind:'system',reason}` (EXPLICIT system authority — a non-empty reason is required; no service-role bypass); the runner refuses `owner_only_forbidden` on prepare AND execute AFTER `foreign_tenant` and BEFORE any approval/dispatch, fail-closed on a missing/unknown authority, and records the system reason on the receipt detail (HONEST CAVEAT §13: the canonical Rail writer `makeCanonicalRailReceipt` DROPS `detail`, so that is best-effort attribution on the receipt object / the operational `mcp_connection_receipts` writer, NOT a persisted Rail field — Rail persistence is a `record_capability_run` contract change out of INT-082 scope). `connection.ts` surfaces `visibility` and fail-closed-normalizes anything-but-'tenant' to `owner_only`. `verify_mcp_connection_approval` is UNCHANGED (visibility is not a consent facet); `mcp-client.ts` is NOT touched. LIBRARY-ONLY — the runner is imported by no deployed function (§37 independent scout re-confirmed: sole producers of the changed contracts are the smoke + the type def + 2 pgTAP; `get_mcp_connection_secret`'s sole TS consumer `connection.ts` reads named keys with no strict validation, so `visibility` is additive-safe; the receipt writer discards `detail`). Proof: NEW pgTAP `supabase/tests/mcp_gateway_owner_only_visibility.sql` (real Postgres, wired into `paige-spine-contract.yml`) — get_secret returns visibility (owner_only + tenant) AND still returns `endpoint_hash`; the `_mcp_caller_capabilities` role matrix: owner→cap, admin→cap, member→∅ (even carrying a GLOBAL staff role — §59 trap), NULL actor→∅ (no service-role bypass), platform owner (super_admin)→cap, other-tenant owner→∅ for this tenant / cap for their own. smoke `mcp-gateway` **145→168** (+23: loader surfaces + fail-closed-normalizes visibility; owner_only refuses a missing/wrong-capability authority on prepare+execute and never dispatches; the restricted capability ALLOWS regardless of role — a delegated grant holder — INT-089; explicit system-with-reason ALLOWS and records the reason; system-without-reason refuses; foreign_tenant refuses BEFORE owner_only; a tenant-visibility connection needs no authority; an owner_only mutation is refused before the approval gate; `makeRpcCapabilityResolver` maps the RPC and fails closed on error) — LOAD-BEARING (proven: neutering the runner's `if (canon.visibility === 'owner_only')` gate flips 7 assertions — connection contacted, false 'prepared', read_observed instead of refused, empty attribution, mutation reaching the approval gate — exit 1; restored → 168). `smoke:mcp-transport` unchanged; migration-version (unique vs origin/main, `20270328000000`) + definer-fn (service_role-only, no anon) lints green; tsc `--skipLibCheck` on the 3 changed modules = 0 errors + the esbuild bundle compiles them (deno unavailable locally — the proxy blocks the deno.land install; these `_shared/mcp-gateway` files are imported by no deployed function, so — consistently with INT-078/INT-079 — the affected-function deno ratchet resolves 0 and does not type-check them in CI either). Registry: INT-082 → CLOSED in the n8n + zapier-mcp entries. Migration version chosen last vs origin/main; edge NO-OP expected (the 3 mcp-gateway TS files match `supabase/functions/**` so `deploy-edge-functions` RUNS, but zero deployed importers → zero deploys, `edge-live` unmoved). Codex exact-head reviews (auto-on-Ready found 2 P2s; the requested re-review CLEAN): **P2-1 (tenant-bind the capability) FIXED** — the capabilities authority now carries `tenantId` and `resolveRestrictedUse` accepts it only bound to the loaded connection's tenant (via `sameId`); `makeRpcCapabilityResolver` returns a tenant-bound authority; smoke 168→169, tenant-bind load-bearing. **P2-2 (durable persistence of the system-authority reason) REASONED-DECLINED** on the thread + flagged to the coordinator as a PR-2 forward-constraint (system path is INT-089-mandated; durable Rail persistence is an out-of-scope `record_capability_run` contract change; runner unwired; §13 caveat documented in code/log/smoke). §39 SHIP. CI at merge (head `fd227bf8`): `ci/verify` failing set EXACTLY the inherited baseline `{9,18,28,29,39,74,80}` (zero new red; smoke step 91 + typecheck + build + migration-version/definer/registry lints green), `PAIGE Spine contract` GREEN (`database-contract` ran the new `mcp_gateway_owner_only_visibility.sql` pgTAP on real Postgres). §32 PERSISTED: `deploy-migrations` run `35534659994` success — the PERSISTED-verify step passed, `20270328000000` recorded on prod, **`db-live` `000c973c`→`6757e487`**; `deploy-edge-functions` run `35534659883` RAN but the affected-function resolver found ZERO deployed importers → Deploy/Record SKIPPED, **`edge-live` unmoved at `c7e386a3`**. PROD OBJECT SELECT of the replaced `get_mcp_connection_secret` body (visibility key) + the new `_mcp_caller_capabilities` remains OWED to a prod-SQL-capable session (headless MCP prod SQL permission-denied). §4.0 Shipped Delivery Log row added in this same closeout.**
- **PAIGE Chat dictation · INT-086 — tap-to-toggle UX repair BUILT + DELIVERED to draft PR [#1290](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1290), proceeding through the guarded INT-083 release gate (2026-09-20; exact code head `e03df45fb09cbc2493119a668eddc7d99ceb2ee6`; current-main integration `7747a755d3e69898b98704232235dac9aca8df10`).** Owner-locked invariant: one tap starts capture and recording persists across pointer release and natural pauses until one tap stops it; microphone capture begins before auth/WebSocket setup and buffers startup PCM so opening words are retained; `Listening` appears only after provider readiness. A 5-minute sustained-silence guard and 30-minute hard cap are forgotten-mic safety exits, both preserving text and stating why capture stopped. Final text inserts at the captured cursor; stop/disable/unmount/route or account change closes tracks, recorder, socket, timers, and fences stale callbacks. Provider stays Deepgram through unchanged `paige-dictate`; ElevenLabs/Scribe, Live Conversation, playback, send-error wording INT-085, `stt-router.ts`, `paige-tts`, `paige-live-session`, `paige-ai-chat/index.ts`, migrations, and customer data are excluded. Failing-first seven assertions RED across the original and review-repair shapes; repaired focused suite **22/22**; pointer-release, stop-during-connect, and composer-caret mutations reintroduce their failures; tsc ratchet **12→12** and build green; four required Solo viewport pairs rendered with zero overflow. `AUTHENTICATED_RUNTIME`, real mic/Deepgram, Safari/macOS, iPhone, zoom/reflow, Vercel production identity and exact-head CI remain owed at this pre-merge record. The single Codex review on `7fe41bc20d93683dcaf04118fc758326c47f020f` returned valid P1 stop-during-connect and P2 caret-preservation findings; both were fixed failing-first in `e03df45fb09cbc2493119a668eddc7d99ceb2ee6`, with no second review requested per the coordinator rule.
- **Solo Owner/Team label from authoritative membership — INT-071 census repair, presentation only — SHIPPED / MERGED + VERCEL PRODUCTION DEPLOYED under owner standing authority INT-083 (2026-09-20; PR [#1285](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1285) MERGED to `main` as [`7747a755d3e69898b98704232235dac9aca8df10`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/7747a755d3e69898b98704232235dac9aca8df10); merged branch head [`b1d52508`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/b1d52508bdc992d3402fd1c3a0cf394ca027814e); code head [`d4f80150`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/d4f80150ec142d57533b9b9a5776952f818d0023); Vercel production `dpl_264TRrMvSgeAtJrHeATEkYr5rbQJ` coordinator-verified READY; NO edge or migration deploy; zero `supabase/` diff; CI gate at merge: `ci/verify` failing steps EXACTLY the inherited baseline {9,18,28,29,39,74,80} — zero new red vs `main` — with #83 green and `Validate UI delivery evidence`/`Security Audit`/`web-fetch-hardening-smoke` green; only other red the non-required Code-scanning platform crash).** The Solo shell's Owner/Team presentation derives from authoritative membership ownership — the tenant+user-keyed `has_tenant_role` 'owner' verdict over `tenant_members` is_owner / role='owner' — never from the display-only `tenants.owner_user_id` pointer (census: Owner seats on tenants `7e700000…` and `d8a0a880…` were shown "Team workspace"). The existing one-pair-per-tenant+user probe runs whenever the shell identity is resolved and carries owner/admin verdicts separately (owner → label; owner OR admin → the unchanged Setup-notice CTA half); fails to Team while unresolved or keyed to a prior identity. Authorization untouched (`solo_setup_access_scope()`, `isPrimaryOwner`, RLS — separate plan-first seam). Notice 26/26 (7 failing-first), journey 16/16, 4 load-bearing mutations, eslint + tsc ratchet clean, UI-evidence green with `FLOW_BY_FLOW: WAIVED` (#1280 grammar; skill truly not installed) and `MATERIAL_FLOW_CHANGE: NO` (presentation-only per the paige-ui-design material-flow test). **AUTHENTICATED_RUNTIME + 8 viewports UNVERIFIED — owed after deploy (owner of a pointer-mismatched tenant sees Owner workspace; admin/member see Team; account switch resolves Team-first).**
- **SPINE CHANGE REQUEST — the three-state Mind evidence contract is now a CI-guarded, deployment-inert invariant (PR-A) — BUILT, pending merge under INT-083 (2026-09-20).** The Mind surface (the tenant-scoped 3D mind feed and the Chat handler that shares its projection) must resolve everything it presents to exactly `recorded | no_evidence | unavailable`, from ONE projection home (`supabase/functions/_shared/paige-spine/mindEvidence.ts`), fail-closed, with honest absence wording — the honesty invariant that stops a rendered surface from implying activity that did not happen (§13/§00). PR-A codifies that as a structural guard: new `scripts/ci/mind-contract-lint.mjs` (`lint:mind-contract` + `:test` self-test), one appended-LAST step in the `verify` job of `.github/workflows/ci.yml`, fixtures under `scripts/fixtures/mind-contract/`, and `src/__tests__/mind-contract.test.ts` (five failing-first tests: MC1 canonical union · MC2 fail-closed projector · MC3 honest absence copy · MC4 one projection home / a second needs an SCR · MC5 Chat is a caller not a re-author). The RULING it records: **generalising the Mind projection beyond its one home is a shared-primitive change that requires a new Spine Change Request** (declared via `sharedPrimitiveChange: "SCR-<ref>"` or `// mind-projection-scr: <ref>`), never a silent import (§18). Scope is deployment-inert by construction — **zero edit to any `supabase/` file** (so no edge bundle changes and `paige-ai-chat`, owned by INT-086, is untouched), no `src/solo`, no migration; only CI/test tooling. Guard self-test PASS (all five planted violations caught, compliant fixtures clean); guard PASS on the live contract; failing-first proven both ways (neutered guard → all five tests + self-test RED; five real-contract breaks → guard exits 1 with exactly MC1–MC5, restored clean). Merge SHA + the Shipped Delivery Log row to be recorded on merge.
- **Connected MCP Gateway · MCP PR-1B-b — INT-078: consent is bound to the LOADED endpoint — BUILT + DELIVERED to a draft PR, going to merge under owner standing authority INT-083 (2026-09-20; second of the three sequential single-invariant PR-1B slices, branched off current main `64521c5f96e9c2c34554e282a124243c99827a7c`).** Closes the load↔verify TOCTOU (#1262 Codex R1 P1): the runner VERIFIES consent by `connection_id` (`verify_mcp_connection_approval`) but DISPATCHES to the endpoint the loader resolved (`get_mcp_connection_secret`). Verify re-derived only the connection's CURRENT endpoint, so a session that LOADED endpoint A could be authorized by an approval matching the row's endpoint at verify time (B) and then dispatch to A — consent for one endpoint authorizing a call to another; the 20270322000000 delete-trigger + the writer's FOR UPDATE serialize the common case but not a load performed before a re-point that verify sees after. Migration `20270327000000`: (1) `get_mcp_connection_secret` ALSO returns `endpoint_hash` = `_mcp_endpoint_hash(the same decrypted endpoint it returns as server_url)` — additive, every existing key byte-identical, and it CARRIES PR-1B-a's url exemption (`auth_kind NOT IN ('none','url')`) since it is CREATE OR REPLACE'd again (the url pgTAP re-runs against this body and stays green as the proof); (2) `verify_mcp_connection_approval` gains `_loaded_endpoint_hash` as a REQUIRED (non-defaulted 4th) parameter — NULL/empty → `loaded_endpoint_hash_required`, and the approved endpoint must equal the LOADED one → else `endpoint_load_mismatch`; the pre-existing CURRENT-endpoint re-derive stays as defense in depth (`endpoint_changed`). The 4-arg signature is DROPPED (`_args_shape_hash` moves to 5th, defaulted); §37 (independent scout confirmed): the ONLY real callers were `consent.ts` (`makeRpcApprovalVerifier`, named-arg) and the durable-consent pgTAP (8 positional call sites) — BOTH moved to the 5-arg signature in this PR; no view/trigger/function/policy depends on it, so the DROP is dependency-safe; the loader (`connection.ts`) surfaces `endpointHash` and refuses a configured+enabled row lacking a well-formed one; `runner.ts` passes `canon.endpointHash` through as `loadedEndpointHash`. LIBRARY-ONLY — the runner is imported by no deployed function; `mcp-client.ts` is NOT touched (transport smoke unchanged). Proof: NEW pgTAP `supabase/tests/mcp_gateway_endpoint_load_binding.sql` (real Postgres, wired into `paige-spine-contract.yml`) — get_secret returns `endpoint_hash`; loaded=approved=current → authorized; loaded≠approved WHILE current=approved → `endpoint_load_mismatch` (LOAD-BEARING — the pre-INT-078 verify returned authorized here); moved row → `endpoint_changed`; NULL/empty loaded → `loaded_endpoint_hash_required`; the durable-consent pgTAP updated to the 5-arg signature and re-runs green; smoke `mcp-gateway` **138→145** (loader surfaces endpointHash + refuses missing/malformed; a mutation whose loaded endpoint equals the approved one executes and passes the loaded hash to consent; a repoint-window mutation is refused `endpoint_load_mismatch` and never dispatched) — LOAD-BEARING (proven: removing the runner's `loadedEndpointHash` thread flips the new assertions and every downstream mutation-consent assertion to `loaded_endpoint_hash_required`, exit 1). `smoke:mcp-transport` **84** unchanged; migration-version (unique vs origin/main) + definer + integration-registry (real+self-test) + ci:regression lints green. Registry: INT-078 → CLOSED in the n8n + zapier-mcp entries; INT-082 → MCP PR-1B-c (owner INT-089 addendum 2026-09-20: PR-1B-c models the runner's caller authority as a server-resolved CAPABILITY, e.g. `mcp.connections.use_restricted`, owner+admin in today's mapping, extensible by mapping-only; `owner_only` stays the owner's per-connection choice, docs-rename only). Migration version chosen last vs origin/main; edge NO-OP expected (no `supabase/functions/**` bundle deploy path — the runner is unwired; `edge-live` unmoved); `deploy-migrations` persists `20270327000000`. PROD OBJECT SELECT of the replaced function bodies remains OWED to a prod-SQL-capable session (headless MCP prod SQL permission-denied). §4.0 Shipped Delivery Log row added post-merge (needs the merge SHA + deploy evidence).
- **Connected MCP Gateway · MCP PR-1B-b — INT-078: consent is bound to the LOADED endpoint — SHIPPED / MERGED under owner standing authority INT-083 (2026-09-20; second of the three sequential single-invariant PR-1B slices, branched off `64521c5f`; PR [#1288](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1288) MERGED to `main` via a merge commit as [`000c973c3064b4a549fb48858d0c5c1d314c4471`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/000c973c3064b4a549fb48858d0c5c1d314c4471); merged branch head [`8f50d5954a7cce75488116ed6fde1a1aa957f18c`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/8f50d5954a7cce75488116ed6fde1a1aa957f18c); §39 SHIP + Codex CLEAN on the code (carried across two base-merges of a moving main + a test-fixture fix, all code byte-identical); `deploy-migrations` run 35528707625 PERSISTED `20270327000000` — prod `schema_migrations` caught up, `db-live` `82e27270`→`000c973c` zero drift; `deploy-edge-functions` run 35528707661 RAN (the 3 mcp-gateway library files matched `supabase/functions/**`) but the affected-function resolver found ZERO deployed importers, so the deploy/record steps were SKIPPED — zero functions redeployed, `edge-live` unmoved at `c7e386a3`).** Closes the load↔verify TOCTOU (#1262 Codex R1 P1): the runner VERIFIES consent by `connection_id` (`verify_mcp_connection_approval`) but DISPATCHES to the endpoint the loader resolved (`get_mcp_connection_secret`). Verify re-derived only the connection's CURRENT endpoint, so a session that LOADED endpoint A could be authorized by an approval matching the row's endpoint at verify time (B) and then dispatch to A — consent for one endpoint authorizing a call to another; the 20270322000000 delete-trigger + the writer's FOR UPDATE serialize the common case but not a load performed before a re-point that verify sees after. Migration `20270327000000`: (1) `get_mcp_connection_secret` ALSO returns `endpoint_hash` = `_mcp_endpoint_hash(the same decrypted endpoint it returns as server_url)` — additive, every existing key byte-identical, and it CARRIES PR-1B-a's url exemption (`auth_kind NOT IN ('none','url')`) since it is CREATE OR REPLACE'd again (the url pgTAP re-runs against this body and stays green as the proof); (2) `verify_mcp_connection_approval` gains `_loaded_endpoint_hash` as a REQUIRED (non-defaulted 4th) parameter — NULL/empty → `loaded_endpoint_hash_required`, and the approved endpoint must equal the LOADED one → else `endpoint_load_mismatch`; the pre-existing CURRENT-endpoint re-derive stays as defense in depth (`endpoint_changed`). The 4-arg signature is DROPPED (`_args_shape_hash` moves to 5th, defaulted); §37 (independent scout confirmed): the ONLY real callers were `consent.ts` (`makeRpcApprovalVerifier`, named-arg) and the durable-consent pgTAP (8 positional call sites) — BOTH moved to the 5-arg signature in this PR; no view/trigger/function/policy depends on it, so the DROP is dependency-safe; the loader (`connection.ts`) surfaces `endpointHash` and refuses a configured+enabled row lacking a well-formed one; `runner.ts` passes `canon.endpointHash` through as `loadedEndpointHash`. LIBRARY-ONLY — the runner is imported by no deployed function; `mcp-client.ts` is NOT touched (transport smoke unchanged). Proof: NEW pgTAP `supabase/tests/mcp_gateway_endpoint_load_binding.sql` (real Postgres, wired into `paige-spine-contract.yml`) — get_secret returns `endpoint_hash`; loaded=approved=current → authorized; loaded≠approved WHILE current=approved → `endpoint_load_mismatch` (LOAD-BEARING — the pre-INT-078 verify returned authorized here); moved row → `endpoint_changed`; NULL/empty loaded → `loaded_endpoint_hash_required`; the durable-consent pgTAP updated to the 5-arg signature and re-runs green; smoke `mcp-gateway` **138→145** (loader surfaces endpointHash + refuses missing/malformed; a mutation whose loaded endpoint equals the approved one executes and passes the loaded hash to consent; a repoint-window mutation is refused `endpoint_load_mismatch` and never dispatched) — LOAD-BEARING (proven: removing the runner's `loadedEndpointHash` thread flips the new assertions and every downstream mutation-consent assertion to `loaded_endpoint_hash_required`, exit 1). `smoke:mcp-transport` **84** unchanged; migration-version (unique vs origin/main) + definer + integration-registry (real+self-test) + ci:regression lints green. Registry: INT-078 → CLOSED in the n8n + zapier-mcp entries; INT-082 → MCP PR-1B-c (owner INT-089 addendum 2026-09-20: PR-1B-c models the runner's caller authority as a server-resolved CAPABILITY, e.g. `mcp.connections.use_restricted`, owner+admin in today's mapping, extensible by mapping-only; `owner_only` stays the owner's per-connection choice, docs-rename only). CI gate at merge (head 8f50d595): `ci/verify` failing set EXACTLY the inherited baseline `{9,18,28,29,39,74,80}` (zero new red; the #1287 `mind-contract` step 97 and my registry/typecheck/build/gateway-smoke steps all green); `PAIGE Spine contract` GREEN — its `database-contract` job ran the new `mcp_gateway_endpoint_load_binding.sql` (step 34), the 5-arg durable-consent pgTAP (32), the url-auth pgTAP (33) and the revokes/consent-race proofs all on real Postgres. CI CAUGHT a real test-fixture bug first (the load-binding fixture defaulted `auth_kind='bearer'` so `get_mcp_connection_secret` returned `configured:false`; fixed to `auth_kind='none'` — the migration itself was proven correct by the same run's durable/url/revokes pgTAPs). **Edge NO-OP is effective, not path-based**: the 3 mcp-gateway TS files DO match `supabase/functions/**`, so `deploy-edge-functions` runs, but zero deployed functions import them → zero deploys, `edge-live` unmoved. PROD OBJECT SELECT of the two replaced function bodies (`get_mcp_connection_secret`, `verify_mcp_connection_approval`) remains OWED to a prod-SQL-capable session (headless MCP prod SQL permission-denied). §4.0 Shipped Delivery Log row added in this same closeout.
- **SPINE CHANGE REQUEST — the three-state Mind evidence contract is now a CI-guarded, deployment-inert invariant (PR-A) — SHIPPED/MERGED under INT-083 (2026-09-20).** The Mind surface (the tenant-scoped 3D mind feed and the Chat handler that shares its projection) must resolve everything it presents to exactly `recorded | no_evidence | unavailable`, from ONE projection home (`supabase/functions/_shared/paige-spine/mindEvidence.ts`), fail-closed, with honest absence wording — the honesty invariant that stops a rendered surface from implying activity that did not happen (§13/§00). PR-A codifies that as a structural guard: new `scripts/ci/mind-contract-lint.mjs` (`lint:mind-contract` + `:test` self-test), one appended-LAST step in the `verify` job of `.github/workflows/ci.yml`, fixtures under `scripts/fixtures/mind-contract/`, and `src/__tests__/mind-contract.test.ts` (five failing-first tests: MC1 canonical union · MC2 fail-closed projector · MC3 honest absence copy · MC4 one projection home / a second needs an SCR · MC5 Chat is a caller not a re-author). The RULING it records: **generalising the Mind projection beyond its one home is a shared-primitive change that requires a new Spine Change Request** (declared via `sharedPrimitiveChange: "SCR-<ref>"` or `// mind-projection-scr: <ref>`), never a silent import (§18). Scope is deployment-inert by construction — **zero edit to any `supabase/` file** (so no edge bundle changes and `paige-ai-chat`, owned by INT-086, is untouched), no `src/solo`, no migration; only CI/test tooling. Guard self-test PASS (all five planted violations caught, compliant fixtures clean); guard PASS on the live contract; failing-first proven both ways (neutered guard → all five tests + self-test RED; five real-contract breaks → guard exits 1 with exactly MC1–MC5, restored clean). **MERGED** via a merge commit to `main` as [`16b3856752707b5c7916e4a5bafc426f00d0e761`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/16b3856752707b5c7916e4a5bafc426f00d0e761) (merged branch head `34f6fb9e`); `ci/verify` failing steps EXACTLY the inherited baseline `{9,18,28,29,39,74,80}` with the new LAST step green; zero `supabase/` diff → no `deploy-edge-functions` run, `edge-live` unmoved at `c7e386a3`, no migration. Codex reviewed `4b3610ab` (6 findings, 1 P1 + 5 P2, all fixed in `34f6fb9e`); the merged head was NOT re-reviewed before merge — an auto-triggered second Codex review of `34f6fb9e` completed post-merge and surfaced 3 narrower guard-completeness residuals (P1 export-const projection detection, P2 canonical-path-exists check, P2 Chat-export-body scoping), tracked for a follow-up guard-hardening PR — not a shipped-contract defect (the guard passes on the live contract). §4.0 Shipped Delivery Log row added.
- **Connected MCP Gateway · MCP PR-1B-a — INT-079: a url-auth connection is CONFIGURED — SHIPPED / MERGED under owner standing authority INT-083 (2026-09-20; single-invariant PR of the three sequential PR-1B slices, branched off current main `ddbccbb2787e99e8b57a7017d27688fc4bfd3109`; PR [#1284](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1284) MERGED to `main` via a merge commit as [`82e27270c3e6abb0fbd83d4a9179c8ec1b42e110`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/82e27270c3e6abb0fbd83d4a9179c8ec1b42e110); merged branch head [`25a39a37ed583ff399c740c661e270a01f19c685`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/25a39a37ed583ff399c740c661e270a01f19c685); Codex-clean code head `f7cd4a14` carried by the comment-only `25a39a37`; `deploy-migrations` run 35524319424 PERSISTED `20270326000000` — prod `schema_migrations` caught up ("every local version is recorded remotely"), `db-live` `27488aa8`→`82e27270` zero drift; edge NO-OP — no `supabase/functions/**` change, no `deploy-edge-functions` run on `82e27270`, `edge-live` unmoved at `c7e386a3`).** `get_mcp_connection_secret` (mig 20270319000000) refused an `auth_kind='url'` row (Zapier's credential-in-URL scheme; the credential is IN the endpoint, so BOTH token columns are legitimately null) as `configured:false`, because the null-token guard exempted only `auth_kind='none'`. Migration `20270326000000` exempts `url` alongside `none` — `_row.auth_kind NOT IN ('none','url')` — so a url row whose `server_url_ct` is present resolves `configured:true`; a bearer/header/oauth row with both tokens null still fails closed; the endpoint requirement and every other branch are byte-identical (additive, only widens a false-refuse). The loader already allow-lists `url` (`MCP_EXECUTABLE_AUTH_KINDS`) and `authFromSecret` maps `url`→`{kind:'none'}` (credential already in the URL, no header added), so this closes the RPC-side false-refuse end-to-end with NO code change to `connection.ts`/`mcp-client.ts`. Proof: pgTAP `supabase/tests/mcp_gateway_url_auth_configured.sql` (real Postgres, wired into `paige-spine-contract.yml`) — url→`configured:true`/`auth_kind='url'`/endpoint decrypted, disabled url→`enabled:false`, bearer-no-token→`configured:false`, url-no-endpoint→`configured:false`; smoke `mcp-gateway` **136→138** (loader resolves a url row `ok:true` with `{kind:'none'}` auth; runner `read_observed`) — LOAD-BEARING (proven: dropping `url` from `MCP_EXECUTABLE_AUTH_KINDS` flips both new assertions to `connection_unusable`, count 138→136, exit 1). `smoke:mcp-transport` **84** unchanged (not edited); migration-version + definer + integration-registry (real+self-test) lints green. LIBRARY-ONLY — the runner is imported by no deployed function; `mcp-client.ts` is NOT touched. INT-078 → MCP PR-1B-b, INT-082 → MCP PR-1B-c (sequential). Migration version chosen last vs origin/main (`lint:migration-versions` green). CI gate at merge: `ci/verify` failing steps EXACTLY the inherited baseline `{9,18,28,29,39,74,80}` (zero new red vs `main`; #8 regression/#46-47 registry/#83 knowledge-scope/#91 gateway-smoke/#33-34 migration-version/#71 typecheck/#72 build all green), `PAIGE Spine contract` GREEN (ran the new pgTAP), `migration-lint`/`ui-delivery-evidence`/`Security Audit` GREEN; the only other red is the non-required `Code scanning AI findings` (Copilot Autofix infra crash, empty output). PROD OBJECT SELECT of the replaced function body remains OWED to a prod-SQL-capable session (headless MCP prod SQL permission-denied — re-confirmed on a READ attempt this session: `list_projects` empty, `execute_sql` on the prod ref returns "You do not have permission to perform this action"). §4.0 Shipped Delivery Log row added in this same closeout.
- **Solo Setup is not an access gate — owner-adjudicated canonical access repair — SHIPPED / MERGED + VERCEL PRODUCTION DEPLOYED under owner standing authority INT-083 (2026-09-20; PR [#1277](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1277) MERGED to `main` as [`23197918c8bda50f8ea0f3c8d2a1d5ea033bfeb8`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/23197918c8bda50f8ea0f3c8d2a1d5ea033bfeb8); merged branch head [`a918434dbcbae861f89b2e52b559f4aee8a645e4`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/a918434dbcbae861f89b2e52b559f4aee8a645e4); attested code head `a68503fa` — six non-evidence files byte-identical; Vercel deployment `dpl_FFuDb8DsCrmP8HEHUP3HxD2AUt7B` Production success; NO edge or migration deploy — zero `supabase/` diff).** `RequireSetupComplete` solo tier never redirects and renders children ONLY; the dismissible `SoloSetupReadinessNotice` lives inside SoloApp's height-owned paige-solo column (tenant+user-keyed dismissal; CTA gated to owner/membership-admin; sub-account redirect and outer guards byte-equivalent). Owner rulings of 2026-09-20 (INT-083/INT-084) recorded in the evidence record via the #1280 grammar: `FLOW_BY_FLOW: WAIVED` (skill not installed in the delivery environment), `MATERIAL_FLOW_CHANGE: YES`, `FLOW_PROTOTYPE: WAIVED` (Setup must never hold up a new user; PAIGE collects business context through Chat). Journey 16/16, notice 19/19, matrix 15/15; mutation pins load-bearing; verify red inherited-only ({9,18,28,29,39,74,80} identical to `c7e386a3`), #83 green, UI-evidence green with the WAIVED values; Codex on `284b1019` = one P2 evidence-wording finding, fixed once as `a918434d`. **AUTHENTICATED_RUNTIME + the eight SOLO_* viewports UNVERIFIED — the owner's post-deploy acceptance act (trapped account → Command Center → Business Game Plan with notice and PAIGE Chat reachable; known-good account → no notice; 8-viewport matrix to be driven).**
- **Paige Chat tool contract repair (INT-080 production Chat outage) — SHIPPED / MERGED + DEPLOYED under owner standing authority INT-083 (2026-09-20; PR [#1281](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1281) squash-merged to `main` as [`c7e386a38cb7a9288bec8e874144d79eceb50d3b`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/c7e386a38cb7a9288bec8e874144d79eceb50d3b); six files incl. the int-080 evidence record; `deploy-edge-functions` run `35516213796`, `edge-live = c7e386a3`).** The INT-080 chat tool contract repair across `paige-ai-chat` (`_shared/claude.ts`, `_shared/crm-command/catalog.ts`), its adoption test, and two CI scripts. **Production verified on tenant `3062585`, trace `1e36dca8`; trace `3212141` confirmation PENDING** (the coordinator-verified facts; the #1281 agent return package had not been received at closeout). No customer release note recorded.
- **UI delivery evidence governance — owner-waiver (WAIVED) grammar — SHIPPED / MERGED under owner standing authority INT-083; resolves INT-084 (2026-09-20; PR [#1280](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1280) MERGED to `main` as [`9d174268c4587b722e89afc2da6e62acc33301ef`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/9d174268c4587b722e89afc2da6e62acc33301ef); merged branch head [`79374300`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/79374300bce92955e9885700a8b02b26927943ce) — four files byte-identical to the Codex-clean head [`0c2dbf64`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/0c2dbf645c4d4d30933cdae4794065a1d0557039); governance only — no deployable code).** The validator lets a record truthfully state the owner WAIVED a gate instead of forcing a false PASS: `FLOW_BY_FLOW` accepts `WAIVED: owner-decision=<recognizable reference>; reason=<substantive reason>`; `FLOW_PROTOTYPE` only under `MATERIAL_FLOW_CHANGE: YES:`; `WAIVED` invalid everywhere else. Fail-closed eligibility: unresolved-token rejection, recognizable reference + ≥4 distinct words, filler rejection, whole-reason negation sweep, predicate BOUND to the Flow-by-Flow mention, universal availability veto (temporal/adjectival "present" exempt). Suite 75/75 (from 33); Codex "Didn't find any major issues" on `0c2dbf64`; verify red inherited-only, #83 green, UI-evidence green. This is the grammar under which #1277's waivers are recorded.- **Connected MCP Gateway · MCP PR-1 — canonical connection single-source (server/library only) — SHIPPED / MERGED under owner standing authority INT-083 (2026-09-20; small-PR train, one invariant; PR [#1278](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1278) opened off `main` `27488aa8c28852c7958211bacdf03b98624f40b7`, re-grounded onto `3079bdae719757d746aec6199f31686c596196e7`, then synced to current `main` `ca10e5d577a46b007f532b82be3c6ec026a5c136` (#1267) via a merge commit and MERGED as [`8324100fea60b159738e39a774c0598aa4b3de7a`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/8324100fea60b159738e39a774c0598aa4b3de7a); merged branch head `88f0d84102d2c05ee93ce58f43a54ba0ee6ede37`; Codex-clean code head `954ddb5fdf9b7e9057b850127b414f09aa60cf17` (byte-identical seven files after the sync); `deploy-edge-functions` run 35486262926 success, `edge-live = 8324100f` zero drift, 4 functions redeployed via `_shared/mcp-client.ts` — call-zapier-action, paige-ai-chat, tenant-mcp-connect, tenant-n8n-oauth — runner still imported by no deployed function; MCP NOT customer-live; post-deploy runtime-error log inspection OWED (prod function logs permission-denied to the headless merging session; no provider call made); PR-2 NOT started).** Closes the #1268 forward-risk: the runner used to VERIFY consent against the connection row (by `connection_id`) but DISPATCH to a `serverUrl` the caller supplied on the request — two independent inputs, so a caller could verify against endpoint A and dispatch to B. Fix: the runner no longer accepts a URL. `RunnerRequest` now carries only `connectionId` + the caller's server-derived `tenantId`; a new `deps.loadConnection` (production `makeRpcConnectionLoader` over the existing service-role `get_mcp_connection_secret`, `_shared/mcp-gateway/connection.ts`, reusing `authFromSecret` §18) resolves the ONE canonical row, and BOTH dispatch (`canon.serverUrl`/`auth`) and consent (`verify_mcp_connection_approval` by the same `connectionId`) derive from it. §9: `get_mcp_connection_secret` is tenant-agnostic, so the runner refuses `foreign_tenant` when the row's tenant ≠ the caller's; missing → `no_connection`, disabled → `connection_disabled`, unusable → `connection_unusable`, no loader wired → fail closed; and (§39 defense-in-depth) a loader that resolves to a row for a DIFFERENT `connection_id` than the caller named is refused `connection_mismatch`, so the invariant is loader-independent (consent id === dispatch row id). §37: sole producer of the old request shape was the smoke (runner imported by no deployed function). No migration, no Chat, no endpoint-setter, no live-provider execution. Governance (coordinator amendment 2026-09-20): the Integration Capability Registry (`docs/integration-registry/integration-capability-registry.json`, the amended 7th file of this PR) updates the n8n + Zapier outbound-worker entries' governance truth to name `mcp_connections`/`get_mcp_connection_secret` as the canonical PR-1 outbound connection-loader seam (backfilled 1:1 from `tenant_mcp_connections` + `tenant_n8n_connections`, still the sole LIVE path; library-only; zero deployed importer; INT-078/INT-079 OPEN for PR-1B before the endpoint setter; READ-only is a staged certification milestone, the final gateway architecture is MUTATION-CAPABLE and governed) — no status/lane elevation, nothing claimed connected/live because the loader exists (AGENTS.md delivery rule; `lint:integration-registry` real + self-test green). `prepare` now validates the connection (existence + id + tenant) BEFORE affirming `prepared` — a foreign/missing connection in prepare mode is refused, never a false success (Codex R1 P2). Both identity comparisons (connection_id, tenant) are by `sameId` (canonical-UUID equality for valid UUIDs via a VALIDATING `canonicalUuid` — only a value matching PostgreSQL's own `uuid` grammar — 32 hex in eight 4-digit groups with an optional hyphen at any of the 7 boundaries (canonical, hyphenless, fully-hyphenated, braced, any case), else null; exact non-stripping compare otherwise), so a valid UUID named in ANY accepted spelling (uppercase, hyphenless, brace-wrapped) is not falsely refused as a mismatch/foreign_tenant, while a MALFORMED value (a garbage-prefixed alias like `zzabcdef…`, an all-non-hex string) is REFUSED — never stripped down to a key that collides with a valid UUID (Codex R2 P2 + the braced/hyphenless follow-up + the 304ce3c6 malformed-rejection follow-up: the earlier strip-only reducer equated `zz`+canonical with canonical, defeating the loader-independent guard; + the 6a289b48 hyphen-layout follow-up: a permissive hex+hyphens-anywhere parser still let a stray-hyphen value (`-`+32hex, 32hex+`-`, wrong-group hyphens) strip to the canonical key, so the LAYOUT was validated before any hyphen removal; + the b907d1b4 grammar-completion follow-up: the two-layout-only validator then FALSELY REFUSED PostgreSQL's valid alternative groupings (hyphen after every 4-digit group), so `canonicalUuid` now mirrors Postgres's `uuid` grammar exactly — 32 hex in eight 4-digit groups with an optional hyphen at any of the 7 boundaries — accepting every DB-valid spelling while still rejecting non-hex / wrong-length / leading / trailing / mid-group / doubled hyphens; proven by a standalone grammar battery). A listed connection the MCP client cannot execute — a non-`http` transport (`sse`/`stdio`) or the n8n REST `api_key` facet (backfilled from `tenant_n8n_connections`, REST base URL) — is refused `connection_unusable` BEFORE `ok:true` (the loader allow-lists transport + auth kind; the RPC already returns both, so no migration), so prepare cannot affirm and execute cannot contact it (Codex R3 P2). An OAuth row whose access token has already EXPIRED, and a custom-header facet whose name is reserved/invalid (accepted by `authFromSecret` but rejected by `authHeaders` at dispatch), are likewise refused `connection_unusable` before `ok:true` (Codex R4 P2-A/P2-B; §18 shared `authUsable`/`isUsableHeaderName` helper in `mcp-client.ts` — the loader and `authHeaders` read the same regex + reserved set; no refresh/rotate write, deferred; the expiry refusal is scoped to `auth_kind='oauth'` so a connection switched off oauth carrying a stale `access_token_expires_at` is not falsely refused — Codex R5 P2). `smoke:mcp-gateway` **136 assertions** (single-source proofs: valid resolves; foreign/missing/disabled/no-loader/id-mismatch refuse; a caller-smuggled `serverUrl` is ignored and the rogue endpoint is never contacted; consent + dispatch key off the same canonical id; stale endpoint → `endpoint_changed`; prepare validates before affirming; a hyphenless/brace-wrapped connection_id/tenant that canonicalizes to the row is NOT falsely refused, and a malformed (garbage-prefixed OR stray-hyphen, non-UUID) connection_id/tenant IS refused connection_mismatch/foreign_tenant — never stripped to a colliding key — AND a PostgreSQL-valid alternative grouping (hyphen after every 4-digit group) that canonicalizes to the row is NOT falsely refused (load-bearing: the strip-only and hex+hyphens-anywhere reducers flip the rejections to a false pass, and a two-layout-only validator flips the grouping to a false connection_mismatch). **Codex on bba9d5d3 (final bounded code round, coordinator-authorized) — two P2s FIXED:** (a) a row declaring `auth_kind='header'` with a null/empty/blank `auth_header_name` falls through `authFromSecret` to BEARER, so the LOADER now refuses it `connection_unusable` (never sends the credential as `Authorization` instead of the custom header) — the shared missing-name→bearer fallback in `mcp-client.ts` is UNCHANGED (pinned by `smoke:mcp-transport`, §37); (b) `sameId` compares a non-UUID (opaque) identifier EXACTLY — case-sensitive, no lowercase/trim — so distinct opaque ids differing only in case (`CONN/FOO` vs `conn/foo`) are not equated (a lowercasing fallback let consent verify one id while dispatch resolves another). +7 load-bearing proofs (129→136); `smoke:mcp-transport` unchanged at 84; the executable-facet gate refuses api_key/stdio/sse, reserved (incl. the transport-owned `Mcp-Session-Id`, added after a fresh-main Codex round) / invalid custom-header names, and expired-OAuth tokens at loader AND runner level with zero dispatch, and ADMITS a public tokenless `auth_kind='none'` server as executable (Codex P2 follow-up on 57028d1a — a schema-supported, RPC-configured, client-supported kind was falsely refused; fix in `authFromSecret` (`none`→`{kind:'none'}` before the token guard) + the allow-list, no migration; load-bearing: dropping `none` from either refuses it), exercising the real `makeRpcConnectionLoader`); load-bearing mutations — letting a caller URL override fails the "rogue never contacted" proof, and widening the executable allow-list to admit api_key/stdio flips every refusal (8 assertions fail, endpoint contacted); both restored. **§39 adversarial peer-gate + Codex R1: NO BLOCKER/MAJOR; findings dispositioned** — NIT-1 → the `connection_mismatch` guard; Codex R1 P2 (prepare validation) → FIXED; Codex R2 P2 (case-insensitive connection_id/tenant) → FIXED; Codex R3 P2 (executable-facet gate — reject non-`http` transports + the n8n REST `api_key` facet before `ok:true`) → FIXED; Codex R4 P2-A (refuse expired-OAuth tokens) + P2-B (refuse reserved/invalid custom-header names via §18 shared `authUsable`) → FIXED; Codex R5 P2 (scope the expiry refusal to `auth_kind='oauth'`) → FIXED; **RETURNED to the coordinator; coordinator RULED (2026-09-19) both belong in MCP PR-1B — migration-backed canonical connection/consent contract closure, which comes AFTER #1278 and BEFORE PR-2 (endpoint-setter), NOT PR-2. (1) INT-078 (Codex R1 P1)**: bind consent to the LOADED endpoint VERSION/hash, not just the id — an in-flight-repoint TOCTOU where a session opened to endpoint A can be authorized by an approval re-created on endpoint B; needs migrations to `get_mcp_connection_secret` (return the endpoint hash) + `verify_mcp_connection_approval` (require the loaded hash), beyond this bounded "load from one row" PR and beyond the owner's PR-1 proof list; runner unwired so not exploitable today, and #1268's revoke covers the common repoint case. **(2) INT-079 (Codex R6 P2)** — `get_mcp_connection_secret` refuses `auth_kind='url'` (Zapier, credential-in-URL, both token columns null) rows as `configured:false` because only `none` is token-exempt (migration 20270319000000:302-304), so a url-auth row is refused `no_connection` before the loader's `url` allow-list runs; the loader already allow-lists `url` (forward-compatible), so the fix is an RPC migration (PR-1 is no-migration) + a production-shaped regression test, in PR-1B; a false-refuse (fail-closed), not exploitable today (runner unwired). **(3) INT-082 (Codex R? P2, on head 7ed4578e)** — `get_mcp_connection_secret` returns the row without `visibility`, and the runner carries no caller-role authority, so once wired an ordinary tenant member holding an `owner_only` connection's id could resolve/execute it, bypassing what `get_mcp_connections_v2` hides from non-admins. The truthful fix needs an RPC migration (return `visibility`) + a new server-derived caller-authority input — both outside PR-1; coordinator-RULED (2026-09-20) INT-082, sequenced into MCP PR-1B with INT-078/INT-079 (explicitly NO library-only approximation, NO caller-supplied role flag, NO second RPC call). Not exploitable today (runner unwired). PR-1B NOT started; PR-2 NOT started. The `authFromSecret` `auth_kind='none'` MINOR + refusal-oracle NIT also recorded as deployed-door follow-ups. `ci/verify` inherited-red only (#1208 + #1255).
- **Knowledge-scope · fail-closed active-account revalidation before general-document extraction (#1255) — SHIPPED (2026-09-20; PR [#1267](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1267) MERGED to `main` as [`ca10e5d577a46b007f532b82be3c6ec026a5c136`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/ca10e5d577a46b007f532b82be3c6ec026a5c136); merged branch head `43c2e1d3b8d590b797d4d5025ccef05fb8665776`; classification internal-only (patch); SHIPPED under DIRECT owner authorization — NOT INT-083; the INT-083 standing authority's uses are #1278, #1280, #1281, #1277).** Fail-closed active-account revalidation BEFORE the general-document extraction provider call: a switched-account DOCX/image turn makes ZERO provider calls; a switched-account general PDF makes EXACTLY ONE caller-document-only pre-resolution read-check; `runDocumentReadCheck` was NOT relocated; the extraction is tenant-attributed. **MERGED and DEPLOYED (not customer-verified):** `deploy-edge-functions` run 35484814492 success; `edge-live` moved to `ca10e5d5`; knowledge-scope 369/0 and `ci/verify` step #83 green on `main`. **PROOF OWED (§32.c/§13):** authenticated production verification OWED — the INT-080 blocker is CLEARED by the #1281 repair (deployed, `edge-live = c7e386a3`, production verified on tenant `3062585`, trace `1e36dca8`; trace `3212141` confirmation pending). Customer-release eligibility: none (patch, no release note).
- **Connected MCP Gateway · Phase C hard-entry safeguards (server-only, DB + runner library) — SHIPPED (2026-09-19; owner authorized the merge after Codex R5 clean + exact-head CI; PR [#1268](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1268) merged to `main` as [`27488aa8c28852c7958211bacdf03b98624f40b7`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/27488aa8c28852c7958211bacdf03b98624f40b7); `deploy-migrations` run #279 persisted `20270323000000` — prod `schema_migrations` caught up, `db-live = 27488aa8c28852c7958211bacdf03b98624f40b7`; runner remains library-only/non-live).** The server-only entry gate the MCP runner must pass before any live wiring (#1262), additive/non-destructive, runner still imported by no deployed function. Migration `20270323000000`: adds nullable `endpoint_hash`/`args_shape_hash`/`expires_at` to `mcp_connection_approvals`; hardens `set_mcp_connection_approval` with a `SELECT … FOR UPDATE` row lock (serializes vs endpoint change — closes the Codex P1 race) + records the endpoint binding; NEW service-role `verify_mcp_connection_approval` (the durable consent SPEND — pin + endpoint identity + optional action shape + expiry; legacy NULL binding fails closed); widens the receipt outcome CHECK with `tool_error`. Library (`_shared/mcp-gateway/`): `effect-policy.ts` (server floor reusing `MUTATION_VERB` — provider `_meta` may only RAISE), `result.ts` (`isError`/unaccepted shape → `tool_error`, never `executed`), `rail-receipt.ts` (outcome truth via canonical `record_capability_run`, no parallel story), `consent.ts` (the `verifyApproval` seam + one-home action-shape hash); runner rewired, `RunnerRequest.approval` removed. §37: `set_mcp_connection_approval` had zero producers. Proofs green on real PG16 (durable-consent psql `BEGIN…ROLLBACK` + two-session race proof, both wired into `paige-spine-contract.yml`) + `smoke:mcp-gateway` 79 assertions; all migration/security lints pass (incl. `lint:migration-versions` — `20270323000000` unique vs origin/main); `ci/verify` red is inherited-only (#1208 + #1255). **Codex review to R5 + an independent §39 adversarial pass (verdict: no BLOCKER/MAJOR); all findings dispositioned in-slice:** (a) [R3] the runner carries a truthful `RunnerResult.receipt` filing state (`filed`/`reason`) so a completed action whose canonical-Rail write FAILS is reported completed-but-unrecorded, never a silent recorded-success (§13/§32 — the outcome itself is never downgraded); (b) [R3] the race proof hard-asserts ZERO surviving approvals in BOTH orderings + a NON-racing deterministic tripwire proves the `20270322000000` revoke trigger deletes on endpoint change (verified to fail loudly with the trigger absent), and the racy handler swallows ONLY `MCP_ENDPOINT_CHANGED` (any other writer error re-raises, so it can never pass vacuously); (c) [R4 P2-A, safety] a MUTATION that dispatched then returned `isError`/malformed/unrecognized is `outcome_unknown` (→ Rail "may or may not have taken effect; check before running again"), NEVER `tool_error`/`capability_failed` (which renders "nothing was left half-done" — catastrophically wrong for a possibly-landed mutation and a duplicate-action risk); a READ stays `tool_error`; (d) [R4 P2-B] a malformed non-boolean `isError` fails closed (never slips through `=== true` as success); (e) [adversarial MINOR] a landed `executed` effect with no actor to file under is `record_failed` (owed-but-unrecorded), not benign `not_applicable`; (f) [R5 P2, safety] a post-dispatch TRANSPORT throw (HTTP error/timeout/malformed envelope) carries the same read-vs-mutation distinction, resolved BEFORE dispatch and carried into the catch (never re-derived from provider metadata): a READ → `tool_error`/`capability_failed` (no side effect to leave ambiguous), a MUTATION → `outcome_unknown` (may have landed, never auto-retried) — load-bearing test proven by removing the distinction (the READ case fails) then restoring. **Fenced (untouched):** `paige-ai-chat`, `scripts/knowledge-scope`, `_shared/mcp-outcome.ts`, the shipped `20270322000000` trigger. **PERSISTED (§32.a):** `deploy-migrations` run #279 applied `20270323000000` on the #1268 merge — `migration list` shows the version recorded remotely, prod `schema_migrations` caught up, `db-live = 27488aa8c28852c7958211bacdf03b98624f40b7` (direct prod object `SELECT` remains a nicety owed to a prod-SQL-capable session; prod SQL is permission-denied to the headless session). **Parked pending #1255:** live runner/Chat wiring, a live endpoint-setter, real-provider execution.
- **Connected MCP Gateway · endpoint-change revokes connection approvals (#1262 finding 5) — SHIPPED (2026-09-14, Gate; owner authorized a guarded merge — "do not merge until every final review incl. Codex completed and any relevant finding resolved or returned"; PR [#1263](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1263) squash-merged to `main` as [`406c1e2`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/406c1e2a65c4b308d7cde3424a4a690b5bd75c37); `deploy-migrations` run #276 persisted `20270322000000` — prod `schema_migrations` records it per the PERSISTED-verify step, `db-live` `fee6bd3`→`406c1e2`, zero migration drift; edge NO-OP — no `supabase/functions/**` change, `edge-live` unmoved).** Narrow standalone migration closing finding 5 of #1262: the Phase S connection-keyed `mcp_connection_approvals` store had no endpoint-change revocation (the legacy `tenant_mcp_connections` has had one since `20261012000000`), so consent for server A could authorize a same-fingerprint tool on server B after a re-point — a §58 safeguard regression. Adds migration `20270322000000` — an `AFTER UPDATE` trigger `_mcp_gw_revoke_approvals_on_endpoint_change()` on `mcp_connections` that DELETEs every approval for a `connection_id` when its **decrypted** endpoint changes (re-point or disconnect→NULL); a non-endpoint edit or a same-address re-encryption does NOT revoke (decrypted comparison because `platform_encrypt` is non-deterministic). Connection-keyed twin of the legacy trigger; `SECURITY DEFINER` + §59-safe (trigger-only, EXECUTE revoked from PUBLIC/anon/authenticated, fires only on an owner-authorized `mcp_connections` UPDATE, touches only `NEW.connection_id`). Regression proof wired into `paige-spine-contract.yml` (4 cases incl. sibling-scope + a non-vacuity guard on the re-encrypt case). **Not touched:** runner, Chat, Spine/Orchestration wiring, provider calls, Phase C scope. Reviews folded pre-merge (§5 SHIP; §39 no BLOCKER/MAJOR; Codex P2 proof-non-vacuity FIXED `1088839`); final **Codex CLEAN on the exact merge head `dd4dcbb`**. A concurrent `20270320000000` version collision was resolved by renumber→`20270322000000` (byte-identical) + sibling reconcile (#1264/#1265). `ci / verify` red is inherited-only (`lint:views` #1208 + `test:knowledge-scope` #1255 — SQL/CI-only diff touches neither). **Codex P1 (endpoint/approval serialization race) is REAL and owner-approved for deferral to Phase C as a HARD ENTRY GATE** (tracked in #1262): before any runner/Chat/Spine/Orchestration/provider wiring goes live, Phase C MUST serialize approval writes against endpoint changes and bind approval/consent to the endpoint identity, not merely a tool fingerprint. **PROOF OWED (§32.a):** direct prod object SELECT of the trigger/function in `pg_trigger`/`pg_proc` (MCP prod SQL permission-denied this session; pipeline PERSISTED-verify + `db-live` advance + zero drift is the persistence proof); no authenticated live-drive owed — the trigger wires no user surface and `mcp_connection_approvals` is unconsumed today, so the safeguard is dormant-but-correct.
- **Connected MCP Capability Gateway · Phase S — additive multi-connection foundation (server/schema-first) — SHIPPED (2026-09-14, Gate; owner authorized merge/deploy on exact head `573f8a3` under a guarded gate; PR [#1258](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1258) squash-merged to `main` as [`6c745007`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/6c745007d6ecdf982558fe9f217203c219ced49a); `deploy-migrations` run #272 persisted `20270319000000` — prod `schema_migrations` recorded per the PERSISTED-verify step, `db-live`=`6c745007`, zero migration drift; edge NO-OP — empty affected set, `edge-live` unmoved).** Paige-as-an-MCP-**client** foundation (outbound "Connected MCP Capability Gateway", NOT the inbound `paige-mcp` server), **additive/non-destructive**: legacy `tenant_mcp_connections` + `tenant_n8n_connections` remain the sole LIVE path, untouched. Adds the connection-keyed registry (immutable `connection_id`, many-per-tenant/provider, `mcp_providers` data-descriptor with a `generic-remote` no-op so a curated descriptor is never required), connection-keyed tools/approvals/receipts, a 1:1 idempotent backfill (real pins only §58; collision-free label picker), owner-only RLS + service-role-only decrypt/probe (§9/§59), and provider-agnostic `_shared/mcp-gateway/*` (read-only intake, model-safe capability summary, generic runner: read/prepare always, external effects only after owner approval, **fail-closed on undeclared effects**, pin-drift refusal, no auto-retry). **Chat uses NONE of it** — proven library code awaiting Phase C wiring; **Phase C PARKED pending #1255.** Reviews folded pre-merge (§5 no-blocker; §39 one BLOCKER + one MAJOR fixed). `ci / verify` red proven **inherited-only** (`lint:views` #1208 `paige_unclassified_inbound`; `test:knowledge-scope` 357/1 assertion `15.9` = parked #1255) — both reproduce on `main`, this PR adds zero views and touches neither `paige-ai-chat` nor `scripts/knowledge-scope/*`. **PROOF OWED (§208):** direct prod `\d`/object SELECT (MCP prod SQL permission-denied this session; pipeline PERSISTED-verify + `db-live` + zero drift is the persistence proof) + per-tier pgTAP + real-provider runner/intake (Phase C). `forget_paige_workflow`: owner decision **C** (resync fallback unchanged); Option A follow-up = [#1260](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/issues/1260).
- **C5 s2 follow-up (#15) · Orchestration DB guard extended to INSERT + approve-invariant CI lock — SHIPPED (2026-09-13, Gate A; owner directed the scope; PR [#1250](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1250) squash-merged to `main` as [`cd503301`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/cd503301e6c4dfde5e995474ec956524b709393f); `deploy-migrations` run #270 persisted `20270317000000` (prod `schema_migrations` records it per the PERSISTED-verify step; db-live=`cd503301`, zero migration drift); edge NOT_APPLICABLE).** The owner ruled "do not merge #15 as-is" and directed pulling the orchestration INSERT protection + a fail-closed source-guard tweak into #15, keeping claims narrow, and creating the broader §59 non-orchestration approvals-RLS redesign as the NEXT slice (task #18, ahead of #17). Delivered: (1) DELETED the two UNMOUNTED `/admin` approval pages (§18/§70 — hardening dead code was the trap; the mounted `ApprovalRow`/Command-Center hooks already route approve through `execute-approval`); (2) ADDED `scripts/ci/approval-direct-write-lint.mjs` — a fail-closed CI tripwire locking "no direct `paige_pending_approvals` status='approved' write in `src/`" (46-case self-test; flags an unparseable payload); (3) migration `20270317000000` makes `paige_guard_orchestration_direct_approve()` `tg_op`-aware and re-creates its trigger `BEFORE INSERT OR UPDATE` — a JWT caller can now neither CREATE an already-approved orchestration row, transition one to approved, nor launder a non-orchestration row INTO orchestration outside `execute-approval`; service-role create stays ledger-gated. **NARROW (§13):** #15 closes the orchestration INSERT+UPDATE approve path ONLY — it does NOT claim the DB globally blocks every direct approval write; the non-orchestration §59 RLS redesign is task #18. Proof: real-Postgres guard proof **13/13**; **60-assertion** pgTAP green on `database-contract`; §37 producer inventory clean (only the mint writes `source='paige_orchestration'`, always `pending`). Reviews: §5 SHIP + §39 no-confirmed-defect + Codex P1 (launder-in) FIXED + three evidence-consistency P2s fixed; the escaped-table-literal P2 DECLINED per the owner's mini-lexer-scope ruling (documented as the accepted defense-in-depth boundary). PROOF OWED (§32.c): a fine-grained direct prod object SELECT of the broadened trigger/function (MCP prod SQL permission-denied this session; pipeline verify + db-live advance + zero drift is the persistence proof); no authenticated live-drive owed (no new user surface). Evidence `docs/evidence/ui-delivery/admin-approvals-direct-write-guard.md`.
- **E7 · Governed Calendar-link sharing — SHIPPED LIVE (2026-09-13, Gate A — owner authorized the release; PR [#1252](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1252) squash-merged to `main` as [`5fbb7c1b`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/5fbb7c1baeb277b6bbeaaeb04f2ce533df389943) off `main` `ec59415d`; `deploy-migrations` run #269 persisted `20270315000000`+`20270316000000` (prod `schema_migrations` records both per the PERSISTED-verify step; db-live=`5fbb7c1b`, zero migration drift), `deploy-edge-functions` run #323 redeployed `paige-ai-chat` (edge-live=`5fbb7c1b`, zero edge drift); MAJOR-2 catalogue-preservation PROVEN on real PG16 pre-merge — baseline `20270305000000`=134 rows, `20270316000000`=135, **0 dropped/overwritten, +1 `calendar_link_send`**, every prior row preserved; §39 adversarial SOUND + §5 compliance SHIP + Codex COMPLETED clean on `647b30f6`).** NEW capability class: Paige prepares a PUBLISHED calendar's public `/book/{slug}` link and, after the confirm gate, SENDS it to a tenant contact by EMAIL or SMS — a **comms-governed send of a read-only link**, distinct from the E3/E5 booking-preset lifecycle and from the FU-2 booking/reschedule/cancel WRITE path. **Reuses the canonical seams, invents none (§18):** send-message (email/SMS, caller-JWT forwarded so its §9 gate stays live), runPreSend (consent/DND/suppression/quiet-hours), _assert_can_manage_preset (calendar caller scope), send-message's own `comms.outbound` Rail (no second Rail event), social_account_status (connected-channel read). **Social = COPY-READY ONLY (owner ruling 2026-09-13):** the governed social-POST executor does not exist and the owner's "no second social adapter" rule forbids inventing one, so E7 enumerates connected accounts + hands back post text and NEVER posts/claims a post (surfaced as the §00 incompatibility; owner chose copy-ready over building the un-built Social Ops publisher). **Ships in the draft (proven):** the NEW shareability gate `public.calendar_link_shareable` (migration `20270315000000`, SECURITY DEFINER §59, mirrors public-booking loadCalendar `enabled=true` AND ≥1 host) — pgTAP `calendar_link_shareable.sql` **24/24 green on real Postgres** against the REAL `_assert_can_manage_preset`; the tenant-brain adapter `_shared/calendar-link-tenant-brain.ts` (prepare/send/social_copy) with the **READBACK-TRUTH invariant** — success reported ONLY when send-message returns `outcome==="sent"` (a queued/blocked/failed/`needs_config` send is reported as itself, never as sent; send-message returns HTTP 200 + wire `status` that is NOT the disposition — the map keys on `outcome`) — adapter smoke **39/39 green** headless; the Spine domain `calendar_link.ts` (2 READS registered) + `calendar_link_send` classified `high` in action-risk + catalogued in `list_tool_autonomy` (`20270316000000`, all prior rows preserved §58) + 6 additive paige-ai-chat wiring sites; both proofs wired into `calendar-preset-seam.yml`. **§37:** send-message contract UNCHANGED (E7 is a new consumer, adds no field). **Collision:** additive/union-safe vs PR #1234 (registry.ts spread, action-risk row, paige-ai-chat else-if). **Held OUT:** a real social-post executor, the Spine-manifest row for the send (needs the registry validator's edge-executor lane — the riskiest #1234 line, deferred, no enforcement lost), and the E6-era MED-1/MED-3/LOW-2/LOW-3-B2 follow-ups. **PROOF — discharged at release:** `deno check`/typecheck-ratchet/build green in CI on `647b30f6` (zero new diagnostics); migration PERSISTED on prod (`deploy-migrations` #269 verify + zero drift). **STILL OWED (§32.c/§70, browser/JWT/prod-SQL session):** authenticated in-chat live-drive (prepare→confirm→send→contact receives), `/book` origin confirmation, and the finer direct prod-SQL object read (`calendar_link_shareable` in `pg_proc` + `list_tool_autonomy` returns `calendar_link_send`). **Pre-existing (NOT E7):** action-risk/tool-catalogue lints still flag `nav_pull_business_credit`/`smartcredit_pull_snapshot` on `main` — untouched by E7. Package: `docs/evidence/proofs/calendar-link-sharing-e7/README.md`.
- **Agent-control experience — grounding ACCEPTED, PROPOSED / NOT BUILT, gated behind #1234 (owner ruling 2026-09-13).** Owner accepted the read-only backend grounding for a future owner-facing agent-control monitor (watch + govern Paige's + her agents' work). **Recorded, nothing built, no capability claimed.** Canonical seam to REUSE = the **Layer C approve→resume→readback fabric** (`paige_act_executions` ledger `20270126000000` + `paige_approve_act_execution` `20270202000000` + companion `paige_pending_approvals` inbox + reconciler cron `20270303000000` + `approve-executor.ts` RESUME + receipts/Rail on `advanceLedger`). Per-control truth (§13): **status-list PARTIAL** (per-store readbacks — `list_actions`, `paige_approval_queue_v`, ledger SELECT, `paige_workflow_runs`, tasks — but NO unified query and NO surface reads the ledger today); **resume/readback REUSE** (NATIVE acts only; n8n `not_supported_in_slice`); **cancel PARTIAL** (decline-a-*pending* → `cancelled` guarded to `approval_pending` only; `advance_action→dismissed`; task cancel — but NO stop-in-flight); **retry PARTIAL** (auto resume/reconcile/workflow-sweeper only; the `retrying` outcome is defined but written by no code path; no "retry now" RPC); **pause UNAVAILABLE** (no in-flight pause state; `paige_automations.state='paused'` disarms a process definition, not a run; needs new ledger state + engine gate). **BUILD SEQUENCING (owner directive):** do NOT build the agent-control backend or coordinate edits against **PR #1234** (CRM/Pipeline, HIGH collision on `paige-ai-chat`/`_shared/action-risk.ts`/capability-gateway/spine-registry); **wait for #1234 to merge + establish its canonical `crm-command` surface, THEN re-ground on fresh `main` and build the monitor backend from the real seams.** #1235 (orchestration fix-forward) is MERGED — corrected executor in `main`. The **visual prototype proceeds NOW via the design agent** (§00; required workflow `npx impeccable update` → `flow-prototype`) and MUST render pause/retry/cancel + any unsupported state as explicitly unavailable/absent, never functioning controls; it MAY truthfully prototype status/history, approval-needed, approve/decline, and supported resume/readback on the Layer-C model. Full map: `docs/delivery/agent-control-capability-map.md`.
- **E6 · Calendar security/proof/reviews — SHIPPED LIVE (2026-09-13, Gate A; PR #1246 squash [`e560a4ab`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/e560a4ab983ad3f54703479d96d9368b662b16a4); db-live=edge-live=`e560a4ab`, zero migration + edge drift; deploy-migrations #268 persisted `20270312000000`, deploy-edge-functions #322 redeployed public-booking + booking-manage, seam pgTAP gate green in CI #2). Owner AUTHORIZED the release after the §39 peer-gate + §5 compliance passed — the earlier HELD is discharged. PROOF OWED (§32.c/§70): authenticated /book+/manage live-drive + direct prod-SQL row read (MCP prod SQL permission-denied this session).** E6 is NOT net-new capability: E3/E4/E5 already shipped live and the 11 Calendar DEFINER fns are already §59-compliant in code — E6 is the consolidation/hardening/proof + independent-review pass. **Ships in the draft (proven):** a REPEATABLE pgTAP CI gate for the E3 server seam (`supabase/tests/calendar_booking_preset_seam.sql`, **112 assertions re-run green by the integrator**, not just the builder's claim) + `.github/workflows/calendar-preset-seam.yml` — converts the one-off `booking-preset-lifecycle/` transcript into a permanent gate (§32/§24); the **MED-2 anon host-email PII leak FIX in BOTH guest-facing `resolveHostNames` copies** (`public-booking` + `booking-manage` — names-only, no auth-email fallback; pre-launch blast radius ≈0; the final §39 peer-gate caught that the first commit fixed only `public-booking` and overclaimed "structurally impossible" repo-wide — corrected, and a §18 unify-to-one-helper follow-up named); the **LOW-1** `create_class_booking` §59 tenant-consistency guard (migration `20270312000000`, proven 5/5 — the safe tenant half; host-membership half deferred); §124 SECURITY_DEFINER catalog registration of the calendar seam (was stale since 2026-07-02); FU-2/FU-3 contract + Spine-comment + tier-matrix currency fixes. **Independent review:** §39 adversarial (anon boundary SOLID, no HIGH; verified findings MED-1/MED-3/LOW-1/LOW-2/LOW-3) + §5 compliance (B1 closed, B2 routed to owner, B3/B4 closed). **Routed to OWNER decision (exact patches in the package, NOT applied — product/write-contract/behavior + need authenticated verify):** MED-1 (direct `UPDATE calendars SET enabled=true` bypasses the RPC publish-bar — intra-tenant, not cross-tenant; the `20270301000000` header's "cannot be bypassed by a direct table write" claim is FALSE), MED-3 (host-removal auto-pause), LOW-2 (fail-open throttle), LOW-3/B2 (two born-live minters bypass draft-by-default — reconcile vs. signed exception), and a §53 review of `cancel_/reschedule_internal_booking` (`has_any_role` tenant-role usage). **OWED to a capable session (§32.c/§70):** prod-SQL persisted-confirm of the E3 migrations, E4 authenticated live-drive + 8 viewport captures, E5 in-chat live-drive + prod-SQL catalogue read, and item-C resolver Deno invariant tests (the proper MED-2 proof). Deferred (named): FU-1 (§67/§68), FU-3 richer emission + #244. Package: `docs/evidence/proofs/booking-preset-e6/README.md`.
- **Owner VISUAL APPROVAL of the Settings › Connections › Calendars surface (§28, 2026-09-13).** The owner accepted the released Calendar experience — the "New calendar" language, the prominent primary button, the "Booking calendars" framing, the Active/Archived grouping, and the booking-model chooser — as the right human-facing direction, and directed that it be recorded as owner visual approval with no cosmetic churn unless a real usability / accessibility / truthfulness / responsive-layout defect is found. The previously-provisional chooser copy (E4, `solo-booking-preset-lifecycle.md`) is now APPROVED-FROZEN; Claude Design retains authority over any future wording change (§00 — this records the owner's approval, not a CC design decision). Independent of E6 (Calendar security/proof, HELD) and E7 (link sharing, queued).
- **C5 slice 2 fix-forward SHIPPED — readable + ledger-proven companion-mint approvals (2026-09-13, Gate A; PR #1235 — MERGED squash `e6850fbb`; migration `20270306000000` PERSISTED via `deploy-migrations` run #267, `db-live`=`e6850fbb` zero drift §32.a; §39 peer + §5 compliance SHIP; Codex CLEAN on the exact merge head `bea2eca8`).** An external Codex review of the merged C5 slice 2 (#1225, `17a23de0`) found six real defects in the companion-mint seam — all fixed-forward by `CREATE OR REPLACE` of THREE of the four `20270303000000` functions (mint / direct-approve guard / reconciler; cancellation-sync UNTOUCHED) + one NEW pure non-DEFINER helper `paige_orchestration_approval_content()` + an idempotent backfill/repair: (1) **§70 readable approval** — the mint wrote `draft_content='{}'`/no `summary`, so `ApprovalRow.tsx`/`ApprovalsInbox.tsx` (which render only `summary`) showed "(no summary)" on a high-risk approval; the helper humanizes capability+snapshot-args+event-client into `summary`+`draft_content.subject/preview/body`, folding the args INTO `summary` so WHICH target is visible; (2) **no client-writable metadata bypass** — approval/exec state proven from the canonical `paige_act_executions` ledger, never browser JSON; (3) **direct-approve guard** gates on the STORED `old.source`, refuses JWT callers (`42501`), requires a terminal ledger (`executed|failed`), and pins `source`+ledger-coords (`metadata.event_id/act_id/act_execution_id`) immutable so the row can neither be reclassified nor repointed to launder past the executor-only gate; trigger broadened `BEFORE UPDATE OF status`→`BEFORE UPDATE` (coverage-only, §58); (4) **reconciler** stamps the monotonic RPC's RETURNED outcome, never the stale requested one; (5) **idempotent backfill + legacy repair** (`NOT EXISTS`+`ON CONFLICT DO NOTHING`; empty pre-fix companions recomputed readable, `contact_id` untouched §9); (6) **recovery doc** corrected — an applied additive migration is repaired by a FORWARD cleanup migration that DROPs only the new helper and KEEPS the slice-1 index `paige_ppa_orchestration_act_uk`, never a git revert. 53-assertion pgTAP wired into the required `database-contract` (green on `bea2eca8`, both migrations replayed in order). One Codex finding (a "missing Visible-Flow-Impact trailer") was verified incorrect and refuted with evidence (the trailer IS present base..head; `ui-delivery-evidence` green). **PROOF OWED (§13/§32.c/§70):** (a) the authenticated approve→execute live drive on a real held act (browser/JWT-capable session — never a real customer record headless); (b) a fine-grained direct prod object SELECT confirming the helper (non-DEFINER) + the three replaced functions + the broadened trigger def + the surviving slice-1 index in `pg_proc`/`pg_trigger`/`pg_indexes` — the migration IS persisted per the pipeline `migration list` verify + zero drift; Supabase MCP prod SQL is permission-denied THIS headless session (confirmed on a READ attempt), so the object-granularity SELECT is owed to a prod-SQL-capable session. Follow-ups still open: route the two `/admin` approve surfaces through `execute-approval` (task #15); the broader table-wide approvals-RLS §59 global-role trap + fresh-orchestration-companion INSERT-forge (owner-decided).
- **Shipped Delivery Log row added for the closeout-accuracy fix-forwards #1240/#1242 (identity record, 2026-09-13, Gate A).** A third Codex P1 (on #1242) correctly noted, per AGENTS.md L89, that the docs-only fix-forwards #1240 (`d124b18a`) and #1242 (`281672e4`) reached `main` without their own §4.0 Shipped Delivery Log row, and that the closeout-only exemption (L91) did not cover them because they carried normative lessons-learned additions. Added ONE consolidated §4.0 row recording both deliveries (identity, docs-only proof boundary, evidence, customer-release eligibility = none). This entry + the row are a mechanical identity record (AGENTS.md L91) — no product/policy/capability/status change — so this PR needs no recursive row of its own. Closes the closeout-accuracy fix-forward sub-workstream.
- **kb-ingest-url SSRF fork migrated onto the canonical `assertPublicHttpUrl` (2026-09-13, Gate A; PR #1241 — SHIPPED: MERGED squash `127e2de7`, edge-deployed edge-live=`127e2de7` zero drift §32.a; §39 peer SOUND + §5 compliance SHIP).** The tracked §18 follow-up from web_fetch #1227. `kb-ingest-url` carried its OWN per-hostname regex blocklist (`BLOCKED_HOST_PATTERNS`/`unsafeReason`) that matched only the hostname STRING and never resolved DNS — so a public hostname resolving (or 3xx-rebinding) to a private address, and encoded-IP literals (decimal/hex/IPv4-mapped-IPv6), sailed through and got fetched server-side (a real SSRF surface on a tenant-reachable URL-ingest endpoint). Swapped it for the canonical `_shared/ssrfGuard.ts assertPublicHttpUrl` (§18 one home — resolves the host + validates every resolved IP numerically, https-only, rejects embedded credentials), applied on the initial URL AND re-validated on every redirect hop INSIDE kb-ingest-url's existing manual redirect loop — so redirect-following (up to 5 hops) is PRESERVED (no redirect-posture product decision; the canonical `safeFetch`'s refuse-redirects behavior is deliberately NOT adopted here since KB ingest legitimately follows canonicalizing redirects, only the validation is shared). `SsrfError` mapped to honest 403/400 responses that never leak the resolved address. **§37 producer inventory:** sole runtime caller is `src/pages/admin/TenantKnowledgeAdmin.tsx`; request/response contract unchanged; no edge/cron/MCP caller — only attack URLs newly refused, happy path identical. **§32 proof:** new `scripts/kb-ingest-url-ssrf-smoke.mjs` 20/20 (real imported guard refuses DNS→private/encoded/non-https/creds; kb redirect loop re-validates each hop — 302→private refused, 302→public followed, direct 200 returned; anti-vacuity vs the old regex; honest mapper) — now a CI gate in the Node-22 job (needs `--experimental-transform-types` for `SsrfError`'s parameter property, same as web_fetch, §24). deno check owed to CI (no local Deno). **Collision check clean** (no open PR touches kb-ingest-url; #591 = paige-ai-chat/knowledge-scope, #1234 = CRM surface). **§58 behavior note (§39 P3-1):** the old hostname-regex silently allowed `https://user:pass@host`; the canonical guard now rejects embedded credentials (400) — a correct security tightening, not an approved-capability removal (fetching credentialed URLs into a KB was never an intended feature). **PROOF OWED (§32.c/§70):** authenticated live-drive of the ingest flow (public URL indexes / attack URL refused). **FOLLOW-UP (§31/§18, tracked):** kb-ingest-url's `res.text()` body read is unbounded before the 200 KB slice (a pre-existing memory-bound concern), and its hand-rolled redirect loop is a parallel transport to the canonical `safeFetch` — both §39/§5 recommend closing them together by adding an opt-in redirect-follow mode to the canonical `safeFetch` (collapses the loop AND bounds the body). Out of this SSRF-migration's scope. (Task #35.)
- **Security-accuracy fix-forward — a SECOND Codex P1 on #1240's recovery text (docs-only, 2026-09-13, Gate A).** Codex's post-merge review of #1240 caught a genuine P1: my corrected recovery still said "a forward migration that `CREATE OR REPLACE`s `match_paige_memory` **back to the prior body**" — but the prior body IS the forged-ID / global-role / unbounded-param IDOR this slice closed, so following that recovery would REOPEN the cross-tenant read. Fixed the recovery *target* (the #1240 fix had corrected only the *mechanics*): recovery is now **fail-closed — never restore the pre-fix body** — either forward-fix the hardened body while preserving its §53/§59 authority properties, or `REVOKE EXECUTE`/deny the seam (take it offline), never reinstate the vulnerable version. Refined the lessons-learned rule (b) with the deeper lesson: a security fix's rollback target is fail-closed, never restore-the-vulnerable-body. Docs-only; no code/migration/behavior change.
- **E5 · Paige governed adoption of the Calendar booking-preset action path — SHIPPED LIVE (2026-09-13, pre-launch merge-on-verified §4/§69; PR #1220 squash [`688613db`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/688613db7ef846745ad1620f358181a495811737), migration-renumber hotfix #1237 [`5d65a96f`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/5d65a96facf907757b191ac8b70e1fe1603eb24f)).** Paige now creates/revises/publishes/pauses/duplicates/archives/restores booking calendars through the EXACT canonical Calendar RPCs the Settings › Connections › Calendars UI drives — never a parallel model (§10). `_shared/calendar-preset-tenant-brain.ts` (read-before → canonical RPC via caller JWT → tenant re-resolve → `get_calendar_presets` readback → per-verb post-condition → Rail only on verified match); registered in `paige-spine/registry.ts`; risk-clamped in `action-risk.ts` (publish/revise/archive `high`, create/pause/duplicate/restore `ordinary`); catalogued in `list_tool_autonomy` migration `20270305000000` at the `confirm` default (§58: 7 added, 0 removed). edge-live=`688613db`, db-live=`5d65a96f`, zero migration drift; migration PERSISTED via the `deploy-migrations` §32 verify step + `db-live` advance. TWO migration-version collisions caught (#1225 took `20270303000000`; #1233 `match_paige_memory` took `20270304000000` inside the merge window, FAILING deploy-migrations #265) — fix-forward hotfix #1237 renumbered E5 to `20270305000000`, deploy-migrations #266 SUCCESS restored the shared pipeline (the §32.a merge-window-race lesson; a pre-merge version-uniqueness hardening follow-up is owed). Review: §5 compliance SHIP, §39 peer-gate FIX-FIRST→fixed (MEDIUM: `toolCallLabel` mislabeled a verified-but-Rail-failed publish as "not live" for 6/7 verbs; Codex P1 unnamed approval card; P2b no-op-revise fabrication) all resolved. Proof: 26-case adversarial unit suite + 30 spine tests pass, tsc 0-new, migration replay clean. OWED (§32.c/§70): authenticated in-chat live-drive (the owner's validation pass on deployed Studio) + a finer direct prod-SQL read of the catalogue rows (MCP prod SQL permission-denied to this session). E6 (security/proof/reviews) + E7 (governed link sharing) are strictly separate and unreleased. Evidence: `docs/evidence/proofs/booking-preset-chat-e5/`, `docs/evidence/ui-delivery/paige-chat-booking-preset-e5.md`; contract `docs/architecture/booking-preset-capability-adoption.md`.
- **Closeout-accuracy fix-forward — 3 Codex P1 findings on the merged #1233/#1236 slice (docs-only, 2026-09-13, Gate A).** Post-merge Codex review of the merged `match_paige_memory` slice + its closeout raised three legitimate §13/§70 accuracy defects; actioned as a docs-only fix-forward (support/review lane; #1233/#1236 already merged, so a fresh branch off `main`). **(a) #1236-B (genuine, operationally dangerous error):** the §4.0 recovery procedure said `git revert 0060f575` restores the prior body — WRONG. Version `20270304000000` is already recorded in prod `schema_migrations`, so `supabase db push --include-all` SKIPS it on any re-run; a revert removes the file but leaves the hardened body LIVE. Corrected to a NEW higher-version forward migration that `CREATE OR REPLACE`s back to the prior body (§32 — a persisted `CREATE OR REPLACE` is never undone by revert). **(b) #1233-P1 (label honesty):** bare `✅ RESOLVED/DONE/FIXED` tokens across the R1 audit §4c/§7, role-taxonomy §6, roles-permissions, memory-contract, and master-ref R3a entry relabeled `✅ FIX SHIPPED … production runtime UNVERIFIED (PROOF OWED §32.c/§70)` — a merged migration + a synthetic boundary test is not production proof (§70). **(c) #1236-A (status consistency):** `harness-completion-map.md` still directed the program to "continue phase by phase" (line 10 + §10) after the COMPLETE ruling; added a dated STATUS UPDATE + SUPERSEDED marker, and clarified here that "Completion Program construction complete" is a construction-program status, NOT a claim every A–G layer is verified/LIVE (per-layer PARTIAL truth unchanged). No code/CI/migration/behavior change; the fix itself remains FIX SHIPPED + PROOF OWED (§32.c/§70).
- **Persistent tasks/history Chat slice STOOD DOWN — subsumed by active CRM collision PR #1234 (2026-09-13; owner-named stop condition).** Owner directed the persistent-tasks/history slice with an explicit guard: "reuse the canonical CRM task model, governed command door, tenant authority, confirmations, receipts, and readback—never create its own task table/status/queue/approval path… stop only for an active CRM collision, a missing canonical seam, or a material product decision." Fresh collision check found **PR #1234 `codex/governed-crm-pipeline-2026-09-13` "feat(crm): complete governed Paige operational access"** (draft, base `main`, +2504/-28, 28 files) already delivering the **complete** governed task-command surface through a canonical door: a new `supabase/functions/crm-command/index.ts` + `_shared/crm-command/catalog.ts` mapping `task.create/update/assign/reschedule/complete/reopen/cancel/delete`; all task actions registered in `_shared/action-risk.ts` (`crm_update_task`/`crm_reschedule_task`/`crm_complete_task`/`crm_reopen_task`/`crm_cancel_task`/`crm_assign_task`/`crm_delete_task`) with risk tiers; every mutating task command bound by `task_id` (exact UUID) + `expected_updated_at` (optimistic concurrency = the owner's stale-state/idempotency/no-false-success requirement); approval-card path for destructive/ownership; receipts + Rail readback; and it edits the **exact files** a `crm_update_task`+`crm_get_task` slice would (`paige-ai-chat/index.ts`, `_shared/action-risk.ts`, WRITE_TARGET). Building the slice now would be a duplicate task-command silo (§18 / §1 RED-LINE #3) and a guaranteed file collision. **Decision: do NOT build `crm_update_task`/`crm_get_task` in `paige-ai-chat`; the canonical task-command home is #1234's `crm-command` door.** Any remaining task-chat work sequences AFTER #1234 lands and reuses that door — never a parallel path. No code changed by this grounding; reported to owner. (Next session: do NOT re-attempt the tasks slice as separate chat tools.)
- **web_fetch — operator/tenant-owner seat LIVE proof received; client-portal-seat still owed (2026-09-13, §32.c/§70).** Owner drove `web_fetch` on his own active Paige session (input `https://www.mogulmakeracademy.com/`): Paige fetched the real public page and summarized specific content (site thesis, four service pathways, proof points, testimonials), then returned a relevant follow-up rather than claiming an unsupported action — no private/loopback access, no redirect issue, no fabricated success, no injection-driven behavior observed. This DISCHARGES §32.c/§70 for the authenticated **operator/tenant-owner** seat (the standing PROOF OWED from #1227). **NOT overstated to client-seat proof** (owner directive): the owner drove his own operator/tenant-owner account, not a client-portal account, and the client seat carries `CLIENT_SEAT_ALLOW` gating + client-scoped resolution as a distinct path — so the narrower authenticated **CLIENT-PORTAL-seat** live-drive remains owed. Records updated in place (§58, not rewritten): `docs/evidence/ui-delivery/web-fetch-hardened-routing.md` (AUTHENTICATED_RUNTIME → PASS scoped to operator seat; UNVERIFIED narrowed to client-portal seat), master §4.0 row, chat-completion-matrix web_fetch row. Prior #1227 entry (below) left intact.
- **Paige Runtime Harness Completion Program — COMPLETE (owner-ruled 2026-09-13).** The last Harness-owned implementation — the systemic `is_platform_owner()`/§53/§59 authority-correctness sweep — shipped as the `match_paige_memory` resource-scoped-authz slice (PR #1233, `0060f575`, entry below): confirmed-open defect closed in the deployed + persisted (`db-live` zero drift) + boundary-proven (CI) function body, §39/§5 reviewed — the authenticated production drive remains PROOF OWED (§32.c/§70), i.e. FIX SHIPPED, not production-verified. **Active Harness construction STOPS here.** This is a **construction-program** ruling: it does NOT assert every A–G layer is verified/LIVE — per-layer truth stays exactly as `docs/delivery/harness-completion-map.md` (§4/§5) and master-ref §4/§5 record it (several layers PARTIAL), and the remaining layer work is parked for its own workstreams. Remaining Harness involvement is support/review only, when another workstream needs the shared governance seam. PARKED for their own workstreams: Marketplace, Financial Integrations, provider activation, Plaid/iSoftpull, and the remaining authority audits (R2b wrapper-closure, R3 rest-of-c1/c2 — the sweep fixed only the ONE source-confirmed defect, per "replace unsafe checks ONLY where source evidence requires").
- **match_paige_memory resource-scoped authorization — §53/§59 caller-scope fix (2026-09-13, Gate A; SHIPPED `0060f575` (PR #1233), migration `20270304000000` persisted via `deploy-migrations` #264, `db-live` == merge SHA zero drift; row-count reconfirm + §32.c authenticated drive remain PROOF OWED).** The systemic authority-correctness hardening the owner scoped, delivered as ONE narrow slice fixing the confirmed-open §59 defect (R1 audit §4c; memory-contract follow-up). `public.match_paige_memory` (SECURITY DEFINER, granted `authenticated`) had (1) a **forged-ID structural bypass** — `_target_client_id := auth.uid()` + `_target_user_id := victim` made `IS DISTINCT FROM _target_client_id` false, the RAISE never fired, victim's `client_memory.content`/`chat_message_embeddings.content_excerpt` returned (caller-supplied `_match_threshold := -1` dumped); and (2) the **global-role trap** (`has_role('admin')` tenant-agnostic → any tenant owner reads cross-tenant). Fix: authority per-target — cross-USER limited to self / `is_platform_operator`; staff cross-CONTACT via canonical `can_access_contact` (per-contact tenant-correct); each data-branch gated on its own flag (authorized-for-A can't leak B), threshold clamped `[0,1]` + counts `[0,50]`, and an explicit `service_role` trust branch. §39/§5 peer review caught Finding 1 (a per-user coach/tenant-admin grant is not per-row-tenant — `chat_message_embeddings` has no tenant column — so it was dropped; cross-user staff has no producer) (governed-memory-seam pattern) so the legitimate `paige-ai-chat` path — which the old guard left DEAD (service role → auth.uid() NULL → always RAISE) — works via server-resolved authority. Grant end-state anon-revoked → authenticated+service_role. LATENT at the 2026-08-18 audit (0 rows both tables); row-count re-confirm PROOF OWED. Boundary proof `supabase/tests/match_paige_memory_authz.sql` (16 cases incl. forged-ID/global-admin/per-branch-gating/Finding-1-multi-tenant/clamp-discriminator refusals) green in CI (`.github/workflows/match-paige-memory-authz.yml`, pgvector) — local run blocked (no registry). `lint:definer-fns`+migration-lint PASS; `test:client-memory-authz` 268/1 (the 1 = pre-existing base 19.8, reproduced on clean main). `ci:tsc` only the pre-existing #1186 (zero TS in diff). Evidence: `docs/evidence/match-paige-memory-authz.md`.
- **Funding & Coaching provider expansion PARKED / handed off (owner ruling 2026-09-13).** Do NOT start Plaid, iSoftpull, provider activation, Marketplace package seeding, Marketplace entitlement/UI, Financial connection/consent/health/disconnect UI, QuickBooks, Nav, SmartCredit, or any provider expansion — separate workstreams, own agents. Preserve current state exactly: deployed provider gate fail-closed, package UNAVAILABLE (unseeded), nothing activated. **Ownership split:** Marketplace owns the installable Funding & Coaching Tools package + entitlement experience; Financial Integrations owns tenant connection/consent/health/disconnect surfaces; later provider-adapter work activates individual providers only AFTER both are live. No placeholder PR opened to park (this durable record is the handoff).
- **Layer C · C5 (slice 2) companion-minting — SHIPPED: MERGED to `main` + PERSISTED + edge-deployed (2026-09-13, Gate A; PR #1225 squash [`17a23de0`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/17a23de046b71e5b4f91e661adb37914117869e8)).** Owner lifted the merge gate and authorized full completion. Pre-release re-verified against fresh main (no collision — the one new commit `ae57acfd` was docs-only, trial-merge clean; migration `20270303000000` unique; `get_reviews` empty → no §39/§5/Codex blocker), marked ready, squash-merged. `deploy-migrations` #263 **success** — "Verify prod is caught up (PERSISTED, §32)" green; `db-live`→`17a23de0`, `db-live..main` on migrations **empty (zero drift)** → `20270303000000` persisted on prod. `deploy-edge-functions` #319 **success**; `edge-live`→`17a23de0`, edge drift **empty** → `execute-approval` live. Required CI green on the merged head (database-contract 34-assertion pgTAP, contract, audit, lint, ui-delivery-evidence); vitest 20/20. Migration renamed `20270203000000`→`20270303000000` on the rebase (collision with main's `solo_beta_welcome_lifecycle`, §32.a). Non-required `verify` red = pre-existing repo-wide baseline (identical on main's base `86476a83`); this branch's only contribution (ESLint on the test double) fixed at `f55fd235`; not re-litigated per owner. **PROOF OWED (§32.c/§70) — exact owner-drivable path:** in a scoped test tenant, create ONE approval-required governed act (a `confirm`-lane / high-risk-clamped native act, e.g. `crm.advance_journey_stage`) → confirm EXACTLY ONE matching inbox row (`paige_pending_approvals`, `source='paige_orchestration'`, `contact_id` NULL) → approve it ONCE via `execute-approval` → confirm EXACTLY ONE canonical domain action (the journey-stage transition), durable readback (`paige_act_executions.outcome='executed'`, settled), ONE Rail event, ONE receipt, and NO duplicate execution (replay/re-approve is idempotent). Plus a fine-grained prod object SELECT (`pg_trigger`/`pg_proc`/`pg_indexes`/`cron.job`) confirming the four objects. Owed to a browser/JWT/prod-SQL-capable session — this headless session has no authenticated user session and its MCP prod SQL is permission-denied; never fabricated, never a real customer record headless. Master §4.0 Shipped Delivery Log row added in this closeout. Supersedes the prior "RELEASE-READY DRAFT held at gate" line below (§58 — recorded, not removed).
- **Layer C · C5 (slice 2) companion-minting + tracked follow-ups — BUILT, reviewed, local-proven, RELEASE-READY DRAFT held at the production-release gate (2026-09-13, migration `20270303000000`; branch `claude/blissful-einstein-pbnraa`; per the owner's slice-2 instruction the merge is the one HOLD).** Slice 1 shipped the executor + approve door but nothing PUT a held act in front of a human. Slice 2 mints the companion into the EXISTING `paige_pending_approvals` inbox, atomically in the monotonic RPC's txn, REUSING the inbox schema/authority/claim/Rail/tenant resolution — NO second approval system, NO UI change (§00); `lint:approval-gate` confirms "one approval channel". **(1) mint trigger** `paige_mint_orchestration_approval` (AFTER INSERT OR UPDATE ON `paige_act_executions` WHEN outcome='approval_pending') mirrors `advance_action`'s companion-mint column list; `source='paige_orchestration'`, keyed on `metadata.event_id`+`act_id` (NEVER `action_id`, so the existing `trg_ppa_sync_action` no-ops — verified L168-170); `type='other'`, `category`=capability_key, `risk_level` maps detail.risk high→high else medium, `contact_id=NULL` (§9 — the held act must not leak into the client portal via the "clients read own-record approvals" RLS), `draft_content='{}'`; idempotent via a PARTIAL UNIQUE INDEX `paige_ppa_orchestration_act_uk` + ON CONFLICT DO NOTHING. **(2) cancellation-sync** `paige_sync_orchestration_act_on_decision` (AFTER UPDATE OF status): reject/skip settles the held ledger act to `cancelled`, guarded to advance ONLY `approval_pending` (never clobbers an executed act). **(3) direct-approve guard** `paige_guard_orchestration_direct_approve` (BEFORE UPDATE OF status): an orchestration approval reaches `approved` ONLY when the executor stamped `metadata.act_outcome` — blocks bulk/bypass approve that would mark the inbox approved while the ledger act never ran (§13/§70). **(4) durable reconciler** `paige_reconcile_orchestration_acts()` (pg_cron `*/5`, §64/§59 service-role-only in-body): sweeps orphaned advanceable NATIVE acts on already-`done` events the native-event sweeper cannot re-drive — a landed correlation transition → `executed` (also stamps the companion truthful); >24h with none → `failed` (reconcile_exhausted); fresh/recent left as-is. Mirrors `native-adapter.reconcileByCorrelation`'s ONE journey query (§18 documented; the §18 seam to extend for a future native executor). **Follow-ups bundled:** (a) §68 — the accepted-never-landed RESUME re-dispatch now re-runs the availability+governed gate before re-firing (fixes the slice-1 tracked gap); (c) execute-approval releases the claim on a final-stamp failure AND the Layer-C branch runs BEFORE the isComms heuristic (source is authoritative, §37/§32). §37 producer/consumer inventory: the mint's one new producer + its consumers (queue reads, execute-approval, MCP `decide_pending_approval` refusal, apply_approval_policy, trg_notify, trg_ppa_sync_action) verified; the §5 compliance pass then caught two consumers the first walk MISSED — the legacy `/admin` direct-write approve paths `ApprovalsInbox.bulk("approve")` (an atomic `.in()` batch the new guard would abort for a co-selected orchestration row) and `ApprovalDetail.setStatus('approved')`. These are FAIL-SAFE (the direct-approve guard refuses — no mis-approval, no leak — it never falsely marks an orchestration row approved) and the PRIMARY approve surface (Command Center `ApprovalRow` → execute-approval) already handles orchestration approvals correctly. Routing those two `/admin` surfaces through execute-approval is a FRONTEND change that must carry its own §00/§69 UI-delivery evidence, so it is DEFERRED as a tracked follow-up (recorded here + in the PR #1225 body) rather than smuggled into this backend slice — the honest §5-sanctioned minimum for pre-launch (§13: the earlier "all verified safe" claim is corrected, and the gap is fail-safe, not half-hardened-dangerous). PROOF (§32/§13): vitest 20/20 on the approve-executor suite (2 NEW §68 tests: revoked-after-redemption STOPS the re-fire); `deno check` EXIT 0 on approve-executor.ts + execute-approval; the four DB objects proven by a 34-assertion behavioural test run LOCALLY against a native PG16 cluster with the REAL monotonic RPC (all 34 PASS) + the full migration re-applied clean end-to-end (cron.schedule + REVOKE/GRANT) with the supabase roles present; `lint:migration-versions`/`lint:definer-fns`/`lint:approval-gate`/`lint:governed-execution`/`lint:action-authority`/`lint:write-targets`/`lint:managed-schema` all green. The pgTAP `paige_orchestration_companion_minting.sql` wired into the REQUIRED database-contract job (runs against the fully-replayed schema in CI). **PROOF OWED (§32.a/§32.c/§70):** persisted-apply of `20270303000000` (deploy-migrations `db-live` move) lands on the owner's merge; the authenticated mint→inbox→approve→execute drive on a real held act (owed to a capable session, never a real customer record headless). SCOPE (§13): built + unit/local-proven + CI-wired; HELD at the merge gate per the owner's slice-2 instruction.
- **web_fetch hardened tenant-safe routing — inert client-seat tool made functional + shared fetch SSRF-hardened (2026-09-13, Gate A; PR #1227 — MERGED squash `1a5d8020`, edge-deployed: edge-live=`1a5d8020`, zero drift §32.a).** Owner ruled: do NOT retire `web_fetch`; build it for the client seat too (it was in `CLIENT_SEAT_ALLOW` + advertised to clients, but the handler returned `"Web fetch not executed in this flow"` — a §13/§70 advertised-but-dead gap). **(1)** `fetch-url-content` migrated its outbound fetch onto the canonical `_shared/ssrfGuard.ts safeFetch` (§18 one home) — replacing a per-hostname regex blocklist that could not see DNS→private / IPv4-mapped-IPv6 / link-local; `safeFetch` resolves the host + validates every IP numerically, refuses redirects, bounds time+bytes; adds `title`/`truncated` provenance + honest `SsrfError` reason→status. Response contract `{success,url,content}` preserved → auto-fetch path, paige-deep-research, CI mocks unaffected (§37 inventory complete). **(2)** the `web_fetch` tool handler is functional: routes to the hardened fn, FENCES the attacker-controlled page body (`RETRIEVED_KNOWLEDGE_UNTRUSTED_NOTICE` + `sanitizeUntrustedText`) before it re-enters the model, returns provenance `{url,title,fetched_at,truncated}`, honest refusal/failure (never a fabricated page), takes NO identity from tool args (seat/tenant resolved server-side). Client portal gains working link-fetch. §58: nothing removed (web_fetch stays declared + non-confirm; 3 CI assertions hold). **§32 proof** (no local Deno; `deno check` owed to CI): new `scripts/web-fetch-hardening-smoke.mjs` 29/29 (SSRF refusals incl. DNS→loopback, anti-vacuity vs old regex, safeFetch redirect-refusal + bounded read, fence, handler shape) — **now a standing CI gate in its OWN Node-22 job** (`smoke:web-fetch-hardening`; the smoke needs `--experimental-transform-types` for `SsrfError`'s parameter property, which the Node-20 `verify` job can't run, §24; CI-confirmed the job passed on 1a5d8020); `__checks__` 22/22 (real index.ts imports); client-memory-authz 268 + knowledge-scope 351 pass (each 1 pre-existing baseline fail, identical on clean main). **§39 peer SOUND** (no P1/P2 — SSRF guard fires unconditionally incl. service-role path; no identity from args; content always fenced; contract preserved); internal P3s folded: failure-branch error/reason now fenced, fetch-url-content 500 contract cosmetics (success:false + distinct `internal_error` reason). **§5 compliance SHIP** — mutation-tested the smoke (non-vacuous), flagged the CI-wiring gap (now closed). **Codex review: 4 findings** (P1 Node-version → dedicated Node-22 job; P1 visible-flow-evidence → `Visible-Flow-Impact: yes` trailer + `docs/evidence/ui-delivery/web-fetch-hardened-routing.md`, gate validated locally; P1 shipped-log-timing → §4.0 row moved to this post-merge closeout; P2 turn-budget → each fetch bounded by `min(15s, remaining WALL_CLOCK_MS)`, skip when spent) — ALL folded + threads resolved before merge. **PROOF OWED (§32.c/§70):** authenticated client-seat web_fetch live-drive. **KNOWN LIMITATION (disclosed, not a regression, §58):** safeFetch refuses redirects by design → a client pasting a link-shortener / www-canonicalizing URL gets an honest refusal, not a follow (the canonical SSRF posture; a bounded single re-validated hop is a future owner/product call, NOT silently relaxed). **FOLLOW-UP (§18, tracked):** `kb-ingest-url` still has its own weaker regex SSRF fork — migrate onto safeFetch (its comment was corrected from the now-false "identical to fetch-url-content's guard"). (Task #33.)
- **Funding & Coaching Tools Harness fail-closed gate SHIPPED (item 2 slice 1) + Codex 2nd-pass fix-forward (2026-09-13, Gate A).** GATE (PR #1222, merge `8c7fb131`, edge deploy run #315 success): new `_shared/funding-coaching-gate.ts` — the ONE HOME (§18) for "may this finance/credit provider run for THIS workspace right now?" — wired into `smartcredit-pull-snapshot`, `nav-pull-profile`, and `business-verifier` (before the ALL_ADAPTERS fan-out), refusing BEFORE any provider contact/write unless the tenant is package-entitled (the EXACT `is_finance`→`funding_enabled` predicate; no parallel system) AND has a per-tenant Financial connection/consent; fails closed on any read error. Capability-status resolver extended with `requiresPackage`/`packageEntitled` (backward-compatible). Nothing seeded/activated; every provider `unavailable` today (connection model absent + bundle unseeded). `ci:tsc` caught a TS2339 (`FundingConnectionFact` union access the repo tsc doesn't narrow; `deno check` had missed it) → fixed `689035f4`; vitest 28/28; §39 peer SHIP + §5 compliance SHIP. FIX-FORWARD MERGED (PR #1228, squash `86476a83`, edge deploy run #318 success §32.a) closing Codex's four P1s on the merged #1222 (layered defense — external review caught what the internal §39/§5 missed): **F1** the HTTP-200 refusal (`ok:false`) was recorded as SUCCESS by two server-to-server callers `skill-runner`(verify_business_sos) + `paige-mcp`(verify_business) — the §37 inventory had verified only the FRONTEND consumers; both now map the refusal to a non-success outcome (no `success_count` inflation, no false "tool completed"). **F2** `setup_required` ("install the package") was a DEAD remediation while the package is unseeded → new module constant `FUNDING_TOOLS_REMEDIATION_LIVE=false` makes every refusal resolve the honest `unavailable` (state preserved for audit); flips to `setup_required` only when Marketplace #670 + Financial #6 ship the surfaces — never a grant flag, only fails MORE closed. **F3** the three registry controlling entries said the gate was OWED → updated to the shipped contract + gate code_anchors (lint green, 70 anchors resolve). **F4** no `Visible-Flow-Impact` trailer/evidence → added `docs/evidence/ui-delivery/funding-coaching-tools-gate-refusal-propagation.md` + trailer. vitest 31/31; `ci:tsc` clean of this diff (only the pre-existing #1186 `signupMobile` baseline). A SECOND Codex pass on #1228 then found F2 (operational failures — BUSINESS_LOOKUP_FAILED / the BUSINESS_VERIFICATION_FAILED catch-all, both HTTP-200 `ok:false` — were mis-mapped to the policy `cancelled` instead of `failed`, hiding real failures from monitoring) + F3 (`BEHAVIORAL_EVIDENCE: PASS` claimed the caller propagation was proven, but no test exercised the callers); both fixed by extracting a UNIT-TESTED pure classifier `_shared/business-verify-outcome.ts` (ran/policy_refusal/operational_failure) that BOTH callers apply (vitest 45/45). F1 'missing trailer' was a FALSE POSITIVE — Codex parsed the ephemeral PR merge commit; the `Visible-Flow-Impact: yes` trailer is on the code commit and the ui-delivery-evidence CI check passed green. Layered defense (§39): external review caught what the internal §39 peer + §5 compliance both missed, twice. STILL OWED: the per-tenant connection/consent model (Financial #6), the Marketplace bundle (#670), §32.c authenticated live-drive; slice 2 (task #30) = Plaid + iSoftpull. Package stays `UNAVAILABLE`; no provider activated.
- **PAIGE Chat Runtime Correctness Phase 0 (#576) MERGED + edge-deployed — three silent runtime defects in paige-ai-chat fixed, regrounded on fresh main (2026-09-13, Gate A; squash `d33f6718`, edge-live=`d33f6718` zero drift; owner-authorized drive of the long-parked #576).** Three same-class defects (an identifier used where nothing declares it → a `ReferenceError` swallowed by a surrounding catch): (1) `runGeneralDocumentExtraction` called-but-never-defined → no extraction proposal was ever produced; implemented against the shipped `ExtractionProposal`→`data:{extraction_proposal}` SSE→`ExtractionProposalCard` contract, fabricates nothing (honest `null` for no-content/failed-call/unparseable/nothing-found), every emitted key a canonical `paige-write-back` field_path, §2 coaching-generic (no finance; no SSN/DOB §9/§13); (2) the tenant domain-identity read used out-of-scope `admin` → threw every turn, identity never reached the prompt; now via the caller-JWT `supabaseClient` (`resolve_tenant_domain_identity` self-scopes off `auth.uid()` — a §9 tightening, not a loosening); (3) the KB-save path assigned an undeclared `result` → false "did NOT save" on every genuine save; declared. **REGROUND:** #576's base (`00108e45`) was an orphaned pre-squash commit with NO common ancestor to main, so it was regrounded onto current main — the 3 repairs + their `__checks__` preserved exactly, no other work absorbed (owner directive); the `__checks__` run-command moved to `--experimental-transform-types` (current main's transitive `_shared` tree carries a TS parameter property strip-only mode can't parse). **§32 proof:** `__checks__` **22/22** (scope via the TS compiler + behavioural importing+executing the real `index.ts` with an injected model seam + the field-catalog contract) + anti-vacuity reproduces all 3 defects against the pre-fix source. **§5 compliance SHIP**; **§39 peer SOUND** — the peer caught a reground artifact (a now-stale "UNREACHABLE AT RUNTIME" comment on the domain-identity read that fix 2 invalidated) + NB-1 (the in-file run-command flag), both folded pre-merge. §32.a: `edge-live`=`d33f6718`, zero edge drift, `paige-ai-chat` live. `deno check` owed to CI (no local Deno — the behavioural import executing + the scope probe are the local proxies); `verify`/`Supabase Preview` red = pre-existing non-required baseline (stash-verified identical without this change; the historical `disputes` DDL, zero migrations here) — a standing-down comment posted on the PR. **PROOF OWED (§32.c/§70):** the authenticated live-drive of the 3 repaired flows (no isolated test tenant headless). **END-TO-END (corrected per §39/Codex review on #1221):** the frontend ALREADY consumes this — `PaigeAIChat.tsx` parses the `data:{extraction_proposal}` SSE frame (L993) and renders `ExtractionProposalCard` (L1624; `applyExtraction` on confirm/skip), `PaigeAIChat.documentProposal.test.tsx` exercising it — so #576's server fix COMPLETES a working end-to-end extraction-proposal flow (server emits → client renders the field-by-field confirm card → confirmed fields write; authenticated live-drive still §32.c PROOF OWED). #576's original "no frontend parses it yet" note referenced the RETIRED `PaigeChat.tsx`, not the live `PaigeAIChat.tsx`, and was wrong. **This CLEARS the shared `index.ts`/`PaigeAIChat.tsx` collision** → the remaining Track B slices + task #33 (web_fetch hardened routing) are now unblocked. (Closeout docs PR #1221.)
- **Funding & Coaching Tools — owner ownership split + GO to build the Harness fail-closed gate (2026-09-13).** Owner decision, no further product decision owed. OWNERSHIP SPLIT: Marketplace (#670) owns the F&CT bundle/catalog entry, tenant install entitlement, package copy + Marketplace UI; Financial Integrations owns Settings → Integrations → Financial (connection cards, consent, connection lifecycle, health, disconnect/reconnect, provider-specific setup UX); **Harness owns the SERVER-SIDE enforcement** — Gateway truth, entitlement/connection/consent checks, provider-call refusal, audit/receipt path, registry truth, tests; domain/provider adapters keep their own canonical provider logic. NO parallel entitlement system / connection registry / provider router (§18). BUILD GO (item 2): build the fail-closed gate NOW reusing the existing `is_finance`/`funding_enabled` marketplace entitlement seam + the existing encrypted tenant-connection pattern (`tenant_mcp_connections`-shape); do NOT seed a live entitlement or activate any provider in this slice. Every F&CT provider REFUSES before provider contact unless the server confirms (a) the tenant installed the package, (b) the tenant has the required Financial connection OR recorded consent/setup state, (c) the caller has tenant-scoped authority to the target, (d) Gateway/risk/approval/budget policy permits — else an honest `setup_required`/`unavailable` result and NO provider call. A mutable global config flag or a "dormant" producer is NOT an acceptable gate (item 4). Public research sources (SoS/OpenCorporates/SEC EDGAR): no fake OAuth — use the canonical package-consent/setup state required by the Financial connection model, while still requiring package entitlement + Gateway authorization (item 5). Runtime-safe FIRST; do NOT block the server refusal path on Marketplace/Financial-Integrations UI work (item 6). Do NOT activate ANY of the ten providers (Plaid, Nav, Array, QuickBooks, SmartCredit, D&B, LexisNexis, TransUnion Business, SoS, OpenCorporates, SEC EDGAR) for any tenant until the complete entitlement + connection/consent + authority path is merged, deployed, and proven (item 7); keep the package honestly UNAVAILABLE until the Marketplace bundle + Financial connection surfaces are actually merged + deployed. §58 FLAG (owner-directed, not silent): gating `business-verifier` makes its currently-reachable PUBLIC sources (OpenCorporates/SEC EDGAR direct; SoS via Firecrawl) return `setup_required` until the package is installed — an owner-directed tightening (item 7), authorized, flagged here. Tracked: task #29; builds as its own PR AFTER the docs fix-forward PR (priority 1) merges, on the designated branch reset to fresh main per the merged-PR rule.
- **smartcredit-pull-snapshot authorization bypass closed + shared `contact-authz` door extracted (2026-09-13, Gate A; branch `claude/paige-os-integration-82ywpq`, PR #1214 — MERGED to main squash `eb51ca4e04da886a59206a9f2fe68b0ee410d5c0`; `deploy-edge-functions` run #309 SUCCESS (Actions run 34747436387, completed=success, confirmed via API) → smartcredit-pull-snapshot + the shared contact-authz door LIVE on prod (§32.a persisted); §32.c authenticated live-drive still owed).** Slice 4 of the A/B governed-adoption program (item 2, bypass #4). `smartcredit-pull-snapshot` (consumer 3-bureau credit pull — highly sensitive PII) had the SAME §53/§59 trap as nav-pull: `requireAdmin` (GLOBAL admin) + body `contact_id` + service-role `clients` read with no tenant filter + paid SmartCredit call + write to `paige_owner_credit_snapshots`. §18: rather than mint a 4th near-identical adapter, EXTRACTED the shared `_shared/contact-authz/governed-adapter.ts` (`authorizeContactScopedAction` + a parameterized `buildContactScopedAudit`/`contactScopedGovernedAuditRow` taking the caller's capability key / actionType / actionPrefix) — nav-pull's `authorizeNavPull` was already 100% generic, so this is the ONE HOME for the contact_id→`can_access_contact` pattern (smartcredit now, the paige-plaid-* batch next). FIX: authenticate person/system; a person authorized via `can_access_contact(auth.uid(), contact_id)` (the canonical tenant-scoped helper the target table's RLS uses); actor from the JWT; authorize BEFORE any provider contact or write; governed audit on every decision; the FCRA `assertNoDisputeFields` scope guard preserved (now after authz). NEW risk key `smartcredit_pull_snapshot`=`high`. §67 autonomy clamp deferred upstream, recorded honestly (`enforcement:"authority"`, `autonomy_gate:"deferred_upstream"`). §37: ZERO code producers — grep-confirmed no UI/edge/cron/MCP caller — so this was a PREEMPTIVE IDOR close, no §58 removal. §13 CORRECTION (Codex 2nd-pass P2, 2026-09-13): the zero-blast-radius claim rests on the ZERO-producers fact [verified by grep], INDEPENDENT of the config flag — even if `smartcredit_enabled` were flipped true in prod, nothing invokes the function. The config DEFAULT is false [verified: migration 20260627194735:138 `smartcredit_enabled boolean DEFAULT false`], but the LIVE prod value of that mutable single-row flag is UNVERIFIED (no authenticated prod query attached) — so "config-gated OFF" is NOT claimed as a live fact; dormancy stands on zero producers. PROOF: 8/8 vitest on the shared adapter (matrix + smartcredit receipt shape) + `deno check` clean on both edge files + action-risk.ts; tsc-ratchet only the 2 pre-existing #1186 baseline errors. PROOF OWED (§32.c): authenticated runtime live-drive once the provider is activated. FOLLOW-UP (§18, noted not done — §58 don't-refactor-live-code): the already-LIVE nav-pull-profile keeps its own equivalent adapter; it can migrate to the shared door in a later cleanup, NOT risked here for a cosmetic refactor of a deployed function.
- **§13 FIX-FORWARD on merged #1214 (Codex 2nd-pass P1 + OWNER PRODUCT-BOUNDARY RULING 2026-09-13; docs-only).** Codex's second review pass (posted after #1214 merged) raised P1 (registry entries owed) + P2 (flag-state honesty, corrected above). Fix-forward: promoted **SmartCredit, Nav.com (business credit), and business-verification** to full Integration Capability Registry controlling entries (the delivery rule — #1207/#1213/#1214 had each merged owing them), reduced `uncatalogued_wired_providers` to iSoftpull + FRED, added README rows. The compliance+fidelity verifier caught three real over-claims pre-write (all integrated): (B1) SoS is **Firecrawl-mediated** (api.firecrawl.dev, FIRECRAWL_API_KEY), NOT a direct public endpoint — only OpenCorporates + SEC EDGAR are direct; (B2) Nav is NOT producer-free — `src/pages/admin/BusinessCreditAdmin.tsx` is a live producer, and `nav-refresh-scores` is cron-shaped but has NO in-repo schedule (prod scheduling unverified); (B3) deploy #309 confirmed success via the Actions API, not inferred from the stale local `edge-live` tag. Honest statuses: SmartCredit/Nav = PROOF_OWED; business-verification = PARTIAL (OpenCorporates/SEC direct + SoS-via-Firecrawl real; D&B/LexisNexis/TransUnion/Array inert stubs); all money_movement:false → m1:none (paid READS, not payments). **OWNER RULING 2026-09-13 — 'Funding & Coaching Tools' package boundary:** the ten providers (SmartCredit, Plaid, Nav, Array, SoS, OpenCorporates, SEC EDGAR, D&B, LexisNexis, TransUnion Business) are ONE optional Marketplace package, NOT universal Solo capabilities. Every provider fn must FAIL CLOSED unless the tenant has the package entitlement — reuse the existing **is_finance marketplace gate → funding_enabled** (one home `_shared/client-context.ts`; NOT a new system) — PLUS a per-tenant **Financial connection/consent** (Settings → Integrations → Financial). Registry entries now name package membership + entitlement + connection requirement. **HONEST GAP (§13):** the functions enforce only global config + can_access_contact/RLS authority today — they do NOT yet enforce package entitlement or per-tenant connection; that fail-closed gate is OWED (item 2, task #29), built on the existing finance gate + connection seam and COORDINATED with the Marketplace + Financial-Integrations workstreams (no parallel entitlement/connection system). Do NOT claim "Funding & Coaching Tools is available" until entitlement + connection + runtime are proven. Registry lint green (27 providers, 66 code_anchors resolve).
- **Track B (Main Paige Chat completion) grounding reconciliation + web_fetch §37/§58 finding (2026-09-13, grounded on then-fresh main `f1db1a18`; no code shipped — findings record, §BRAIN/§13).** Re-grounded Track B after #1177 merged. **§18 reconciliation — task #24 ("capability truth + first governed vertical") headline deliverables are SUBSTANTIALLY ALREADY SHIPPED on main (source-verified; authenticated-runtime PROOF still OWED per §70, so "shipped" ≠ "proven-usable"): do NOT rebuild.** (1) capability truth / the 6 truth states = `CapabilityAvailability` (8-value union) in `_shared/paige-capability-status/{resolver,signals,gatherer,render}.ts` + `paige-capability-gateway` (#1166/#10/#11); (2) contact-action outcome model created/already-existed/needs-clarification/failed = the `crm_create_contact` handler (`index.ts:10160-10253`) + `_shared/crm-capability-outcome.ts` (#1040/#22); (3) a first governed vertical runs end-to-end for `crm.advance_journey_stage` via the Layer-C engine (`paige-orchestration/engine.ts` → `native-adapter.ts` → `paige_act_executions` → `record_capability_run` → `emitAutomationRail`), now with the C5 approval-executor (#1212), chat-inline CRM writes running the equivalent inline governance. **Genuinely-remaining Track B = chat-completion-matrix slices 3-5 (web_fetch retire/route · tenant-KB citations · agent control), ALL in `paige-ai-chat/index.ts`/`PaigeAIChat.tsx` — the files the two STALLED drafts #576 (index.ts runtime-correctness, stale base `00108e45`, `unstable`) and #1044 (`dirty`/conflicted) own.** #576 surfaced as an owner task card (drive-to-merge: fixes 3 real silent chat bugs — extraction never produced, tenant domain-identity never reaches the prompt, KB-save falsely reports failure — AND unblocks index.ts); its exact hunks (1307-1359/2477-2512/11960-11985) are known so index.ts can be worked collision-safe. **web_fetch §37/§58 finding (the planned "Slice 3a retire dead tool" is NOT a clean autonomous merge):** `web_fetch` is inert platform-wide (def `index.ts:5237`, handler `8664-8665` returns "Web fetch not executed in this flow", stub note `130-131`), BUT it is one of only TWO tools in the CLIENT seat allow-list (`_shared/actorTier.ts:62-65` `CLIENT_SEAT_ALLOW`) and is documented to the client (`_shared/client-context.ts:972-978`), while auto-fetch-on-pasted-link (`index.ts:1390`) is NON-client only — so the **client portal has NO working link-fetch at all** (§13/§70 capability gap). Retiring it REMOVES a declared (inert) client-seat capability → §58 requires explicit owner sign-off; it also forces a retire-vs-route PRODUCT decision (§69) — route to a hardened fetch would make client link-fetch actually work but is §9/SSRF-sensitive for a client tier. It further touches a CI baseline (`scripts/ci/chat-tool-baseline.txt:107`) + two CI assertions (`client-memory-authz/check.mjs:1223` asserts `web_fetch` IS declared; `knowledge-scope/stage1-check.mjs:3300` lists it) + 3 docs. Surfaced for owner decision, NOT merged (overnight, §58/§69). **Net overnight posture (§13):** no unproven/§58/§9-risky merge manufactured; Track B's remaining increments are owner-gated or stalled-draft-blocked; authenticated-runtime PROOF-OWED posture across the cockpit unchanged (the universal proof gate — least-privilege Solo test tenant + `LIVE_DRIVE_*` — remains the #1 dependency).
- **Layer C · C5 (slice 1) approval-executor SHIPPED + edge-deployed + migration persisted (2026-09-13, Gate A; merge SHA `790b456d`, PR #1212; migration `20270202000000`).** The final Layer-C gap: a `confirm`-lane / high-risk-clamped act lands `approval_pending` in `paige_act_executions` and, until C5, DEAD-ENDED. C5 ships the execution mechanism + the ONE approve door WITHOUT weakening the C4 monotonic contract. (1) Guarded RPC `paige_approve_act_execution` = the SOLE sanctioned `approval_pending → accepted_for_execution` transition (`approval_pending` stays FINAL for the generic RPC so a re-drain can't clobber; SECURITY DEFINER service-role-only in-body `auth.uid() IS NULL`, §59; idempotent). (2) The executor (`_shared/paige-orchestration/approve-executor.ts`): re-resolves availability THROUGH the canonical Gateway (never a Layer-C literal, C2 correction); re-runs the ONE governed pathway with the human's yes as the approval claim → `execute`; redeems; dispatches the native adapter; advances executed|failed|ambiguous + receipt + owner Rail (Rail suppressed on a no-op). NATIVE only (n8n held act → `not_supported_in_slice`, never fired). RESUMABLE + honest: IMMUTABLE snapshot args (engine snapshots governed args into `detail.snapshot_args` at approval_pending; executor dispatches that snapshot + the ledger's own `capability_key`, never the mutable act row — closes a §70.2 confused-deputy); ledger-write-failure returns the PRIOR persisted state, never the adapter outcome (§13); ambiguous/accepted reconcile by correlation on re-approval, never blind re-fired. (3) execute-approval delegates orchestration-sourced approvals to the executor (the ONE door §18), consumes `executed|failed` ONLY (non-terminal → release claim, stay reclaimable); MCP `decide_pending_approval` refuses orchestration rows (routes to the one seam); comms/ACK untouched (§58). DORMANT until slice-2 minting. Reviews on the final committed diff (MESSAGE A): §5 SHIP (4 non-blocking); §39 peer SOUND + found ONE blocking regression — a cross-tenant outcome-enum DISCLOSURE (tenant-auth guard echoed `row.outcome` before the resumable gate, consumable as an attempt) — FIXED (constant non-terminal sentinel) + regression-tested, delta re-reviewed CLEAN; §68 non-blocking note (accepted-resume skips availability re-check — matches engine phase-5) documented + tracked. Local `deno check` (§32) caught 2 Deno-strict edge errors the green vitest/tsc masked (ApproveExecutorDb→optional-args rpc; execute-approval `admin as unknown as ApproveExecutorDb`) → FIXED (deno check EXIT 0 on all 3 affected entrypoints). §32.a PERSISTED: `edge-live` at `790b456d`, ZERO edge drift (execute-approval + orchestration graph + paige-mcp live); `deploy-migrations` `db push` + `migration list` verified prod caught up (`20270202000000` recorded remotely → migration applied), and after two transient GitHub-infra failures on the drift-tag push (500, then startup_failure) a workflow re-run re-synced `db-live` → `790b456d`, ZERO migration drift. Required CI green (Spine contract incl. pgTAP, Security Audit, migration-lint, ui-delivery-evidence). PROOF: vitest 109/109 across the 6 orchestration suites; tsc 14=14 (zero in C5 files); non-required `verify` red is the pre-existing `src/solo/*`+`signupMobile` baseline. PROOF OWED (§13/§32.c/§70): (a) a direct MCP prod object-query + BEGIN..ROLLBACK transition proof — Supabase MCP permission-denied THIS session (the CI migration-list caught-up verify transitively proves the function exists); (b) the authenticated approve→execute drive — owed once slice-2 mints a held act. SCOPE (§13): the execution core + door, unit-proven, dormant. Slice 2 = companion-minting into `paige_pending_approvals` (+ §37 producer inventory) + tracked follow-ups: §68 availability re-resolution on accepted-resume re-dispatch, a durable ambiguous-reconcile sweeper, release-claim-on-final-stamp-failure.
- **nav-pull-profile authorization bypass closed — governed authority adoption (2026-09-13, Gate A; branch `claude/paige-os-integration-82ywpq`, PR #1213 — MERGED to main squash `ce212e28978afae340879d653a934b605919165f`; `deploy-edge-functions` run #308 SUCCESS → nav-pull-profile + nav-refresh-scores LIVE on prod (§32.a persisted); §32.c authenticated live-drive still owed).** Slice 3 of the A/B governed-adoption program (item 2, bypass #3), mirroring the business-verifier fix. `nav-pull-profile` was `requireAdmin`-gated — the GLOBAL `user_roles` admin role, tenant-AGNOSTIC (§53/§59 trap) — then trusted the BODY `contact_id`, read `clients` via a SERVICE-ROLE client with NO tenant filter, contacted the PAID Nav.com business-credit API, and wrote `paige_business_credit_profiles` + fired a `business_credit_score_changed` bridge. So any global admin of any tenant could pull + persist ANY tenant's contact's business credit (§9 cross-tenant IDOR) and burn Nav.com budget. FIX: authenticate the caller — verified user JWT → `person` (actor = `auth.uid()`), else bearer == service-role key → trusted `system` (the `nav-refresh-scores` cron), else 401; authorize a person via `can_access_contact(auth.uid(), contact_id)` — the canonical tenant-scoped contact helper (SECURITY DEFINER, keyed on the passed uid, granted to service_role) that the TARGET table's own RLS uses, folding super_admin + tenant owner/admin + agency parent + direct/coach relationship into ONE predicate (simpler than business-verifier's three branches); derive the actor from the JWT never the body (nav-pull takes NO `triggered_by`); authorize BEFORE any provider contact or write (a denied caller burns ZERO budget); governed audit receipt for every decision. New pure adapter `_shared/nav-pull-profile/governed-adapter.ts` (§18) + NEW canonical risk key `nav_pull_business_credit` = `high` in action-risk.ts (distinct from `business_verify` and the `ordinary` `ingest_credit_scores`). §67 DEFERRED, not skipped (same as business-verifier — `high` caller-initiated sync flow; the autonomy/budget clamp for WHO may spend is an upstream §67 concern, recorded `risk:"high"` + `enforcement:"authority"` + `autonomy_gate:"deferred_upstream"`). §37 (2 producers): `BusinessCreditAdmin.tsx` (person JWT → can_access_contact) + `nav-refresh-scores` (the service-role batch). SIBLING-DOOR HARDENING (Codex P1 + internal §39 peer P3 BOTH flagged it — the business-verifier 'close the sibling door too' lesson applied): `nav-refresh-scores` was itself `requireAdmin`-gated (global admin) and service-role-selects stale profiles across ALL tenants, then fans out with the service key → nav-pull-profile's `system` path (which bypasses can_access_contact), so a tenant admin could still trigger cross-tenant paid pulls through it (and no cron could run it, since requireAdmin needs a person JWT). FIXED IN THIS PR: nav-refresh-scores re-gated with the same person/system pattern — a PERSON must be `is_platform_owner` (super_admin — a cross-tenant batch is the operator's action, §53), a `system` service-role caller (a genuine cron) is allowed; the cross-tenant select + fan-out now runs only for a caller authorized cross-tenant. No MCP/skill sibling door (grep-confirmed). §58/§13 HONEST scope change: authority now MIRRORS the contact's own RLS (`can_access_contact`) — NARROWER cross-tenant (IDOR closed) but BROADER same-tenant than the prior global-admin gate (a coach/staff with a direct relationship to the contact can now pull, if they can reach the surface); this is the faithful 'mirror the surface' choice (the business-verifier lesson), no legitimate capability removed; narrowing WHO may spend to tenant-admin only is a later product decision, not bundled into a security-IDOR close. PROOF: 9/9 vitest on the pure adapter (contact-access matrix + system + receipt shape) + `deno check` clean on both edge files + action-risk.ts; tsc-ratchet only the 2 pre-existing #1186 baseline errors (neither in touched files). PROOF OWED (§32.c): authenticated runtime live-drive of the Business Credit pull.

- **Booking-preset Draft→Publish lifecycle + guided chooser + shared server seam BUILT (2026-09-13, branch `claude/busy-archimedes-bsvrdi`; DRAFT — owner withheld release, NOT merged or deployed).** Approved Calendar capability expansion, one tenant-generic slice. BACKEND (migration `20270301000000`, additive): a `calendars.published_at` column (backfilled `= created_at` for already-enabled rows) + five SECURITY DEFINER RPCs — `create_calendar_preset` (draft-by-default: `enabled=false`, registers the creator as host, never flips live — fixing the old client-side insert-then-flip that made a new preset's `/book/:slug` live on creation), `update_calendar_preset` (partial-patch config edit, allowlisted; never touches enabled/published_at/slug), `publish_calendar_preset` (the ONLY seam that sets `enabled=true`; server-revalidates ≥2 hosts for round_robin/collective, an open window, a usable method — refuses with a specific reason otherwise; idempotent, preserves published_at), `pause_calendar_preset` (enabled=false, keeps published_at → Paused ≠ Draft), `get_calendar_presets` (tenant-scoped read + derived lifecycle). All §59 in-body caller scope (authenticated → can_manage/membership; service-role trusted only for the tenant it names), anon revoked. Lifecycle derives from `enabled` + `published_at` only — no second stored status (§57). Resolver `public-booking/index.ts` unchanged behaviorally (added a documenting comment); its `enabled=true` gate already refuses Draft AND Paused. FRONTEND: the Settings › Connections › Calendars surface becomes master/detail (compact preset cards → focused editor with the SAME ten-area builder behind it), the bare inline name creator is replaced by a "New booking preset" chooser (six recognizable models + starter templates + Custom; scheduling-model copy PORTED from the approved pack `super-admin-shell-v3/paige-ia.js`, §00), and the editor carries Publish/Pause + draft-honesty copy (a Draft/Paused link reads "Reserved URL — inactive until published" / "paused, not public", Copy says "future URL — not public yet", Open disabled; §13). `useCalendarConnections` now drives the RPCs instead of direct table writes; `config.ts` gains `published_at`, `presetLifecycle`, `publishReadiness` (client mirror of the server gate), and the `PRESET_CATALOG`/`draftForTemplate` (no baked tenant/brand/service identity, §2/§63). PAIGE CAPABILITY: the Spine domain `domains/calendar_preset.ts` (create/revise/publish/pause/list onto the SAME RPCs; publish/revise `high`, create/pause `ordinary`, list read) is authored and validated but DELIBERATELY not registered — registering a mutating capability asserts a LIVE chat binding that is only true once the Chat handler wires the tools (§13), so the chat adoption is handed off in `docs/architecture/booking-preset-capability-adoption.md`; the shared SERVER seam (the RPCs) is the deliverable. Also folds in the isolated removal of the redundant `SOLO SETTINGS / Connections` masthead. Lands the FU-2 (preset-config server-authorized write) + FU-3 (audit_logs provenance) SUBSET of `docs/doctrine/calendar-capability-contract.md`; FU-1 full per-capability Trust-Compass clamp still owed (§67/§68), and the Rail EVENT emission rides the (handed-off) capability layer. PROOF: local-Postgres replay of the real migration = 41 assertions, 0 fail (draft-by-default, §59 scope matrix, publish validation incl. the round_robin/collective 2-host honesty floor, pause-keeps-published_at, patch-injection immunity, backfill, tenant-scoped read + derived lifecycle, resolver `enabled=true` refusing Draft+Paused); `config.test.ts` 47, `useCalendarConnections.test.tsx` 10, `paige-capability-gateway.test.ts` 13; tsc 0 errors; eslint clean; lint:action-risk/action-authority/definer-fns/governed-execution/approval-gate green. PROOF OWED (§32.c/§70): the authenticated live owner drive (chooser→draft→configure→Publish with persisted + publicly-bookable readback, Draft/Paused NOT bookable), rendered Solo-viewport captures, and prod persisted-apply of the migration — all owed to a browser-capable session / the owner. FOLLOW-UPS (flagged, not silently done): legacy agency `CalendarsPanel` still creates live-on-create (reconciliation); `provision_tenant_default_calendar` still provisions the default calendar live; a create idempotency key; the chat wiring per the hand-off doc.
- **Layer C · C4 reusable ActionAdapter contract + exact state semantics LOCKED (2026-09-13, Gate A; merge SHA `2286e3ec`, PR #1208).** C1 built the ledger + monotonic RPC, C2 the native adapter, C3 the n8n adapter — so most of C4's mandate ("fail-closed unsupported; one `paige_act_executions` ledger; exact state semantics") was already delivered. C4 closed the genuine UNPROTECTED invariants, NO new engine, NO migration: (1) **ONE HOME** for the outcome vocabulary + FINAL/SETTLED set (`_shared/paige-orchestration/outcomes.ts`, §18) — the vocabulary had lived in FOUR hand-maintained copies (`decide.ts` `ActOutcome`, the SQL domain `paige_act_outcome`, `engine.ts` `FINAL_OR_SETTLED`, the SQL `_final` array) with no drift guard; `ActOutcome` + `FINAL_OR_SETTLED` now DERIVE from it, `ActOutcome` re-exported so importers are unchanged (§58). (2) **Bidirectional TS↔SQL drift guard** (decide test parses the already-shipped migration `20270126000000` and asserts SQL domain == `ACT_OUTCOMES` AND SQL `_final` == `FINAL_OUTCOMES`, set-for-set, both directions — the F5 registry-sync pattern applied to state semantics; replaces the prior one-way substring check that had no `_final` coverage). (3) **Uniform contract-conformance suite** across every registered adapter (native + n8n) — kind round-trip, `resolveCapability` exact (own→non-null+one-ledger-channel; foreign→null), fail-closed unsupported (unknown→null adapter), `DispatchResult.outcome` ⊆ vocabulary — plus a **self-enforcing coverage meta-assertion** (`registeredAdapterKinds()` exported; a new adapter FAILS the suite until covered) and a **true compile-time exhaustiveness** lock (`Record<DispatchResult["outcome"], true>`). §39 peer SOUND (zero confirmed defects; hand-traced the regexes/derived-sets/re-export/Deno-compat); §5 SHIP (even FALSIFIED the guard — injected `phantom_drift` → 2 tests failed). Three non-blocking review notes folded in pre-merge (self-enforcing coverage, true exhaustiveness, exercise `isFinalOutcome`). PROOF: affected vitest 89/89 across the 5 Layer-C suites; full suite adds ZERO new failures (the 4 red files are the pre-existing `src/solo/*` + `src/components/tenant-shell/*` baseline — zero `paige-orchestration` failures); tsc 14=14 (zero new, none in touched files); `outcomes.ts` strict-clean in isolation; new files lint-clean. §32.a PERSISTED (observed): `deploy-edge-functions` moved `edge-live` to `2286e3ec5c7bb4048ac04684346c01854454c17e`; `paige-native-event-dispatch` (the sole orchestration-graph importer) redeployed with behaviorally-identical source; zero edge drift; NO migration → `db-live` unmoved. Non-required `verify` red is the proven pre-existing baseline, not this PR's. PROOF OWED (§13/§32.c): none new — a contract lock with no tenant runtime surface. SCOPE (§13): C4 finishes the reusable CONTRACT; C5 (event/safety fabric completeness + AI COO realness) is the remaining Layer C slice.
- **business-verifier authorization bypass closed — governed authority adoption (2026-09-13, Gate A; branch `claude/paige-os-integration-82ywpq`, PR #1207 — MERGED to main squash `94470c910f12c6fee70d9fa43bd261bd22ec3f33`; `deploy-edge-functions` run #307 SUCCESS → business-verifier + paige-mcp LIVE on prod (§32.a persisted); §32.c authenticated live-drive still owed).** Slice 2 of the A/B governed-adoption program (item 2, bypass #2). `business-verifier` was invocation-gated (`verify_jwt=true`) but NEVER used the caller identity: it read `business_id` + `triggered_by` from the BODY, used a SERVICE-ROLE client with NO tenant filter, then contacted external registries + auto-activated PAID adapters (D&B/LexisNexis/TransUnion/Array = real budget) — so any authenticated user of any tenant could verify any business (§9 IDOR) and spoof the actor via body `triggered_by`. FIX (mirrors the write-back authority adapter): authenticate the caller — a verified user JWT → `person` (actor = `auth.uid()`), else the bearer == service-role key → trusted `system` (skill-runner/paige-mcp/DB trigger), else 401 (closes the anon-key-passes-verify_jwt hole); authorize a person by the surface's OWN read gate — can the caller SELECT the business under RLS via their JWT client (`businesses_tenant_staff_select`: `is_platform_owner()` OR the business's `owner_user_id` OR same-active-tenant staff of ANY staff app_role admin/coach/sales_rep/cs_rep/finance/viewer) — with an `is_platform_owner` fast-path (also the only verifier of a tenantless business) and an `agency_can_manage_child` fallback for the delegated case the RLS may miss; derive the actor from the JWT, never the body; authorize BEFORE any run insert or provider contact (a denied person burns ZERO budget); governed audit receipt for every decision. New pure adapter `_shared/business-verifier/governed-adapter.ts` (§18, reuses canonical key `business_verify`). §67 DEFERRED, not skipped: `business_verify` is `high`, so `decideGovernedExecution` on the default lane would clamp auto→confirm→propose, breaking this synchronous caller-INITIATED flow (all 3 producers) — so this door adopts the authority re-resolution + audit but executes on a passed verdict; the autonomy clamp is for Paige-AUTONOMOUS invocation, a separate §67 concern, recorded (`risk:"high"` on every audit row) not hidden. §37 (4 producers, none broken): card (person JWT → authorized by the same RLS read that renders the card, so every staff app_role that sees it still verifies), paige-mcp (system; NOW tenant-scopes `business_id` to `actorTenantId()` before dispatch, this PR), skill-runner (system, already tenant-scopes before calling), `trg_business_auto_verify` DB trigger (posts `Bearer service_role_key` → system; unaffected — separately non-functional today, follow-up (b)). §58: only the IDOR + actor-spoof closed; NO same-tenant staffer who can reach the Verify card loses access — authority MIRRORS the card's own RLS read rather than a narrower `tenant_members.role` gate, deliberately: `map_app_role_to_tenant_role` collapses sales_rep/cs_rep/finance/viewer→`member` while `businesses_tenant_staff_select` still admits them, so a role-list gate would silently 403 real staff the card is shown to (the §58/§70 trap this pivot avoids; caught by grounding the role model against the migrations before merge, not by the green suite). SCOPE-COMPLETION (internal §39 peer-gate on 60fa946d surfaced the over-claim; fixed in the same PR so "closes the cross-tenant IDOR" is true of the CAPABILITY, not just the direct endpoint): paige-mcp `verify_business` (index.ts:3556) forwarded an arbitrary body `business_id` with the service-role key → `system` → blanket allow, so the IDOR stayed open through that door. NOW scoped exactly as skill-runner does — `actorTenantId()` + `businesses.eq("id",business_id).eq("tenant_id",tenantId)` before dispatch, else `business_not_found_in_your_workspace`. Also (P3a robustness) the business-verifier `isPlatformOwner` dep now calls `is_platform_owner(_user_id)` through the SERVICE-ROLE client (answer depends only on the explicit verified uid), so it no longer fails closed if a future migration REVOKEs the (uuid) overload's authenticated grant. FOLLOW-UPS (noted, not fixed — scope): (a) the BROADER paige-mcp governed adoption (routing all its mutating tools through the governed seam + receipt) remains a separate target — only the business-verify IDOR door is closed here; (b) `trg_business_auto_verify` (mig 20260630013855) hardcodes a STALE `_project_url` (bfmyebsjyuoecmjskqhs ≠ prod xygzykjyynhzqytbqnzu) + swallows errors → pre-existing infra question; (c) MATERIAL FINDING `export-clients-csv` gated only on global `user_roles` admin → exports every tenant's profiles+FICO+auth-emails platform-wide (§9 cross-tenant READ exfiltration) — surfaced to owner, candidate future slice. PROOF: 14/14 vitest on the pure adapter (full person/system + tenant matrix + receipt shape) + `deno check` clean on both edge files. PROOF OWED (§32.c): authenticated runtime live-drive.
- **paige-write-back — RESIDUAL global-role trap closed + audit-error handling (2026-09-13, Gate A; fast-follow to #1197, PR #1201 MERGED `822e513f`; edge-deploy run #302 SUCCESS — live).** Post-merge Codex review of #1197 found two real findings my §39 peer + §5 compliance crew BOTH missed (the layered-defense lesson — external review caught what internal review did not). **P1 (privilege escalation):** slice 1 closed the CROSS-tenant half of the §53/§59 global-role trap but left the role SOURCE global — it read the caller's `admin`/`coach` from the tenant-agnostic `user_roles`. Because `current_user_tenant_id()` admits an active membership at ANY role, a caller who is a global admin/coach of tenant A but only a PLAIN MEMBER of tenant B could set B as their active workspace and pass both the global-role check and the tenant bond, writing B's client PII despite not being an admin/coach OF B. FIX: resolve the caller's authority TENANT-SCOPED from `tenant_members.role` for the resolved workspace (owner/admin → admin; coach → coach), never the global `user_roles` — the same correction migration `20261180000000` already made elsewhere (`is_tenant_admin()` over `has_role()`). The adapter now resolves the workspace FIRST, then the role in it; a global role is no longer an input. **P2 (swallowed audit error):** `writeGovernedWriteBackAudit` relied on try/catch, but supabase-js `.insert()` resolves a DB rejection as `{ error }` rather than throwing — so a failed audit insert (the ONLY trace of a refused cross-user write) was silently dropped; now inspects and logs the returned `error`. **P3 (agency-delegation gap, Codex's SECOND pass — found before merge; scope corrected by the §39 peer):** the first cut of the P1 fix resolved authority from a DIRECT `tenant_members` row only. Codex flagged that an agency operator managing a CHILD holds membership on the PARENT, so `callerRoleInTenant(child)` is null → denied. The §39 peer then narrowed the real scope: the CANONICAL agency entry (`agency_enter_subaccount`) FORCE-INSERTS a direct `tenant_members(child, admin, active)` row and never revokes it, so that flow resolves `admin` and was NEVER regressed. The genuine gap is the EDGE `current_user_tenant_id()` itself handles via its `agency_can_manage_child` branch: `active_tenant_id` = a managed child with NO active direct row (e.g. the membership removed post-entry) — reachable and platform-sanctioned, where a direct-only check wrongly denies. FIX (principled, not belt-and-suspenders for the edge): when there is no direct admin role, also resolve agency delegation via `agency_can_manage_child(callerTenantId, caller)` — the SAME predicate `current_user_tenant_id()` uses to let that operator operate in the child; granted to service_role → basis `agency_manager`. This aligns write-back authority exactly with the tenant-resolution logic, is harmless on the canonical path (directAdmin short-circuits it), and a plain member does NOT manage the child, so it never re-opens the escalation; the tenant bond still gates the target. §37: request contract unchanged → producers identical; the legit paige-ai-chat flow (a tenant admin/coach in their OWN tenant, AND now an agency operator in a managed child) resolves authority and is unaffected; only the escalation is closed (§58 — no legit capability removed). PROOF: 22/22 vitest incl. regression guards for BOTH the escalation (plain member → DENIED) and the agency flow (agency operator, no direct row → ALLOWED `agency_manager`; cross-tenant target still DENIED) + `deno check` clean. PROOF OWED (§32.c): authenticated runtime live-drive.- **Layer C · C4 reusable ActionAdapter contract + exact state semantics LOCKED (2026-09-13, Gate A; merge SHA `2286e3ec`, PR #1208).** C1 built the ledger + monotonic RPC, C2 the native adapter, C3 the n8n adapter — so most of C4's mandate ("fail-closed unsupported; one `paige_act_executions` ledger; exact state semantics") was already delivered. C4 closed the genuine UNPROTECTED invariants, NO new engine, NO migration: (1) **ONE HOME** for the outcome vocabulary + FINAL/SETTLED set (`_shared/paige-orchestration/outcomes.ts`, §18) — the vocabulary had lived in FOUR hand-maintained copies (`decide.ts` `ActOutcome`, the SQL domain `paige_act_outcome`, `engine.ts` `FINAL_OR_SETTLED`, the SQL `_final` array) with no drift guard; `ActOutcome` + `FINAL_OR_SETTLED` now DERIVE from it, `ActOutcome` re-exported so importers are unchanged (§58). (2) **Bidirectional TS↔SQL drift guard** (decide test parses the already-shipped migration `20270126000000` and asserts SQL domain == `ACT_OUTCOMES` AND SQL `_final` == `FINAL_OUTCOMES`, set-for-set, both directions — the F5 registry-sync pattern applied to state semantics; replaces the prior one-way substring check that had no `_final` coverage). (3) **Uniform contract-conformance suite** across every registered adapter (native + n8n) — kind round-trip, `resolveCapability` exact (own→non-null+one-ledger-channel; foreign→null), fail-closed unsupported (unknown→null adapter), `DispatchResult.outcome` ⊆ vocabulary — plus a **self-enforcing coverage meta-assertion** (`registeredAdapterKinds()` exported; a new adapter FAILS the suite until covered) and a **true compile-time exhaustiveness** lock (`Record<DispatchResult["outcome"], true>`). §39 peer SOUND (zero confirmed defects; hand-traced the regexes/derived-sets/re-export/Deno-compat); §5 SHIP (even FALSIFIED the guard — injected `phantom_drift` → 2 tests failed). Three non-blocking review notes folded in pre-merge (self-enforcing coverage, true exhaustiveness, exercise `isFinalOutcome`). PROOF: affected vitest 89/89 across the 5 Layer-C suites; full suite adds ZERO new failures (the 4 red files are the pre-existing `src/solo/*` + `src/components/tenant-shell/*` baseline — zero `paige-orchestration` failures); tsc 14=14 (zero new, none in touched files); `outcomes.ts` strict-clean in isolation; new files lint-clean. §32.a PERSISTED (observed): `deploy-edge-functions` moved `edge-live` to `2286e3ec5c7bb4048ac04684346c01854454c17e`; `paige-native-event-dispatch` (the sole orchestration-graph importer) redeployed with behaviorally-identical source; zero edge drift; NO migration → `db-live` unmoved. Non-required `verify` red is the proven pre-existing baseline, not this PR's. PROOF OWED (§13/§32.c): none new — a contract lock with no tenant runtime surface. SCOPE (§13): C4 finishes the reusable CONTRACT; C5 (event/safety fabric completeness + AI COO realness) is the remaining Layer C slice.
- **Durable task↔thread link hardened to TRUSTED server-side context (2026-09-13, Gate A, branch `claude/paige-cowork-handoff-rpf6dw`).** Second dependency-ordered Chat Product Completion slice. The gap was NOT "nobody sets `tasks.source_thread_id`" — the `crm_create_task` chat handler DID stamp it, from the **raw client request-body `threadId`** via a **service-role insert that bypasses RLS**, with **no validation** that the thread belonged to the caller. A client could therefore record ANOTHER tenant's conversation (or a forged uuid) as a task's provenance — the "client-supplied source_thread_id" the owner forbids (§9 integrity hole; not a transcript leak, since a dangling uuid grants no read). **Fix:** validate the claimed thread server-side as the caller's OWN thread in the RESOLVED tenant — an RLS read on the caller-JWT client (`supabaseClient`, anon key + caller Authorization), explicitly filtered `id = <claim> AND tenant_id = crmTenantId AND caller_user_id = user.id` — BEFORE stamping; a foreign / forged / expired / absent thread (and an account-switched caller, which re-resolves `user.id`+`crmTenantId`) → stamp `NULL`. The service-role `admin` client still does the insert, but only with the VALIDATED id. Decision rule in ONE home (`_shared/source-thread-link.ts`, §18); NO new SECURITY DEFINER (§59 — an RLS read, not a definer); NO second task/conversation registry. The link is **traceability only, never task-completion proof** (§13) — `status` is untouched. `crm_create_task` tool schema + the `tasks` insert column set + `crm_list_tasks` response shape are unchanged (§37: no producer/consumer contract break). **Proof by class:** pure-helper unit (`src/__tests__/source-thread-link.test.ts`, the 6 owner scenarios over the injected validation); source-assertion wiring (`src/__tests__/task-thread-link-wiring.test.ts` — validation on the RLS client with owner+tenant filters, stamps the validated id, old raw-body stamp gone, link≠status); and a SET-ROLE RLS proof (`supabase/tests/source_thread_link_scope.sql` — same-tenant validate / cross-tenant denial / account-switch denial / no-discovery-or-leak via a foreign link) wired into the `database-contract` CI job (spine-contract workflow). 14 new tests + 92 existing crm/confirmation tests green; helper + index transpile clean. §39 adversarial peer-gate + §5 compliance run on the real pushed diff. **Owed:** the authenticated end-to-end chat drive (§32.c/§70) — headless-impossible here; the universal proof gate (least-privilege Solo test tenant + `LIVE_DRIVE_*` secrets).
- **P0 confirm-loop repair fix-forward — a lookup failure during an approval now ends in the honest terminal, not the loop (2026-09-13, Gate A; PR #1195 → `main` squash `ba1103eb`, production).** Fix-forward of #1185's FIX A approved-set lookup, closing two Codex P1s on the merged P0 that my own §39 pass missed (layered defense: §39 + §5 + external review + CI, none alone sufficient). A returned PostgREST error OR a thrown jsonb-path-filter rejection mid-approval left `approvedSetAmbiguous` false, so with a non-empty approved set execution fell through to `recordConfirmation` and minted a drifted proposal — recreating the exact re-ask loop the P0 exists to stop. Both the returned-error and thrown paths now set FIX B's honest terminal (nothing claimed, nothing recorded) and log loudly (§68); the block's `approvedConfirmations.size > 0` entry guard means a first proposal still gets its own card. Added the `Visible-Flow-Impact: yes` trailer + `docs/evidence/ui-delivery/chat-confirm-gate-terminal.md` (Codex Finding 2). **§32 persisted deploy:** `deploy-edge-functions` run 300 redeployed `paige-ai-chat` (911 kB bundle, bundles `confirm-fingerprint`); `edge-live` moved `8114284f`→`ba1103eb`; `git diff edge-live..main` on `supabase/functions/**` is empty (zero drift). confirm-fingerprint 13/13 + confirm-gate wiring 13/13 green; exact-head CI (`workflow_dispatch` run 34738689203) `verify` red PROVEN pre-existing main-wide debt — the only smoke failure (#78 client-memory-authz `19.8 every executable mutation names the entity it touches`) is the #1186 social tools unmapped in `WRITE_TARGET`, and `_shared/action-risk.ts` is byte-identical HEAD↔main with the same three tools unmapped there; Deno ratchet / Build / integration-registry / ESLint all green, so this diff adds ZERO new failures. §39: no reviewer finding on the PR; no unresolved threads. **PROOF OWED (§32.c/§70):** the authenticated deployed-surface three-outcome battery + the live lookup-failure terminal drive (no test tenant / prod MCP / browser-drive this session).
- **Layer C · C3 governed n8n workflow-run ActionAdapter SHIPPED + edge-deployed (2026-09-13, Gate A; merge SHA `52f32866`, PR #1203).** The n8n workflow-runtime adapter (fire + POLL readback) as a reusable Layer-C ActionAdapter, driving the SAME fire/poll core `paige-n8n` drives — never a forked n8n client (owner directive, §18). NEW shared seam `_shared/n8n-run.ts` (the one home): `n8nFetch` (SSRF-guarded REST transport, moved verbatim), `resolveN8nConnection` (`get_tenant_n8n_secret` + instance-URL SSRF vet), and the `fireN8nWebhook`/`getN8nExecution` cores — each parameterized by an EXPLICIT tenant + a service-role client, resolving creds internally so the API key never leaves the seam. WHY a shared seam and not `functions.invoke`: `paige-n8n` is JWT-tenant-gated and the headless Layer-C engine holds a service-role client + no JWT (it would 401), so the core is shared not forked. `paige-n8n` refactored to consume the seam, PROVEN byte-equivalent (§58) by the live-handler smokes (`smoke:n8n-ssrf` 33/33, `smoke:n8n-egress` 39/39 — SSRF, creds, and response projections unchanged). NEW `_shared/paige-orchestration/n8n-adapter.ts` wired into `adapters.ts`: ASYNC (n8n has no completion callback → poll is the only readback). dispatch fires → `accepted_for_execution` + the execution id (NEVER `executed` on a bare 2xx; a definitive refusal/non-2xx → terminal `failed`); readback polls → success (no node error) → `executed`, node error/non-success → `failed`, running/waiting → `ambiguous`, no id/not-found/infra → `ambiguous` (never a blind re-fire). Correlation stamped into the webhook payload (spoof-safe — the ref is set last); binds the poll by execution id (→ provider_ref). Pure mappers, unit-proven; `delivered` is a detail, never flips a successful RUN to failed. HIGH-risk hold PRESERVED + TRIPLE-guarded (governed clamp auto→confirm→approval_pending; engine execute-plan native-only at engine.ts:304; phase-5 native-only at engine.ts:521) — an unattended auto-lane n8n act HOLDS for approval and never dispatches; NO engine change. §39: one PLAUSIBLE finding (the refactored `run` case handled only 4 of 7 refusal codes with no catch-all → a race could false-`ok:true`) fixed with a catch-all restoring parity with `execution_get`. §5 SHIP (all criteria; byte-equivalence proven, hold triple-guarded, tenant scope + key boundary correct). PROOF: 199 vitest across 8 orchestration/capability suites incl. the new n8n adapter suite + the unchanged engine hold test; real differential Deno Edge typecheck clean→clean on `paige-n8n` and `paige-native-event-dispatch`; tsc-ratchet only the 2 pre-existing main-inherited errors. §32.a: `edge-live` at `52f32866`, ZERO edge drift (paige-n8n + drainer live); no migration. PROOF OWED (§13/§32.c): an authenticated live-tenant n8n fire→poll drive (no isolated test tenant / real n8n instance headless — never a real customer record). SCOPE (§13): C3 is the reusable adapter mechanism + shared seam + preserved hold, unit-proven WITHOUT firing a real workflow; the approval-executor that fires an APPROVED n8n act is a later slice (C4/C5).

- **paige-write-back cross-tenant write IDOR closed + governed-seam adoption (2026-09-13, Gate A; branch `claude/paige-os-integration-82ywpq`, PR #1197).** Slice 1 of the A/B governed-adoption program (item 2: close the highest-risk consequential bypass). `paige-write-back` writes sensitive tenant data (`businesses`/`profiles`/`intake.*`/`foundation.ein`/`credit_negative_items`/`credit_accounts`) through a SERVICE-ROLE client with RLS bypassed; for a cross-user write (`target_user_id != caller`) the prior guard consulted the GLOBAL `user_roles` table for an `admin`/`coach` role — no tenant predicate (the §53/§59 global-role trap) — so a tenant-A admin/coach could write a tenant-B user's credit/identity record (a §9 cross-tenant write IDOR). FIX: cross-user write permitted only for a PLATFORM OWNER (`is_platform_owner()`, super_admin ONLY per §53 — matching the sibling gate at `paige-ai-chat:808`), OR an `admin`/`coach` whose SERVER-RESOLVED active workspace (`current_user_tenant_id()`) OWNS the target, with the existing `coach_clients` assignment still required for a coach; fails closed on unresolved tenant/target/read error. Routed through `decideGovernedExecution` via a new pure door-adapter `_shared/paige-write-back/governed-adapter.ts` (mirrors the skill/mcp doors; reuses canonical key `update_client_data`, §18; governed receipt to `paige_audit_log`). §58: the ONLY behavior change is the IDOR closure — same-tenant admin + same-tenant assigned coach unchanged; `coach_clients` check identical to prior code. §37 inventory: ProfileSettings.tsx + PaigeChat.tsx are self-writes (no `target_user_id`, unchanged path); paige-ai-chat:8559 forwards the caller JWT and is already same-tenant-gated upstream; no trigger/cron/Action/webhook/n8n/Zapier/MCP caller. `target_user_id` is `z.string().uuid()` — no `.or()` filter-injection surface. PROOF: 17/17 vitest on the shipped adapter (full IDOR matrix) + `deno check` clean. MERGED to main as `8114284f` (squash, Gate A); §32.a edge-deploy PROVEN — `deploy-edge-functions` run #299 on `8114284f` = SUCCESS, paige-write-back live. PROOF OWED (§32.c): authenticated runtime against the deployed function — owed to a browser-capable session, not claimed headless. FOLLOW-UP (same program): post-merge Codex review found a RESIDUAL global-role trap + a swallowed audit-insert error that §39+§5 missed — fixed in the next entry.
- **Layer C · C2 native journey-advance auto-execute vertical SHIPPED + edge-deployed (2026-09-13, Gate A; merge SHA `24005a80`, PR #1188).** The one bounded native, synchronous auto-execute vertical: an act `action_kind` `crm.advance_journey_stage` → the tenant-aware `set_journey_stage` RPC, on the C1 governed event→act ledger. Availability resolves THROUGH the canonical Gateway seam (`resolveNativeCapabilityStatus`, composing `getActorTier` + a `user_roles` read + `resolve_tool_autonomy` + `clampLaneByRisk` + the pure `buildCapabilitySignals`/`resolveCapabilityStatus`) — NEVER a Layer-C availability literal (owner correction 2026-09-13). B1 (both reviewers): the vertical was FK-dark — `paige_automation_acts.action_kind` FK-binds `paige_action_kinds(slug)` and the slug was unregistered; migration `20270128100000` registers it (platform default `tenant_id` NULL, coaching-generic §2, executor `record_only`, `default_autonomy_lane='auto'` — a floor, not standing authority — `requires_approval=false`; `chk_auto_lane_safe` permits `auto` for `record_only`; `action-risk` = `ordinary`). Dotted `action_kind`/manifest-key ↔ underscore tool/risk/receipt-key seam. §39 F2: phase-4 skips native, phase-5 read-current-first (a re-drain never overwrites an advanceable row with a flipped refusal). §39 F3/F4: confirmation is BY CORRELATION ONLY (a transition stamped `source_event=correlationRef`, never a bare current-slug match; readback needs no slug so a crash-recovery drain still reconciles). §39 F5: a test pins `NATIVE_EXECUTORS` keys == `nativeCapabilityBindingKeys()`. §39 F1: a native `ambiguous`/non-terminal outcome had no reconcile trigger once the event completed `done` — the phase-5 reconcile was unreachable in prod; fixed by `EngineResult.reconcile_pending` → the drainer keeps the event reclaimable so the EXISTING sweeper (attempts<5 → terminal `error`, never a silent `done`) re-drives it and phase-5 reconciles by correlation (no new cron, §18). §37: the sole `EngineResult` consumer (paige-native-event-dispatch) supplies the new `journeyAdvanceLane` fact in paige-ai-chat too. Reviews on the final diff: §5 SHIP (all 8 owner criteria met); §39 one CONFIRMED finding (F1), fixed and independently re-cleared. PROOF: 323+ vitest across 16 Layer-C suites; real differential Deno Edge typecheck clean→clean on the drainer, 17→17 on chat (zero new); migration version-collision + `lint_migrations.py` + `lint:definer-fns` green; required CI (PAIGE Spine contract, Security Audit, migration-lint, ui-delivery-evidence) green. Persisted-apply confirmed (§32.a): `deploy-migrations` SUCCESS on `24005a80`, `db-live` at `24005a80`, ZERO migration drift, `20270128100000` live; `deploy-edge-functions` SUCCESS, `edge-live` at `24005a80`, ZERO edge drift. PROOF OWED (§13/§32.c): an authenticated live-tenant end-to-end journey-advance drive through the deployed drainer (no isolated test tenant available headless — never a real customer record). Surfaced (out of Layer-C scope, tracked separately): the `verify` job's reds are ALL pre-existing/main-inherited/non-required (from the #1186 solo-beta + social merge) — 2 tsc-ratchet (`paige-capability-gateway.test.ts` exhaustive-Record gap; `signupMobile.ts` `libphonenumber-js/min`), 18 vitest in 4 `src/solo`/`src/lib/auth` files, and a **HIGH-priority §9/§59/#116 finding**: `public.paige_unclassified_inbound` (migration `20270115000000`) is a public view missing `security_invoker` that bypasses `messages` RLS — needs its own bounded forward-migration fix + §37 producer inventory.
- **Solo Calendar Settings write gate fixed to the VIEWED tenant + New-appointment failure observability (2026-09-13, Gate A; branch `claude/busy-archimedes-bsvrdi`).** DEFECT 2 (real, fixed): the Settings › Connections › Calendars write gate (`useCalendarConnections`) read `canWrite` from `is_current_user_tenant_admin()` — admin of the caller's OWN `current_user_tenant_id()` (= `profiles.active_tenant_id`), NOT the VIEWED `activeTenantId`. On a stale profile pointer, a tenant switch before the profile persists, or an operator acting-as, it hid 'New preset' (and edit/enable/hosts) from an owner the write would accept. Fix (client-only, live on frontend deploy — no migration): `canWrite = is_tenant_admin(activeTenantId) OR is_platform_admin()`, evaluated against the viewed tenant, mirroring the `calendars` manage-RLS. Both helpers already deployed + client-granted. DEFECT 1 (New appointment): the server `create_internal_booking` seam was PROVEN sound by a faithful local-Postgres replay of the real DDL + verbatim 17-param function (create ok + persisted; overlap→23P01; cross-tenant/unauth→42501; only the deployed 17-param overload exists, 11-param DROPped in 20260712120000); no code defect found. Kept the RPC untouched; improved client observability instead — `createBooking` now routes refusals through `classifyBookingWriteError` → honest `{category,message}` (conflict/forbidden/not_found/invalid/unavailable/network/unknown), so a live PGRST202/203 (RPC not resolving) reads as 'nothing was booked, try again' rather than a raw dump, and never as a booked appointment (CreateDrawer only closes on ok). PROOF: hook test (viewed-tenant call, stale-profile grant, platform-admin, tenant-switch re-eval, deny) + local-PG RLS-parity harness (gate predicate == real manage-RLS INSERT: owner@own allowed, owner@other refused, non-member refused) + classifier unit tests + New-appointment refusal/success workspace tests; 216 tenant-calendar+connections tests green; tsc-ratchet adds 0 new errors; eslint clean. PROOF BOUNDARY (§32.c/§70): the authenticated live drive — New appointment end-to-end and the 'New preset' button enabled for the signed-in owner with persisted readback — is PROOF OWED to a browser-capable session; the owner is capturing the exact live create request. No public booking preset was created (diagnosis was read-only).
- **Solo-only Beta acquisition + database release — production application/database (2026-09-12, #1148 → `a725633b` + #1190 → `42ad0d36`, owner-approved Gate A; record carried forward 2026-09-20 from stale closeout #1192 under INT-103).** Solo-only fixed 30-day trial then $74.50/month, fail-closed unsupported enrollment/duplicates/legacy destinations, atomic webhook fulfillment with server-readback destination, US-default + international phone entry (E.164 storage). VERIFIED now: both SHAs on main; deploy-migrations run 34735838226 success at 42ad0d36. Owner-attested 2026-09-20 (not independently verified): production deploy run 6417319452, db-live-at-the-time equality/zero drift, 178/178 rendered checks. Stripe stayed TEST mode; authenticated fulfillment + live financial activation remained PROOF OWED.

- **Solo Calendar owner reschedule + edit wired to the live governed booking engine (2026-09-12, Gate A; branch `claude/busy-archimedes-bsvrdi`, PR #1176).** The mounted Solo calendar was already real (`useSoloCalendar` + `list_team_bookings`/`create_internal_booking`/`reschedule_internal_booking`/`cancel_internal_booking`), so the genuine gap was owner-driven *edit* and a reschedule affordance. Added migration `20270125000000_calendar_update_internal_booking.sql` (`update_internal_booking` SECURITY DEFINER, §59 in-body tenant-scoped caller guard). Codex §39 review then surfaced that the whole booking-mutation family guarded on the tenant-agnostic `has_any_role` (§59 global-role trap): fixed across update/reschedule/cancel to a `tenant_members` check on the row's tenant (§37 inventory: reschedule's only caller is this PR's UI, cancel none, update new — no legit caller regressed, §58). Also: reschedule withheld for class sessions (single-row move would orphan seats), and 22023 split by message prefix for honest edit errors. §32/§70 authenticated behavioral proof is PROOF OWED to a browser-capable session — not claimed from structural/migration proof alone. MERGED to main as `4fdf471` (squash, Gate A); §32 persisted — `deploy-migrations` moved the `db-live` tag to `4fdf471`, migration present in the db-live tree, zero drift.
- **Two governance rulings — Registry Steward assigned, `ui-delivery-evidence` made required (2026-09-12, owner).**
  **(1) Registry Steward → the Harness workstream.** A governance and consistency role, explicitly
  NOT a feature-development bottleneck and NOT an approval queue: Harness owns the canonical registry
  contract, the allowed truth states, cross-domain reconciliation, and escalation of unowned/unsafe
  entries; domain workstreams keep their own provider adapters and propose their own capability/
  connection entries, and the per-entry `owner` field still names the domain owner. No new registry,
  queue or dependency chain. Recorded in `integration-capability-registry.json`
  (`known_owed_entries.registry_steward_role`, previously "UNDEFINED") and in the portfolio artifact.
  This makes the standing order's "notify the Registry Steward" path executable for the first time —
  but the missing piece was never only the owner: **nothing in CI couples a provider-touching diff to
  its registry entry**, which is why Upload-Post (`6e35c60e`) and fal.ai (`d5387376`/#1153) both
  merged owing entries. The mechanism (a `code_anchors` schema field + resolve check) is sequenced
  separately.
  **(2) `ui-delivery-evidence` becomes a required status check on `main`,** scoped behaviour
  preserved (UI change → evidence required; non-UI change → honest N/A). Exact context:
  `ui-delivery-evidence / Validate UI delivery evidence` (workflow/job — the job name alone will not
  match). **§13 — NOT YET IN FORCE:** branch protection is a repository setting, not a file; a
  headless session measured `403 Resource not accessible by integration` on
  `GET /branches/main/protection`, so it can neither set nor verify it, and workflow YAML is never
  proof that merges are blocked. Owner action + read-back owed. Measured at ruling time: **41 open
  PRs**, many UI-touching — those lacking a conforming evidence record will be blocked, which is the
  intended effect, recorded so it is not a surprise. Both rulings land with PR #1175.

- **Current-main Platform Health & Security is reconciled in the existing consolidated audit, not a new audit system (2026-09-12; documentation update, not a runtime repair; pinned main `9cac02d66ab068a7cff6a032bca267c9a928a3c6`; audit `2026-09-12T20:23:44.2006964-04:00`).** Current health may cite only merged-main artifacts; PRs/issues identify ownership/collisions, workflow definitions identify intended controls, and neither changes runtime truth. Source truth, production/deployed truth, authenticated proof and proposed remediation remain separate. The current matrix lives at `docs/assessments/CONSOLIDATED_PLATFORM_AUDIT.md`; historical warnings are retained, retired or reopened only after fresh corroboration. Merged #1175's Capability Portfolio routing/Registry Steward ruling and `9cac02d6`'s fal result-readback fix are incorporated without turning portfolio or commit prose into runtime proof. Later repairs append a new exact-SHA reconciliation entry; they do not silently rewrite this snapshot. The decision packets for `handle-inbound-webhook`, unknown-budget fail-closed behavior, branch-protection readback, and credential remediation remain owner decisions. Tracked-`.env` metadata and private issue #788 are distinct credential packets; neither authorizes guessed scope, rotation, or history rewriting. No runtime/schema/provider/credential/deployment change, and no Shipped Log or release claim, is created by this decision.

- **Social publishing INTERIM server-side containment merged; Codex #1162 is canonical (2026-09-12, owner-ruled, Gate A; PR #1164, main `19ccb133699837d745c053769070ca7a3659f06f`, production channel).** Owner chose "merge #1164 now as the interim containment" over deferring to Codex's #1162. Denies Social PUBLICATION at both content-publication seams — `paige-social` `post` (Upload-Post) + `meta-schedule-post` (Meta Graph FB `/feed` + IG `/media_publish`) — with an honest `UNAVAILABLE` returned BEFORE any provider call / success / receipt/Rail. Pure `_shared/social-publish-containment.ts` decides on the action name ALONE, so a configured `UPLOAD_POST_API_KEY`/`META_PAGE_ACCESS_TOKEN` can't flip it (§13/§38 — connected/configured ≠ authorized). Contained body carries `success:false`+`ok:false`+`error:"social_publish_contained"`+`contained:true` — fixes the §39-found false-"succeeded" `paige_audit_log` receipt. Reads (`accounts`/`analytics`/`post_analytics`/`audience`/`comments`/`status`/`scheduled`) + `cancel_scheduled` preserved; account `connect` is NOT wired (returns `unknown_action`) and no longer over-claimed in the copy. §37: the two seams are the only content publishers (Chat `social_post`, service-role, cron all pass through them). Proven: `social-publish-containment` 13/13 (unit+structural+perturbation), `lint:action-risk`+`lint:governed-execution` green, `ui-delivery-evidence` PASS on PR #1164 (the `Visible-Flow-Impact: yes` trailer on branch commit `2b8ec97` routed the change; CI validated the record). §13 precision (Codex #1167 P2): the squash-merge did NOT carry that trailer into main `19ccb13` (only Co-Authored-By/Claude-Session parse as trailers) — expected, since `ui-delivery-evidence` is a PR-time gate and the evidence record is on main. Codex review folded: P1 (visible-flow evidence record + trailer), P2 (honest copy — dropped the account-connection over-claim). **OWNER RULING — INTERIM, not canonical:** Codex PR #1162 (Social Operations Gate A — containment + foundation) is the canonical Social build; when it rebases onto fresh main it MUST preserve/strengthen EVERY denial here (no silent re-enable of `social_post`, Upload-Post, Meta publishing, false receipts, or any alternate publication seam) and resolve the `paige-social/index.ts` overlap ONCE — retaining the stronger canonical impl, never two permanent containment modules. "Contain first; replace the interim guard only with the complete proven foundation." **§32.c OWED:** authenticated live-drive of the refusal owed to a browser-capable session (headless here). **§66:** Social publishing gating outcome unchanged (already UNAVAILABLE everywhere per tier-matrix Social section + integration-registry Meta entry) — this ships the server-side ENFORCEMENT layer; tier-matrix Social section updated with the interim-containment note; the definitive Social tier reality is owned by canonical #1162. Edge deploy: `paige-social` + `meta-schedule-post` via `deploy-edge-functions.yml` (run #282). Evidence: `docs/evidence/ui-delivery/social-publish-containment.md`.

- **P0 live-chat fix: truthful self-knowledge + approval-loop repair (2026-09-12, Gate A).** Two live defects in the authenticated Solo-owner Paige chat. **Defect 1 — capability over-claim:** "what can you do here?" answered from the tool list + persona (CRM, pipeline, social posting, campaigns, n8n/Zapier, SMS, team management as if all live). Fix grounds the answer in a server-resolved manifest: `_shared/paige-capability-status/signals.ts` EXPANDED past the 3-capability MVP to every over-claimed family, each maturity **read from the Spine registry** (`getSpineCapability(key)?.maturity`, never hardcoded — the only hardcoded LIVE is the two documented CRM seams that have no `crm.*` registry row); an ACTION family with **no registered governed seam** (`social.publish`, `comms.send`, `team.manage` — confirmed absent from the registry) resolves `null → UNAVAILABLE → "planned"` (the anti-over-claim). New pure `render.ts` emits an AUTHORITATIVE per-turn system block that OVERRIDES the tool/persona impression, forbids claiming post/text/run/manage unless available, grouped can-do-now / prepare-for-approval / needs-setup / not-built-yet. One shared `gatherCapabilityManifest` (index.ts, after hoisting `resolveToolAutonomy` above prompt assembly) feeds BOTH the prompt block AND the `capability_status` tool (§18 — can't diverge), server-side facts only (tier + ceiling-clamped lanes + registry maturities + real n8n connection), gated tenant-only/non-client. Honest fresh-Solo result: reads (contacts, connections, pipeline, inbox, social accounts, team, campaigns) LIVE; contact/campaign-brief writes NEEDS_APPROVAL; n8n NEEDS_SETUP until connected; social publish / SMS send / team manage PLANNED. **Defect 2 — approval loop on dismissing a batch of 3 drafts:** root cause `confirmFingerprint` hashed the model-authored `decision_rationale`, which the model rephrases on the approval turn, so the fingerprint drifted and the card never bound. Fix A: `confirmFingerprint` extracted to `_shared/confirm-fingerprint.ts` (pure, unit-tested) with a NARROW per-tool `NON_IDENTITY_ARGS` allowlist (`{action_advance:["decision_rationale"]}`) dropping non-identity free-text; identity (action_id+to_status) + all consequential params stay in the hash; safe because the seam runs the STORED proposal args, never the re-emission. Fix B: a non-looping note pointing the operator to the already-rendered card once. Also fixed the source file's raw NUL byte → ` ` escape (identical runtime hash, reviewable text diff). **Lavelle/heartbeat/reminder audit:** the chat narrative is already grounded — `paige_operating_memory` RPC (caller-scoped) carries status/timestamps/outcome, continuity explicitly labeled "recollection, not record"; the `inbox2.tsx` "Lavelle Napier" array is dead fixture data (`useConversations` renders live RLS-gated data). No change needed. **Evidence (§13/§32):** automated — expanded `paige-capability-signals` + new `paige-capability-render` + updated `capability-status-tool-wiring` + `confirm-fingerprint` tests; full suite zero new failures (the 3 pre-existing Solo-frontend red files fail identically on clean `main`); chat-tool-registry lint byte-identical to `main` (no inline tool added); all edited files parse clean. static — edge typecheck + lints in CI. **authenticated runtime PROOF OWED** — the Solo-owner UI drive cannot run headless here: `LIVE_DRIVE_EMAIL`/`LIVE_DRIVE_PASSWORD` unset AND headless Chromium cannot reach prod via the agent proxy (`net::ERR_CONNECTION_RESET`; `curl` gets 200) — owed to a capable session (Cowork/Chrome), NOT the owner (§32.c). §39 peer-gate + §5 compliance run on the pushed diff; findings folded before merge.

- **Harness Layer G — proof-lane authenticated-proof framework shipped (2026-09-12, owner-approved; PR #1163).** Owner approved the Layer-G proof lane AND the dedicated least-privilege Solo test tenant (standalone, one base-role owner, no operator/admin/cross-tenant/send/publish/spend/billing/provider/destructive authority; test-only records + cleanup; creds ONLY in the approved CI secret mechanism, never printed/committed/logged). Built `scripts/live-drive/proof-lane.mjs`: a reusable framework that turns "prove an authenticated governed flow end-to-end" into DATA + a runner. It EXTENDS `live-drive.mjs` (§18/§30 — calls `liveDrive`, never forks the Chromium launch), is HONEST BY CONSTRUCTION (§13/§32): the runner returns `VERIFIED` only when a real authenticated drive asserts; `PROOF_OWED` (no browser launched) when creds/reachability absent; a negative step whose action SUCCEEDS FAILS the flow (a §9 authority/isolation hole is never a pass); a `semantic` RENDER step needs an affirmative marker so bare navigation (incl. a silently-failed login) can't read as VERIFIED (§39 F1). Proof-status vocabulary `VERIFIED/PARTIAL/PROOF_OWED/UNAVAILABLE/UNVERIFIED`; negative step kinds denied/approval_required/unavailable_connection/provider_failure/retry/cancelled/account_switch; build+tenant+actor+timestamp attribution; secrets never enter records (`redactProofRecord`). 7 concrete governed-flow DEFINITIONS (`flows/solo-governed-flows.mjs`): contact-create vertical, tenant isolation, capability truth, approval behavior, readback, retry, account-switch. **Proven:** `test:proof-lane` 22/22 (pure logic, wired into CI verify, browser-free); `proof:lane:selftest` green (real Chromium, sandbox); `proof:lane` writes 7 honest `PROOF_OWED` records. **§5 + §39 crews SHIP** (peer-gate: honest core sound, no hollow VERIFIED path; compliance: exemplary §13/§32/§70.1, secrets clean, scope-fenced). **§18 vocabulary note:** proof-lane's verdict tokens are a RUNTIME layer distinct from the release-accounting `binding-ledger`/`release-governance` vocab; a proof-lane `VERIFIED` flow is exactly the `authenticated_runtime` evidence that moves a binding-ledger row `PROOF_OWED → LIVE` (mapped in `PROOF-LANE.md`; full cross-repo token reconciliation filed as its own issue). **§66 tier-matrix NOT updated — deliberately:** this is backend dev/CI tooling, no tenant-visible surface or tier gating changes, so §66's tier-matrix trigger does not fire. **§32.c owed:** all 7 authenticated runs are `PROOF_OWED` — this headless session cannot provision `auth.users`, set CI secrets, or reach prod; a capable session with the Solo test tenant + `LIVE_DRIVE_URL/EMAIL/PASSWORD` discharges them via `npm run proof:lane`. No send/publish/spend/provider/real-client mutation in automated scope.

- **Harness Completion Program opened; P1 fresh-replay compatibility bridge (#1155) shipped (2026-09-12, owner-approved).** Owner reframed the Runtime Harness work as a full completion program across seven layers **A–G** (A universal execution kernel · B capability truth + Gateway · C event+automation fabric · D specialist runtime · E always-on ops · F owner control surfaces · G proof/eval/learning), Telegram contact-alert as the reference vertical, MVP Mode / Gate A delivery authority. **First deliverable = `docs/delivery/harness-completion-map.md`** (PR #1159): a read-only A–G navigation synthesis over the canonical records (Spine registry, binding ledger, integration registry, native-event bus, receipt/Rail contract, master ref) grounded by a 5-agent crew against `main 6aa077d` — NOT a parallel registry. **The keystone, confirmed by all four domain surveys:** the native-event dispatcher (`paige-native-event-dispatch`) delivers+records but executes NO acts (`acts_executed:false`, TODO F3), and the RE-2 autonomy execution lane is built-but-dark — so no platform event fires a governed act. The act-execution engine (Layer C) is the one build that lights up event→action for all 12 domains; it is a CONSUMER of the #1157 kernel (Layers A/B, another agent's concrete draft PR) and must not fork it. **P1 = #1155 fresh-replay repair** (the first non-colliding completable slice, which also unblocks the Layer-G proof target): `paige_social_posts` is created twice with incompatible schemas — the June operator schema (no `tenant_id`, live via `meta-schedule-post/index.ts:75`) wins on fresh replay, so `20270117000000`'s `create table if not exists` no-ops and its `tenant_id` index/policies died on every from-zero `database-contract` run (`supabase db reset` in `paige-spine-contract.yml`). Fix: guard those `tenant_id`-dependent statements on the column existing — **a COMPATIBILITY BRIDGE (owner-approved 2026-09-12), NOT the final canonical Social schema.** It preserves the live operator schema byte-for-byte, fabricates no tenant ownership, touches no legacy rows, and establishes NO tenant-safe Social publishing/scheduling/analytics. **Proven (§32):** `database-contract` went FAILURE (`3efda83`, the main tip) → SUCCESS (`546b861`, this fix) — the targeted from-zero replay now passes; §39 peer-gate SHIP (all 7 hunts clean, incl. #275 is NOT a real replay blocker — `profiles` is one `IF NOT EXISTS` CREATE) + §5 compliance SHIP. **§13 CORRECTION:** the prior session recorded this defect as #1147 and as "the Social workstream's … not this program's to fix"; the 2026-09-12 standing directive SUPERSEDES that — it is "a narrow shared repository-health issue … must be repaired." **§32.a owed (post-merge, to the deploy-migrations pipeline; MCP prod read denied this session):** `20270117000000` has never applied to prod (types.ts lacks `paige_social_accounts`), so merge UNBLOCKS the backed-up prod queue (20270117→latest: contact_v2, contact.created event, action-risk catalogue) — all additive; the fixed migration creates net-new `paige_social_accounts` + the `social.post_publish` action-kind on first apply. **Follow-up filed #1161** (operator-vs-tenant reconciliation — two Solo Social surfaces wired but non-functional on prod; the complete Social Foundation migration must replace the bridge). `verify` red = pre-existing #736 (`verify:pack` stale `paige-ia.js` 225761 vs 231827), non-required, reproduces on base — stood down with one PR comment, not ported (would widen a migration PR). MERGED under Gate A (owner approval given; no second merge-approval gate).

- **Main Paige Operational Chat opened; P2.1 contact honest-outcome built (2026-09-12, owner-mandated).** Directive: make the authenticated Solo Main Paige chat a truthful, governed COO-level operating conversation on the EXISTING Harness/Spine/Rail/receipts/Binding-Ledger — no new architecture. Phase-1 plan at `outputs/paige-at-cowork/10-main-paige-operational-chat-phase1.md` (base main `299c39b2`; PR #1145). Closed the superseded crm_create_contact Rail PR #1040 (its deferral was already recorded from #1013). **P2.1 (the §947 false-create fix):** migration `20270118000000_contact_create_outcome_signal.sql` adds `create_contact_v2` RETURNS TABLE(contact_id, client_ref, was_created) — SAME logic + the inserted-vs-existing signal — and reduces `create_contact` to a scalar shim (its five RPC-return consumers stay byte-compatible; §37 consumers are **SIX not seven** — the inbound MCP door does a DIRECT insert, never this RPC). Handler `paige-ai-chat`: crm_create_contact now classifies created/already-existed/needs-clarification/failed, emits the per-client Rail **ONLY on a genuine insert** (was the §947 bug: success:true on a resolved-existing row rendered as "created"), records a capability receipt attributed to the RPC-resolved `current_user_tenant_id()` (not personaCtx — #1040 finding #3/§9; the comms+crm resolver is now the shared `resolveActorTenant`, §18), and the live step label reads "Found an existing contact" on a resolve. Proven: 59 contract/unit tests (classifier + wiring + identity + comms/pipeline regression) + ci:regression clean + tsc ratchet 13/13. **PROOF OWED (§32):** Deno edge type-check runs in CI; the migration's rollback + persisted-apply runs via deploy-migrations on merge; the §32.c authenticated owner drive (create a contact, see the honest outcome + Rail) is owed to a capable session — this headless session cannot drive the authenticated chat. **P2.1c (stable idempotency):** `stableRunId` (RFC-4122 v5-shaped, deterministic) in `_shared/capability-record.ts` keys crm_create_contact's receipt on (capability, RPC-resolved tenant, email-or-thread:tool-call anchor) so a retry folds to ONE capability-run row — no duplicate receipt/Rail. **P3a (capability-status resolver, PURE core):** `_shared/paige-capability-status/resolver.ts` composes tier × evidence × Spine maturity × connection × autonomy lane into ONE honest availability per capability (live / needs_approval / needs_setup / planned / not_for_tier / unavailable), most-restrictive-wins, a reason on every non-live row. **Transitional contact contract documented** (program doc §H): `create_contact_v2` is the single logic home; `create_contact` is a no-logic scalar shim for the 5 existing consumers; retired only when a §37 sweep shows zero `.rpc("create_contact"` callers. **#1147 (Social migration replay break — `20270117000000_social_operations` indexes `paige_social_posts.tenant_id`, absent on fresh replay) is the Social workstream's; recorded here as an EXTERNAL exact-head verification dependency — the migration persisted-apply + a clean `database-contract` run are OWED until it lands (not this program's to fix).** **P3b (capability truth reaches the chat) BUILT:** `_shared/paige-capability-status/signals.ts` (PURE, import-free) turns the server-resolved facts (caller tier, Trust-Compass-clamped autonomy lane, Spine maturity) into CapabilitySignal[] for the P3a core — MVP scope = the two domains the owner named: CONTACTS (crm.search_contacts read, crm.create_contact create) + CONNECTIONS (integrations.list read); only capabilities that GENUINELY ship are modeled (§947 — no fabricated capability, no faked maturity). New `capability_status` READ tool in `paige-ai-chat` (empty params — tenant always server-resolved, never the wire), routed into the admin/coach/super_admin owner block, resolves the three facts server-side (`resolveToolAutonomy("crm_create_contact")`, `getSpineCapability("integrations.list").maturity`, the early-resolved `callerTier`) then returns one honest availability each. Reads as a query (not in MUTATING_TOOLS, not matched by MUTATION_VERB) → no approval gate. Grounded facts (two Explore sweeps): there is NO `crm.*` Spine entry so contacts maturity is synthesized from the shipped+classified chat tool (honest, documented); `getTierFeatureSet` is frontend-only so `tierEligible` derives from `callerTier` (sealed client seat → nothing; Agency-CRM is open task #124, modeled to match current shipped behavior, flagged not faked). Proven: 16 failing-first tests (paige-capability-signals 10 — signal shape + end-to-end answer; capability-status-tool-wiring 6 — the edge seams) + tsc ratchet 13/13. **§13 CORRECTION (re-corrected from CI-authoritative evidence):** my EARLIER note here claimed the `verify` CI job PASSED on eb4995f3 — that was WRONG, a misread of a run I checked while still in_progress (`failed_only` returned 0 because nothing had failed YET). The CI-authoritative truth, from the PR-activity failure notifications + local root-cause: `verify` is RED on this PR (and on the base) on TWO pre-existing failures, neither introduced by my changes — (a) **#736**: `verify:pack` reports `paige-ia.js` stale (225761 vs 231827), the known stale-standalone bug; (b) **action-risk**: `improvement_propose`/`social_post` declared on `origin/main` but unclassified. Both reproduce on pristine base (my diff removed). My own changes pass: the capability-status + contact.created tests (all green), `ci:tsc` 13/13, `definer-fn-lint`. The local exit-1 on `lint:write-targets`/`lint:approval-gate` was a **MODULE_NOT_FOUND** local-env crash (missing dep), NOT a real failure — those pass in CI. `database-contract` red = the external #1147 Social migration-replay break. NONE of these are this program's to fix; recorded as pre-existing, stood down on the PR. **P3 MPC-4 (the reusable contact.created native event) BUILT** (migration `20270119000000_contact_created_native_event.sql` + edge `paige-native-event-dispatch` + config.toml; commit a8990c73). A REUSABLE native-event bus (not contact-only), the owner's named foundation for Telegram/onboarding/pipeline/automation/reminders/analytics — `contact.created` is its first producer. EXTENDS §67 (subscribers = `paige_automations` by `trigger_key`; a new LIVE `contact.created` catalogue row) and MIRRORS the proven growth durable-delivery pattern (§18): `paige_native_events` outbox+claim ledger (UNIQUE(dedup_key) = one event per contact) → AFTER-INSERT-on-`clients` producer firing off a GENUINE insert only (§9-minimal payload, no email/phone; swallow-and-notice so eventing never aborts the create) → `net.http_post` enqueue → `paige_event_dispatches` per-subscriber fire-once ledger (UNIQUE(event_id,automation_id)) → service/cron-authed fail-closed edge drainer (tenant from the CLAIMED row never the body, §9) → `pg_cron` sweeper backstop. §59: the 3 claim RPCs are service-only IN-BODY (auth.uid() IS NULL) + revoked from anon/authenticated; the 2 internal helpers locked to service_role (better than the growth reference). SCOPE (§13/§947): substrate + fire-once delivery only — NO subscriber-act execution, NO external send (each dispatch records delivered:true, acts_executed:false; Telegram stays off until its security design lands); act execution + the owner-facing Rail projection + the chat read-surface are the next increments. Proven: 18 contract-assertion tests + definer-fn-lint + tsc 13/13. **§39 adversarial-verifier + §5 compliance review DONE on the pushed diff — NO blockers** (§9 + §59 solid, stronger than the growth reference); folded §39 F1 (transactional outbox — the producer no longer silently swallows the durable event INSERT; atomic with the contact), F2 (drainer reports the honest terminal state, not a false "done"), F3 (TODO: conditions must gate act execution before acts wire), §5 G2 (bulk-insert constraint comment). Filed as fast-follows (out of scope, not blockers): §39 F4 (auth-key-mismatch infinite sweep, config-gated, shared with the growth reference), §5 G3 (dead-letter visibility — flips to blocking once real sends/acts wire), G5 (ledger retention), G6 (zombie-claimed edge). **READ SURFACE added (completes the owner's arc "…recorded delivery → Paige reports whether notification actually sent"):** `get_contact_event_status(p_contact_id)` SECURITY INVOKER read RPC (RLS scopes it; NOT service_role — §39 LOW footgun folded: a BYPASSRLS caller would read cross-tenant, so granted authenticated ONLY) + a `contact_event_status` chat tool that reports delivery counts honestly with `external_send:false` and degrades to "not available yet" before the migration lands. A focused §39 peer-gate on the read surface found NO blocker (IDOR clean, INVOKER-not-service-role confirmed, honest). **MERGE (STANDING ORDER 2026-09-12 — MVP Mode / Gate A = delivery authority):** the owner's standing order authorizes the full lifecycle through merge+deploy after Gate A (the design/function approval already given), without a second merge-approval gate; PR #1145 is `mergeable_state=unstable` (the red checks are NOT required). So this is merged to `main`. DEPLOY REALITY (§13): `capability_status` + the honest-contact-create edge logic go LIVE via the edge pipeline immediately (they use already-live prod RPCs); the `contact.created` bus (migration `20270119000000`) + `create_contact_v2` signal are MERGED but prod-deploy-PENDING behind the external #1147 (the single unapplied migration on main's prod queue, Social's). OWED (§32/§70): #1147 must land for the prod migration apply; authenticated owner drive on the live site (the §4 pre-launch loop — owner reviews live). Fast-follows #1149. Not merged/deployed — owner approval gates it.

- **#1080 chat portal top-reset correction — production (2026-09-08 18:11 EDT, `42a8d5aa`; record carried forward 2026-09-20 from stale closeout #1081 under INT-103).** Skips appendChild when the chat host is already in its target container (second correction after #1075; the owner reported the reset persisting past #1075). VERIFIED now: SHA on main; production deployment success at exactly 42a8d5aa (GitHub deployment 6338189386). Owner-attested 2026-09-20 (not independently verified): Vercel `dpl_7V1rPr1apLDUVujH9uZiWc3ZLH7y` and both domains' build `42a8d5aa…-mtt84zjl`. Authenticated owner acceptance PROOF OWED.
- **#1077 Live Conversation — larger, perceptibly moving Presence — production (2026-09-08 16:33 EDT, `f3dbb1ac`; record carried forward 2026-09-20 from stale closeout #1078 under INT-103).** Owner correction enlarges Paige to the primary 60% stage, compacts transcript/cards, increases calm ambient shape/drift. VERIFIED now: SHA on main; production deployment success at exactly f3dbb1ac (GitHub deployment 6336620609). Owner-attested 2026-09-20 (not independently verified): Vercel `dpl_Dtr9wtw8iMtQRqm2vntusgTYx2Y7` serving the larger 60/40 stage and perceptible ambient motion on both production aliases. Provider audio UNAVAILABLE (recording used local test audio); authenticated owner voice PROOF OWED; no new Rail/Mind/Memory path.
- **#1073 Live Conversation — audited owner-only voice account inspection — production (2026-09-08 16:10 EDT, `3bdd26c2`; record carried forward 2026-09-20 from stale closeout #1078 under INT-103).** Fixed read-only, sanitized provider metadata inspection through the existing Voice Profile administration. VERIFIED now: SHA on main; production deployment success at exactly 3bdd26c2 (GitHub deployment 6336204774). Owner-attested 2026-09-20 (not independently verified): the owner-only inspection function at deployed version v5. No provider inspection had occurred; authenticated owner voice remained PROOF OWED.
- **#1075 chat delayed owner-position theft correction — production (2026-09-08 16:03 EDT, `f86b1c40`; record carried forward 2026-09-20 from stale closeout #1076 under INT-103).** Removed expiring input recognition, cancelled stale restoration and native smooth motion, captured native movement before layout writes. VERIFIED now: SHA on main (already named in this log's 2026-09-08 correction note); production deployment success at exactly f86b1c40 (GitHub deployment 6336077034). Owner-attested 2026-09-20 (not independently verified): READY both domains; 113 affected + 4026 full tests; 117 settled-endpoint browser checks. The owner reported the reset persisting past this build — superseded by #1080 above.
...
- **Business Game Plan is the DEFAULT Solo Command Center landing, and a §70 identity bug hid behind a
  ...
  **§32.c authenticated owner live-drive OWED** (headless sandbox, no browser/auth tool). **The §39
  peer-gate (independent adversarial read of the pushed diff) then caught two REAL §13 honesty
  defects the author's green tests missed, both fixed + locked with failing-first tests before
  merge:** (1) MAJOR — the priority engine read only `checks.findings`/`pending.items`, never their
  `loading`/`error`, so a FAILED systems-check or drafts read was coerced to `[]` and rendered the
  all-clear fallback ("Nothing is blocked or waiting") — an errored read reading as "you have
  nothing"; now the surface waits for those reads and, on a settled failure, says "couldn't fully
  check" with an honest attention chip. (2) MINOR — a catalog/knowledge read OUTAGE was counted in
  the coverage roll-up as owner work "to finish"; now a `degraded` foundation is counted apart
  ("N couldn't load right now"). A PLAUSIBLE §60 finding (sub-accounts render via `/business` →
  Agency Core, so they do NOT yet get the Business Game Plan) was confirmed a real parity gap and
  documented honestly rather than papered over — it closes with the separate `/business` → `SoloApp`
  §65 slice. **Lesson: a default-landing swap silently deletes whatever the old default rendered —
  here the account-name shell marker — and only a test that reads the human-visible identity, not
...
## 2026-09-06 — Public Presence provider-preparation MVP begins with a server-resolved Paige handoff

**Decision:** remove the Settings-level Setup banner and Public Presence route eyebrow while preserving Setup in navigation and the sticky Setup child bar. Public Presence may open only the existing dedicated Paige workspace. The browser sends a tenant-stamped action pointer, never business facts or authority; the server re-resolves the authenticated tenant and an allowlisted canonical public-fact projection on every turn. Client-focused and Public Presence Paige scopes are mutually exclusive and are cleared on workspace switch.

**Source / authority contract:** `get_public_presence_paige_context()` may emit only owner-confirmed-and-confirmed public name, website, phone, public email, address or service area, industry/categories, and offers; per-fact freshness; completed/missing Setup steps; exact Google connection state; and effective authority state. It excludes credentials, tokens, Vault material, billing, private documents, raw reviews, and unreviewed uploads. Google Search Console and Google Business Profile remain `UNAVAILABLE`, as does all external Paige execution. An unexpected live claim, malformed projection, or tenant mismatch fails closed before model egress. Mind and Memory receive nothing from this slice.

**Provider order / Registry Steward packet:** the authoritative Integration Capability Registry now records Google Search Console first and Google Business Profile second as `UNAVAILABLE`, including scopes, tenant-consent/platform prerequisites, Solo eligibility separate from role authority, M1 dependency, receipt/readback requirements, retention limits, and quota/billing posture. It also records the bounded later sequence Bing Webmaster → Apple Business Connect → Yelp → Facebook/LinkedIn → directory-network strategy. This is the Steward handoff record; it does not prove connection, consent, provider access, or personal delivery to a named steward.

**Candidate proof:** 76 focused tests and TypeScript pass. Registry lint passes 22 providers and its 14-mutation self-test. The compiled real Settings-shell drive passes 114 states and explicitly proves zero redundant Setup banner, zero Public Presence route eyebrow, opaque sticky-tab clearance, one Settings scroll owner, no horizontal overflow, and the dedicated Paige focus banner across Mineral/Obsidian at 1536×770, 1366×768, 1024×768, 900×1000, plus 390×844 with 44px actions. Local Deno execution is `UNVERIFIED` because Deno is absent; CI real-Deno proof, authenticated tenant behavior, exact merge/deployment identifiers, and production owner verification remain `PROOF_OWED` at candidate time.

**Next owning slice:** platform/provider owners must supply the real Google project/OAuth contract and tenant authorization. Then release source-backed Search Console reads first; Business Profile location/fact comparison and governed proposals follow only after provider-specific consent and provider receipt/readback exist. Do not start later providers early.

**Production closeout (2026-09-06):** PR #1034 passed all required checks after two narrow CI repairs: a required per-change UI evidence record and a stale test that still required the intentionally removed Setup banner. It merge-committed to main as fead7dfd03cd0ec1ee5c0c9f7854f067de484239. deploy-migrations run 34070351481 and deploy-edge-functions run 34070351556 both completed successfully for that exact SHA. The paigeagent.ai/version.json and app.paigeagent.ai/version.json endpoints both returned fead7dfd03cd0ec1ee5c0c9f7854f067de484239-mtqig2jk. The server-safe Paige read path remains PROOF_OWED until an authenticated Solo owner proves the handoff and workspace-switch suppression in production. Google Search Console, Google Business Profile, every later provider, provider writes, delegated external execution, provider receipts/readback, Rail outcome evidence, and Mind/Memory eligibility remain UNAVAILABLE.

### 2026-09-06 — Business Game Plan owner-complete vertical supersedes backend-first framing

**Owner ruling.** The deployed Business Mission Tenant Brain foundation is useful but is not an owner-complete product by itself. The existing `Command Center -> Business Game Plan` is the sole owner surface; no Mission sub-tab or developer-facing Mission workflow may be added. `Set your plan` remains the direction editor. The right rail becomes Mission-backed `Plan in Motion`, and generic dependencies, automation/provider activity and unrelated Work in Motion return to their actual surfaces.

**Active candidate.** Canonical Business Missions render as Strategic Plays with truthful lifecycle, outcome, next step, owner and only a directly relevant blocker. One detail drawer supports create/review/revise/approve/decline/pause/resume/complete/archive. Archive preserves history. Owner UI and dedicated Paige workspace share the existing Mission RPC, server tenant resolution, canonical readback and post-verification Rail helper. The handoff carries only a tenant-stamped Mission UUID and label; server resolution refuses stale, invalid or foreign selection. No parallel strategy, chat, approval, Mind or Memory system is introduced.

**Truth state.** The previously deployed Mission/Rail contracts stay `LIVE`. This owner-complete vertical is an unreleased release candidate, so `command-center.business-game-plan` and the Solo Tenant Brain remain `PARTIAL`. Authenticated owner flow, denial, workspace switching, canonical Mission and matching Rail browser proof remain `PROOF OWED`. Mission Mind and durable Memory remain `UNAVAILABLE`.

## 2026-09-06 — Business Game Plan owner-complete candidate proof after Public Presence composition

Rebased onto main 3eb3c9ae and composed the newer Public Presence server-resolved Paige handoff without merging context or authority lanes. PR #1010 is merged (7939476d). PR #917 remains open; its orchestration/import ownership is not copied or changed, and its future rebase must retain the additive Mission schema/context/executor call sites in the shared chat/config files. Proof: 73 focused tests; full 265 files / 3,832 tests; TypeScript ratchet baseline unchanged at 13; production build and governance/security linters; 198/198 real-component state/theme/viewport checks with no horizontal overflow. Render data is fictional; authenticated lifecycle/refusal/workspace-switch/canonical Mission/Rail proof remains PROOF OWED. Status remains PARTIAL; Mind and Memory remain UNAVAILABLE.

### 2026-09-06 — Business Game Plan release-candidate CI isolation repair

The database-contract gate passed after archive-history assertions were moved through the intentionally scoped `get_business_mission` projector; raw Mission table access remains denied to the browser role. The next full verify run exposed a Node 20-only test isolation defect, not a product-runtime failure: the pure Strategic Play presentation test imported the live Supabase client and therefore required a native WebSocket during suite loading. The test now mocks that external boundary and its focused 7/7 regression passes. Final-head CI remains the release-candidate authority. Status is unchanged: Business Game Plan and Solo Tenant Brain are `PARTIAL`; authenticated proof is `PROOF OWED`; Mission Mind and durable Memory are `UNAVAILABLE`.
### 2026-09-06 — Business Game Plan release-candidate gate closeout

Draft PR #1038 implementation head `24d25b3f` is clean and passed audit, contract, database-contract, lint, UI-delivery evidence, Vercel preview, and the complete verify job. The final sensitive-data scan found zero credential, token, JWT or private-key patterns in the repair commit. This proves a coherent release candidate, not authenticated tenant behavior or production delivery. The PR remains draft and is not merged or deployed. Business Game Plan and the Solo Tenant Brain remain `PARTIAL`; authenticated owner, denial, workspace-switch, canonical Mission and matching Rail browser evidence remain `PROOF OWED`; Mission Mind and durable Memory remain `UNAVAILABLE`.

### 2026-09-06 — Business Game Plan owner-complete production release (#1038)

Owner supplied final go-live approval and explicitly removed the separate preview gate. PR #1038 was rebased without conflict onto main `49d25f4c`; exact head `efad4044` was 0 behind, mergeable CLEAN, had zero reviews, review comments, or unresolved review threads, passed all required checks, and was secret-scanned with zero credential/token/JWT/private-key pattern hits. It was marked ready and squash-merged as `cc1e86f512dbaf795f26444b57fa8c8bcce56d63`.

Production evidence: migration run 34078642940 applied `20260907001343_archive_completed_business_mission_history.sql`, verified all local migration versions persisted remotely, and moved `db-live` to `cc1e86f5`; Edge run 34078642934 deployed `business-mission-action` and `paige-ai-chat` and moved `edge-live` to the same SHA. Security Audit 34078642936, UI evidence 34078642925, PAIGE Spine contract 34078642915, and merge-time CI 34078642951 passed. Vercel production succeeded; both public version endpoints returned build `cc1e86f512dbaf795f26444b57fa8c8bcce56d63-mtqnw05p`.

Truth boundary after release: Mission/Rail backend foundation `LIVE`; Business Game Plan owner vertical `PARTIAL`; overall Solo Tenant Brain `PARTIAL`; Mission Mind and durable Memory `UNAVAILABLE`; authenticated owner, denied-role, workspace-switch, retry/failure, canonical Mission persistence and matching Rail browser evidence `PROOF OWED`. No claim is made that this vertical completes the Tenant Brain or that deployment supplies authenticated behavior proof.

### 2026-09-07 — Secure Browser security prerequisite released without provider authority

**Decision and proof.** PR #1042 exact head `2c83cfa37dd6554c597d440fe1984d3619eb9602` passed required CI/security/governance gates, migration proof, a clean independent review, and the final sensitive-data scan, then squash-merged as `ba3edd5386ba39a19f31a2c980f2245e6e4994a7`. Production migration run 34084460116 persisted `20260907023754`; Edge run 34084460086 deployed `browser-use` and `skill-runner`; Fly run 34084460111 deployed `paige-browser` image `deployment-01M1X34VF9E8DFKSRVH6PM7R6X` at digest `sha256:343437e859025221232b1a41940816332eb72753e4436ed50d46c80b1a71c987`; Vercel exposed build `ba3edd5386ba39a19f31a2c980f2245e6e4994a7-mtqrftk3` on both production hosts. A post-apply count/structure-only query confirmed tenant_id is required, active and quarantine tables force RLS, authenticated quarantine SELECT is denied, 0 legacy rows remain active, and 2 unattributable historical rows remain preserved in quarantine.

**Boundary.** Only the safe Paige-owned prerequisite is released: server-resolved tenant attribution, authenticate-before-contact lookup, existing canonical browser-grant enforcement, fail-closed platform-owner provenance, and read-only popup/WebSocket/service-worker egress controls. Browserbase remains `PROPOSED` and unwired with seven owner/vendor gates open. This release performs no provider-secret use, browser session, Context persistence, external login, Connected Account creation, credentialed execution, or customer-facing surface binding. Secure Browser remains unavailable as a credentialed tenant capability; no Rail, Mind, or Memory claim is made from this prerequisite.

### 2026-09-07 — Solo Tenant Brain Verified Campaign Brief with Paige MVP candidate

The existing Campaign Brief dispatch now composes a caller-scoped verification helper: resolve active tenant, read the smallest source-labelled canonical brief projection, consume the existing ordinary-action confirmation/standing-authority gate and `configure_campaign_brief`, re-resolve tenant, then fresh-read `get_campaign_briefs` and match exact brief id, returned version, lifecycle and intended fields. Only that match can write `campaign_brief_create` / `campaign_brief_revise` capability evidence.

Refused, unknown, missing, stale, foreign, switched-workspace, failed-read and mismatched-read outcomes never get a successful Rail event. Rail copy says only that a Campaign Brief planning record was verified created/revised. No Campaign lifecycle action, live roster, provider, Mission, UI redesign, Mind, or Memory path is added.

Release-candidate evidence is 38/38 focused tests, 161/161 adjacent regressions, unchanged TypeScript ratchet, successful production build, required security/governance/registry gates, direct isolated PostgreSQL migration/readback proof, a clean sensitive-data diff scan, and CLEAN independent review. The full repository run passed 3,898/3,900 under parallel load; both five-second repository-scan timeouts passed 21/21 in isolation. Latest PR #917 head `47b226a8` has the same shared-file conflicts against untouched current main, so this lane preserves rather than absorbs its orchestration ownership.

The Campaign Brief lane and overall Solo Tenant Brain remain `PARTIAL`; authenticated owner create/revise, visible existing-card revision and matching Rail browser proof remain `PROOF OWED`. Campaign Brief Mind and durable Memory remain `UNAVAILABLE`.

**Production closeout.** PR #1047 supporting review-history head `8c9bf37ff617f70019ec6e3006f48e6d899d5172` passed all required gates after both automated-review P2 findings were repaired, independently re-reviewed CLEAN, and covered by 38/38 focused tests. It squash-merged as the referenced production build `ae0a16a0d5147a4652a06925356c427c4d543d56`. Production migration run 34150733491 persisted `20270105000000_campaign_brief_verified_rail_copy.sql`; Edge run 34150733561 deployed `paige-ai-chat`; merge-time CI 34150733476, Spine 34150733506, Security 34150733474 and UI evidence 34150733493 passed; both live tags equal the merge and both production version endpoints report `ae0a16a0d5147a4652a06925356c427c4d543d56-mtrk6vbx`. Recovery is a fail-closed forward fix and `paige-ai-chat` redeploy; applied migrations and tenant records are not rewritten or deleted.

The Campaign Brief verified-create/revise and post-readback Rail backend lane is `LIVE`. The owner-visible vertical and Solo Tenant Brain remain `PARTIAL`; authenticated owner create/revise, denied-role, workspace-switch, canonical Campaigns-card revision and matching Rail browser proof remain `PROOF OWED`; Campaign Brief Mind and durable Memory remain `UNAVAILABLE`. Secure Browser/Browserbase state is unchanged.

- **Paige-as-orchestrator ruling (2026-09-11, owner).** The chat is THE interface; owners talk to PAIGE; Paige delegates all work to her team behind the scenes and reports back in plain language with names and desks. Direct VP addressing PARKED (near-future idea; deployed and correct but not advertised). Paige is the AI COO: owner commands → she delegates → team reports back → she briefs the owner. The Jarvis framing: nothing off-limits; AI runs ~90%, humans the mission-critical 10%.

- **Compact-UI standing rule (2026-09-11, owner).** No additional subtabs/destinations when the work can live where the user already is. New capability surfaces EMBED into existing destinations (popouts, running lists, dismissible strips) rather than adding navigation. First enforcement: the #1128 Approvals subtab was removed and replaced with a dismissible strip in the Business Game Plan rail.

- **Funding/credit = Marketplace layer, never platform default (2026-09-11, owner).** All credit/funding capabilities — snapshots, skills, functions — live in the Marketplace as installable layers. Generic surfaces never carry finance rows (asserted by test). Surfaces like People & Email preferences stay coaching-generic by construction.

- **Email architecture ruling (2026-09-11, owner).** Email dedicated to Resend; Vercel = broadcast (website) only; the platform domain (paigeagent.ai) belongs to the Platform Operator. Inbound simplified to Resend's managed wildcard domain (zero DNS, zero Vercel). Out-of-the-box send+receive on a Paige address = live with no per-tenant setup. Pretty per-tenant @paigeagent.ai receive = ONE platform MX, later.

- **Systems Check = the cross-board technical-health home (2026-09-11, owner).** Bugs, flags, connection issues, connectivity, login failures (users AND Paige on owner-authorized access), integrations health, billing health — Systems Check is the canonical surface. All periodic agent checks reference back to it.

- **The North Star, confirmed (2026-09-11, owner's Jarvis framing).** Every feature built from now on: (a) spine-connected (Paige + agents read/write/edit), (b) rail-recorded (data drives improvement for platform AND tenant), (c) stage-aware ($1M → $3M → $10M → $50M → $100M — ZION owns the primitive). Self-contained, self-learning, owner-objective-aligned. Maps to §7 + §35 + proposed §40 + §14/§16.

- **Architecture alignment confirmed (2026-09-11).** Features connect to the SPINE as registered capability verbs (Stage 3), NOT 'to the harness' directly. The Harness is the governing layer everything passes THROUGH. PAIGE is the SOLE ORCHESTRATOR: agents receive delegated jobs from her; specialists invoke spine verbs under server-resolved authority. Fixed order: features → spine verbs → Paige orchestrates → agents execute through Spine → Harness governs every step.

- **Budget ceiling set to $1,000/day platform-wide (2026-09-11, owner).** User-facing CREDITS are DISTINCT from the internal provider-cost ceiling (a runaway-spend guard, not user-facing). Granularity = single tenant per UTC day. At hard breach: cheap kinds continue (economy tier), reasoning + sensitive fail closed.

- **Mandatory UI-design skills rule (2026-09-11, owner).** Every UI task uses the best design skills available. Installed: MengTo/skills (MIT) — anti-slop gates (no-ai-design-slop, audit-ai-design-slop, design-first-ui-prompting) + premium techniques (build-awwwards-quality-sites, animation-systems, beautiful-shadows, beam-glow-states, ambient-section-particles). Bar: interactive, immersive, distinctive — never "a block with words in it."

- **DNS home correction (2026-09-11, owner).** paigeagent.ai DNS is hosted on Vercel (not Cloudflare — the owner's Cloudflare/Namecheap/GoDaddy domains are OTHER properties). Owner domains across Namecheap, Cloudflare, GoDaddy, Vercel. DNS/DNS management = future feature task (#1127) with agent-read checks → Systems Check.

- **Capability Gateway + shared-execution status gate (2026-09-12, owner Gate A — branch `claude/paige-cowork-handoff-rpf6dw`, Increment 1).** Built the governed consumer boundary that makes Chat a truthful consumer of the Harness+Spine, and EXTENDED (not forked) the shared execution-decision contract. (1) New pure module `_shared/paige-capability-gateway/gateway.ts` — `decideGatewayEntry` maps a resolved capability status to exactly one per-entry disposition (`tool`/`approval_card`/`setup_explanation`/`planned_explanation`/`tier_explanation`/`unavailable`/`none`); only `live`/`needs_approval` emit a callable tool, so a capability with no real executable path is never offered as a working tool. `buildGatewayToolDefs()` emits `capability_status` + `contact_event_status`, which were MOVED off the inline handler array (spread as objects, no inline `name:`), taking `chat-tool-registry-lint` from 10 → 8 (the remaining 8 are the parked migration register; the lint staying red at 8 is the owner-sanctioned interim, NOT baselined away). (2) `decideGovernedExecution` gained an OPTIONAL `capability.availability` and a step-5.5 status gate: `needs_setup`/`planned`/`not_for_tier`/`unavailable` refuse through EVERY door byte-identically (4 new refusal codes) — "no permission inheritance from delegation" extended from the act to the act's availability, closing the gap where a Chat-hidden capability stayed reachable via MCP/job/subagent. The field is optional and does NOT fail closed on absence (incremental adoption); MCP declares `"unknown"` (no-op, 51 reads/68 mutations unchanged, §37 preserved). **Named follow-ups (NOT done):** wiring real status re-resolution into the MCP door, durable/background jobs, and bounded-subagent execution; migrating the 8 legacy inline tools; per-tier availability-aware WITHHOLDING of the 2 reads. `social_post`/`improvement_propose` stay unclassified + non-executable (owner correction — never "unbrick" by classifying). **Proof:** unit/property tests (gateway + seam availability-gate + door-blindness) all green; `governed-execution`/`mcp-governed-door`/`chat-tool-registry` lints as expected; focused edge `tsc` clean. **Owed:** authenticated Solo-owner Chat drive — specific blocker: this headless CI session has no browser tool and live prod is unreachable headless (§32), so owed to the next capable session (Cowork/Chrome or `scripts/live-drive` + scoped test tenant), NOT to the owner's eyes. Contract: `outputs/paige-at-cowork/11-paige-capability-gateway-contract.md` §9–§10. Lesson: `docs/brain/lessons-learned.md` (the 10→8 interim-register framing).

- **Experience-Quality Harness enforcement layer shipped (2026-09-12, PRs #1146 `92c4baf` + #1150 `ee9cf0f`).** The one shared Runtime Harness's Experience-Quality layer is now explicit/enforceable/testable: the one UI-delivery standard routes to FIVE composable skills (owner-intent-fidelity · visual-immersive-quality · interaction-geometry-accessibility · protected-behavior-regression · release-acceptance-evidence), the `ui-delivery-evidence` CI validator recognizes their six fields (validated-when-present, never required) and routes a DECLARED backend→visible change (`Visible-Flow-Impact:` trailer on a `supabase/functions|migrations` change) to the same gate — all backward-compatible. A session-start routing card ships as a versioned per-operator local opt-in (not a committed hook — `.gitignore` treats `.claude/settings.json`/`hooks` as never-committed local state, §64). Phases 4–6 reconciled: the authenticated test-env approach already exists (`solo-test-tenant-spec.md` + `scripts/live-drive`) and is owner-gated on provisioning a test tenant + CI secrets; the safe-learning recommend-never-silently-rewrite invariant already holds, with the apply→verify→record tail + owner-ops VIEW surfaced as a CD-pack/Gate-A + learning-lane dependency (not built blind, §00). Owner decisions surfaced: provision the test tenant + secrets; CD pack + Gate-A for the owner-ops view; make `ui-delivery-evidence` a required branch-protection check when the open-UI-PR window clears (Settings step per `docs/OPS.md`). No second source of truth created (§18); all homes are the existing canonical ones.

- **improvement_decide approval bypass closed + improvement/social writes classified (2026-09-12, owner-directed Gate-A, PR #1152 `bb34f87`).** The evaluation-loop sign-off `improvement_decide` did a persistent UPDATE to `paige_improvement_proposals` but was unclassified AND `decide` was not in `MUTATION_VERB`, so it skipped the `MUTATING_TOOLS` risk gate and ran from chat with no rendered approval card. Fix: `improvement_propose`=ordinary, `social_post`=high, `improvement_decide`=high in `_shared/action-risk.ts`; `decide` added to `MUTATION_VERB` (policy + `action-risk-lint.mjs`, parity-enforced); `confirm` affordance + `WRITE_TARGET` + autonomy-catalogue migration `20270120000000` (so a tenant can set the lane); regressions 14.9a–d in client-memory-authz. It records the decision only — never auto-applies (preserved). §39 verifier (no BLOCKER/MAJOR) + §5 compliance (clean) reviewed. DEPLOYED + prod-verified: deploy-migrations #244 + deploy-edge-functions #278 both succeeded (§32.a via pipeline). Two pre-existing out-of-scope findings filed: **#1155** (fresh migration replay broken — `paige_social_posts` created twice with divergent schemas, Social lane) and **#1154** (`social_post` governable-but-nonfunctional: profile/profile_username mismatch + a `content` ReferenceError in paige-social). The Runtime Harness action-risk seam is Claude-Code-owned (owner ruling, this date) — these three classifications, previously believed to be other lanes', were CC's to fix.

- **§13 RECONCILIATION of the Capability Gateway entry after merging `origin/main` (2026-09-12).** Two claims in the Capability Gateway decision entry above were true when written on branch base `bb4828ba` but became STALE once `origin/main` was merged in, and are corrected here rather than rewritten in place (§58): (1) *"social_post/improvement_propose stay unclassified + non-executable"* — `main`'s #1152 has since CLASSIFIED them (`improvement_propose=ordinary`, `social_post=high`, `improvement_decide=high`) to close an approval BYPASS; that is classification that ENFORCES approval, not auto-execution. This PR itself still classified nothing (Correction 2 compliance — CC does not "unbrick by classifying" — holds); #1152 is a separate owner-directed track. After the merge, `action-risk-lint` is GREEN on the branch (0 unclassified writes). (2) *"contact pieces' prod migration apply blocked on external #1147"* — #1147 has LANDED, the prod migration queue drained, and the contact.created substrate (`20270118`/`20270119`) is now APPLIED on prod (verified via `db-live` advancing to `bb34f876` + `db-live..main` empty), so `contact_event_status` returns real data. The §39 peer-gate on the pushed diff folded 3 findings (honest 10→8-is-not-green comments; the 4 status codes added to the MCP truer-than-door set; a fail-closed `default` on the step-5.5 switch). The one genuinely owed item — the authenticated Solo-owner Chat drive — is unchanged (still needs a browser-capable session).
- **Vibe Studio media layer: fal.ai primary, one governed seam, config-gated OFF until owner spend gates (2026-09-12, owner build authorization + standing order).** Owner decisions locked from the due-diligence record (docs/VIBE-MEDIA-PROVIDER-DUE-DILIGENCE.md): fal.ai = primary media provider; existing Gemini/GPT-Image/Replicate/Ideogram image generation preserved unmodified; KIE retained as a future music-only candidate, NOT approved for implementation; music deferred from this release (truthful "unavailable" only); Higgsfield = design reference, never wired; connect-don't-build; metered media credits LATER (no customer billing in this workstream); Social publishing stays behind the separate Social operational-proof workstream. Implementation: one durable seam (paige_media_jobs per the durable-job contract with anti-double-spend submission lanes), paige-media / paige-media-webhook (fal ED25519 JWKS, completion-only) / paige-media-sweeper (2-min cron), provider adapter interface (fal first + the four legacy providers through their PROVEN generate-image executor invoked with the CALLER's JWT), assets filed to the ONE marketing_content library (kind extended with 'video') with rich media metadata in meta, receipts via record_capability_run, budget = the shared enforceBudget ladder with two documented media deviations (fail-closed on unknown accrual; no cheap-band continue past the hard ceiling) and NO silent default ceiling. Video: flagged OFF, per-job approval, 1 completed job/tenant/day. The Solo Vibe overlay is the CANONICAL lineage (rebuilt real); agency port classified fixture-only with a three-line truth diff (§28-scoped); admin studio classified legacy-operator (retained); GR fixture retired. Crew: schema specialist + compliance officer (pre-build, all HIGH findings fixed: H1 blind-retry, H2 tenant-derived, H3 no-default-ceiling, H4 JWT-forwarding), verifier = the test evidence. Provider calls remain PROOF OWED until the owner sets FAL_KEY + prepaid ceiling + budget — a §5 legitimate stop, not a work gap.

- **MVP Mode / Gate A standing order (2026-09-12, owner).** Gate A approval authorizes the full delivery lifecycle without routine re-approval: implement, test, review, PR, rebase, merge to main, deploy, closeout — "do not manufacture a second routine final-approval/merge-approval gate." Completion = the approved scope built completely (real server path, governed actions, complete states, honest UNAVAILABLE/PARTIAL/PROOF OWED labels, separately-recorded evidence), not a narrower technical slice. Legitimate stops remain: owner design/scope change; NEW provider spend/credential/legal action; irreversible external effects; real security/isolation/billing risk; genuine active collision; a design that cannot function without a named backend change. Development autonomy is separate from Paige's runtime approval governance (unchanged). This supersedes the 2026-09-12 build authorization's "not authorized to deploy/merge" for routine delivery; the provider-call prohibition STANDS (credential + prepaid balance are owner-only).

- **Vibe media seam DELIVERED to production, config-gated OFF (2026-09-12, PR #1153 → main `d5387376`, Gate A + owner pre-merge corrections honored).** Merged only after the #1159 replay bridge let database-contract reach and validate `20270121000000` (migration renamed from the collided 20270118 slot); §32 persisted-apply confirmed by read-only prod queries (schema_migrations, table+5 policies, claim RPC, cron, realtime); three functions deployed and probed (webhook verify_jwt=false + in-function ED25519 fail-closed answered on prod; paige-media/sweeper behind the JWT wall). Two delivery repairs recorded: config-only pushes don't trigger the edge pipeline (webhook redeployed manually with the flag), and CI's paige-media deploy did not stick (redeployed manually — worth watching on the next functions PR). Provider activation remains OWNER-ONLY: FAL_KEY + prepaid ceiling + media budget + explicit max test spend; no paid call has been made. Capability truth: media generation UNAVAILABLE by config; music UNAVAILABLE; Social publishing non-executable until the Social operational proof.

- **Chat Product Completion · Slice 1 — the honest capability manifest completed (2026-09-12, Gate A, branch `claude/paige-cowork-handoff-rpf6dw`).** First dependency-ordered slice of the Chat Product Completion workstream (the grounding artifact `docs/delivery/chat-completion-matrix.md` identified "truthful capability discovery" as the foundation). The "what can you do here?" manifest (`_shared/paige-capability-status/{signals,resolver,render}.ts`) widened from contacts+connections to the full honest cockpit picture, pure-core + gatherer wiring ONLY (no new table/authority engine/tool; decision core unchanged, §18). **Anti-UNDER-claim:** surfaced 5 genuinely-governed chat capabilities the manifest had been silent about — `documents.create`, `knowledge.save`, `planning.create`, `agentteam.delegate` (the §8 "hire her team" capability), each grounded in its REAL ceiling-clamped lane (`resolve_tool_autonomy`), and `research.web` gated on the REAL `FIRECRAWL_API_KEY` presence (verified to match `paige-web-search`/`paige-deep-research`'s own `configured` degrade — §13/§34 names-not-values). **Anti-OVER-claim:** `skills.run` + `browser.secure_session` hardcoded UNAVAILABLE → "planned" (no chat seam / worker gated off — the safe direction). **Two new resolver dispositions** to complete the owner's taxonomy: `proof_owed` (attempt-but-never-promise) and `no_applicable_capability` (named in the render directive; never a faked per-capability row). **Bug caught mid-slice:** the `integrations.n8n_run_workflow` signal had been dropped in an earlier rewrite while its facts were still passed by the gatherer — re-added + re-proven (same class as the earlier `integrations.list` drop; see lessons-learned). **Reviews (real diff):** §5 compliance — COMPLIANT, no blockers; both should-fix items folded. §39 adversarial peer-gate — found ONE real BLOCKER: the manifest threaded `resolve_tool_autonomy` (the §67 trust-compass clamp) but NOT the action-risk `high`→`confirm` clamp that the runtime dispatch applies, so a HIGH tool (`delegate_to_subagent`, `n8n_run_workflow`) on an `auto` grant at a permissive rung resolved `live` ("no approval") while the dispatch always forces the card — an over-claim on the authoritative surface. **Fixed** by extracting the clamp into a shared `clampLaneByRisk` helper in `_shared/action-risk.ts` (§18 one home) that BOTH the chat dispatch AND the manifest gatherer (`resolveEffectiveLane`) resolve through, so they cannot diverge again; the dispatch's inline clamp was refactored onto the same helper (behavior-identical, warn preserved). §39 notes also folded: research re-framed off "connect a provider" (platform key → `unavailable`, not a tenant-connect step) and research tier-eligibility relaxed to non-client (web_search/deep_research carry no owner-ops gate at runtime — a confirmed under-claim). **Proof:** 69/69 capability + fingerprint tests green (incl. a new clamp block: HIGH-on-auto → needs_approval end-to-end); action-risk + signals + index transpile 0-syntax-errors; research-key honesty verified against `paige-web-search`/`paige-deep-research`. **Owed:** authenticated Solo-owner Chat drive (§32.c/§70) — the universal proof gate (least-privilege Solo test tenant + `LIVE_DRIVE_*` secrets), unchanged. **CI reality (§13):** a consumer slipped the §39 re-review — `src/solo/paige-team-capability.test.ts` read the dispatch SOURCE and pinned the old inline-clamp literals, so the clampLaneByRisk refactor broke its source assertions (CI `verify` red). Fixed by asserting the clamp RULE behaviorally on the real helper + keeping only the source check that the clamp is APPLIED above the branch (lesson recorded: a source-assertion test is a §37 consumer). The remaining `verify` failures are the 18 PRE-EXISTING Solo-frontend tests in 3 files this diff never touches (SoloGamePlanWorkspace, resend-receipt-handler, settings.connections-actions) — proven by the full vitest run (18 failed | 4312 passed, all in untouched files) and the #1162 precedent (merged to main with `verify`=failure); `verify` is non-required, the required checks are green.

- **Vibe Media credits DELIVERED; the controlled proof proven to the provider boundary (2026-09-12, PR #1168 + fix-forwards).** Product rules live as config: Solo Beta includes 300 Media Credits monthly ($3.00 of provider cost at $0.01/credit); packs are margin-priced DATA ONLY ($5→350cr, $15→1100cr, $40→3000cr — ≥20% contribution after 2.9%+$0.30 processing; no Stripe products exist); plan price unchanged $74.50/30-day trial; Chat/automations visible-but-included; no unlimited promise anywhere. The truthful-naming correction shipped: `media_spend_ceiling_usd` is the ENFORCED platform-wide daily guard (checked inside media_credit_hold under a global-class lock); `media_provider_ceiling_usd` is documented as the activation gate it always was. The proof (≤$1.25 authorized): an isolated least-privilege test tenant exercised the ENTIRE ladder live — the only failure was fal rejecting the FAL_KEY credential itself (403; secret present by name, value never read); the anti-leakage settlement returned the reserved credits and platform spend read $0. Owner action to resume the paid proof: re-issue the fal key in the id:secret form as the FAL_KEY edge secret.

- **Platform Operator Command Center released by owner mandate (2026-09-12, Gate A).** The owner's build mandate (this session) releases the parked `operator.platform` seam ("no broad operator/subaccount work until the owner releases it" — that was this): build Paige's internal operating headquarters on the existing owner-locked v3 console at `/operator` (six-slot IA, `RequireOperator` server-enforced guard — Phase 1 already shipped), Phase 2 first view = the pack's Analytics → Platform health, then tenant triage (Phase 3), governed controls via the one approval gate + Layer C as landed (Phase 4), Paige-as-operator (Phase 5). Authority boundaries fixed by the mandate: separate server-enforced operator plane, no tenant identity/context/data crossing, no fabricated health, honest states only, no parallel platforms/seams. Phase 0 map + the ten AGENTS.md capability-routing answers live in `docs/delivery/platform-operator-command-center.md`. First slice delivered as PR #1181 (Platform health view, real seams, §58 Team-Pulse displacement recorded, `operator.platform` UNAVAILABLE→PARTIAL + portfolio snapshot moved in-change).

- **skill-runner routed through the canonical governed seam; §9 cross-tenant leak + spoofable authority closed (2026-09-13, Harness runway #1, PR #1179).** The direct `skill-runner` Edge route now enforces the governed truth itself rather than trusting an upstream caller: `resolveSkillCaller` derives actor + tenant + approving authority SERVER-SIDE from the verified JWT (or a trusted service credential, which the seam then refuses for this `high` mutation — a machine credential is nobody's yes), never from `body.tenant_id`, `body.contact_id`, or `body.invoker_kind`; `decideSkillRun` runs the call through the shared `decideGovernedExecution`. Every subject lookup (`draft_and_email_document`, `build_game_plan`, `verify_business_sos`, and the interpreter path) is scoped `.eq("tenant_id", callerTenantId)`, so a cross-tenant `contact_id`/`business_id` resolves to nothing instead of leaking tenant B under tenant B's identity. The one external send (`draft_and_email_document`) is bound to a durable, single-use, fingerprint-bound approval in the EXISTING `paige_pending_confirmations` table (fingerprint over action+tenant+contact+recipient+sender+doc_type+content SHA-256; atomic compare-and-set claim excluding the minting request; §18 — no parallel approval system). No migration (table + `confirm-fingerprint` reused), no new deps. Pre-merge, two INDEPENDENT reviewers (§39 adversarial + §5 compliance) read the real pushed diff and both caught a §13 response-honesty defect the author+integrator had missed — a FAILED external send was reported `status:"succeeded"` (durable records honest; HTTP response not); folded before merge as a distinct `send_failed` outcome → 502/`status:"failed"` + a failed-send test (the approval is consume-then-execute, so it is spent and the caller re-drafts). Proof (run locally): `src/__tests__/skill-runner-governed.test.ts` 19/19 + reused-seam 142/142; `lint:action-risk`/`lint:definer-fns`/`lint:governed-execution`/`lint:write-targets` ✓; zero new `ci:tsc` errors (the +1 `paige-capability-gateway.test.ts` error is pre-existing on `main`, a separate filed follow-up). §37 impact: `scripts/skills-s58-harness.mjs` (service-role) is now correctly refused — it must use a scoped test-tenant admin JWT; no service-role execute backdoor added. LOW residuals (non-email-skill propose-enforcement via the existing confirm gate, `active_tenant_id` integrity, broad member access) are pre-existing/disclosed follow-ups, deliberately not widened. PROOF OWED: `ci.yml`'s `deno` check did not run (GitHub suppresses Actions on this branch's PR-create/synchronize events under recursion-prevention) and authenticated live-drive of the deployed function (JWT→tenant derivation lives in Deno) is owed to a browser-capable session (§32.c).

- **P0 consequential-action confirm loop + bug-report honesty — FIX BUILT on a fresh-main emergency branch; NOT merged, NOT shipped; batch-repair firing + live rows/error are PROOF OWED (2026-09-13, branch `claude/p0-confirm-loop-repair`, separate emergency PR).** Owner-reported production incident: an owner-confirmed `action_advance` dismissal repeatedly re-issued a fresh confirmation/fingerprint then "failed flat"; a bug-report proposal failed to create; nothing dismissed/filed/sent. **Grounded vs owed (§13/§32.c):** the loop MECHANISM is established by reading the DEPLOYED SOURCE (the `#711` `paige_tool_confirmations` token path is unwired in chat; the live gate is `paige_pending_confirmations` + `confirmFingerprint` + a request nonce; a BATCH is disambiguated only by the exact fingerprint, which a drifted hashed arg defeats → both fallbacks refuse on ≥2 → the drifted fp records a NEW proposal → a silent re-ask loop). It is NOT a runtime trace of the incident — the actual failing confirmation/action rows and server error category need prod log/DB access this session lacks, and remain PROOF OWED. #1166 had closed only `decision_rationale` drift. **FIX A:** disambiguate a batch by the stable, required, model-reproduced subject id (`action_advance.action_id`) WITHIN the operator's already-approved fingerprint set — narrows, never widens, what is claimable; stored args execute (new pure `CONFIRM_IDENTITY_KEY`/`confirmIdentityValue` in `_shared/confirm-fingerprint.ts`, §18; jsonb narrow uses the proven `.filter(col,"eq",val)` form). **FIX B:** un-pinnable approval → ONE honest `execution_unavailable` terminal ("nothing changed or sent" + `action_ref` + `correlation_id`), records nothing. **FIX C:** `improvement_propose` returns an explicit "nothing was filed" for a tenant-less/role-refused caller instead of a throw. **Reviews:** §39 design-adversary cleared all seven gate invariants + two folded guards (by-scope claim confined to card-less surfaces; ambiguous terminal only on a genuine approval attempt); §5 compliance NO BLOCKER. `lint:approval-gate` + `lint:governed-execution` pass (no second approval channel). Local proof: unit 13/13, wiring 10/10, neighbouring confirm/governed 172/172, `index.ts` parses clean. **STATUS (honest):** containment is LOCALLY PROVEN (fail-closed logic + tests — an un-pinnable approval cannot loop or execute); the batch REPAIR actually firing (the `args->>action_id` match on the deployed chat, failure-safe → the terminal) and the full owner battery (benign dismissal, batch resolve, expired/replayed/forged/cross-tenant, receipt/Rail, no-outbound) are PROOF OWED on the deployed surface. **Do NOT report contained as fixed, and add NO Shipped Log (master §4) entry until the real merge SHA + deployment identity + persisted post-deploy proof exist.** Kept OFF PR #1177 (task↔thread slice only) per owner ruling 2026-09-13; this is its own emergency PR from current main. §66: no surface/gating/tier-visibility change. Cross-refs #711/#1166.
- **Layer C · C1 merged + persisted — mechanical closeout (2026-09-13, PR #1173 → `main` 7784571944, release channel production).** The connector-neutral governed event→act engine foundation shipped: the per-act outcome ledger `public.paige_act_executions` + the atomic monotonic `paige_record_act_execution` RPC (migration `20270126000000`), the decision core, the `ActionAdapter` registry, the person-attributed engine, the subject-derived tenant-integrity check, and the dispatcher wiring; §39 adversarial + §5 compliance reviewed pre-merge (both ITERATE → findings folded). §32 persisted: deploy-migrations #251 success · `db-live` at 7784571944 · `20270126000000` in the db-live tree · `db-live..main` zero drift · `edge-live` moved. The substantive record (owner corrections #2/#3/#4/#6, the C1 outcome semantics, and the roadmap's C1–C5 sequence) is PR #1173 / commit 7784571944 and the master-doc §4.0 Shipped Log row; this closeout carries no new policy/capability/status change (AGENTS.md closeout rule).
### 2026-09-12 — Solo-only public acquisition and verified entitlement candidate

**Decision:** Paige is publicly enrollable only as the paid Solo Beta with one server-fixed 30-day trial, then `$74.50/month` in test mode unless canceled before the first paid renewal. New acquisition never selects Agency, Platform, Client Portal, sub-account, annual, a caller-chosen/reusable trial, free, or planless variants. Checkout collects payment details for the first renewal; checkout success in the URL is never entitlement. Only signed provider evidence proving the exact trial and Price plus atomic provisioning and fresh receipt/subscription/membership readback can emit the current Solo destination.

**Preservation:** Existing authorized non-Solo and multi-account users keep their server-derived access and account-choice behavior. PR #899's deterministic payment-setup ordering remains separate and carried forward. PR #724's canonical Solo route contract remains the destination authority; generic `/app`/account-choice fallbacks are superseded only for ordinary new public Solo enrollment.

**Proof boundary:** candidate code, automated/static checks, fresh migration replay, and Solo authorization/offer pgTAP are `VERIFIED`; authenticated non-production Supabase, Stripe test-mode, deployed production, and owner acceptance are `PROOF OWED`/`UNVERIFIED`. No charge, live provider configuration, production customer mutation, or customer-ready claim is authorized by this entry.

- **skill-runner receipt-honesty fix-forward (Codex P1/P2, 2026-09-13, PR #1183).** After #1179 merged, a THIRD independent reviewer (Codex, on the merged head `26bd6d2`) flagged two §13 receipt-honesty gaps in `runDraftAndEmailDocument`'s `recordReceipt` that the §39 adversarial + §5 compliance passes had missed: (P1) the `paige_skill_runs` insert destructured only `{data}`, so a failed receipt write after a successful Resend send still returned HTTP 200/`succeeded` with a null run id and no durable history (AGENTS.md L48-49 — no completed action without its receipt; the `communication_log` insert likewise swallowed its error); (P2) the receipt recorded the redemption `body.inputs` rather than the APPROVED claimed inputs, so a phase-2 caller redeeming a valid token with altered `doc_type`/`prompt` could make the run row misdescribe the message that was actually sent from the stored claim. Both verified real against the code and fixed forward in #1183: the receipt write is CHECKED (`receiptPersisted`; a sent-but-unrecorded send returns `succeeded_unrecorded`/`receipt_not_persisted`, never a silent clean success — the email is consume-then-execute so it cannot be un-sent, but the missing audit row is surfaced), the `communication_log` error is logged not swallowed, and the run row records the claimed `doc_type`/`contact_id`/`content_sha256`. Host-code hardening (the vitest matrix injects its own `recordReceipt`, so this layer is covered by the deno edge-ratchet typecheck + the owed authenticated live-drive, not the unit matrix). Lesson: an external-effect seam's receipt write must be checked like any other, and the receipt must describe the CLAIM, not the redemption request.

- **CI exact-ref manual dispatch enabled (2026-09-13, branch `claude/ci-exact-head-dispatch`).** A bot-authored PR's `pull_request`/`push` events are withheld from GitHub Actions (recursion prevention), so such a PR can never earn a real `ci / verify` run from its own events — which repeatedly left exact-head verification owed on bot PRs (e.g. #1185). Added a `workflow_dispatch` trigger to `.github/workflows/ci.yml` and taught every previously `pull_request`-only gate (merge-base, changed files, tsc-baseline, regression lint, eslint/gold changed-src, resolve-affected edge fns, the Deno edge ratchet + evidence) to also run under a dispatch, resolving base from the live `origin/main` tip and head from the dispatched ref. **Safe by construction:** for `pull_request` and `push` every widened condition + env value evaluates identically to before, so existing CI is byte-unchanged; the dispatch branch and null-PR env fallbacks activate only on a manual dispatch. No check is weakened, skipped, replaced, or de-required (owner-authorized 2026-09-13 overnight mandate: "a separate, narrowly reviewed CI-reliability change allowing the existing CI workflow to be manually dispatched on an exact ref"). How to use: `gh workflow run ci.yml --ref <branch>` (or the Actions dispatch API), then read the run's `verify` job.
- **Integration Registry `code_anchors` accountability delivered (2026-09-13, grounded origin/main 91c815f).** The Integration Capability Registry (`docs/integration-registry/integration-capability-registry.json`) catalogued 23 providers but no mechanism coupled a provider's claimed DELIVERY state to the real code that backs it — the exact gap its own `known_owed_entries.registry_steward_role` named as why Upload-Post (6e35c60e) and fal.ai (d5387376/#1153) merged owing entries. Delivered the smallest non-duplicative slice: (1) a new `field_schema.code_anchors` contract (array of `{path, role, note?}` citing the ACTUAL adapter/entry-point code; REQUIRED non-empty for runtime-claiming statuses LIVE/PARTIAL/PROOF_OWED, ABSENT/[] for unbuilt UNAVAILABLE/DEFERRED/PROPOSED — distinct from `sources`, which are doc citations); (2) backfilled `code_anchors` on all 16 runtime-claiming providers (50 anchors total, every path verified to exist on disk), with two §13 honesty escalations preserved in the anchor notes — **meta** is PARTIAL because its adapters (`paige-social/`, `meta-schedule-post/`) are runtime-contained to HTTP 503 `attempted:false` via `_shared/socialUnavailable.ts` (NOT LIVE), and **google-workspace** is PARTIAL because only Google Calendar OAuth code exists — there is NO Drive/Docs (productivity_documents) adapter, so only the calendar-OAuth code is cited with the gap noted; (3) extended `scripts/ci/integration-registry-lint.mjs` (reusing binding-ledger-lint's `findDeadAnchors` pattern, §18 — no new lint, no new npm script, no CI-step change) with status-coupled structural rules in the pure `validateRegistry` plus a new exported pure `findDeadCodeAnchors(reg, exists)` dead-anchor resolver that fails CI on any cited path missing from disk (a single-`*` last-segment glob matcher handles glob anchors). Schema bumped 1.1.0 -> 1.2.0. OWNER REFINEMENT (2026-09-13, folded in before completion): each anchor's `role` is now a CONTROLLED, lint-enforced vocabulary of exactly four kinds — `provider_adapter` (the provider's own integration/API/OAuth/send code) · `callback_readback` (webhook/status-callback reading back the provider's outcome) · `fail_closed_containment` (a path that deliberately refuses/contains, e.g. a 503) · `proven_runtime` (ONLY where genuine authenticated §32.c runtime proof exists — DEFAULT NONE, and none is used today) — validated in the pure `validateRegistry` (CODE_ANCHOR_ROLES) with a self-test mutation proving an out-of-vocab role fails; a code anchor proves only that a code PATH EXISTS, never that the provider works/connected/LIVE, and this mechanism raised NO provider's status (traceability only). Meta's four anchors (paige-social, meta-schedule-post, handle-meta-webhook, socialUnavailable) are ALL `fail_closed_containment` — every one returns HTTP 503 attempted:false — and the socialUnavailable note states it must NEVER imply LIVE publishing/analytics, anchored only so drift is detectable (status stays PARTIAL). Every runtime-claiming provider cites ≥1 PROVIDER-SPECIFIC anchor; the one generic dispatcher present (resend's send-message) is explicitly flagged and is never the sole anchor (enforced by construction + honest note, not a brittle lint). Proofs: self-test green (42 mutations + dead-anchor resolver + glob-matcher proofs), main lint green (23 providers, 50 anchors all resolve), independent `jq + test -e` fs sweep shows 0 missing. Reviewed on the real diff by an independent §39 adversarial pass + a §5 compliance pass (both SHIP after one 53->52 count correction), then by Codex on PR #1189 which flagged one P2 — a non-object `field_schema` (null/string/number/array) would skip the code_anchors contract check and let CI accept a registry with the accountability field removed; hardened to reject it, with two self-test mutations proving the catch (38 -> 40). A SECOND Codex P2 caught a mis-attributed anchor — `elevenlabs` cited `paige-stt/`, which actually opens a Deepgram Nova-3 STT session (`provider:"deepgram"`, via `_shared/stt-router.ts`), NOT ElevenLabs — so it was removed (52 -> 51 anchors) and Deepgram was recorded in `uncatalogued_wired_providers` as honest coverage (a genuinely-wired STT provider, never an invented catalogued entry, never status-set). Both Codex P2s were caught by the external reviewer AFTER my §39/§5 crew passed — the layered-defense point (§39: none alone is sufficient). A THIRD Codex P2 (on cfa15fe2) then caught that 38 of 51 anchors were DIRECTORIES, so `fs.existsSync(dir)` stayed green if the specific adapter file was deleted while a sibling survived (e.g. `_shared/email/providers.ts` — the Resend branch — vanishing while `branding.ts`/`shell.ts` kept the dir alive), defeating the dead-anchor check's core promise. Fixed by converting EVERY anchor to its exact FILE entry point (edge fns -> `/index.ts`; `_shared/email/` -> `providers.ts`; `_shared/paige-mcp/` -> `governed-adapter.ts`; `services/paige-browser/` -> `server.js`) and tightening `anchorExists` to require a FILE — a bare directory is now rejected (self-test proves it), so file-level drift (a moved/deleted adapter file) fails loudly. All 50 anchors resolve to real files. TWO more post-review accuracy fixes on the final diff: a §39 adversarial pass flagged the `platform-marketplace` anchor `_shared/marketplace-authority-containment.ts` as `provider_adapter` when it is a fail-closed authority/containment helper (no provider API/send seam) — corrected to `fail_closed_containment` to match its structurally-identical siblings (`governed-adapter.ts`, `secure-browser-authority.ts`); and a FOURTH Codex P2 caught that `posthog` anchored the ORPHAN `_shared/posthog.ts` (0 importers) rather than the reachable client `src/lib/observability.ts` (posthog-js, init'd in src/main.tsx) — re-homed to the reachable adapter so drift in the code that ACTUALLY runs is what fails CI. TWO further Codex P2s on the final commit: (5) `browser-use/index.ts` is the Browserbase (3rd-party, PROPOSED) stateful STUB — its header marks the external worker deliberately unavailable and `services/paige-browser/README.md` names it the Browserbase stub — NOT the reachable self-hosted `paige-browser-research` capability (server.js + paige-deep-research), so it was removed (51 -> 50 anchors) while Browserbase stays PROPOSED/unanchored; (6) a path-containment hardening — `anchorExists` AND the pure `validateRegistry` now reject an absolute path or a `..` escape, so a future anchor cannot satisfy the dead-anchor check with a CI-host file like /etc/passwd (two self-test mutations + two anchorExists assertions prove it). Six external-reviewer catches total; all fixed; all 50 anchors resolve to real repo files. A seventh Codex P2 flagged a symlink escape (a committed `adapter.ts -> /etc/passwd` symlink would pass via statSync) — hardened: `anchorExists` now uses `lstatSync`, so a symlink is isSymbolicLink()=false-as-a-file and rejected (a real regular file still passes). TRACKED FOLLOW-UPS / ESCALATIONS (not built here): (a) a reverse coupling check forcing a provider-touching DIFF to update its entry — this slice closes the anchor-accountability half only; (b) REGISTRY-STEWARD ESCALATION to the google-workspace domain owner — an eighth Codex P2 correctly observed google-workspace's productivity_documents PARTIAL rests only on SHARED Google Calendar OAuth code (the same anchors as google-calendar) with NO Drive/Docs adapter. Whether that entry should stay PARTIAL (OAuth foundation real, Drive/Docs gap disclosed) or move to DEFERRED/UNAVAILABLE is a DOMAIN-OWNER truth decision this mechanism must not make (owner rule: Registry Steward owns schema/anchor-validity/escalation, never domain capability truth or provider status). Status left UNCHANGED; the Drive/Docs gap is disclosed in-anchor; escalated here for the domain owner to rule. Reuses the §18 mechanism from the binding-ledger (`findDeadAnchors`) and the view/definer/tier-feature CI-guard family (#116/#117). FIRST LIVE CATCH (post-merge, 2026-09-13): on merge to main the guard immediately failed on `upload_post_social` (PROOF_OWED, the Upload-Post social provider) — a 24th provider added by a concurrent PR between this slice's grounding (91c815f) and merge, which my PR's own CI never saw. Hotfixed: anchored it to its real provider-specific code (`_shared/social-provider/upload-post.ts` provider_adapter + `paige-social-callback/index.ts` callback_readback), 50 -> 52 anchors, lint green on main. This is exactly the accountability the guard exists for — a runtime-claiming provider can no longer merge owing its code anchors.

- **P0 confirm-loop repair MERGED + DEPLOYED (2026-09-13, PR #1185 → `main` `ff5fc9ed`, production).** The consequential-action confirm-gate fix shipped: (A) a batch of same-tool confirmations is disambiguated by the stable subject id (`action_advance.action_id`) WITHIN the operator's already-approved set (narrows, never widens; stored args execute); (B) an un-pinnable/ambiguous approval ends in ONE honest terminal ("Action execution is temporarily unavailable; nothing changed or sent." + correlation id) that records nothing and cannot loop/execute; (C) `improvement_propose` returns an honest "nothing was filed" for a tenant-less/role-refused caller. No task↔thread behavior (separate from #1177). §32 persisted deploy: `deploy-edge-functions` run 34737466149 success; `edge-live` at `ff5fc9ed`; `paige-ai-chat` + `_shared/confirm-fingerprint.ts` zero drift vs prod. The exact-head CI required before merge was obtained by DISPATCHING `ci.yml` (run 34736741411) — bot-PR `pull_request`/`push` Actions events are suppressed, so #1191 (separate, §39-reviewed, owner-authorized) added `workflow_dispatch` to the gate (safe-by-construction; PR/push byte-unchanged). That exact-head `verify` was RED on **pre-existing main-wide debt ONLY** — tsc baseline (`paige-capability-gateway.test.ts`, `signupMobile.ts`/#1190), chat-tool-registry Spine-migration ratchet, 18 Solo-frontend test failures in 4 untouched files, and the views/pack-lineage/operator-reach/integration-registry/secure-browser/client-memory full-tree guards — EACH reproduced byte-identically on `origin/main`, proving this diff adds zero new failures (stand-down comment on #1185). `verify` is non-required; required (Vercel) checks green. §39 adversarial read of the full pushed diff: NO BLOCKERS. **LESSON / honest limit:** the P0 is "contained + deployed", NOT "fixed" — the authenticated deployed-surface battery (benign dismissal · batch resolve-or-one-honest-terminal-without-looping · expired/replayed/forged/cross-tenant denied · receipt/Rail persistence · no outbound on denial/failure · unavailable bug-report says nothing filed) is PROOF OWED (§32.c) to a browser-capable / prod-log session (no prod MCP, no test tenant, no browser-drive this session).

- **Solo Beta live billing binding shipped with proof boundary (2026-09-13, owner-authorized).** PR #1200 exact head `f018ac57` squash-merged as `4ccb8a9a`. The sole canonical `paige-solo-beta-monthly-v1` offer is live-ready and server-bound to the verified active live Paige Solo Beta Product and USD 7450 monthly Price; the non-reusable 30-day trial remains a Checkout subscription parameter. Production migration/Edge/Vercel, provider catalog/webhook, database grants/constraints, public offer status, forged-signature denial, unauthenticated Checkout denial, and signed-out four-viewport rendering are verified. No customer, subscription, invoice, payment, or charge was created for proof. Capability remains `PARTIAL`: first authorized live Checkout + signed fulfillment + exactly-one tenant/membership/entitlement/receipt/current-shell and lifecycle proof are owed; Stripe Billing Portal has no active live configuration and remains fail-closed/`PROOF OWED`.

### 2026-09-13 — Promotional Solo access is explicit, service-only, metered, and non-provider-bound

Owner-authorized grandfathering is not a free/trial fallback and is not public enrollment. PR #1202 shipped `platform_grant_promotional_solo`; PR #1210 recovered a production migration-catalog collision forward under `20270201000000`. Production run `34743013452` persisted the contract. Browser roles are denied; a service caller must supply a real platform-owner actor, an existing identity, a standalone-safe topology, and a reason. The transaction creates or reuses exactly one personal Solo workspace and owner membership, writes explicit `promotional` classification and metered/no-expiry subscription truth, preserves unrelated memberships, emits one audit plus usage receipt, and fails closed on paid/provider conflicts. The Antonio Cook grant and idempotent retry are verified in production; Antonio Daniel LLC and Mogul Maker Academy remain active promotional tenants. No Stripe object, charge, trial, or provider binding is created. Authenticated browser sign-in, Billing rendering, and account switching remain `PROOF OWED`. The Integration Capability Registry is unchanged because this is a native platform access decision, not a third-party integration.

### 2026-09-13 — Fresh login always pauses for established workspace choice

Every established authenticated person with at least one active direct `tenant_members` row pauses at `/choose-account` on each fresh login, including a person with only one workspace. The existing authorized card remains visible until deliberately selected and enters only through the guarded `switchTenant` seam. Platform remains an explicit peer context for authorized staff. A stale Solo acquisition intent or `/welcome?checkout=success|recovery` URL cannot supersede established access; `solo-beta-enrollment-status` returns a bounded `choose_account` recovery with the fixed `/choose-account` destination when active membership exists and no enrollment exists. Genuine membership-less Solo intake/checkout and signed invite acceptance remain unchanged. The in-workspace exit control still appears only when another enterable context exists. No Stripe configuration, entitlement grant, tenant mutation, or unsupported account enrollment is part of this decision.

Production closeout: PR #1219 squash-merged as `4bc9cf771e992614701d6f4e1b94f1d5c3d513d6`. Vercel deployed that merge and the current production descendant `c287a4f86ad42f35903a7dc297012514e1480c8a` contains the scoped files unchanged. Edge run `34749881389` deployed `solo-beta-enrollment-status`; current `edge-live` `d33f67180f2a55691e55dec39fd807610d71fc63` is a descendant with no scoped function drift. Production merge, web identity, public-route reachability, and Edge deployment are verified; authenticated legacy/promotional sign-in and persisted workspace selection remain `PROOF OWED`.

### 2026-09-13 — Stripe owns live Checkout payment-method eligibility; welcome follows verified fulfillment only

PR #1226 (`03bf91ea`, merged as `0ba624b7599eca61eae83b21c0d1aea3270069c4`) preserves the owner's live Stripe Dashboard payment-method configuration by intentionally sending no fixed `payment_method_types`, no payment-method configuration override, and no exclusion list. Stripe therefore owns session eligibility; Paige makes no claim that every enabled method appears for every customer. Paige still resolves the sole live Solo offer, Price, 30-day trial, identity, and fulfillment server-side.
