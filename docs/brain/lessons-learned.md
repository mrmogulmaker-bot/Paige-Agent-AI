...
- **Rule.** A SECURITY DEFINER reader must derive authority from SERVER facts PER TARGET (self = `auth.uid() = _target_user_id`; cross-user = active relationship / same-tenant `is_tenant_admin` / `is_platform_operator` / the canonical resource helper `can_access_contact` for a `clients.id`), and **gate each data-predicate branch on its own per-target flag** so authorization for target A never returns target B through a different branch. Bound every caller-supplied search parameter (`threshold`→`[0,1]`, counts→`[0,50]`). And when the sole caller is service-role, decide the service-role posture DELIBERATELY — trust it to pass server-resolved ids (the governed-memory-seam pattern) only where a producer inventory proves the server resolved scope upstream; otherwise it fails closed and the feature is dead, which is a §32 finding in its own right. The class of proof that catches all of this is a **caller inventory against the tenant matrix** (who calls this, as which role, having proven what?) — never a unit suite over the code path a dead function's absent callers never run.
