...
| M2 | No cancellation-policy or scheduled-cancellation field exists on an item or an install. `marketplace_installs.status` is active/uninstalled only. | `src/integrations/supabase/types.ts` `marketplace_installs` Row |
