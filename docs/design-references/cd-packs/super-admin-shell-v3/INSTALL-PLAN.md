...
| **R6** | **The SSN/EIN already in git history.** I neutralised them going forward (commit `8194ce7c`); the originals remain in `c1f30f57` and in every clone. | Leave history alone unless you want the surgery. They are near-certainly CD-invented, and the forward fix stops the real risk (a later session porting a real-looking identifier into a shipped surface). |
