...
STATIC_EVIDENCE: PASS: tsc-ratchet — no new errors from this PR's files (2 pre-existing main-drift errors in untouched files paige-capability-gateway.test.ts / Step1Welcome.tsx, verified by stash on the clean tree); CI database-contract PASS on the rebased head (the trigger replays cleanly; migration renumbered twice onto 20270324000000 after a second version collision, main took the slot).
...
- Collision check performed twice: migration version collided twice with concurrent main traffic (20270122 taken by social_foundation; 20270126 taken during the build) — renumbered to 20270324000000 and rebased onto the taking commit each time.
