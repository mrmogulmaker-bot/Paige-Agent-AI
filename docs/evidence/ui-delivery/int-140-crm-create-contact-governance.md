...
AUTOMATED_EVIDENCE: PASS: initial class-level failing-first produced 4 failures / 20 passes; the equivalent-name test independently failed and produced distinct retry keys before the canonical boundary was added; repaired focused suites pass 33/33 and the directly affected 15-file CRM/governance family passes 343/343; restoring presence-only name handling makes the malformed-name regression red, restoring the missing-counterpart condition makes the absent-name-part regression red, and removing pre-hash/invocation canonicalization makes the equivalent-retry regression red; independently removing display trim, Unicode-whitespace collapse, identity case folding, nonsemantic zero-width filtering, NFC/NFKC normalization, legacy field-alias resolution, lifecycle-alias resolution, the own-property alias guard, the runtime canonical-command marker, the contact-only execution envelope, or the database effective-command projection makes its focused regression red; expanding the filter to remove meaningful join controls also makes its preservation regression red; restoring every step returns one key across canonical, leading/trailing/repeated-space, mixed-case, non-breaking-space, nonsemantic-zero-width, decomposed-Unicode and legacy-alias spellings; the exact 147-tool manifest contract passes 9/9 at 215944 serialized bytes; knowledge scope passes 369/369; extraction proposal passes 15/15; apply extraction passes 26/26; trace wiring passes 12/12; action-risk lint reports 157 classified actions with 0 unclassified writes; tool-catalogue lint reports 156 runtime-gated / 160 catalogued / 0 governed-but-invisible tools
STATIC_EVIDENCE: PASS: the create-contact nested patch declares only canonical executor fields with additionalProperties=false and canonical lifecycle values; one shared person-name boundary produces a display/storage form (NFC, Unicode-whitespace collapse, trim, case preserved, meaningful format controls preserved) and a comparison/hash form (NFKC, removal of only U+200B/U+2060/U+FEFF, Unicode-whitespace collapse, trim and locale-independent lowercase); normalization runs immediately after crm-command request validation and before tenant readback, autonomy resolution, confirmation, idempotency or execution; a branded canonical-command type plus runtime WeakMap rejects raw/unmarked commands at the fingerprint boundary; PAIGE Chat hashes the canonical identity projection and invokes the same canonical command; migration 20270410000000 makes database replay consume that same server-derived projection and fails closed if its predecessor hash seam drifts; focused changed-file ESLint passes, TypeScript ratchet remains baseline=12/current=12, migration/security/schema/regression lints pass, and the production build passes; exact-head Linux verify remains proof owed until push
RENDERED_EVIDENCE: NOT_APPLICABLE: no interface source, visual state, geometry, copy, motion or control changed
BEHAVIORAL_EVIDENCE: PASS: controlled offline tests reproduce the production-shaped name/lifecycle payload, fill absent or malformed canonical name parts from the usable legacy alias without overwriting usable canonical values, normalize names without mutating the original input, preserve human-facing spelling and meaningful Persian/Indic/Mongolian join controls for storage/invocation, bind canonical, legacy-alias, leading/trailing/repeated-space, mixed-case, non-breaking-space, nonsemantic-zero-width and canonically decomposed Unicode forms to one comparison identity and fallback retry key, reject inherited object keys as lifecycle aliases, reject a raw command at the hash boundary, invoke the same canonical command whose identity projection was hashed, retain unrelated malformed fields for canonical fail-closed validation, and preserve the existing confirm, execute, readback and receipt contracts; a disposable local PostgreSQL application of migration 20270410000000 proved two display spellings produce one semantic hash and that the installed reversible executor hashes public.crm_effective_command(_command), then the database and temporary roles were removed; this is not authenticated production execution
...
INTERNAL_BUILD_IDENTITY: 36a8626c8a43d733e63d5c74e19dd35f763e2a57; deployment=none-pre-merge; environment=development; migrations=PROOF_OWED(20270410000000_crm_contact_name_idempotency after merge); edge=PROOF_OWED(crm-command and paige-ai-chat automated deployment/source match after merge); evidence=INT-140-focused-regression-mutation-and-local-database-proof
...
- Product-code head after the conflict-free current-main merge and migration renumber: 36a8626c8a43d733e63d5c74e19dd35f763e2a57 (base 283547eccf8020d474598d5f77fea5958b623f90).
- Failing-first: the class-level extension produced 4 failures / 20 passes on the then-current repair; review follow-up separately reproduced malformed-name, absent-name-part, equivalent-retry, database-replay, meaningful-join-control and prototype-alias failures before the complete boundary was restored.
- Load-bearing mutations: restoring presence-only canonical-name handling makes the malformed-name regression fail; restoring the missing-counterpart condition makes the absent-name-part regression fail; removing PAIGE Chat's pre-hash/invocation canonicalization makes the equivalent-retry regression fail; independently removing display trim, Unicode-whitespace collapse, lowercase identity, nonsemantic-zero-width filtering, NFC/NFKC normalization, legacy name-field alias resolution, lifecycle alias resolution, the own-property alias guard, contact-only identity envelope, runtime canonical marker, or database effective-command projection makes the matching regression fail; overbroadly removing meaningful join controls also fails; every mutation was restored before the passing run.
- Focused proof: CRM adoption, action-door contract and canonical executor/readback/Rail contract = 33/33.
- Wider proof: directly affected 15-file CRM/governance family 343/343; exact manifest 9/9; Knowledge scope 369/369; extraction proposal 15/15; apply extraction 26/26; trace wiring 12/12; TypeScript ratchet 12/12 baseline/current; production build passed; local PostgreSQL migration apply and semantic-hash proof passed with full cleanup.
