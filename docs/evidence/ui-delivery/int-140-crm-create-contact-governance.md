...
AUTOMATED_EVIDENCE: PASS: initial failing-first on current main produced 4 named failures; the two review-found defects each failed before repair; repaired focused suites pass 26/26 and the wider affected family passes 136/136; restoring presence-only name handling makes the malformed-name regression red, and removing pre-hash/invocation canonicalization makes the equivalent-retry regression red; restoration returns both green; the exact 147-tool manifest contract passes 9/9; knowledge scope passes 369/369; extraction proposal passes 15/15; apply extraction passes 26/26; trace wiring passes 12/12; action-risk lint reports 157 classified actions with 0 unclassified writes; tool-catalogue lint reports 0 growth in governed-but-invisible tools
...
INTERNAL_BUILD_IDENTITY: 20938ed76104ec5ee1ee01b5033c0125982b5b03; deployment=none-pre-merge; environment=development; migrations=NOT_APPLICABLE; edge=PROOF_OWED(crm-command and paige-ai-chat automated deployment/source match after merge); evidence=INT-140-focused-and-regression-suites
...
- Product-code head after the conflict-free current-main merge: 20938ed76104ec5ee1ee01b5033c0125982b5b03.
- Failing-first: the original contract repair produced 4 failures / 12 passes on untouched product code; review follow-up separately reproduced a malformed-name failure and an equivalent-retry failure before either correction.
- Load-bearing mutations: restoring presence-only canonical-name handling makes the malformed-name regression fail; removing PAIGE Chat's pre-hash/invocation canonicalization makes the equivalent-retry regression fail; restoring the repairs returns both green.
- Focused proof: CRM adoption 10/10, action-door contract 8/8, canonical executor/readback/Rail contract 8/8 = 26/26.
- Wider proof: affected CRM/governance family 136/136; exact manifest 9/9; Knowledge scope 369/369; extraction proposal 15/15; apply extraction 26/26; trace wiring 12/12; TypeScript ratchet 12/12 baseline/current; production build passed.
...
did not exist while legacy arguments were rejected outright: malformed canonical name
parts must not discard a usable alias, and equivalent retry spellings must not mint
