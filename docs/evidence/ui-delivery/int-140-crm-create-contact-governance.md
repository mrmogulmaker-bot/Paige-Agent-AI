...
AUTOMATED_EVIDENCE: PASS: initial failing-first on current main produced 4 named failures; all four review-found defects failed before repair; repaired focused suites pass 28/28 and the wider affected family passes 248/248; restoring presence-only name handling makes the malformed-name regression red, restoring the missing-counterpart condition makes the absent-name-part regression red, removing pre-hash/invocation canonicalization makes the equivalent-retry regression red, and removing canonical-name trimming makes the whitespace-equivalent retry regression red; restoration returns all four green; the exact 147-tool manifest contract passes 9/9; knowledge scope passes 369/369; extraction proposal passes 15/15; apply extraction passes 26/26; trace wiring passes 12/12; action-risk lint reports 157 classified actions with 0 unclassified writes; tool-catalogue lint reports 0 growth in governed-but-invisible tools
...
BEHAVIORAL_EVIDENCE: PASS: controlled offline tests reproduce the production-shaped name/lifecycle payload, fill absent or malformed canonical name parts from the usable legacy alias without overwriting usable canonical values, trim usable canonical name parts before hashing and invocation, normalize without mutating the original input, bind legacy, canonical and whitespace-equivalent forms to the same approval subject and fallback retry key, invoke the same canonical command that was hashed, retain unrelated malformed fields for canonical fail-closed validation, and preserve the existing confirm, execute, readback and receipt contracts; this is not authenticated production execution
...
INTERNAL_BUILD_IDENTITY: 57f13c04b49af0819bddc21fa0ad0f527a7185a9; deployment=none-pre-merge; environment=development; migrations=NOT_APPLICABLE; edge=PROOF_OWED(crm-command and paige-ai-chat automated deployment/source match after merge); evidence=INT-140-focused-and-regression-suites
...
- Product-code head after the conflict-free current-main merge: 57f13c04b49af0819bddc21fa0ad0f527a7185a9.
- Failing-first: the original contract repair produced 4 failures / 12 passes on untouched product code; review follow-up separately reproduced malformed-name, absent-name-part, equivalent-retry and whitespace-equivalent retry failures before their corrections.
- Load-bearing mutations: restoring presence-only canonical-name handling makes the malformed-name regression fail; restoring the missing-counterpart condition makes the absent-name-part regression fail; removing PAIGE Chat's pre-hash/invocation canonicalization makes the equivalent-retry regression fail; removing canonical-name trimming makes the whitespace-equivalent retry regression fail; restoring the repairs returns all four green.
...
name parts must not discard a usable alias, and equivalent retry spellings, including canonical
names with surrounding whitespace, must not mint different fallback keys. The repair narrows only contact.create to the executor-owned
