...
FLOW_BY_FLOW: PASS: Flow-by-Flow (bug — behaviour/reachability on a shipped, owner-approved surface) applied — diagnosed the real height chain (CommandHub content div → tabpanel → `.gp`), identified the block-wrapper root cause, applied the minimal flex-column fix, then REPRODUCED the bug and PROVED the fix in Chromium (scripts/live-drive/game-plan-scroll-drive.mjs, 6/6 incl. the faithful Paige-dock case). Pre-edit frame + per-viewport measurement recorded in this PR.
...
BEHAVIORAL_EVIDENCE: PASS: new scripts/live-drive/game-plan-scroll-drive.mjs — 6/6 in Chromium at content-overflowing viewports. With the shipped block wrapper the bug reproduces (`.gp` overflows its container: 966px vs 680px desktop, 1534px vs 680px narrow, 1002px vs 680px dock → clipped, unreachable). With the flex-column fix `.gp` is bounded (== container height) and EVERY intended scroll region reaches its bottom with NO horizontal overflow (bodyOX=0, gpOX=0): desktop `.gp-rail` scrolls 286px; narrow `.gp-field` scrolls 516px; and the FAITHFUL Paige-dock-open case (1366px viewport + a 956px content column, so the real two-column path stays active — not the viewport-shrink approach that would cross the 1040px breakpoint) `.gp-rail` scrolls 321px, all regions reachable, no h-overflow.
...
KEYBOARD_FOCUS: UNVERIFIED: the fix does not touch any focus handler, tab order, or control (a layout wrapper only), so existing focus behaviour is unchanged — but neither browser drive exercises the keyboard (the scroll drive sets scrollTop directly; the render drive measures geometry), so tab-into-view of a below-the-fold control and visible-focus preservation are NOT proven here. Owed to a keyboard/browser-capable pass (Codex #980 P1). Keyboard navigation was not the reported bug (mouse scroll reachability), which IS proven.
ZOOM_REFLOW: UNVERIFIED: the fix bounds `.gp` so its per-width internal scroll activates, and reachability + zero horizontal overflow are confirmed at desktop (1440px), narrow (900px) and the faithful dock (1366px viewport / 956px content) columns — but a real 400%/320px browser zoom / enlarged-text pass was NOT run, and a width-only render is not equivalent to zoom (an enlarged operating brief could consume height); owed to a browser-capable pass (Codex #980 P1).
...
SOLO_1366X768_PAIGE_OPEN: PASS: proven FAITHFULLY by the scroll-drive dock case — a 1366px viewport with a 956px content column (so the real two-column path stays active; the render-drive's viewport-shrink to 956px would instead cross the 1040px breakpoint and test the wrong single-column scroller, Codex #980 P1): `.gp` bounded, `.gp-col`/`.gp-rail` scroll and reach their bottoms, bodyOX=0, gpOX=0 (no horizontal overflow).
...
UNVERIFIED: (1) the authenticated DEPLOYED scroll drive of the signed-in Solo surface with the real Paige dock — owed to a browser-capable session (§32.c); the scroll mechanism is proven headless in a faithful reproduction of the real shell chain (including the dock's two-column-at-956px geometry) and corroborated by the owner's live report. (2) KEYBOARD tab-into-view of a below-the-fold control + visible-focus preservation (Codex #980 P1) — the fix changes no focus handler, but the keyboard path was not driven. (3) A real 400%/320px browser zoom / enlarged-text pass (Codex #980 P1). Note: the Paige-dock-open geometry (1366px viewport / 956px content, two-column path) IS now faithfully verified by the scroll-drive — it is no longer owed.
...
- `scripts/live-drive/game-plan-scroll-drive.mjs` — the 6/6 bug-reproduction + fix proof (incl. the faithful Paige-dock case) (Chromium).
...
- Commands (repo root): `node scripts/live-drive/game-plan-scroll-drive.mjs` (6/6); `node scripts/live-drive/game-plan-render-drive.mjs` (128/128); `npm run build` (green); `npx vitest run` (green).
