...
AUTOMATED_EVIDENCE: PASS: scripts/ci/approval-direct-write-lint.mjs is FAIL-CLOSED — it flags every src/ from(the approvals inbox).update()/.upsert() whose payload is not statically provable approve-free (a single inline object literal, no spread, whose top-level status is absent or a decline string literal). Ships an 18-case --self-test: catches an inline approved literal, a ternary, a non-first-field approved, a variable payload, a variable/computed status, a shorthand status, a spread, an .upsert, and a backtick table+literal; ignores decline literals, an update that sets no status (a reassign), a different table's approved write, a read-then-different-table write (no cross-statement conflation), the canonical execute seam call, and comments; honors the exemption escape; reports the real line number — all 18 pass. Real scan GREEN across 1184 src files. Wired into the CI audit job as lint:approval-direct-write + :test.
STATIC_EVIDENCE: PASS: eslint clean on the changed files; the two deletions break no imports (verified — nothing in src outside the removed files references them); no migration and no edge bundle changed (git diff touches only src deletions, scripts/ci, package.json, .github/workflows/ci.yml, and docs/ — this evidence record plus two stale-doc corrections: docs/handoff/tenant-capability-recovery-matrix.md and docs/delivery/agent-control-capability-map.md no longer list the deleted ApprovalsInbox/ApprovalDetail as live/connected and now point at the mounted seam-routed surfaces).
...
UNVERIFIED: nothing for this change — it is fully proven headless (guard self-test 18/18 + green scan across 1184 files + no-import verification for the deletions), and it changes no runtime behavior so no runtime/authenticated proof is owed.

<!-- RELEASE_GOVERNANCE_POLICY — read docs/doctrine/release-governance-and-customer-update-policy.md -->

INTERNAL_BUILD_IDENTITY: 18a87ca4009e252fcddf4100d4e21a6eaa424d24; deployment=none-pre-merge; environment=development; migrations=NOT_APPLICABLE; edge=NOT_APPLICABLE; evidence=this-PR-and-scripts/ci/approval-direct-write-lint.mjs(18-case-self-test)
...
RELEASE_TRUTH_BOUNDARY: LIVE: the guard and the dead-code removal are fully proven headless (self-test 18/18, green scan across 1184 src files, no-import verification) and CI-enforced on merge; the change alters no runtime behavior, so there is no PARTIAL/PROOF-OWED runtime remainder
