...
AUTOMATED_EVIDENCE: PASS: vitest 31/31 across funding-coaching-gate.test.ts (both remediationLive states + the shipped-default unavailable + fail-closed matrix) and paige-capability-status-resolver.test.ts; integration-registry lint green (27 providers, 70 code_anchors resolve)
STATIC_EVIDENCE: PASS: ci:tsc (tsc-ratchet) reports zero new type errors from this diff — only the pre-existing #1186 signupMobile.ts baseline, which this diff does not touch; the registry JSON parses and lints clean
...
BEHAVIORAL_EVIDENCE: PASS: unit tests assert the refusal shape (ok:false, result:unavailable, diagnostic state preserved) that the server callers now key on; the two caller guards are simple outcome-reading conditionals confirmed by the edge deno-check in CI and the §39 diff read; the authenticated end-to-end drive is recorded under AUTHENTICATED_RUNTIME
...
UNVERIFIED: the authenticated live-drive of the refusal path across the two admin surfaces plus the skill-runner/paige-mcp non-success recording is owed to a browser-capable session — no isolated test tenant is available headless this session (§32.c); every other claim is proven by unit tests, ci:tsc, the registry lint, and the §39 diff review
...
INTERNAL_BUILD_IDENTITY: d620b1b787ce651abc4ead4fe46dd10ff7c23889; deployment=PR-preview; environment=preview; migrations=NOT_APPLICABLE; edge=PROOF_OWED(smartcredit-pull-snapshot + nav-pull-profile + business-verifier + skill-runner + paige-mcp production deploy via deploy-edge-functions on merge); evidence=vitest-31-of-31-and-ci-tsc-clean-of-this-diff
...
RELEASE_TRUTH_BOUNDARY: UNAVAILABLE: the Funding & Coaching Tools package is unseeded and every provider resolves unavailable today; the gate refusal and server-caller honesty are LIVE at the code level (the gate merged as 8c7fb131, this fix-forward at d620b1b7), while the authenticated live-drive is owed
RELEASE_RECOVERY: position=revert this fix-forward commit d620b1b7, which leaves the #1222 gate in place and returns behavior to the merged gate's setup_required messaging and the prior caller status-keying; reference=commit d620b1b787ce651abc4ead4fe46dd10ff7c23889
...
- Commit: d620b1b787ce651abc4ead4fe46dd10ff7c23889 (this fix-forward); gate merged as 8c7fb131 (deploy run #315).
- Automated proof: `npx vitest run src/__tests__/funding-coaching-gate.test.ts src/__tests__/paige-capability-status-resolver.test.ts` → 31/31; `npm run ci:tsc` → only the pre-existing #1186 signupMobile.ts error; `npm run lint:integration-registry` → 27 providers, 70 anchors resolve.
- Source: supabase/functions/_shared/funding-coaching-gate.ts (FUNDING_TOOLS_REMEDIATION_LIVE + decideFundingCoachingGate), supabase/functions/skill-runner/index.ts (verify_business_sos), supabase/functions/paige-mcp/index.ts (verify_business).
