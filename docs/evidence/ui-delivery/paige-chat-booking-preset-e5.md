...
MATERIAL_FLOW_CHANGE: YES: a genuinely new owner-visible capability flow (seven governed booking-calendar verbs from chat) with new consent-card and result-card copy — not a tweak to an existing flow.
...
INTERNAL_BUILD_IDENTITY: head of branch claude/calendar-paige-e5 (exact SHA stamped in the post-deploy closeout); deployment=CI on merge to main; environment=production; migrations=20270304000000_calendar_preset_tool_autonomy_catalogue.sql; edge=paige-ai-chat (+ _shared bundle); evidence=docs/evidence/proofs/booking-preset-chat-e5/ + this record.
