...
AUTOMATED_EVIDENCE: PASS: vitest — the Mind set is 64/64 (src/solo/mind-orb 49 across synapseForm §32 smoke + dustCap A1 form-floor, mindDomains data layer, and orbInteraction 20; plus SoloMindWorkspace behaviour 15 incl. ruling #2 headline). orbInteraction (20) proves the three #1303 pick/feed fixes — the focus-scaled pointer AND keyboard-Enter pick select the RENDERED node (and MISS at the pre-focus position, so the fix is load-bearing), reduced-motion fires NO flash, a feed arriving during the engine import is held not dropped — AND the round-3/4 cadence fixes: a simulated 30 FPS device reports ~30 fps and takes the density step-down while a 60 FPS device reports 60 and does not, the old CPU-time metric would have hidden the 30 FPS device, reduced-motion/idle-wake frames are never sampled, and a hide→show within 250 ms clears the cadence state so the hidden gap is not sampled. CORRECTION — the earlier "vitest 55/55 … orbInteraction (11)" line was STALE after the round-3/4 cadence work (the added 30/60-FPS, idle-wake, and hide→show tests); the current focused counts above are the real ones
