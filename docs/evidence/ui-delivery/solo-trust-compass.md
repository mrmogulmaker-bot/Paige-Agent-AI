...
AUTOMATED_EVIDENCE: PASS: full src/solo + src/lib/routing + src/components/tenant-shell suites green (1878 tests) at code head 05b23e6 (this record's doc-only update is the sole change on top) — governance derivation and ceiling-probe honesty (useSoloToolGovernance.test.ts, useSoloToolGovernance.ceiling.test.tsx incl. the write-serializer failure paths), risk-class drift guard (capabilityTools.test.ts), fabrication guard (compass.fabrications.test.ts), render + a11y incl. cancelled-drag and failed-write drain (compass.render.test.tsx), routing round-trip and counts (tierBranches.test.ts, TenantCommandCenterSubtabs.test.tsx).
STATIC_EVIDENCE: PASS: tsc ratchet clean (baseline 13, current 13); eslint reports 0 errors on the changed files; governance lints green (lint:action-risk 117 classified, lint:tool-catalogue 0 governed-invisible, lint:tier-features); production build green.
...
STATE_COVERAGE: PASS: the governed instrument covers loading (skeleton), not-configured and read-error (honest failure state plus retry), non-admin READ-ONLY (a read-only note and static posture pills, never an active slider that would only fail on the server — is_current_user_tenant_admin gates settable), and configured/editable; the pending and recorded sections each carry loading, ready, error, and empty; a workspace switch re-keys every read through accountEpoch and each write carries that tenant so a stale cross-workspace write is server-rejected; asserted across compass.render.test.tsx, useSoloToolGovernance.ceiling.test.tsx, and the activity-feed tests.
...
- Commands: npx vitest run src/solo src/lib/routing src/components/tenant-shell (1878 passed); node scripts/ci/tsc-ratchet.mjs (baseline 13, current 13); npm run lint:action-risk (117 classified) / lint:tool-catalogue (0 governed-invisible) / lint:tier-features (green); eslint on the changed files (0 errors); production build green.
...
- Head: code at 05b23e6 on branch claude/modest-curie-zip2i5 (merge of main through #981 + Codex rounds 2–4); this evidence-record update is the only change committed on top of it.
- Redacted: no secrets or customer-sensitive data included; no real tenant identifiers.

## Review and limitations

Independent review this session (§5/§39, Codex + adversarial verifier): findings across four rounds were addressed against the real diff with fixes and new tests before this record — ceiling-probe honesty and tenant binding, held-back attribution, knob value clamping, post-save flicker, full write-authority predicate (platform-owner act-as) and honest unproven-authority, per-tool write serialization across the domain and child knobs, cancelled-drag discard, removal of the unsupported "set it in chat" claim (§70.1), and the write-serializer failure paths (never strand or falsely-succeed the user's last, more-restrictive choice). Remaining limitations are the UNVERIFIED items above — real-browser rendered evidence at the eight Solo viewports, a real-browser behavioral drive, zoom and reflow, and authenticated-runtime persistence — all owed to the owner's live review because this session is headless. No claim of working authenticated behavior is made from rendering, fixtures, mocks, or static tests.
