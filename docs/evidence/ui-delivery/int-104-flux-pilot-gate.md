...
AUTOMATED_EVIDENCE: PASS: scripts/voice-stt-smoke.mts covers mandatory MIP opt-out and the Flux parser with a fake socket; scripts/paige-live-ticket-smoke.mts covers strict boolean availability, direct and agency standing checks and both pre-adapter server gates; the existing Live stage test fails first when a minimized user's workspace loses availability and now passes with UNAVAILABLE, no reconnect, and no microphone; the Live client test preserves the control-plane revocation response; zero network and provider calls
