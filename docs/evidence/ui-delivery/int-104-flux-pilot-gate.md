...
AUTOMATED_EVIDENCE: PASS: scripts/voice-stt-smoke.mts covers mandatory MIP opt-out and the Flux parser with a fake socket; scripts/paige-live-ticket-smoke.mts covers strict boolean availability, active-user membership and both pre-adapter server gates; the existing Live stage test fails first when a minimized user's workspace loses availability and now passes with UNAVAILABLE, no reconnect, and no microphone; the Live client test preserves the control-plane revocation response; zero network and provider calls
...
INTERNAL_BUILD_IDENTITY: code=706ebfdb6be246a9d56f14fb8aa3011f56365ebf; deployment=PROOF_OWED; environment=development; migrations=PROOF_OWED(20270408000000_paige_live_pilot_feature_guard-after-merge); edge=PROOF_OWED(paige-dictate-paige-stt-paige-live-session-paige-live-relay-after-merge); evidence=scripts/voice-stt-smoke.mts-and-scripts/paige-live-ticket-smoke.mts
