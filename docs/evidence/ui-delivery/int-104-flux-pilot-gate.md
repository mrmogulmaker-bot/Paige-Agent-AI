...
AUTOMATED_EVIDENCE: PASS: scripts/voice-stt-smoke.mts covers mandatory MIP opt-out and the Flux parser with a fake socket; scripts/paige-live-ticket-smoke.mts covers strict boolean availability, direct and agency standing, operator exit, first-member fallback, and both pre-adapter server gates; the existing Live stage test fails first when a minimized user's workspace loses availability and now passes with UNAVAILABLE, no reconnect, and no microphone; the Live client test preserves the control-plane revocation response; zero network and provider calls
...
STATE_COVERAGE: PASS: missing, malformed and false platform availability values fail closed in the local test; the server refuses ticket issuance and relay upgrade without an enabled workspace row; failed renewal replaces stale PARTIAL with UNAVAILABLE and stops playback/relay, while text chat remains available
...
PROTECTED_SEAMS: affected — service-only workspace availability, server-resolved active membership, caller-owned thread, one-use ticket, WSS pre-upgrade gate, existing Live-stage renewal/unavailable display, Deepgram MIP opt-out, registry truth; unchanged — chat runtime, governed approvals, billing, dictation capture and read-aloud playback.

INTERNAL_BUILD_IDENTITY: code=PROOF_OWED(squash-merge-commit-after-PR-1344); pr=1344; reviewed_head=PROOF_OWED(final-exact-head-review); supporting_branch_history=706ebfdb6be246a9d56f14fb8aa3011f56365ebf; deployment=PROOF_OWED; environment=development; migrations=PROOF_OWED(20270408000000_paige_live_pilot_feature_guard-after-merge); edge=PROOF_OWED(paige-dictate-paige-stt-paige-live-session-paige-live-relay-after-merge); evidence=scripts/voice-stt-smoke.mts-and-scripts/paige-live-ticket-smoke.mts
...
The pre-squash branch commit above is supporting history only, never the release build identity. At squash merge, the exact merge commit and final reviewed PR head must be recorded in the post-merge closeout; migration/deployment identity is also owed then. The workspace holder set is operational database data and is never recorded in this artifact. The exact existing-system inventory is `docs/delivery/paige-live-existing-system-inventory.md`. Presence and mute follow-up #1343 is a pre-activation requirement, not a claimed pass here.
