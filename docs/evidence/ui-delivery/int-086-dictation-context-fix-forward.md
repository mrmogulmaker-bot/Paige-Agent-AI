...
AUTOMATED_EVIDENCE: PASS: Failing-first produced 6 failures on the merged implementation. The requested-review regression and first Ready-review draft-boundary regression each failed 1/5 before repair. The repeat Ready-review Live Retry regression failed 1/16 before repair (15 passed). Round 3 failed 1/32 on current code because PaigeAIChat still keyed the mic; after repair the focused suite passes 32/32 and the final affected AppShell, PAIGE engine, Conversations, MessageComposer, dictation, and Live Conversation selection passes 107/107.
...
BEHAVIORAL_EVIDENCE: PASS: The real useDictation hook is driven with controlled recorder/WebSocket/timer seams. Tests prove all three mic mounts remain mounted, the render-time scope fence rejects stale final, interim, and error callbacks that arrive before passive cleanup, a scope change stops active recording and closes its socket, visible silence/error outcomes remain available, Send stays held through finalization, and disabled Live Retry cannot tear down the open session.
...
STATE_COVERAGE: PASS: Idle, requesting, connecting, listening, user stop, transcribing, clean final, permission denied, provider failure, silence stop, hard maximum, route/user/tenant/thread/edit-draft switch, draft cancellation, unmount, and send hold are covered.
...
ACCEPTANCE_CRITERIA: Start dictation in each composer; verify Send and alternate dispatch controls are held through requesting, listening, and transcribing; stop and receive the trailing final; verify Send then includes that final and alternate-action state remains intact. During capture, change the applicable route/user/tenant/thread/edit-draft identity and verify recorder/socket close, no stale segment inserts, and the new context is clean. In Conversations, cancel an edited draft during capture and verify no late segment enters the cleared reply composer. Trigger a safety stop or permission/provider failure and verify a visible reason.
...
PROTECTED_SEAMS: Tested: DictationMicButton, useDictation epoch teardown, AppShell PaigeChat route/user scope, PaigeAIChat tenant/client/mission/user/thread scope, Conversations tenant/thread/edit-draft scope, MessageComposer send gate, and Live Conversation disabled propagation for an already-open card. Unaffected: paige-dictate, stt-router, paige-tts, paige-live-session, paige-ai-chat Edge runtime, Live audio/session transport, storage, database, and migrations.

INTERNAL_BUILD_IDENTITY: 5e345ed217d62f60c56d3ef8c6cfe5960a7aa488; deployment=none-pre-merge; environment=local; migrations=NOT_APPLICABLE; edge=NOT_APPLICABLE; evidence=focused-tests+affected-regressions+mutation-proof+tsc-ratchet+build+impeccable; current-main=c5d396ebe3f55ae87c6d52268b939683a4e4fe82; current-main-integration=c99582f76ebdfe0a6a4f5c1caacc5ec645d73648
...
- Classification: bounded fix-forward for the three accepted #1290 review findings plus the requested and Ready-triggered exact-head review findings inside the same context/finalization invariant.
...
- Neighboring regressions: attachment-only send, response cancellation, quick/slash actions, confirmation cards, retry, an already-open Live Conversation card, thread and edited-draft selection/cancellation, Conversations send validation, caret insertion, and microphone stop control.
- Collision result: current main c5d396ebe3f55ae87c6d52268b939683a4e4fe82 merged without conflict at c99582f76ebdfe0a6a4f5c1caacc5ec645d73648. Active PRs #1300, #1303, and #1304 do not overlap the six round-3 runtime/test files; #1303 shares decision-log ownership only if a review finding must be deferred.
...
- Exact Ready-review disposition code commit: e13d77d35c88ec1884465551cf14f78b19748b3e.
- Exact repeat-Ready-review disposition code commit: b086f218cdf7d08b3327a4792efab293bf29cf6c.
- Exact round-3 code commit: 5e345ed217d62f60c56d3ef8c6cfe5960a7aa488.
- Current-main: c5d396ebe3f55ae87c6d52268b939683a4e4fe82; conflict-free integration commit: c99582f76ebdfe0a6a4f5c1caacc5ec645d73648.
- Failing-first: 6 failed and 22 passed on the merged implementation.
- Review-round failing-first: Live Retry while disabled failed 1/16 before repair (15 passed); the failure was the enabled Retry control.
- Round-3 failing-first: the focused selection failed 1/32 and named the keyed PaigeAIChat mic; 31 tests passed. Repaired focus: 32/32; final affected regression selection: 107/107.
- Load-bearing mutations: removing teardownRun from the scope-change effect made cleans a pending recorder and opens no socket after an account epoch change fail (expected one recorder stop, received zero). Removing the inherited disabled state from an already-open Live Conversation card made its behavioral test fail (expected disabled true, received false). Removing the render-time `scopeEpochRef.current = scopeEpoch` fence made the new delivery-race test fail because `onText` received `stale final`; restoring it returned the test green. All restorations are present in the recorded build identity.
- Static gates: tsc ratchet baseline 12/current 12; production build 5,707 modules; Impeccable detector [].

## Review and limitations

The single requested Codex review completed on 994b26a75e683efae3f48393802033f397747352 and found one valid P2: alternate dispatch controls could silently no-op while dictation finalized. That finding is dispositioned in 3af43e4c1898fc7a62d7e16306b51b645055e4f7 with failing-first, behavioral, and mutation proof. The first automatic Ready review completed on 81ad7865c19d8f29ecd4a96f904c1ac5a6ffdb46 and found one valid P2: cancelling an edited Conversations draft did not advance the dictation epoch. That finding is dispositioned in e13d77d35c88ec1884465551cf14f78b19748b3e with failing-first proof. The repeat automatic Ready review completed on 0f969f54ebcebbd0f16428b92f66a072ada7193d and found two items. Its P2 Live Retry finding is fixed in b086f218cdf7d08b3327a4792efab293bf29cf6c: the control is disabled and its handler refuses before any teardown, with failing-first proof. Its P1 thread-switch draft finding is dispositioned without a code change: main already leaves the shared input state unchanged in both selectThread and startNewChat, so it is pre-existing; applies equally to typed input; text is visible before send; tracked as INT-102 (per-thread composer drafts), owner decision pending. The coordinator-authorized final round removes remount keys and adds render-time provider-delivery fencing plus teardown proof on all three mounts. The Ready-triggered exact-head review completed on `042c3e65a319be53d337eea192b36b20174d0504` with one P2: Conversations' pre-existing Draft with Paige action is not held through dictation finalization. It is not worse than current main, is recorded under `DICTATION FOLLOW-UPS` in the decision log, and is deferred without a fourth repair round under the terminal rule. Production deployment and authenticated acceptance remain unverified.
