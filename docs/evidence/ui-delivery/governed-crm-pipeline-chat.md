...
DATABASE_RLS_EVIDENCE: PARTIAL: PAIGE Spine `database-contract` passed on exact candidate `92b39e1e` with a fresh Supabase reset and the established SQL suite. The new `supabase/tests/governed_crm_commands.sql` 34-assertion pgTAP file is invoked immediately after fresh database reset in the exact-head Spine workflow; its run status is recorded on PR #1234. Supabase Preview was cancelled at the concurrent preview-branch limit, so preview-branch proof remains PROOF OWED.
