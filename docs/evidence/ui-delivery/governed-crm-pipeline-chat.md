...
STATIC_EVIDENCE: PARTIAL: action-risk catalogue, registry, migration/definer guards, TypeScript ratchet, Edge bundle checks, and production build passed on the source candidate. Exact final-head Deno/CI reruns are required because review found and repaired Edge diagnostics.
DATABASE_RLS_EVIDENCE: PARTIAL: PAIGE Spine `database-contract` passed on exact candidate `92b39e1e` with a fresh Supabase reset and the established SQL suite. The new `supabase/tests/governed_crm_commands.sql` 30-assertion pgTAP file is not invoked by that workflow and Supabase Preview was cancelled at the concurrent preview-branch limit; its execution remains PROOF OWED.
...
Independent §39 and security review run on the real pushed PR diff. Initial findings: restore paid credit-pull high-risk identities, stabilize omitted CRM idempotency keys across unknown-outcome retries, show consequential approval parameters, stop labeling company IDs as exact person routes, add this UI evidence record, and fix new `crm-command` Deno diagnostics. Each diff-owned finding is repaired in focused follow-up commits and rerun against final-head CI/review. Unrelated mainline ratchet failures and the PR conflict with newer `main` are not rewritten into this candidate. Remaining proof gaps are stated above and must not be promoted to LIVE.
