...
RELEASE_CHANNEL: production: OWNER APPROVED 2026-09-21 (comps reviewed; "this is the one I like") — merge to main under INT-083 after exact-head Codex review. SHA is the code commit carrying the layout + the two additions (contrast fix + legend reposition); deployment id reported to the coordinator once Vercel builds the production deploy (a commit cannot contain its own deployment id). Pre-launch §4: verified + non-destructive → merge, owner reviews live on main.
...
RELEASE_RECOVERY: position=merge-bound (owner approved); recovery is a single revert of the squash-merge commit — the change is frontend-only (one .tsx re-parent from the first commit + one .css file), zero migrations/edge/data, so a revert fully restores the prior Mind layout; reference=https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1314
