...
AUTOMATED_EVIDENCE: PASS: EXIT CODE 0 across src/solo/sales-ops.contract.test.tsx, src/hooks/useAnalytics.redaction.test.ts and src/hooks/useAnalytics.wiring.test.ts — 151 tests, 14 files. Guards proven NON-VACUOUS by restoring the exact defect each exists to catch: the false "terms are now active" claim fails 1 of 90; hiding truncation fails 1; dropping the contact id fails 1; the case-sensitive prefix fails 2; the un-redacted referrer fails 2; un-redacted page_path fails 2; the affiliate-readable landing path fails 1. Restored: 151 pass. CORRECTED — this field previously claimed "removing the path redaction fails 2 of 3", which was true only of the pure redaction FUNCTION. Stripping all three redactors out of the posted PAYLOAD left every test green at EXIT 0, so that guard was vacuous with respect to the wiring it was cited to prove. The wiring test replaces it and reads the posted body directly. The routing guard asserts the real destination through a router LocationProbe rather than a spied callback, because a spy would prove only that a handler fired, not that it routed anywhere.
...
INTERNAL_BUILD_IDENTITY: 59af02321eca591c8942c03d9f713ce81e2be4b3; deployment=none-pre-merge; environment=development; migrations=NOT_APPLICABLE; edge=NOT_APPLICABLE; evidence=this-PR+src/solo/sales-ops.contract.test.tsx+src/hooks/useAnalytics.redaction.test.ts+src/hooks/useAnalytics.wiring.test.ts(151-tests-EXIT-0)+scripts/live-drive/sales-ops-drive.mjs(536/536-EXIT-0)+nine-defect-restoration-proofs. SUPERSEDES fdcb4daea4c74cbccdf85e769af4a4f9c8f15ff4, which this record previously named and which is now known to contain two P1 credential-disclosure defects and a vacuous guard; it is recorded here as the superseded revision rather than deleted. The closeout commit that carries this line is documentation-only and changes no shipped code, so 59af0232 remains the exact implementation identity of the delivered behaviour.
