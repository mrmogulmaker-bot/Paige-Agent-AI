...
INTERNAL_BUILD_IDENTITY: 2ef98a7c8d4ce1f484db324bf306431aaace9c70; deployment=none-pre-merge; environment=development; migrations=PROOF_OWED(persisted-apply-on-merge-via-deploy-migrations-db-live); edge=NOT_APPLICABLE; evidence=fix-forward-PR-and-supabase/tests/paige_orchestration_companion_minting.sql
...
- `supabase/tests/paige_orchestration_companion_minting.sql` → 49/49 assertions, proven locally against PostgreSQL 16 with both migrations applied in order (roles seeded, zero apply errors), and wired into the required `database-contract` CI job.
