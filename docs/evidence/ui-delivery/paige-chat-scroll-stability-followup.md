...
AUTOMATED_EVIDENCE: PASS: 25 focused controller/React tests and 97 affected chat/mount tests cover one-pixel wheel, touch inertia, pointer/scrollbar, multi-event and Tab-focus keyboard scrolling, focus-leaving keyup, stream, resize, history, asynchronous thread transition, hidden geometry, client-to-server message-ID reconciliation, and duplicate-content reload after an ephemeral greeting disappears
STATIC_EVIDENCE: PASS: ratcheted types, scoped lint, security, production build, affected suite, and full 278-file / 3935-test regression suite pass locally
...
KEYBOARD_FOCUS: PASS: Arrow/Page/Home/End intent is distinguished from non-scroll keys; focus-leaving Tab keyup releases ownership; rendered PageDown/PageUp/Home/End and composer return remain operable
...
INTERNAL_BUILD_IDENTITY: ba9f97f6b545c1f871c5262e651bdc4d5486f5ff; deployment=local-build; environment=development; migrations=NOT_APPLICABLE; edge=NOT_APPLICABLE; evidence=scripts/live-drive/artifacts/paige-scroll-stability-react/report.json
...
- Focused automated result: 97/97 affected controller, React, Solo workspace, normal chat, and AppShell mount checks, including delayed thread hydration, duplicate-content reload, live duplicate-index refresh, focus-leaving Tab keyup, and replacement of every client message ID during React rehydration; full repository result is 278/278 files and 3935/3935 tests.
