...
INTERNAL_BUILD_IDENTITY: eff296e0d5bceb49f352c0d2a50c522bf801f66c; deployment=NOT_DEPLOYED; environment=local; migrations=PROOF_OWED(exact_head_ci_and_non_production_connection); edge=PROOF_OWED(no_approved_non-production_provider_deployment); evidence=docs/evidence/ui-delivery/solo-beta-acquisition.md
...
Initial base: `299c39b2bc6c10eb0440ce415a626fb0d534f5ca` from fresh `origin/main` on 2026-09-12. Final reconciliation base: `aec940df59d207ec8c4809ceef6b44034665259d`, which contains PR #1159's replay bridge at `b17599cd`; the branch was rebased after that concrete merge, preserving both additive Master/Brain histories. Verified code head before this evidence-only closeout: `eff296e0d5bceb49f352c0d2a50c522bf801f66c`.
...
- **PR #1148 database-contract — PROOF OWED on the exact pushed head:** current main now contains PR #1159's narrow replay bridge (`b17599cd`), and this branch is rebased onto the containing main head `aec940df`. Local Docker is unavailable, so fresh replay and the Solo pgTAP files must be proven by the repository's `database-contract` check after the evidence closeout is pushed; no local database execution is claimed.
