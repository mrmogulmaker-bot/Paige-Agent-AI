...
AUTOMATED_EVIDENCE: PASS: Exact-rebased-head focused Vitest 52/52 and `ci:regression` passed; the whole-repository run reached 310/315 files and 4332/4352 tests, with 20 failures confined to untouched Resend receipt, Settings connection, Business Game Plan, Spline, and Solo-shell contract tests and disclosed separately below.
STATIC_EVIDENCE: PASS: Local production build for code head `eff296e0d5bceb49f352c0d2a50c522bf801f66c`, 12/12 TypeScript ratchet, changed-source ESLint, migration-version, definer, managed-schema, tier, Binding Ledger, Integration Registry, release-governance, shadowed-credential, user-facing-admin-URL, and diff-integrity checks passed. Raw repository `tsc` still reports the 12 recorded current-main baseline errors; Deno and Docker execution are unavailable on this host and are not claimed.
RENDERED_EVIDENCE: PASS: The real exact-head local application passed 152/152 browser checks across Home, Pricing, Auth, retired-route recovery, and checkout-return recovery at all four required viewports in both themes. The run clicked the actual JavaScript CTAs, found no horizontal overflow or uncaught page errors, proved the primary signup action's bottom edge fits inside every initial viewport, exposed visible keyboard focus, allowed zoom, and stopped decorative animation under reduced motion. Generated screenshots were manually inspected for hierarchy, readability, CTA reachability, clipping, and theme contrast; the machine report remains under the existing gitignored live-drive artifact home.
...
INTERNAL_BUILD_IDENTITY: eff296e0d5bceb49f352c0d2a50c522bf801f66c; deployment=NOT_DEPLOYED; environment=local; migrations=APPLIED(20270118010000_solo_beta_atomic_fulfillment,20270118020000_solo_beta_subscription_lifecycle,20270118030000_stripe_event_log_privacy,20270118040000_solo_beta_integrity_fencing,20270119010000_solo_beta_authz_hardening); edge=PROOF_OWED(no_approved_non-production-provider-deployment); evidence=docs/evidence/ui-delivery/solo-beta-acquisition.md
...
Initial base: `299c39b2bc6c10eb0440ce415a626fb0d534f5ca` from fresh `origin/main` on 2026-09-12. Final reconciliation base: `a451278e0d82826a3bf7e6c3c79a74739c557430`, which contains PR #1159's replay bridge at `b17599cd`; the branch was rebased after that concrete merge and again through PRs #1163, #1166, #1164, #1167, #1169, and #1171, preserving additive Master/Brain histories. The production build was verified on code head `eff296e0d5bceb49f352c0d2a50c522bf801f66c`; the fresh migration replay and both Solo pgTAP contracts were verified by GitHub run `34723473934` on exact pre-rebase head `708b6a96454e9337858fd01b4afe7f92bdd4b5a8` and are re-gated on this rebased head.
...
- **Billing (`VERIFIED` automated/static; Stripe test-mode acceptance `PROOF OWED`):** fixed offer code `paige-solo-beta-monthly-v1`, `$74.50`, monthly, USD, one 30-day trial. Checkout forces payment-method collection and uses a stable provider idempotency slot across stale lease recovery. Fulfillment requires a signed exact-duration `trialing` or `active` subscription and exact current agreement; prior subscription history and a provider-created trial permanently close the new-checkout path. After fulfillment, status verifies receipt + immutable subscription + membership before acquisition-agreement checks, so agreement rotation cannot lock out earned access. Lifecycle handling binds persisted product/price/customer/provider/trial facts, accepts granting `trialing/active` and revoking `past_due/canceled/unpaid/paused` states, orders by provider `event.created`, and records one event receipt/evidence.
...
- **Accessibility/rendered (`VERIFIED` structural, interaction, geometry, and manual visual review):** the real exact-head local app passed 152/152 browser checks over Home, Pricing, Auth, legacy recovery, and checkout recovery at `1536×770`, `1366×768`, `1024×768`, and `900×1000` in both supported themes. There was no horizontal overflow or uncaught browser error, actual CTAs reached the canonical flow, every signup primary action fit inside its initial viewport, keyboard focus was visible, zoom remained enabled, and reduced-motion mode had no running decorative animations. Screenshots were manually inspected; authenticated and provider-backed rendering remain `PROOF OWED`.
...
- **Automated — VERIFIED for new scope:** focused Vitest `52/52`; `ci:regression` passes. **Whole repository — PARTIAL:** exact-head Vitest `310/315` files and `4332/4352` tests, with 20 failures in untouched Resend receipt, Settings connection, Business Game Plan, Spline, and Solo-shell contract tests. Those failures are not represented as green.
- **Static/security — VERIFIED for new scope:** production build; TypeScript CI ratchet `12 baseline / 12 current`; changed-source ESLint; migration version `1021` with no reuse; definer, managed-schema, tier-feature, Binding Ledger, Integration Registry, release-governance, shadowed-credential, user-facing-admin-URL, and diff-integrity checks. **Raw repository checks — inherited failing/unavailable:** `tsc` has the 12 ratcheted current-main errors; view security fails on the current-main `paige_unclassified_inbound` migration; Deno and Docker are not installed, so their execution is `UNAVAILABLE`, not passed.
- **Rendered — VERIFIED for the signed-out local flow:** 152/152 checks at four viewports × two themes across actual Home, Pricing, Auth, legacy recovery, and checkout recovery. Screenshots and JSON were produced under `scripts/live-drive/artifacts/solo-beta-acquisition/` and remain gitignored. Manual screenshot inspection passed for hierarchy, readability, contrast, clipping, and primary-action reachability.
...
- **Database/migration — VERIFIED fresh replay; incremental catalog PROOF OWED:** GitHub database-contract run `34722573541` on head `243ceb8163e1687fe8ea34e65175ee52d8b17d50` completed `supabase db reset`, reached every Solo migration, and passed `solo_beta_authz_hardening.sql` (17/17) plus `solo_beta_offer_boundary.sql` (20/20), followed by every remaining database-contract step. No approved non-production Supabase connection was present, so incremental convergence against the approved non-production catalog and provider-backed provisioning remain `PROOF OWED`; production was not queried or mutated.
- **PR #1148 database-contract — VERIFIED on the implementation head:** current main contains PR #1159's narrow replay bridge (`b17599cd`), and this branch is rebased onto containing main head `a451278e0d82826a3bf7e6c3c79a74739c557430`. The repository check explicitly executes both Solo pgTAP files after a zero-state replay; local Docker remains unavailable and is not claimed.
