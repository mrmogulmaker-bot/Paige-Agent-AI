...
| MED-2 | **Anon host-email PII leak — FIXED in BOTH guest-facing copies.** The repo has exactly TWO `resolveHostNames` display helpers — `public-booking/index.ts` (anon availability + guest confirmation) and `booking-manage/index.ts` (the external manage-link holder's booking-details response + reschedule/cancel emails). Both now use `full_name` ONLY and OMIT a nameless host; the per-host `auth.admin.getUserById` + email fallback is removed from each. The legitimate host-notification / recipient paths (`hostInfos` in public-booking, `resolveHostEmails` in booking-manage — host emails as SEND recipients, staff-facing) are untouched. **The final §39 peer-gate caught that my first commit fixed only the public-booking copy and overclaimed the leak "structurally impossible" repo-wide — this row is the corrected, complete fix.** | Structural removal in BOTH copies + build/inspection. Repo-wide grep confirms only those two `resolveHostNames` definitions exist and both are now names-only (the other `getUserById`+email edge functions are the send-recipient pattern, not display leaks). **§18 follow-up (named):** the two byte-identical copies should be unified into one shared `_shared` helper — two copies are what let the first fix miss one. **OWED:** a dedicated resolver Deno invariant test (item C below) + authenticated runtime (§32.c). |
...
|---|---|---|---|
| MED-2 | MED | Anon availability response leaked host auth emails (`resolveHostNames` fallback). Pre-launch blast radius ≈ 0 (no external users; hosts = owner's own team). A byte-identical second copy in `booking-manage` leaked the same to an external manage-link holder. | **FIXED in this draft in BOTH copies** (§1) — the final §39 peer-gate caught that my first commit fixed only `public-booking`; the `booking-manage` copy is now fixed too. §18 follow-up: unify to one shared helper. |
