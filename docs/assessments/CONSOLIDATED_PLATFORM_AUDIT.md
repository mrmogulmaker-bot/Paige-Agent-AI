...
- **PLATFORM_ASSESSMENT B1 (public signup → global admin → super_admin in ~3 calls).** Still open. Same class as #149 but on the auth/roles seam, not the orchestrator seam. Load-bearing.
- **PLATFORM_ASSESSMENT B2 (~20 tenant-scoped tables cross-tenant readable/writable via `OR has_role(admin)` bypass).** Still open.
- **PLATFORM_ASSESSMENT B3 (21 edge functions authorize with global admin then act via service-role client).** Still open. Same class as #149.
