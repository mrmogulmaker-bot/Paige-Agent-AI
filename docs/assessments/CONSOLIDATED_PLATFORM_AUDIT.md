...
| `tenant_staff_owns_user` | `(_actor uuid, _user uuid) → bool` | `_user → clients.linked_user_id → clients.tenant_id` | a **user_id / client_id that is a user id** (the consumer's auth uid) | F1 (9 banking/QB/funding tables), F2 (11 credit/build tables), F3 (planned) |
| `current_user_tenant_id()` + `is_platform_owner()` | `() → uuid` / `() → bool` | agency-aware active-tenant of `auth.uid()` | a **tenant_id column directly** | Slice 1 (client_notes/files/growth_form_submissions/paige_subagents), F4 (planned) |
