...
- Four further defects found while verifying this work are filed and parked, separate and not underway: #733, #734, #735, #736, plus #744, #752 and #753. None is authorized as implementation work; only the owner may convert a parked issue into an assignment.
- Truth status: at the branch head, the full vitest suite (154 files / 1865 tests), the apply-extraction driver (78 checks), `deno check`, the TypeScript ratchet, the production build and every Spine registry, approval-guard, database-contract and lint gate are green, and a real Chromium drives the scope switch and the Skip failure-then-retry flows 13/13, with each new assertion proven to fail against the pre-fix source. A preview deployment exists; a production deployment does not. Authenticated production behaviour and real-RLS proof remain UNVERIFIED. #729 is not shipped, not live and not production-verified; it must not be merged or deployed while #746 is open.
