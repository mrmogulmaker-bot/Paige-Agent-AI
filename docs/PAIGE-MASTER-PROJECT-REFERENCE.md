...
#### Production-version and duplication audit — 2026-09-08 00:47 EDT

| Category | Verified finding | Disposition |
|---|---|---|
| Stale deployed version | **None found after remediation and current-main refresh.** `paigeagent.ai/version.json` and `app.paigeagent.ai/version.json` both return build `ec2336f70ca1016898e4bc91dd7f2efda3816a02-mts6s2mo` with `customerUpdate: null`. Vercel production deployment `dpl_FqW7er4bgB5cpoEnwrqNmUqd2bRn` is `READY` from exact canonical `main` SHA `ec2336f70ca1016898e4bc91dd7f2efda3816a02` and aliases both domains. Durable browser drives on its direct product-code parent at 1366x768 and 900x1000 proved `/premium` and `/legacy` replace-redirect to `/`, Back returns to `/about`, keyboard reaches the Paige Agent link, and 200 percent reflow has no horizontal overflow; #1055 changed only documentation/evidence artifacts. | Current production and canonical `main` match. The #1054 route correction and #1053 Live Conversation delivery remain in this documentation-only descendant's ancestry. No manual promotion or rollback was needed. Public `track-event` requests emitted CORS/401 console errors on both domains; route behavior was unaffected, but telemetry ownership requires separate diagnosis. |
