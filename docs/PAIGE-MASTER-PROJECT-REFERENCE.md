...
**2026-09-19 Connected MCP Gateway — MCP PR-1: canonical connection single-source, server/library only (PR off fresh `main` `27488aa8`; classification **internal-only** (security); small-PR train — one invariant; DELIVERED to a DRAFT for coordinator merge, NOT merged, PR-2 NOT started; runner still imported by no deployed function):** Closes the #1268 forward-risk that the runner verified consent by `connection_id` but dispatched to a caller-supplied `serverUrl`. `RunnerRequest` now carries only `connectionId` + the caller's server-derived `tenantId`; `deps.loadConnection` (production `makeRpcConnectionLoader` over the service-role `get_mcp_connection_secret`, new `_shared/mcp-gateway/connection.ts`, reusing `authFromSecret` §18) resolves the ONE canonical row, and BOTH dispatch (`canon.serverUrl`/`auth`) and consent (`verify_mcp_connection_approval` by the same id) derive from it. §9: tenant-agnostic loader ⇒ runner refuses `foreign_tenant`/`no_connection`/`connection_disabled`/`connection_unusable`, fails closed with no loader, and (§39 defense-in-depth) refuses `connection_mismatch` if the loader returns a row for a different `connection_id` — the invariant is loader-independent. `prepare` validates the connection (existence + id + tenant) before affirming `prepared` (Codex R1 P2 fixed); both identity comparisons are case-insensitive so an uppercase-hex UUID is not a false mismatch (Codex R2 P2 fixed); and a listed connection the client cannot execute — a non-`http` transport (`sse`/`stdio`) or the n8n REST `api_key` facet — is refused `connection_unusable` before `ok:true` via a transport+auth-kind allow-list, so prepare cannot affirm and execute cannot contact it (Codex R3 P2 fixed; the RPC already returns transport+auth_kind, so no migration); an expired-OAuth access token and a reserved/invalid custom-header name are likewise refused (Codex R4 P2-A/P2-B fixed; §18 shared `authUsable` in `mcp-client.ts`; no refresh/rotate write — deferred; the expiry refusal is scoped to `auth_kind='oauth'` so a switched-off-oauth row with a stale timestamp is not falsely refused — Codex R5 P2 fixed). No migration, no Chat, no endpoint-setter, no live provider. `smoke:mcp-gateway` **120 assertions** (single-source proofs incl. a caller-smuggled URL ignored + rogue endpoint never contacted + id-mismatch refused + prepare-validates; the executable-facet gate refusing api_key/stdio/sse, reserved (incl. transport-owned `Mcp-Session-Id`)/invalid headers, and expired-OAuth at loader AND runner level with zero dispatch via the real `makeRpcConnectionLoader`; consent and dispatch key off the same canonical id; load-bearing mutations). §39 peer-gate + Codex R1–R5: NO BLOCKER/MAJOR; NIT → `connection_mismatch` guard; R1/R2/R3/R4/R5 P2 → all fixed. **RETURNED to coordinator for sequencing: Codex R1 P1** — bind consent to the LOADED endpoint VERSION (in-flight-repoint TOCTOU), needs migrations to `get_mcp_connection_secret` + `verify_mcp_connection_approval`, beyond this bounded PR + the PR-1 proof list; runner unwired. **Coordinator ruled (2026-09-19): both belong in MCP PR-1B (migration-backed canonical connection/consent contract closure), AFTER #1278 and BEFORE PR-2 — NOT PR-2.** R1 P1 = **INT-078** (endpoint-version binding). **INT-079 (Codex R6 P2)** — `get_mcp_connection_secret` refuses `auth_kind='url'` (Zapier) rows as `configured:false` (only `none` is token-exempt), so a url-auth row never reaches the loader's `url` allow-list; fix is an RPC migration (PR-1 is no-migration), false-refuse, not exploitable today. PR-1B NOT started; PR-2 NOT started. `authFromSecret` `auth_kind='none'` MINOR + refusal-oracle NIT are deployed-door follow-ups. `ci/verify` inherited-red only (#1208 + #1255). §37: sole producer of the old request shape was the smoke.

**2026-09-19 Connected MCP Gateway — Phase C hard-entry safeguards, server-only (DB + runner library) (PR [#1268](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1268) MERGED to `main` as `27488aa8`; classification **internal-only** (security / execution-preparation); SHIPPED — owner authorized the merge after Codex R5 clean + exact-head CI; `deploy-migrations` #279 persisted `20270323000000`, prod caught up, `db-live = 27488aa8`):**
The server-only safeguards the MCP runner MUST pass before it is ever wired live — the DB + library half of the #1262 Phase C entry gate. Additive, non-destructive; wires **NO** live provider, **NO** Chat, **NO** endpoint-setter; the runner remains imported by no deployed function. Delivers all six hard-entry requirements at the DB/library layer: **(1)** server-authoritative effect floor (`_shared/mcp-gateway/effect-policy.ts` reuses `action-risk.ts`'s `MUTATION_VERB`) so provider `_meta.effects` may RAISE the approval gate but never LOWER it below the name floor (a `["read"]`-mislabeled `send_*`/`delete_*` tool still needs approval; honest residual — a non-verb-named mutation the provider mislabels read is not caught, recorded not implied away); **(2)** durable, endpoint-bound consent — migration `20270323000000` adds nullable `endpoint_hash`/`args_shape_hash`/`expires_at` to `mcp_connection_approvals` and a service-role `verify_mcp_connection_approval` the runner calls via `deps.verifyApproval` (consent is a stored approval whose pin + endpoint identity + optional action shape + expiry all still hold — never a pin carried in the request; a legacy NULL-binding approval fails closed); **(3)** the approval-write vs endpoint-change **race** is closed — `set_mcp_connection_approval` takes `SELECT … FOR UPDATE` on the connection row, serializing it against the endpoint-change UPDATE (which fires the #1263 revoke trigger), with the endpoint-hash binding as defense-in-depth; **(4)** a dispatched call reporting `isError` (or an unaccepted shape) is the new `tool_error` outcome, never dressed as `executed` (`result.ts`); **(5)** the runner routes outcome truth through the canonical Rail `record_capability_run` (`rail-receipt.ts`), no parallel owner-visible receipt story; **(6)** RLS/tenant guard/endpoint-revoke trigger/legacy paths/provider-authority all preserved. §37: `set_mcp_connection_approval` had zero producers, so the added optional params break no caller. **Not touched:** `paige-ai-chat`, `scripts/knowledge-scope`, `_shared/mcp-outcome.ts` (the live n8n/zapier lane), the shipped `20270322000000` trigger. | **Proven, NOT yet merged.** Real Postgres 16 (local harness): both DB proofs green — `supabase/tests/mcp_gateway_durable_endpoint_bound_consent.sql` (writer endpoint binding, `verify` endpoint_changed/contract_changed/approval_not_endpoint_bound/approval_expired/action_shape_changed, §9 cross-tenant writer guard → `MCP_GW_DURABLE_CONSENT_PROVEN`) and `scripts/proof/mcp-gateway-endpoint-consent-race.mjs` (two-session serialization: no stale approval survives). `smoke:mcp-gateway` **79 assertions** (server floor beats a `["read"]` mislabel incl. camelCase/separator normalization, provider-raise, mutation-vs-read post-dispatch outcome for BOTH a returned error and a THROWN transport exception, malformed-isError fail-closed, stale-endpoint/drift refusals, truthful Rail mapping + completed-but-unrecorded receipt signal). Both DB proofs wired into `paige-spine-contract.yml`. **Codex reviewed to R5 + an independent §39 adversarial pass (no BLOCKER/MAJOR); all findings dispositioned in-slice:** (a) [R3] `RunnerResult.receipt` reports whether the canonical-Rail row actually persisted, so a completed action whose Rail write fails is truthful completed-but-unrecorded rather than a silent recorded-success (the outcome itself is never downgraded — §13/§32); (b) [R3] the race proof hard-asserts ZERO surviving approvals in both orderings + a deterministic non-racing tripwire proves the `20270322000000` revoke trigger deletes on endpoint change (verified to fail loudly with the trigger absent), and the racy handler swallows only `MCP_ENDPOINT_CHANGED`; (c) [R4 P2-A] a MUTATION that dispatched then returned `isError`/malformed/unrecognized is `outcome_unknown` (→ "may or may not have taken effect; check before running again"), never `capability_failed` ("nothing was left half-done") which would be a duplicate-action risk for a possibly-landed mutation — a READ stays `tool_error`; (d) [R4 P2-B] a malformed non-boolean `isError` fails closed, never a silent success; (e) [adversarial] a landed `executed` effect with no actor to file under is `record_failed` (owed-but-unrecorded), not benign `not_applicable`; (f) [R5 P2, safety] a post-dispatch TRANSPORT throw (HTTP error/timeout/malformed envelope) preserves the read-vs-mutation distinction — the effect decision resolved before dispatch is carried into the catch (never re-derived from provider metadata): a READ → `tool_error`/`capability_failed`, a MUTATION → `outcome_unknown` (never auto-retried); proven load-bearing. All migration/security lints PASS (`lint:migration-versions` — `20270323000000` unique vs origin/main —, `lint:definer-fns`, `lint:managed-schema`, `lint:rail-grants`). `ci / verify` carries the inherited red only (`lint:views` #1208 + `test:knowledge-scope` #1255 — zero views, no `paige-ai-chat`/`knowledge-scope` in the diff). | **PROOF OWED:** persisted-apply on prod (`deploy-migrations` records `20270323000000`, `db-live` advances, zero drift) is owed to the pipeline on the **owner's merge** (their Gate 2); the direct prod `pg_proc`/`pg_trigger` object SELECT is owed to a prod-SQL-capable session (MCP prod SQL permission-denied this headless session). **Parked pending #1255 (NOT this slice):** wiring the runner into a deployed endpoint or Chat, a live endpoint-setter, real-provider execution — these safeguards are the entry gate that work must pass first. | **No.** Internal backend security/execution-preparation; no customer-facing surface, no provider action, no feature/package state change, no customer release record.
