...
**2026-09-22 Connected MCP Gateway — registry-native connection writers (connect · re-key · disconnect + `mcp.connections.delete`), G1a-1 (PR [#1322](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1322), base `main`; internal build identity = the squash commit (recorded in the PR merge report); release channel **production** via `deploy-migrations`; classification **internal-only** (backend seam, no customer surface); SHIPPED under owner standing authority **INT-083**; built to the "solo shell, no tenant identity" standing rule with the §200 grep proof (`0 hits`) in the PR body):**

Makes `public.mcp_connections` the sole WRITE authority for NATIVE connections (`legacy_source IS NULL`) across the four connect facets (generic remote MCP · Zapier MCP OAuth · n8n MCP OAuth · n8n REST api-key), via migration `20270331000000`: `create_mcp_connection` / `create_mcp_rest_connection` (connect), `set_mcp_connection_endpoint` (MCP re-key, CREATE-OR-REPLACEd onto the shared `_mcp_assert_credential_bundle` helper — single home) / `set_mcp_rest_connection_endpoint` (REST re-key), `disconnect_mcp_connection` (soft disable + history-preserving hard delete), and a third capability `mcp.connections.delete` (owner/tenant-admin only, platform owner excluded). Authority resolved server-side (§9), secrets never returned, SSRF-validated endpoints, `mcp_connection_receipts` FK `CASCADE→SET NULL` so hard-delete preserves history (receipts/Rail survive unlinked, audit records the delete). Two Codex exact-head fix rounds under the coordinator gate: **round 1** — re-key restores `enabled=true` (re-key = reconnect, P1) + `get_mcp_connections_v2.configured` honest for credential-less `url`/`none` rows (P2); **round 2** — soft-disconnect scrubs the url-embedded credential from `server_url_ct` (P2, so a disabled `url` row reports `configured:false` and retains no live credential). Each round ran build+verify (ephemeral-Postgres pgTAP + smoke) → §39 adversarial peer → §5 compliance → integrator read. **Touches no legacy tables/code, no `_shared` TS, no execution wiring, no probe** — the runtime loader still refuses api_key as `connection_unusable`; execution/probe/OAuth-dance/tool-read are later slices (G1a-3 / G1a-4 / G2). Backend contract for the G1b Connections UI: `docs/mcp-gateway/g1a1-connections-contract.md`. | **Proof (reviewed head):** migration applies clean; 3 pgTAP suites `*_PROVEN` (incl. the existing setter proof + loader-parity, unchanged); `smoke:mcp-gateway` **221**; `lint:definer-fns` clean; both fix rounds carried non-vacuity negative controls. `verify` red = the inherited baseline {9,18,28,29,39,74} only (no new red); non-required `github-advanced-security` = the platform Copilot-Autofix infra crash. §200 grep `0 hits`. | **MERGE + DEPLOY (recorded in the PR step-6 merge report):** squash-merged under INT-083 (merged head = reviewed head); on merge `deploy-migrations` applies `20270331000000` and moves `db-live`; the squash SHA, the `deploy-migrations` run, `db-live` movement, and the `schema_migrations` row on prod (ref `xygzykjyynhzqytbqnzu`) are confirmed in that report (§32.a persisted-apply). | **PROOF OWED:** none at the seam level — the RPC contract is proven by pgTAP against a full ephemeral replay; no customer surface and no §32.c authenticated UI drive is owed (the G1b Connections UI is a later slice). | **Customer-release eligibility: none** (backend seam, pre-launch, no customer surface or release note).
