...
**2026-09-20 Per-thread PAIGE composer drafts — INT-102 (PR [#1308](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1308); exact code head [`7c2f8e8d`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/7c2f8e8d0c2d95eec693625ca06bc9bd15787af8); classification **patch**; guarded delivery under owner standing authority **INT-083**; this pre-merge row becomes a shipped record only when #1308 merges):**

Repairs draft ownership without changing the PAIGE composer design: session-memory drafts are keyed by server-resolved tenant + effective user + real thread id, with a stable surface/client/mission-qualified `new-chat` slot that moves to the persisted id on first thread creation. Thread/New-chat/account/user/surface switches resolve only the destination key; returning restores the origin. A composer send clears only its submitted origin after terminal stream `[DONE]`; authentication, transport, non-2xx, truncated 2xx, cancel, and stale-scope exits retain it. Signed-out context renders no draft, a different user cannot inherit it, and same-user recovery lasts only for the still-running module session; reload persistence is explicitly out of scope. Typed and dictated text share the same seam; #1298 scope teardown/late-callback guards remain unchanged. `PaigeAIChat` and AppShell `PaigeChat` are covered. `ConversationsRichComposer` itself is controlled, but its parent `ClientsConversations` still uses one clear-on-selection body; per owner boundary that separate proposal is not folded in. Proof: initial failing-first **10/10 red**, restored **10/10 green**; requested-review regressions **3 red/10 green**, final focused **13/13** and focused+remount **14/14**; load-bearing key mutation **5/6 red**; tsc ratchet **12→12**; production build green; ESLint zero errors. Affected regression **66/67** with the only red inherited from base: `SoloPaigeWorkspace.contract` rejects `_tenant_id:` already present in untouched `SoloApp.tsx`; both files have zero INT-102 diff. The one requested Codex review on `f65a792d` found two valid P2s — cross-surface New-chat aliasing and missing-`[DONE]` false clears — both fixed failing-first in the single permitted round. No visual/layout, attachment, provider, Edge, `_shared`, database, migration, capability, or deferred INT-086 Draft-with-Paige change. Evidence: `docs/evidence/ui-delivery/int-102-per-thread-paige-drafts.md`. | **DELIVERY GATE IN PROGRESS at row authoring.** Conflict-free current-main sync is complete with code diff hash `6fa63cc27618b7bfd9a3427743e58cd4ed9205e7`; final exact-head CI, the Ready-triggered auto review, individual finding disposition, pinned merge, and Vercel production identity remain required. | **PROOF OWED:** authenticated production Solo/AppShell thread switching, New chat, account/user isolation, send success/failure, dictation-origin retention, and sign-out/re-login behavior. | **No.** Existing-flow reliability repair; no new customer capability or release identity.
