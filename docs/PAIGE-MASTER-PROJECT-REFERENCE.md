...
**2026-09-20 PAIGE Chat dictation tap-to-toggle repair — INT-086 (PR [#1290](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1290); exact code head [`d9b68152`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/d9b681520ad8620aaa877dc26ad34cf58ece6221); classification **patch**; guarded delivery under owner standing authority **INT-083**; this pre-merge row becomes a shipped record only when #1290 merges):**

The PAIGE Chat dictation control is repaired to the owner-locked interaction: one tap starts, recording persists through pointer release and natural pauses, and one tap stops. Capture starts before auth/WebSocket setup and PCM is buffered until Deepgram is ready, so opening words are not discarded; the UI says `Listening` only after the provider-ready frame. Finalized text appends at the captured cursor without overwriting typed text. The same obvious stop control remains visible while the mic is live. A **5-minute sustained-silence** forgotten-mic guard and **30-minute hard cap** retain captured text and explain the stop. Stop, disable, unmount, route/account change release mic/recorder/socket/timers and fence stale callbacks. Provider remains Deepgram through unchanged `paige-dictate`; no Edge Function, migration, data, entitlement, Live Conversation, playback, or Chat-send wording change. Evidence: `docs/evidence/ui-delivery/int-086-dictation-tap-toggle.md`; focused suite **20/20**, five failing-first assertions, load-bearing pointer-release mutation, tsc ratchet **12→12**, production build green, required viewport pairs rendered without overflow. | **DELIVERY GATE IN PROGRESS at row authoring.** Draft PR #1290; exact-head CI and exactly one Codex review are required before Ready/merge. Vercel production deployment must follow the merge; no Edge deploy is expected because `paige-dictate` is unchanged. | **PROOF OWED:** authenticated production microphone→Deepgram drive, deployed Vercel identity, Safari/macOS, real iPhone, browser zoom/reflow, and owner acceptance. The local deterministic render is not production/provider proof. | **No.** Existing-flow repair; no new capability or provider.
