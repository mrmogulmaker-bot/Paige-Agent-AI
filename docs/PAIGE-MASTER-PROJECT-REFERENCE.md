...
- **2026-08-11 · §61 Default tier-placement rule — owner-ruled, STOP asking per-feature:** the owner ruled the S1 skills-tier question AND issued a standing meta-directive — *"This is yet another time that you guys have asked me… where things should be placed when we should actually already have this understanding for the entire platform… lock this in as a complete doctrine, in our brain and the master project."* The recurring "which tier gets this?" now has a DEFAULT (don't re-ask the owner): **God gets everything** (§57); a **RESELLABLE capability** (skill/pack/tool/knowledge — an agency can resell it) → **Solo + Agency + Sub-account + Enterprise + God**; an **OPERATIONAL "doing" surface** (own client/campaign book) → all-but-Agency + God (generalizes the existing `customer_portal_invite`/`growth`/`studio` cells); **operator tooling** → God only. Authoritative home `docs/doctrine/tier-matrix.md` §61; CLAUDE.md §61 (PROPOSED, §-number provisional pending Cowork ratification, per the §57–§60 pattern). Brain decision-log + README updated (§BRAIN.3). **Two S1 rulings baked in:** (a) `skills` = resellable → all tier keys; (b) D-vocab = FULL rename (Marketplace "skills" → "capability packs", §58 slice) so "skill" = only the executable recipe. This is a docs-only doctrine PR; the S1 build follows with these rulings encoded.
