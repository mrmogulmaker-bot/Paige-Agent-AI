...
**2026-09-23/24 UTC — INT-104 Live relay join, continuity repair and spoken delivery (post-merge INT-128 closeout; production code, internal-only; capability remains PARTIAL).** Existing Solo Live screen, identity, thread, Presence, controls and canonical PAIGE runtime are reused. Signed runtime output binds speech to the authenticated tenant/user/thread/turn; forged text cannot open the mouth adapter. No second brain, memory or approval path. Spoken assent never approves an action. Neutral UsageSink remains accounting-only; active read-aloud remains OpenAI. Flow-by-Flow 2.0.2 / R3 Deep and independent review applied; S5 interaction record cites Impeccable 4.3.1.

| Release | Last reviewed head = merged branch head | Durable squash identity | Production receipt |
|---|---|---|---|
| [#1387](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1387), real-adapter join | `d0a8ff81e35ed8d431c66b5f5f1eedc42c965485` | `6bd8849d4b9bad7c8d331458cab03c00c61b87aa` | [Edge run 35930845301](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/actions/runs/35930845301), chat + relay only; Vercel `dpl_HWT6WqrC1wRqp3pc2dvFYGeW6Unf` READY on the merge. Independent peer gate replaced twice-requested absent external review; late findings repaired in #1408. |
| [#1408](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1408), Hold/resume and continuous admission | `72395be53f5811709748497624d5ae86a6756a1e` | `1e592542ede48589349b5445376ec2396e948cc6` | [Edge 35935279150](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/actions/runs/35935279150), chat + relay only; [DB 35935279183](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/actions/runs/35935279183) applied `20270411000000`; Vercel `dpl_6vBCkQmHArKHJbtNBc2q37LepRMT` READY. 83 bridge checks and 10 pgTAP assertions passed. |
| [#1405](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1405), spoken style and interrupted-answer continuity | `85bf5a06fabde154aeda1a1b6818a15c21178a7b` | `c234dc3df0b7bbd698bc55de83f552fd8e5235c3` | [Edge 35937983917](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/actions/runs/35937983917) SUCCESS, chat only; Vercel `dpl_6QacabnmJQxEwx5Da2HYA3mJXAbZ` READY on the merge. Exact-head Codex and independent peer clean; 96 focused tests passed. |

**Measured release state:** chat v269, Supabase metadata `ezbr_sha256=1dcffb8dc9ecfcbfe86c2e4e0bd76eddd8aa3aac601eef820e15d5289ecc35ae`; relay v8, `ezbr_sha256=6b1a11b553c0d22eebce9fd963e6cb256247d81ebfa2b5fc392cb9a56e938189`. These are metadata digests, not local bundle hashes. Forced tag refresh: `edge-live=c234dc3d`, `db-live=1e592542`; no function or migration drift against that main. #1405 changed no database object. Its [CI 35937273163](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/actions/runs/35937273163) failed only Test: the same 20 named inherited failures, added/removed both empty, 5364 passed; every other verify step passed. Raw local Deno had 11 diagnostics; the exact-head CI ratchet passed, not a claim of zero diagnostics.

**Truth boundary:** no provider/audio call or synthetic traffic was used. The 34-assertion relay harness measured 959 ms in virtual time, not real latency. Privacy/account/voice entitlement and the runtime signing key remain unverified, transport is OFF, and no human production conversation is claimed. Legacy readiness still requires Voice-specific quota/caps: parked with Budget in [#1407](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/issues/1407), not bypassed or falsely attested. Real microphone-to-Jessica conversation, acoustic take-5 acceptance, actual latency, interruption and clean end remain PROOF OWED. Customer-release eligibility: none. Evidence: [join](evidence/ui-delivery/int-104-live-audio-join.md), [continuity](evidence/ui-delivery/int-104-live-lifecycle-revalidation.md), [spoken delivery](evidence/ui-delivery/int-104-live-spoken-style.md).
...
- **2026-09-23/24 UTC — INT-104 release and proof correction.** #1387's join was not end-to-end delivery. Its late review exposed Hold/resume and stale provider-readiness decisions; #1408 repaired both, including a structured unavailable WebSocket frame at initial refusal. #1405 review exposed buffered first speech, missing DONE handling, transcript rollback/unsafe retry, lost safe receipts and empty-answer false success. Repaired through the existing runtime, stream consumer and turn store, with failing-first tests and mutations. The tools-free answer stream follows a private canonical tool-decision round; that added round means sub-second real latency is UNVERIFIED. A present API key is not privacy, entitlement or signing-key proof. All final build identities below name durable squash commits plus PR/reviewed head, not branch ancestry. Production code is deployed; audio is still OFF and human proof is still owed. No owner identity or operational pilot membership is recorded here.

- **2026-09-23 · INT-163 / #1395 — A DEFECT FIX THAT CLOSED ONE DOOR OF TWO, AND AN EVIDENCE RECORD THAT SAID OTHERWISE (PR [#1398](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1398), third review round).** `agreement-send` used to render an agreement's TEXT body to PDF regardless of where the body came from; an uploaded-file agreement has no text, so it froze and emailed a BLANK page. The fix corrected the send path. **It was not the only way a signer is shown a document.** The owner's surface also offers "Or copy a link", which never touches the edge function — it calls `issue_agreement_signing_link`, which mints a token directly. That RPC refused to FREEZE an uploaded file (correctly: the database cannot read storage) but had nothing to say about a row ALREADY frozen, which is precisely what every pre-fix row is. So after the fix one door refused those rows while the other handed out links to them. **The PR's own evidence record asserted the opposite** — `docs/evidence/ui-delivery/agreements-send-truthful-outcome.md` listed `issue_agreement_signing_link` under "Named and unaffected", reasoning that its refusal "now correctly no-ops once the send has frozen real bytes". The reasoning was sound and the premise was false: for a pre-fix row the frozen bytes are the blank, so the no-op stepped past the defect. Corrected in place; the seam is now listed as impacted. **Three further things the crew caught that the build's own GREEN proof could not (§39).** (1) The refusal's first wording blamed a historical defect — *"this agreement was prepared before a defect was fixed"* — which is FALSE for a row buildable entirely after the fix: a text agreement whose send froze `presented-<uuid>.pdf` and then failed every delivery stays a draft (`agreement-send/index.ts:428`), and a draft's `body_source` is still editable (`20270401000000:544`), so switching it to an upload lands in the same predicate. The refusal is still right; the causal claim was a guess stated as a fact, and was removed. (2) The refusal would never have REACHED a human: `runWrite` reads only `error.code` and `safeWriteMessage` maps `23514` to *"Check the client, the document and the wording, then try again."* — a retry instruction that can never succeed, because the content columns are write-once. The engine now raises its own SQLSTATE `PA001` so the surface can say the one true thing without weakening its no-echo rule. (3) A fixture in the new proof inserted a signer by hand on top of the one `trg_agreement_seed_counterparty` already seeds, producing two rows at `signing_order = 1` — a shape no shipped path can create, since order uniqueness lives in `add_agreement_signer`'s body (`20270405000000:186-190`) and in no index. Corrected so the fixture matches a real row. **A FOURTH THING, found by the peer-gate on the pushed diff, and it is the largest: THE SEND DOOR HAD NEVER OPENED FOR ANYONE.** `agreement-send` and `agreement-document` both gated on `caller.rpc("is_tenant_admin", { _tenant_id: tenantId })`. The only `is_tenant_admin` in the schema takes `_tenant` (`20260629175341:144`), and the generated types, which come from the live schema, carry exactly one overload: `src/integrations/supabase/types.ts:20987`. PostgREST resolves an RPC by argument NAME, so `{_tenant_id}` cannot bind; the call returns no data, both functions destructure `const { data: isAdmin }` and discard `error`, and `isAdmin !== true` then refuses **every caller, including the workspace owner**, with "Only an owner or admin can send an agreement for signature." The two sibling call sites spell it correctly (`export-document/index.ts:124`, `useCalendarConnections.ts:333`), and a contract test already pins the spelling for one of them (`doc-export-contract.test.ts:472`) — so the convention existed and these two missed it. Introduced in `d08769bf4` (PR #1352), not by this work. It survived a build, a typecheck, 104 CI steps and several review rounds because the call is made on an untyped client, so nothing in the type system ever looked at it, and because a fail-closed gate that fails closed on everyone is outwardly indistinguishable from a permissions problem. **It also falsifies a claim this very PR had just added** to the tier matrix, which marked the send seam available to Solo and Sub-account on the strength of reading that gate. Fixed here, with a contract test that reads the argument name out of the generated types — so a future rename of the SQL argument fails the build too — proven to bite by restoring the defect (exit 0, then 1 naming the offending file, then 0). **This is why "an agreement sends the correct document" was still not provable after the document defect was fixed: the document was right and the door was bolted.**

**The carried-forward lesson:** when a defect is fixed at one entry point, the producer inventory owes an answer to "what ELSE reaches this state", not only "what calls this function" — and an evidence record's "unaffected" list is a claim that needs the same proof as the change itself.
