...
Fail-closed active-account revalidation **before** the general-document extraction provider call (#1255). A switched-account **DOCX/image** turn makes **zero** provider calls; a switched-account **general PDF** makes **exactly one** caller-document-only pre-resolution read-check. `runDocumentReadCheck` was **not** relocated. The extraction is tenant-attributed. | **MERGED and DEPLOYED (NOT customer-verified).** `deploy-edge-functions` run `35484814492` **success**; **`edge-live` moved to `ca10e5d577a46b007f532b82be3c6ec026a5c136`**. Knowledge-scope **369/0** and `ci / verify` step **#83 green** on `main`. | **OWED and BLOCKED (§32.c/§13):** authenticated production verification is owed and **BLOCKED BY INT-080** (production Chat outage). | **No.** Patch; no customer release note.
