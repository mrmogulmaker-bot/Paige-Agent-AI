...
## 10. §13 corrections log
 ...
 - **2026-09-22 — INT-117 S1 correction: the persona core must carry NO identity.** The parked #1323
   ...
   structural lesson: ANY identity text in a shared prompt block fights some seat's earlier identity message.
   The correction is architectural, not another per-seat exception: identity moved OUT of the core entirely
   (every lane establishes it earlier — tenant persona block [whose platform default now carries the warmth
   biography], VP block, Studio named agent, owner-desk identity rows), leaving a seat-blind registers block.
   The same lesson generalizes: prompt text shared across seats may carry only seat-neutral content — safety
   lines, registers, naming rules — never seat-specific identity, and §37 conflict sweeps must scan EVERY
   lane that assembles the shared text, operator and CRM blocks included ("every lane is a person lane").
