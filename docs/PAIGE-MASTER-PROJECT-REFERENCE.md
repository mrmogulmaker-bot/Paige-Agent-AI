...
#### Production-version and duplication audit — 2026-09-07 23:35 EDT

| Category | Verified finding | Disposition |
|---|---|---|
| Stale deployed version | **None found.** `paigeagent.ai/version.json` and `app.paigeagent.ai/version.json` both returned build `531045004aa306beecb669b31c385c779c76c4b8-mtry6b9s` with `customerUpdate: null`; both returned HTTP 200 and the same initial hashed JS/CSS assets. Vercel production deployment `dpl_5V2fqn1D8axaEH25Tg2tB6KgKh8P` is `READY`, aliases both domains, and records Git SHA `531045004aa306beecb669b31c385c779c76c4b8`, the then-current canonical `main`. | No release action was needed at audit time. Recheck exact identity after this PR merges. |
...
**Current backend delivery identity at this audit:** production carries 999 migrations, latest `20270105000000_campaign_brief_verified_rail_copy`, and 223 active Edge functions. Relevant active versions include `paige-ai-chat` v209, `business-mission-action` v2, `export-document` v1, and `browser-use` v1. These counts prove deployed inventory only; they do not upgrade authenticated capability proof.
...
| 2026-09-08 00:05 EDT | [#1054](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1054) / [`2719d7d86ccfa23d0e781b841b5ee134ec8e7276`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/2719d7d86ccfa23d0e781b841b5ee134ec8e7276) | **Canonical shipped-log reconciliation and public-home retirement.** Established this sole chronological shipped log, backfilled verified deliveries through #1051, and redirected obsolete `/premium` and `/legacy` page owners to `/`. | **Merged and deployed in current production ancestry.** The route contract and rendered evidence passed; no migration, Edge function, provider capability, customer version, or authenticated tenant behavior changed. Authority: [UI evidence](evidence/ui-delivery/canonical-public-home-route-retirement.md). | **No.** Governance/route cleanup and no approved canonical release record. |
| 2026-09-08 00:28 EDT | [#1053](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1053) / [`f8eb2362beb03ec609dfe645b4d00f43c3eebf2d`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/f8eb2362beb03ec609dfe645b4d00f43c3eebf2d) | **Paige Live Conversation MVP and governed control plane.** Added `Talk live with Paige`, the same-thread immersive stage/companion window, six live card types, accessible session controls, provider-neutral session/profile contracts, and the server-only Paige Voice Profile path. | **UI and fail-closed control plane deployed, `PARTIAL`; provider realtime audio `PROOF OWED`; authenticated owner E2E `UNVERIFIED`.** Both production domains serve exact build `f8eb2362beb03ec609dfe645b4d00f43c3eebf2d-mts66mnc`; `db-live`/`edge-live` point to the merge, migration `20260907155052` is persisted, and the three Live Edge functions deployed. Governed actions still use Spine → verified readback → receipt → Rail; raw audio/transcript/cards do not auto-write Brain/Mind/Memory. Authority: [delivery closeout](delivery/paige-live-conversation-mvp.md) and [UI evidence](evidence/ui-delivery/paige-live-conversation-mvp.md). Next gate: secret-safe account scope/voice/ZRM/retention/quota/concurrency/cost proof, then a separately approved transport enablement and authenticated owner drive. | **No.** No approved canonical release record; audio and authenticated proof remain owed. |
