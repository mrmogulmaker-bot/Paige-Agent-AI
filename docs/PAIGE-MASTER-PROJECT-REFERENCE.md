...
- **Truthful capability awareness — LIVE (edge).** A new `capability_status` chat read tool: before Paige claims a capability she consults a server-resolved, tenant-scoped truth (pure resolver `_shared/paige-capability-status/{resolver,signals}.ts`) composing tier × autonomy-lane × Spine maturity into ONE honest availability per capability (live / needs_approval / needs_setup / planned / not_for_tier / unavailable), most-restrictive-wins. MVP scope = contacts + connections. Deploys live via the edge pipeline on merge (uses only already-live prod RPCs — `resolve_tool_autonomy`, the Spine registry). Authenticated owner-drive §32.c OWED.
