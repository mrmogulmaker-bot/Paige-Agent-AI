...
- **2026-08-09 · Cowork miss #21 (trademark-exposure class, owner-caught):** a Marvel/Disney Iron-Man AI-character codename (the "Initiative" codename + its analog references — the prohibited mark is named in `CLAUDE.md` §50, kept out of this in-repo doc on purpose) had leaked past the internal-only boundary the strategy doc itself set. GitHub `search_code` against `main` confirmed ZERO code hits (rule held on the code side) but **14 hits across 4 doc files** — `docs/product/BRD-MVP-2026-08-08.md` (8), `docs/product/agent-ui-placement-spec.md` (2), `docs/product/interactive-analytics-ui-spec.md` (2), `docs/strategy/owner-trilogy-2026-07-26.md` (2 + the analog mapping table). Repo is public; the character is a distinctive third-party mark. (Cowork's original count was "8 hits"; CC's fresh-clone grep found 14 — logged as the real number, §13.) **Fix:** all references purged in one docs-only PR — reframed to "Systems Check MVP" / "the operator-AI-COO archetype", and the analog mapping table deleted outright; a post-purge `git grep -ri` over `docs/**` returns ZERO hits (the mark now lives ONLY in the `CLAUDE.md` §50 enforcement list, by design). **Prevent-recurrence:** new **§50 Trademark hygiene** doctrine added to `CLAUDE.md` — a mechanical case-insensitive grep on every §5/§39 pass. Owner-caught this class of leak; doctrine now catches it going forward.
