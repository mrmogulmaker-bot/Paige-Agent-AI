...
#### Production-version and duplication audit — 2026-09-08 00:36 EDT

| Category | Verified finding | Disposition |
|---|---|---|
| Stale deployed version | **None found after remediation and current-main refresh.** `paigeagent.ai/version.json` and `app.paigeagent.ai/version.json` both return build `f8eb2362beb03ec609dfe645b4d00f43c3eebf2d-mts66mnc` with `customerUpdate: null`. Vercel production deployment `dpl_79gKFA29LsSstU6tJFEMxNeYWXdw` is `READY` from exact canonical `main` SHA `f8eb2362beb03ec609dfe645b4d00f43c3eebf2d`. Fresh durable browser drives on both domains at 1366x768 and 900x1000 proved `/premium` and `/legacy` replace-redirect to `/`, Back returns to `/about`, keyboard reaches the Paige Agent link, and 200 percent reflow has no horizontal overflow. | Current production and canonical `main` match. The #1054 route correction remains in this newer deployment's ancestry. No manual promotion or rollback was needed. Public `track-event` requests emitted CORS/401 console errors on both domains; route behavior was unaffected, but telemetry ownership requires separate diagnosis. |
