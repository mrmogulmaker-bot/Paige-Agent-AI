...
> **Solo Beta Gate A candidate — not shipped (2026-09-12).** The review candidate narrows public acquisition to the fixed paid Solo offer, adds verified/fenced fulfillment and safe recovery, and preserves existing authorized non-Solo access. Focused new-scope automated/static checks are green; whole-repository Vitest/ESLint retain documented baseline failures; database apply, Stripe test acceptance, authenticated runtime, deployment, and owner acceptance remain `PROOF OWED`/`UNVERIFIED`. It therefore has no Section 4.0 shipped row yet. PR #899's payment-setup ordering intent and PR #724's canonical Solo destination are carried forward; only obsolete new-public-Solo detours are superseded. Authority: [Solo Beta acquisition evidence](evidence/ui-delivery/solo-beta-acquisition.md).
