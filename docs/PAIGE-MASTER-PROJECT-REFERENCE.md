...
#### Production-version and duplication audit — 2026-09-08 00:10 EDT

| Category | Verified finding | Disposition |
|---|---|---|
| Stale deployed version | **None found after remediation.** `paigeagent.ai/version.json` and `app.paigeagent.ai/version.json` both return build `2719d7d86ccfa23d0e781b841b5ee134ec8e7276-mts5d4ii` with `customerUpdate: null`. Vercel production deployment `dpl_CfH3jVVmkzrAm4vSzdUGDTU9Z9gQ` is `READY` from exact canonical `main` SHA `2719d7d86ccfa23d0e781b841b5ee134ec8e7276`. Browser drives on both domains at 1366x768 and 900x1000 proved `/premium` and `/legacy` replace-redirect to `/`, Back returns to `/about`, keyboard reaches the Paige Agent link, and 200 percent reflow has no horizontal overflow. | Current production and canonical `main` match. No manual promotion or rollback was needed. Public `track-event` requests emitted CORS/401 console errors on both domains; route behavior was unaffected, but telemetry ownership requires separate diagnosis. |
