...
#### Production-version and duplication audit — 2026-09-08 01:08 EDT

| Category | Verified finding | Disposition |
|---|---|---|
| Stale deployed version | **None found after remediation and current-main refresh.** `paigeagent.ai/version.json` and `app.paigeagent.ai/version.json` both return build `b36b831b74b661f1720fda8d6caeeefba0026cc0-mts7hmbb` with `customerUpdate: null`. Vercel production deployment `dpl_DtdkQ8ag7XYXxrVg6ZaEt9pSDnyn` is `READY` from exact canonical `main` SHA `b36b831b74b661f1720fda8d6caeeefba0026cc0` and aliases both domains. This is #1061's documentation-only descendant of #1057; the durable public-route proof and product code are unchanged. | Current production and canonical `main` match. The #1054 route correction, #1053 Live Conversation delivery, and #1057 chat-scroll correction remain in the current documentation-only build's ancestry. No manual promotion or rollback was needed. Public `track-event` requests emitted CORS/401 console errors on both domains during route proof; route behavior was unaffected, but telemetry ownership requires separate diagnosis. |
