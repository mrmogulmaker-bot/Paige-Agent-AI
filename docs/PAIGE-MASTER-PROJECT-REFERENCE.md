...
**2026-09-20 Connected MCP Gateway — endpoint setter (INT-099, MCP PR-2) (PR [#1300](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1300); exact code head [`e3aba6fc`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/e3aba6fc); branch `claude/laughing-carson-iw91ey`; internal build identity `e3aba6fc`; release channel **production** (migration `20270329000000` via `deploy-migrations` on merge); classification **internal-only** (endpoint-management RPC hardening; still library-only / zero deployed importer); delivered under owner standing authority **INT-083**; this pre-merge row becomes a shipped record — with the verified `main` merge commit — when #1300 merges):**

New migration `20270329000000_mcp_gateway_endpoint_setter.sql` adds `set_mcp_connection_endpoint(...)`: writes a connection's endpoint + full credential bundle in ONE atomic UPDATE, sourcing every credential column from the arguments so a changed endpoint can NEVER inherit the old secret; the derived `endpoint_hash` follows and the existing `20270322000000` AFTER-UPDATE trigger revokes endpoint-bound consent on a real change (A→B→A cannot resurrect). **D1** refuses a legacy-projected row (no cutover — `tenant_mcp_connections`/`tenant_n8n_connections` remain the sole live write path; native-connection CREATE is a separate future coordinator-numbered PR, so there is no operable native prod row yet). Authority is the new capability `mcp.connections.manage` (CREATE/CONFIGURE/ROTATE, NOT delete — **D2**) on the single mapping `_mcp_caller_capabilities` — owner/tenant-admin of THIS tenant ONLY; a **platform owner is EXCLUDED from `manage`** (keeps `use_restricted` only — **A2**); server-derived actor `auth.uid()`, fail-closed, no service-role bypass (INT-089). **A1** hashes-only audit (old/new endpoint_hash, auth_kind before/after) into the existing `paige_audit_log` in the SAME transaction, audit-fail⇒UPDATE-does-not-commit. **A3** write-only return. **A4** static https/SSRF URL validation (`_mcp_endpoint_write_safe`); runtime egress SSRF stays in `mcp-client.ts`. **A5** EXECUTE to `authenticated` only. **D8** INT-082 pgTAP kept exact-equality against the new full set. Does NOT touch `mcp-client.ts`, `get_mcp_connection_secret`, or `verify_mcp_connection_approval`. **D4 hard gate** recorded (runner system-authority reason persistence must precede any system/headless wiring; this setter has no system path). | **MERGE PENDING at authoring.** **Zero `supabase/functions` diff → `deploy-edge-functions` not triggered, `edge-live` unmoved at `c7e386a3`**; migration `20270329000000` applies via `deploy-migrations` on merge (`db-live` moves then). CI edit confined to `paige-spine-contract.yml` (path filters + one run step); **`ci.yml` untouched** (deconflicts #1295 / zCod PR-P2 which append to `ci.yml`). Merge gate (INT-096): on Ready, wait for the auto-triggered exact-head Codex review before merging. §39/§5/§37 pre-Ready crew (all non-blocking) hardened while draft (`884e471a`→`e3aba6fc`): value-based `inet` SSRF validation (`_mcp_inet_is_public`), authority-before-existence ordering (§9, no cross-tenant oracle), `provider_state`+tool-catalog reset on re-point, and per-case pinned pgTAP refusal reasons. | **PROOF OWED:** CI `database-contract` pgTAP (`mcp_gateway_endpoint_setter.sql` + updated INT-082) green on real Postgres; post-merge prod object SELECT of `set_mcp_connection_endpoint` + `_mcp_caller_capabilities` and `schema_migrations` advanced — owed to a prod-SQL-capable session (headless MCP prod SQL was permission-denied on the prior PR). Local: `_mcp_endpoint_write_safe` SSRF/URL matrix proven on real Postgres 16. | **No.** Internal backend seam; no customer-facing capability, provider, flow, or release identity.
