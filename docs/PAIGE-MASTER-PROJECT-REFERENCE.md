...
**2026-09-13 web_fetch hardened tenant-safe routing — a dead client-seat chat tool made functional + the shared fetch SSRF-hardened (PR [#1227](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1227); internal build identity branch `claude/paige-cowork-handoff-rpf6dw`; release channel **production** (edge-deployed via `deploy-edge-functions` on merge); classification **minor-candidate**; Gate A):**
Owner ruled 2026-09-13: do NOT retire `web_fetch`; build it as a hardened routing slice for the client seat too. It was in `CLIENT_SEAT_ALLOW` and advertised to clients but the handler was **inert** (`"Web fetch not executed in this flow"`) — an advertised-but-dead capability (§13/§70 gap). **Two changes, each on a proven one-home seam (§18): (1)** `fetch-url-content` now routes its outbound fetch through the canonical `_shared/ssrfGuard.ts safeFetch` instead of a per-hostname regex blocklist — closing the DNS→private / IPv4-mapped-IPv6 / link-local bypasses (validates every resolved IP numerically), refusing redirects, and bounding time+bytes; adds `title`/`truncated` provenance and maps every `SsrfError` to an honest stable reason+status. The response contract (`{success,url,content}`) is preserved so the auto-fetch path, `paige-deep-research`, and the CI mocks are unaffected (§37, inventory complete). **(2)** the `web_fetch` chat tool handler is now functional: routes to the hardened `fetch-url-content`, **fences** the attacker-controlled page body (`RETRIEVED_KNOWLEDGE_UNTRUSTED_NOTICE` + `sanitizeUntrustedText`) before it re-enters the model, returns provenance `{url,title,fetched_at,truncated}`, reports refusal/failure honestly (never a fabricated page), and takes NO identity from the tool args — the seat/tenant was resolved once at the top of the handler. The client portal gains working link-fetch. Nothing removed (§58 — `web_fetch` stays declared + non-confirm; the 3 CI assertions requiring it hold). | **Built + verified on branch; merge + edge-deploy pending (this PR).** §32 proof (no local Deno; `deno check` owed to CI): `scripts/web-fetch-hardening-smoke.mjs` **29/29** (SSRF refusals incl. DNS→loopback/RFC1918, public admits, anti-vacuity vs the old regex, `safeFetch` redirect-refusal + bounded read, the injection fence, the handler tool-result shape) — now wired into CI as `smoke:web-fetch-hardening`; `paige-ai-chat/__checks__` **22/22** (the real `index.ts` imports, scope probe clean); `client-memory-authz` 268 pass + `knowledge-scope` 351 pass (each with its 1 pre-existing baseline fail, identical on clean `main`). **§39 adversarial peer SOUND** — no P1/P2 defect (SSRF guard fires unconditionally incl. the service-role path; no identity from args; content always fenced; contract preserved); its P3s folded (failure-branch `error`/`reason` now fenced; `fetch-url-content` 500 contract cosmetics). **§5 compliance SHIP** — it mutation-tested the smoke (non-vacuous) and flagged one standards gap (the smoke was not wired into CI) now closed in-slice. | **PROOF OWED (§32.c/§70):** the authenticated client-seat `web_fetch` live-drive (no isolated test tenant headless). **Known limitation (disclosed, not a regression):** `safeFetch` refuses redirects by design, so a client pasting a **link-shortener** / www-canonicalizing URL gets an honest refusal rather than a follow — the canonical SSRF posture (per-hop re-validation is the TOCTOU the guard deliberately avoids); a bounded single re-validated hop is a future owner/product call, not silently relaxed here. **Follow-up (§18, tracked):** `kb-ingest-url` still carries its own weaker regex SSRF fork — migrate it onto `safeFetch`. | **No.** Internal capability/hardening; the client already saw the advertised tool — no new customer-visible surface, no provider action, no customer release record.
