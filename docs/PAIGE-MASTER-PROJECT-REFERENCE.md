...
New shared `_shared/funding-coaching-gate.ts` — the ONE HOME for "may this finance/credit provider run for THIS workspace right now?" — wired into `smartcredit-pull-snapshot`, `nav-pull-profile`, and `business-verifier` (before the ALL_ADAPTERS fan-out), refusing BEFORE any provider contact/write unless the tenant is package-entitled (the EXACT `is_finance`→`funding_enabled` predicate; no parallel system) AND has a per-tenant Financial connection/consent. Fails closed on any read error. Capability-status resolver extended with `requiresPackage`/`packageEntitled` (backward-compatible). No table, no seed, no activation, no mutable global gate; every provider returns `unavailable` today because the connection/consent read point is absent + the bundle unseeded (`FUNDING_TOOLS_REMEDIATION_LIVE=false`). | **Merged + edge-deployed.** §32.a: `deploy-edge-functions` run #315 **success** on `8c7fb131`, branch `main`; the three functions live. §39 peer-gate SHIP + §5 compliance SHIP. `ci:tsc` caught a TS2339 (`FundingConnectionFact` discriminated-union access the repo tsc doesn't narrow — `deno check` had missed it) → fixed at `689035f4` before merge; vitest 28/28. Required Vercel green; the non-required `verify` red is the pre-existing #1186 `signupMobile.ts` baseline (untouched here). | **Codex fix-forward MERGED — PR [#1228](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1228), squash [`86476a83`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/86476a83519b4d437f2509d4a99d886891d975c2), edge deploy run #318 success (§32.a):** across three Codex passes (layered defense — external review caught what the internal §39 peer + §5 compliance both missed) — F1 the HTTP-200 refusal recorded as SUCCESS by `skill-runner`/`paige-mcp` (§37 server-caller inventory miss) → both now read the OUTCOME via the unit-tested `_shared/business-verify-outcome.ts` classifier (ran/policy_refusal/operational_failure, requiring a literal `ok:true`); F2 `setup_required` was a dead "install" instruction while the package is unseeded → refusals resolve honest `unavailable`; F3 the three registry entries said the gate was OWED → updated to the shipped contract; F4 added the `Visible-Flow-Impact: yes` trailer + evidence record. vitest 45/45; §39 peer + §5 compliance SHIP + Codex pass-4 clean. **PROOF OWED (§32.c):** authenticated live-drive of the refusal path; the per-tenant connection/consent model (Financial Integrations #6) + Marketplace bundle (#670) before any provider can be reached. | **No.** Internal server-side honesty/enforcement slice; nothing activated, package `UNAVAILABLE`; no customer release record.
