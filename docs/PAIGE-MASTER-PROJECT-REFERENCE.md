...
**2026-09-20 Per-thread PAIGE composer drafts — INT-102 (PR [#1308](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1308); exact code head [`999f3794`](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/commit/999f37946f5ee1b45b7de4677a0c38157248a108); classification **patch**; guarded delivery under owner standing authority **INT-083**; this pre-merge row becomes a shipped record only when #1308 merges):**

Repairs draft ownership without changing the PAIGE composer design: session-memory drafts are keyed by server-resolved tenant + effective user + real thread id, with a stable client/mission-qualified `new-chat` slot that moves to the persisted id on first thread creation. Thread/New-chat/account/user switches resolve only the destination key; returning restores the origin. A successful composer send clears only its submitted origin; authentication, transport, non-2xx, cancel, and stale-scope exits retain it. Signed-out context renders no draft, a different user cannot inherit it, and same-user recovery lasts only for the still-running module session; reload persistence is explicitly out of scope. Typed and dictated text share the same seam; #1298 scope teardown/late-callback guards remain unchanged. `PaigeAIChat` and AppShell `PaigeChat` are covered. `ConversationsRichComposer` itself is controlled, but its parent `ClientsConversations` still uses one clear-on-selection body; per owner boundary that separate proposal is not folded in. Proof: failing-first **10/10 red**, restored **10/10 green**, focused+remount **11/11**, load-bearing key mutation **5/6 red**, tsc ratchet **12→12**, production build green, ESLint zero errors. Affected regression **63/64** with the only red inherited from base: `SoloPaigeWorkspace.contract` rejects `_tenant_id:` already present in untouched `SoloApp.tsx`; both files have zero INT-102 diff. No visual/layout, attachment, provider, Edge, `_shared`, database, migration, capability, or deferred INT-086 Draft-with-Paige change. Evidence: `docs/evidence/ui-delivery/int-102-per-thread-paige-drafts.md`. | **DELIVERY GATE IN PROGRESS at row authoring.** Exact current-main sync, exact-head CI, one requested Codex review, the Ready-triggered auto review, finding disposition, pinned merge, and Vercel production identity remain required. | **PROOF OWED:** authenticated production Solo/AppShell thread switching, New chat, account/user isolation, send success/failure, dictation-origin retention, and sign-out/re-login behavior. | **No.** Existing-flow reliability repair; no new customer capability or release identity.
