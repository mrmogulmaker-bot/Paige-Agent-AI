...
- **2026-08-30 · Calendar PR #642 recorded PRE-merge — three corrections, and a BLOCKED (not failed) go-live.** Owner-supplied verified state, recorded so no later session infers a merge, a deployment, an authenticated acceptance, or a backend proof from a green preview or a green CI run.
  - **Production baseline: `main` = `83739ea161e74f80bccd9765f6086519ccfba4d6`** — confirmed by CC against `origin/main`; the tip commit is *"fix(solo): restore Systems Check drawer focus (#645)"*. #645 is **MERGED**, its production deployment reached **READY**, and its authenticated First Sterling Capital Systems Check verification **PASSED** (owner-reported — CC did not drive that authenticated check and does not claim it). **Do not reopen #645** unless a new defect is independently confirmed.
  - **PR #642 status: DRAFT · OPEN · UNMERGED · UNDEPLOYED.** Exact head `fca04110c91befa8b7d0187f59af815a84255b57`, tree `c90e0d11`, base/current main at last exact-head verification `83739ea1`. Scope: **exactly NINE Calendar-owned files** — no Settings, A2P, Conversations, Mind, Marketplace, Systems Check, shared shell, migration, provider or lockfile change. On that exact head: Verify **PASS** · Security Audit **PASS** · Vercel Preview **READY** · Supabase Preview **SKIPPED, correctly, because no Supabase files changed** · mergeability **clean** · blocking reviews **none**.
  - **BLOCKED, not defective.** The sole blocker is GitHub's **GraphQL rate limit** on the draft→"Ready for review" transition. No ready transition, no merge, and no production deployment has occurred. This is an **API capability delay, not a Calendar defect** — do not re-diagnose it as one.
  - **CORRECTION 1 — a prior report naming head `587ca463` with a 16-file scope was from the WRONG, older Calendar worktree** (`wt-cal`, branch `claude/solo-calendar-native`, the #635 checkout). It does **not** describe PR #642. The correct #642 checkout is `wt-calfix` at `fca04110`, **exactly nine files**. The branch was never in the state that report described.
  - **CORRECTION 2 — a local form-fit harness first returned a FALSE GREEN** because a stale server still occupied its port and served an older tree while the new harness had exited non-zero. The stale server was stopped and the proof was **rerun against the correct harness**. A `curl` 200 on a dev port is not proof that *your* build is the one answering.
  - **CORRECTION 3 — a dev-only `MODULE_NOT_FOUND`** occurs **only** when the harness and the browser drive are started within the same shell invocation; three standalone runs were clean. It is **NOT confirmed product behavior** and **NOT a release blocker**. An earlier "startup race" label was withdrawn as unproven.
  - **OWED:** authenticated **First Sterling Capital · Solo Calendar** verification, after release. The session holding this work had **no scoped test credentials** and must not claim that drive happened (§32.c capability-conditional degrade).
  - **BOUNDARY:** Calendar work remains **Clients-owned**. It reuses existing booking/contact references and must **NOT** create a separate Calendar memory, autonomy, or action store (§18 one home).
  - **PR #646 (Calendar↔spine contract): DRAFT · OPEN · UNMERGED · DOCS ONLY.** It *documents* the contract. It does **not** implement or authorize Calendar writes, Trust Compass enforcement, Brain access, or provider changes, and it must remain **separate from #642**.
  - **Shared rails — extend, never fork (read-only audit, same date).** Paige already has a real foundation: **Context Rail** (durable, tenant-scoped evidence + provenance) · **Action Bus** (existing action/workflow/approval/worker lifecycle) · **Trust Compass** (a real platform-level safety posture today, but **NOT YET a server-enforced tenant/capability clamp** — a stored default autonomy value is **not** enforcement) · **Memory/knowledge** (a separate tenant/client substrate — never exposed wholesale as a UI rail) · **PAIGE** (ONE governed workspace, addressed via safe **opaque references**, never raw evidence, messages, prompts, payloads or hidden reasoning) · **Brain** (derived only from proven, tenant-safe records). **Standing rule: reading precedes action.** Every Paige write must be server-authorized for tenant, account, capability, source and scope; durable; auditable; idempotent; recoverable; and **fail closed** on missing, stale, wrong-account, cross-tenant, or insufficient-authority conditions.
  - **OUTCOME (same day, later): #642 MERGED, DEPLOYED and DEPLOYMENT-INTEGRITY VERIFIED.** The GraphQL block on the draft→ready transition cleared; the PR was squash-merged at the authorized exact head `fca04110` (tree `c90e0d11`) onto base `83739ea1`, merge commit **`8ceda26e3e5144342709a3cce90f5cd77806c0ee`**, source branch **retained**. Production deployed: entry `index-Ui1VrVgZ` → `index-BEVu8cX1`, vendor chunks unchanged. **Verified live, not assumed:** the deployed lazy Calendar chunk `useSoloOwner-BEGRua1Y.js` still carries `sc-board`/`sc-truth` (right-chunk sanity) AND all three #642 markers moved **0 → 1** — *"Calendar may be out of date"*, *"We could not refresh the latest booking changes"*, *"Who is booked"* — plus `class_session_id` and `class_seat` **0 → 1** (the fold). The Calendar CSS chunk `useSoloOwner-BrJuY4Vx.css` is **byte-identical** to the merged build (sha256 `831d2075…`), carrying `.sc-truths`/`.sc-truth--stale`/`.sc-stale-retry` — so the grid-row fix that made Retry clickable shipped, not just the markup referencing it. **§13 method note:** production IS reachable from a headless session (contradicting an earlier assumption), so deployment integrity is genuinely provable here; the Calendar code lives in a LAZY chunk, so grepping the entry `index-*.js` would read 0 both before and after and prove nothing. **§13 correction:** a chunk name that 404s to the SPA fallback (HTTP 200, byte-identical to `index.html`) was first reported as a phantom bundle reference — the real cause was a CSS chunk basename with `.js` appended by the resolver regex. The fallback screen was the right guard regardless. **STILL OPEN, not a clean bill of health:** a Codex review triggered on `fca0411` was still *Running* when the merge closed the PR; its outcome is UNKNOWN and is being collected — any finding becomes a NEW follow-up PR off current `main`, never a reopen. **STILL OWED:** the authenticated First Sterling Capital · Solo Calendar drive — credentials unset, surface auth-gated; deployment integrity is proven, the authenticated render is not.
  - **Dependencies + boundaries recorded with it:** **PR #643** remains the migration-history replay / Preview dependency for any work needing real Supabase replay or Preview proof. **Mind safe rail, Marketplace, and Systems Check governed-write work are NOT production-ready merely because their UI/prototypes or local tests are green.** **Settings** owns A2P, business phone, provider readiness, billing and configuration; **Conversations** consumes only **named, proven** readiness. **Analytics** binds to real metric sources and evidence contracts — the Context Rail is **not** a substitute for analytics truth.
