...
**2026-09-20 Agent merge-gate standing rule + PR-A2 closeout (PR [#1296](https://github.com/mrmogulmaker-bot/Paige-Agent-AI/pull/1296); base `main` @ `6757e487`; last Codex-reviewed head before this row-only commit `8a8b9e10`; release channel **development** (docs / agent-governance, no deployable code); classification **internal-only** (repository agent policy + delivery-log closeout); delivered under owner standing authority **INT-083**; this pre-merge row becomes a shipped record — with the verified `main` merge commit — when #1296 merges):**

Adds a new **"Merge gate"** section to `AGENTS.md` binding the coordinator standing rule for every implementation agent — after marking a PR Ready, wait for the auto-triggered exact-head Codex review to complete before merging; disposition every finding; "one review" = one requested + one auto; never merge ahead of either; docs-only closeouts wait too. Because that is a repository **policy** addition (not a mechanical record), this PR is logged as its own delivery rather than under the `AGENTS.md` L91 closeout-only exemption (Codex P1). It ALSO carries the PR-A2 (#1294) closeout — the decision-log SHIPPED entry and the #1294 §4.0 row below. | **MERGE PENDING at authoring.** Docs / `.md` only; **zero `supabase/` diff** (Supabase Preview skipped), no migration, no edge (`edge-live` unmoved `c7e386a3`, `db-live` unmoved). Merge gate applied to this PR: marked Ready → exact-head Codex review returned two P1 governance findings (this delivery-log row + the #1294 row's release channel) → both fixed in one round → re-review + merge to follow. `verify` red = inherited baseline subset only; the non-required `github-advanced-security` red is the known repo-wide Copilot-Autofix infra crash (empty output, unrelated to this docs diff), non-blocking. | **PROOF OWED: none** — documentation / agent-governance only, no runtime or customer surface. | **No.** Repository agent-policy + closeout record; no customer-facing capability, provider, flow, or release identity.
