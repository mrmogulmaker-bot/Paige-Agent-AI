...
#### Production-version and duplication audit — 2026-09-08 01:13 EDT

| Category | Verified finding | Disposition |
|---|---|---|
| Stale deployed version | **None found after remediation and current-main refresh.** `paigeagent.ai/version.json` and `app.paigeagent.ai/version.json` both return build `2a88b5dc2650816cdd05b5ba5ac924f8f15e8ccc-mts7ny03` with `customerUpdate: null`. Vercel production deployment `dpl_9q9BESpDsD3Qa1b26kZgyccvNUfJ` is `READY` from exact canonical `main` SHA `2a88b5dc2650816cdd05b5ba5ac924f8f15e8ccc` and aliases both domains. This is #1056's documentation-only closeout descendant of #1057; the durable public-route proof and product code are unchanged. | Current production and canonical `main` match. The #1054 route correction, #1053 Live Conversation delivery, and #1057 chat-scroll correction remain in the current documentation-only build's ancestry. No manual promotion or rollback was needed. Public `track-event` requests emitted CORS/401 console errors on both domains during route proof; route behavior was unaffected, but telemetry ownership requires separate diagnosis. |
...
#1057 and refreshes the production identity; it likewise changes no capability, proof, or release
