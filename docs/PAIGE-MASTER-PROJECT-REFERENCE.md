...
**2026-09-12 Solo Calendar — owner RESCHEDULE + EDIT wired to the live governed booking engine (branch `claude/busy-archimedes-bsvrdi`, PR #1176, owner-approved Gate A) — ⏳ PENDING-MERGE, NOT YET SHIPPED:** this row is recorded in the delivery PR per §0; the exact merge SHA + `deploy-migrations` ledger proof are filled in by the post-merge closeout (a shipped-ledger row is not "shipped" until persisted-apply is proven, §32). Until then treat this as in-flight, not live.
