...
| **And the two sources can DISAGREE, so precedence is declared: the MOST RESTRICTIVE state wins** | Normalising each value said how to translate a status and not which status to translate. The two are written by different seams — `operator_set_tenant_status` (`20260804150000_operator_fleet_seam.sql:28`) moves `tenants.status` alone, while `customer.subscription.updated` moves `platform_subscriptions.status` alone — so tenant `suspended` with subscription `active`, and tenant `active` with subscription `unpaid`, are both reachable states. Without a rule, two compliant implementations report **opposite** answers for the same workspace, and the wrong one is `active` for a workspace that is suspended or in dunning. Precedence, most restrictive first: `suspended` › `canceled` › `past_due` › **`unsupported_source_value`** › `trialing` › `active`. **And the ordering alone was not the whole rule** — it ranked the five recognised states and said nothing about the two this same row enumerates, so one source reading gibberish while the other read `active` had no defined answer and a compliant implementation could report `active`, contradicting the sentence two lines above it. The rule is therefore three clauses, because absence and disagreement are different questions: (1) **both sources absent** → `unavailable_no_source`; (2) **exactly one source present** → that source's normalisation stands, including `unsupported_source_value` — there is no disagreement to resolve, and a lone uninterpretable value still never becomes `active`; (3) **both present** → most restrictive by the ordering above. `unsupported_source_value` sits below the three restrictive states and above the two permissive ones for one reason: unknown must never silently GRANT access, but a source that says `suspended` is both more restrictive and more informative than one that says nothing intelligible. **This is not my invention either** — the shipped revenue/health query already resolves the same disagreement the same way, flagging dunning when *either* source says so (`20260814300000…:53-54`, an `OR`). The rule here is that one generalised, not a new one. Defaulting an unrecognised billing status to "active" is the failure mode this row exists to prevent. |
...
| **The three notice vocabularies are ENUMERATED, and NOT borrowed from the messaging tables** | `category`: `payment_failed` · `payment_succeeded` · `plan_changed` · `trial_ending` · `renewal_upcoming` · `account_at_risk` · `unavailable_no_source`. `topic_label`: derived one-to-one from the category, so it adds no vocabulary of its own — that is the whole reason it is category-derived (§3.2). `delivery_state`: **reused from `email_send_log`** — `pending` · `sent` · `suppressed` · `failed` · `bounced` · `complained` **and `dlq`** (`20260318203215_email_infra.sql:32` — all seven), plus `unavailable_no_source`. |
| **`dlq` is a terminal OUTCOME, and excluding it was the same defect as inventing a value** | An earlier revision dropped `dlq` from the reused set as “infrastructure, not an outcome an owner reads.” That confused the QUEUE with the LOG ROW. `moveToDlq` writes `email_send_log.status = 'dlq'` (`process-email-queue/index.ts:56-69`) for exactly three conditions — **TTL expiry** (`:200-214`), **max retries exceeded** (`:221`) and a **permanent 403** (`:327-330`) — which are the most permanent failures the system has and the ones an owner most needs to see. Excluding it left the adapter two bad options and no good one: omit the row, and a notice that will never arrive silently vanishes from the history (a §32 false-green shaped like an answer); or emit it, and `safeFacts` rejects an unenumerated value and collapses the **entire read** to `resolver_unavailable`. **I am not taking the normalisation offered** — mapping `dlq` → `failed` — because `failed` is separately written by other seams (`send-transactional-email/index.ts:419`, `auth-email-hook`, and `process-email-queue` itself), so the collapse would erase a distinction the shipped log actually draws: *may still arrive* versus *never will*. For a billing notice that is the distinction that matters. Inventing a synonym like `undeliverable` is the mistake this document already made twice, so the vocabulary is reused WHOLE instead. The objection to the word `dlq` is real and is a RENDERING concern — the Mind turns facts into sentences (§6) — and dropping a state to avoid a word is how the state gets lost. |
| **I very nearly invented this vocabulary too, and the correction is worth keeping** | An earlier revision declared `pending · delivered · failed · suppressed` and justified it by saying no existing vocabulary fits — citing `messages.status` (seven values including `draft`, `received`, `read`) and `operator_messages.status`. Both citations are true and both are the WRONG comparison: they are *conversation* tables. `email_send_log` is a **delivery log**, which is precisely what a notice ledger is, and it was never checked. Two consequences of not checking: I wrote `delivered` where the shipped log says `sent`, and I **omitted `bounced` and `complained` entirely** — so a bounced billing notice would have collapsed into `failed`, losing the one distinction that tells an owner their billing address is dead rather than the send being flaky. Found by grepping my own declared values against the repository after `unpaid` showed that the same mistake had already happened twice. |
...
| **And the cohort is per-AGGREGATE, not workspace-wide — my fix for the over-count under-scoped in the other direction** | Replacing the ledger window with B-2's eligible set fixed the over-count and introduced its mirror image. B-2 identifies who is *generally* eligible; **B-4 owns which recipient each notice actually went to** (it is per-recipient by contract, §4). Threshold the general set and an owner-only notice re-identifies its recipient exactly: five people eligible for billing mail clears a minimum of 5, one of them is the only person a given notice targeted, and `single` then names that person's delivery outcome — the precise re-identification the suppression exists to prevent, arrived at from the safe-looking side. So the cohort is the **current** recipients eligible for *or actually targeted by* the notices included in **this** aggregate, intersected — never the workspace-wide recipient set. That needs B-2 (who is eligible NOW) **and** B-4 (who each included notice targeted); either alone gives a cohort that is wrong in one direction. **This is the third time this one threshold has been defined wrongly** — a ledger window that over-counted, a general set that under-scoped, and before both a bare “5 eligible recipients” that said neither. Each fix was correct about the defect it was answering and silent about the direction it opened, which is why it is written here as an intersection rather than as a third choice of set. |
