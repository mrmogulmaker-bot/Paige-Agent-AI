...
| **Safe projection** | plan/access state · promotional-trial-paid classification *(subject to the open decision in §9)* · renewal or trial-end state **where truly available** · a bounded next-action state (see the row below — it has a named producer or it is not projected). |
...
| **The trial-end state is a BOUNDED state, and the window is 7 DAYS** | Projected from a fixed set — `active` · `ends_within_declared_window` · `ended` · `unavailable_no_source` (§3.0) — where the window is **7 days**, stated here rather than deferred. Naming it "a declared constant" without declaring it left the exact nondeterminism the band was introduced to remove: for a trial ending in ten days, two implementations still could not agree. 7 days is a **default chosen so the contract is decidable**, not a product ruling; if Billing's B-1 contract names a different figure, that one wins — the requirement is that exactly ONE value exists and it is never a downstream choice. |
...
| **The failure band is enumerated, and it is ALWAYS a band** | `none` · `single` · `few` (2–5) · `many` (6 or more) · `unavailable_no_source`. §3.0 makes a scalar's *unavailability* representable but says nothing about what that scalar's other values are, and `safeFacts` accepts only what `factValues` literally lists — so an unenumerated count is not merely imprecise, it is **unrepresentable**, and two implementations could pick incompatible bands while both claiming compliance. **And it is a band in every case, not only where an exact figure would identify someone.** The row below originally banded conditionally, which asks the runtime to make an undecidable re-identification judgement on every read. Always banding is simpler, strictly safer, and removes the judgement. |
