...
| **The three notice vocabularies are ENUMERATED, and NOT borrowed from the messaging tables** | `category`: `payment_failed` · `payment_succeeded` · `plan_changed` · `trial_ending` · `renewal_upcoming` · `account_at_risk` · `unavailable_no_source`. `topic_label`: derived one-to-one from the category, so it adds no vocabulary of its own — that is the whole reason it is category-derived (§3.2). `delivery_state`: **reused from `email_send_log`** — `pending` · `sent` · `suppressed` · `failed` · `bounced` · `complained` (`20260318203215_email_infra.sql:32`), plus `unavailable_no_source`. `dlq` is excluded: a dead-letter queue is infrastructure, not an outcome an owner reads. |
