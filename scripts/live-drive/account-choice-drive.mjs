...
const OUT = path.resolve("docs/evidence/ui-delivery/platform-account-choice-pause");
...
try {
  ...
  await scenarioPage("One-account ordinary user", "/choose-account?case=one", async (page) => {
    ...
    record("One-account ordinary user enters sole direct workspace", page.url().includes("/solo/111111/command-center"));
  });
  ...
  const report = {
    ...
    evidenceClass: "Real ChooseAccount component and production styles; deterministic local authorization adapter; not authenticated production proof",
    viewports: VIEWPORTS,
