...
if (url.hostname !== '127.0.0.1' || !['5203','5213'].includes(url.port)) throw new Error('This mutating fixture drive is restricted to the local Integration harness.');
...
try{
 ...
 for(const theme of ['dark','light'])for(const [width,height] of [[1536,770],[1366,768],[1024,768],[900,1000]]){
  ...
  check(await page.locator('.ig-n8n-summary').innerText().then(t=>t.includes('API connection')&&t.includes('Paige tools (MCP)')&&t.includes('MCP OAuth authorization needed')),tag+' independent overview');
  ...
  check(await page.getByRole('dialog').locator('input').count()===0,tag+' no MCP credential form');check(await page.getByRole('button',{name:'Reconnect authorization',exact:true}).count()===1,tag+' OAuth reconnect available');await page.screenshot({path:path.join(out,tag+'-mcp.png')});
 ...
 for(const data of ['api-error','mcp-error','error','empty','readonly','connected','saved-only']){
  await open(page,data);check(await page.getByRole('dialog').count()===1,data+' drawer reachable');
  if(data==='api-error')check(await page.locator('.ig-n8n-summary').innerText().then(t=>t.includes('MCP OAuth authorization needed')),data+' MCP retained');
  ...
  if(data==='connected')check(await page.locator('.ig-n8n-summary').innerText().then(t=>t.includes('Connected')),'verified API connected');
  if(data==='saved-only')check(await page.locator('.ig-n8n-summary').innerText().then(t=>t.includes('Needs attention')),'saved unverified never green');
 }
 await open(page,'broken');await page.getByRole('button',{name:'Disconnect API',exact:true}).click();await page.getByRole('button',{name:'Keep connection',exact:true}).click();check(await page.getByRole('button',{name:'Disconnect API',exact:true}).count()===1,'API disconnect cancel');await page.getByRole('button',{name:'Disconnect API',exact:true}).click();await page.getByRole('button',{name:'Confirm disconnect',exact:true}).click();await page.getByRole('button',{name:'Connect API',exact:true}).waitFor();check(await page.locator('.ig-n8n-summary').innerText().then(t=>t.includes('MCP OAuth authorization needed')),'API disconnect preserves MCP');
 await open(page,'pending');await page.getByRole('button',{name:/Reconnect API|Edit API connection/}).click();await page.locator('.ig-form input[type=password]').fill('fixture-only');await page.getByRole('button',{name:/^Save/}).click({noWaitAfter:true});check(await page.getByRole('status').filter({hasText:'Saving and checking this connection'}).count()===1,'explicit save checking feedback');check(await page.locator('.ig-card[data-provider="n8n"]').innerText().then(t=>t.includes('Checking')),'tile shares pending API state');await page.evaluate(()=>window.dispatchEvent(new Event('n8n-harness-switch')));check(await page.getByRole('dialog').count()===0,'workspace closes pending drawer');await page.evaluate(()=>window.dispatchEvent(new Event('n8n-harness-finish')));await page.locator('.ig-card[data-provider="n8n"]').click();check(await page.getByRole('button',{name:'Connect API',exact:true}).count()===1,'late save cannot populate new workspace');check(await page.getByRole('dialog').locator('input[type=password]').count()===0,'workspace clears secret draft');
 // Complete owner journey: enter, save+validate, view verified zero, then reopen without secrets.
 await open(page,'empty');await page.getByRole('button',{name:'Connect API',exact:true}).click();await page.locator('.ig-form input[type=url]').fill('https://fixture.example');await page.locator('.ig-form input[type=password]').fill('fixture-only');await page.getByRole('button',{name:/^Save/}).click();await page.locator('.ig-form').waitFor({state:'detached'});
 check(await page.locator('.ig-n8n-summary').innerText().then(t=>t.includes('Connected')&&t.includes('Not connected')),'API success does not authorize MCP');
 check(await page.locator('.ig-facts').innerText().then(t=>t.includes('0 workflows available')&&t.toLowerCase().includes('last successful check')),'verified zero and check time shown');
 check(await page.getByRole('dialog').innerText().then(t=>!t.includes('fixture-only')),'submitted key never echoed');
 await page.screenshot({path:path.join(out,'api-save-validated-zero.png')});await page.getByRole('button',{name:'Close n8n',exact:true}).click();check(await page.locator('.ig-card[data-provider="n8n"]').innerText().then(t=>t.includes('Connected')&&t.includes('Not connected')),'outer tile refreshes both independent outcomes');
 for(const data of ['save-refused','permission-refused','endpoint-error','provider-error']){
  await open(page,data);await page.getByRole('button',{name:'Reconnect API',exact:true}).click();await page.locator('.ig-form input[type=password]').fill('fixture-only');await page.getByRole('button',{name:/^Save/}).click();await page.locator('.ig-form').waitFor({state:'detached'});check(await page.getByRole('button',{name:'Reconnect API',exact:true}).count()===1,data+' same-drawer reconnect');check(await page.locator('.ig-n8n-summary').innerText().then(t=>t.includes('Needs attention')),data+' no false green');check(await page.getByRole('dialog').locator('input[type=password]').count()===0,data+' submitted key cleared');
  if(data==='save-refused')check(await page.getByRole('dialog').innerText().then(t=>t.includes('The saved n8n connection needs attention. Check the API key or its access in n8n, then reconnect.')),'exact refusal guidance');
 }
 ...
 await page.emulateMedia({forcedColors:'active'});check(await page.locator('.ig-n8n-tabs button[aria-selected="true"]').evaluate(e=>getComputedStyle(e).borderTopWidth)==='1px','forced-colors selected tab remains visible');
