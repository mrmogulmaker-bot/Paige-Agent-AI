 ...
 *   C · set individual vs round-robin, and say WHO is in the rotation
...
const result = await liveDrive({
  ...
  steps: async (page) => {
    ...
    if (!WRITES) {
      ...
      // C2 · the roster the rotation actually follows. Round-robin without this
      // is a dead control, so it is proven separately from the type itself.
      const team = await openArea(page, "Team & hosts");
      const picker = team.locator('select[aria-label="Teammate to add as a host"]');
      if (await picker.count()) {
        const opts = await picker.locator("option").evaluateAll((els) => els.map((e) => e.value).filter(Boolean));
        if (opts.length === 0) {
          record("C2 · say who takes the bookings", "UNVERIFIED",
            "everyone available is already a host on this calendar — nobody left to add");
        } else {
          const before = await team.locator(".cc-host").count();
          await picker.selectOption(opts[0]);
          await team.locator("button", { hasText: /Add host/i }).first().click();
          await page.waitForTimeout(1500);
          await page.goto(connectionsUrl(ACCOUNT), { waitUntil: "networkidle" });
          const after = await openArea(page, "Team & hosts").then((a) => a.locator(".cc-host").count());
          record("C2 · say who takes the bookings", after === before + 1 ? "PASS" : "FAIL",
            `hosts ${before} → ${after} after reload`);
          // Put the roster back: this drive does not leave a real calendar changed.
          const undo = await openArea(page, "Team & hosts");
          const remove = undo.locator('button[aria-label^="Remove "]').last();
          if ((await remove.count()) && !(await remove.isDisabled())) {
            await remove.click();
            await page.waitForTimeout(1200);
          }
        }
      } else {
        record("C2 · say who takes the bookings", "FAIL", "no way to add a host is offered");
      }
