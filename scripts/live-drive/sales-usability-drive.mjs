...
//
// KNOWN BROKEN ON origin/main, not by the Agreements change: `open()` waits for a base-view
// 'Quick offer' button and times out. Verified on a clean origin/main worktree — identical
// failure, same selector. Two things this drive will need when it is repaired: the terms view
// must be reached first (the harness opens on Sales Command; click `terms:populated`, then the
// 'Commercial Terms' tab, then wait for 'Record terms'), and Quick offer is no longer on the
// band — it moved INSIDE the agreement editor by owner ruling 2026-09-22, so it is reached as
// 'Record terms' then 'Quick offer'. The band's aria-label below is already updated.
...
await page.reload({waitUntil:'domcontentloaded'});await page.locator('[aria-label="Agreements and terms"] .so-row').first().click();await page.getByRole('button',{name:'Edit commercial terms'}).click();await page.getByPlaceholder('Anything you want to remember about this arrangement').fill('Edited in real Chromium');await page.getByRole('button',{name:'Save changes'}).click();await page.getByRole('dialog').waitFor({state:'detached'});await page.reload({waitUntil:'domcontentloaded'});await page.locator('[aria-label="Agreements and terms"] .so-row').first().click();await page.getByText('Edited in real Chromium',{exact:true}).waitFor();await page.getByRole('dialog').getByRole('button',{name:'Close details',exact:true}).click();
