...
const validContextualReplacement = (path, value) => {
  if (/^\/whats_new\/status\/(?:0|[1-9]\d*)$/.test(path)) return CUSTOMER_STATES.has(value);
  if (/^\/internal_builds\/(?:0|[1-9]\d*)\/(?:migration_status|edge_status)\/state$/.test(path)) return DELIVERY_STATES.has(value);
  return typeof value !== "string" || !isUnresolvedValue(value);
};
...
if (invokedDirectly() && process.argv.includes("--self-test")) {
  ...
  const cases = [
    ...
    ["accepts internal NOT_APPLICABLE correction replacement", { ...valid, record_state: "CORRECTED", history: { supersedes_record_id: "release-0.0.9", reason: "Corrected migration applicability", corrected_values: [{ field_path: "/internal_builds/0/migration_status/state", previous_exists: true, previous_value: "APPLIED", replacement_exists: true, replacement_value: "NOT_APPLICABLE" }] }, customer_release_identity: { ...valid.customer_release_identity, owner_approval: customerApproval } }, false],
    ["rejects arbitrary resolved internal delivery status", { ...valid, record_state: "CORRECTED", history: { supersedes_record_id: "release-0.0.9", reason: "Corrected migration state", corrected_values: [{ field_path: "/internal_builds/0/migration_status/state", previous_exists: true, previous_value: "APPLIED", replacement_exists: true, replacement_value: "READY" }] }, customer_release_identity: { ...valid.customer_release_identity, owner_approval: customerApproval } }, true],
  ...
  const schemaCases = [
    ...
    ["canonical schema accepts NOT_APPLICABLE for internal delivery state", { ...correction, history: { ...correction.history, corrected_values: [{ field_path: "/internal_builds/0/migration_status/state", previous_exists: true, previous_value: "APPLIED", replacement_exists: true, replacement_value: "NOT_APPLICABLE" }] } }, false],
    ["canonical schema rejects arbitrary resolved internal delivery status", { ...correction, history: { ...correction.history, corrected_values: [{ field_path: "/internal_builds/0/migration_status/state", previous_exists: true, previous_value: "APPLIED", replacement_exists: true, replacement_value: "READY" }] } }, true],
    ["canonical schema rejects arbitrary resolved customer status", { ...correction, history: { ...correction.history, corrected_values: [{ field_path: "/whats_new/status/0", previous_exists: true, previous_value: "LIVE", replacement_exists: true, replacement_value: "READY" }] } }, true],
