...
function requireDeliveryState(value, label, findings) {
  ...
  if (value?.state === "PROOF_OWED") {
    ...
    for (const field of ["boundary", "excluded_from_live_claim"]) {
      ...
      if (!nonEmpty(detail) || /^(?:todo|tbd|placeholder|replace_me|none|n\/?a|not applicable)$/i.test(detail)) findings.push(`${label}.proof_owed.${field} must precisely name the owed boundary`);
    }
...
if (invokedDirectly() && process.argv.includes("--self-test")) {
  ...
  const cases = [
    ...
    ["rejects placeholder proof-owed boundary", { ...valid, record_state: "PUBLISHED", customer_release_identity: { ...valid.customer_release_identity, owner_approval: customerApproval }, internal_builds: [{ ...build, edge_status: { state: "PROOF_OWED", evidence: ["authenticated proof pending"], proof_owed: { boundary: "TBD", excluded_from_live_claim: "TODO" } } }], whats_new: { ...valid.whats_new, status: ["LIVE", "PROOF OWED"], known_limitations: "None" } }, true],
