 ...
 *
 * Vitest covers `src/**` only and no edge function carries a test, so this is the
 * §32 smoke for the module that decides WHICH TENANT a newly-connected mailbox,
 * calendar, Zoom account or SMTP server is attached to. Getting it wrong is not a
 * cosmetic bug: too permissive is a §9 cross-tenant write, too strict is the
...
await check("resolves the primary tenant when no preference is given", async () => {
...
await check("honours a signed workspace preference the user is a member of", async () => {
  const c = client({
    primary: PRIMARY,
...
await check("REFUSES a workspace preference the user is not a member of", async () => {
...
await check("REFUSES a workspace the user's membership is not active in", async () => {
  const c = client({
    primary: PRIMARY,
...
await check("surfaces a membership-read failure instead of swallowing it", async () => {
...
await check("returns a clean null for a user who belongs to no tenant", async () => {
...
await check("distinguishes an RPC failure from an honest absence", async () => {
...
await check("refuses an empty user id without querying", async () => {
  const c = client({ primary: PRIMARY });
...
console.log(`resolveTenantForUser smoke: ${results.length} checks passed`);
