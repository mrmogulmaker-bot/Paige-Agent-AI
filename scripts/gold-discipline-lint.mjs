...
function lintPaths(paths) {
  ...
  for (const file of files) {
    // §63 faithful-port exemption: src/solo/** and src/agency/** are the byte-faithful ports
    // of the owner-locked Claude Design packs ("keep it as is, do not redesign in any fashion").
    // Their gold usage is the vendor's approved design, not our primitive-layer surfaces — our
    // gold-discipline retrofit does not apply. (Frozen approved designs, §28-adjacent.)
    if (/(^|\/)src\/(solo|agency)\//.test(file.replace(/\\/g, "/"))) continue;
    let src;
