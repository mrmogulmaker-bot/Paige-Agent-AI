  ...
  if (variant === "hero") {
    return (
      <motion.section
        ...
        className={cn(
          "relative overflow-hidden rounded-[calc(var(--radius)+6px)] p-8 md:p-10 shadow-xl",
          className,
        )}
      ...
      >
        ...
        <div className="relative">
          ...
          <div className="flex items-center gap-2.5">
            {mark && <PaigeMark className="h-9 w-9" />}
            {eyebrow && (
          ...
          <div className="mt-4 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div className="min-w-0">
              <h1 className="max-w-2xl font-display text-3xl md:text-4xl font-semibold leading-[1.1] text-white text-balance">
                {title}
              </h1>
              {description && (
                <p className="mt-3 max-w-xl text-sm md:text-base text-white/70">{description}</p>
              )}
            </div>
