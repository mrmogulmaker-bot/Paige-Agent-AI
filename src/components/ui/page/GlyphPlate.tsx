 ...
 *
 * `ring` overrides that default for DECORATIVE uses (§11 — gold is never spent on a
 * resting/decorative surface): `ring="indigo"` rests the plate on an indigo hairline
 * (e.g. the ProjectCard cover glyph), `ring="gold-armed"` forces the armed gold ring.
 * Omit `ring` to keep the existing faint-gold-at-rest / bright-when-armed behavior
 * everywhere it already ships (Marketplace, PageHeader, SectionCard, StatTile).
 */
export function GlyphPlate({
  ...
  size?: "sm" | "md" | "lg";
  armed?: boolean;
  ring?: "indigo" | "gold-armed";
  className?: string;
}) {
  const dims = { sm: "h-9 w-9", md: "h-11 w-11", lg: "h-14 w-14" }[size];
  const iconDims = { sm: "h-4 w-4", md: "h-5 w-5", lg: "h-6 w-6" }[size];
  // `ring` wins when set; otherwise fall back to the armed/rest gold behavior.
  const ringCls =
    ring === "indigo"
      ? "ring-[hsl(var(--primary-light)/0.7)]"
      : ring === "gold-armed"
        ? "ring-[hsl(var(--gold)/0.6)]"
        : armed
          ? "ring-[hsl(var(--gold)/0.6)]"
          : "ring-[hsl(var(--gold)/0.25)]";
  return (
    <span
      className={cn(
