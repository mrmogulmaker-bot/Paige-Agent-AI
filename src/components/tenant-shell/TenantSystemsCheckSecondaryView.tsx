...
export function TenantSystemsCheckSecondaryView({ view }: { view: SecondaryView }) {
  ...
  const ordered = useMemo(
    () => snapshot.findings.slice().sort((a, b) => (a.priority ?? 999) - (b.priority ?? 999)),
    [snapshot.findings],
  );
  ...
  const proof = snapshot.isError
    ...
    : snapshot.loading
      ? "LOADING"
      : snapshot.run || snapshot.scanPending
        ? "PARTIAL"
        : "UNAVAILABLE";
      ...
      {snapshot.loading ? (
        ...
        ordered.length ? (
          ...
          <div className="tcc-secondary-state" role="status">
            <strong>{snapshot.scanPending ? "Your first scan is still in progress." : "No check directory is available yet."}</strong>
            <span>Checks appear after PAIGE records the first tenant-scoped run.</span>
          </div>
        )
      ) : snapshot.run ? (
        ...
        <div className="tcc-secondary-state" role="status">
          ...
          <span>The latest run appears here after it is persisted.</span>
        </div>
      )}
    </section>
  );
