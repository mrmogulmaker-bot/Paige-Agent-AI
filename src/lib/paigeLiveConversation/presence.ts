...
export function presenceFrame(state: PresenceState, seconds: number, sample: AudioEnergy = SILENT_ENERGY) {
  ...
  const settled = ["held", "unavailable", "disconnected", "interrupted"].includes(state);
  const pace = settled ? .22 : state === "thinking" || state === "working" ? .72 : .38;
  const t = seconds * pace;
  const radii = [96, 112, 69, 110, 93, 61, 102, 119, 74, 104, 71, 109];
  const points = radii.map((radius, i) => {
    const angle = i * Math.PI / 6;
    const r = radius + Math.sin(t + i * 1.7) * (settled ? 2.5 : 6) + energy * Math.sin(i * 2.3 + t) * 16;
  ...
  return { path: `${path}Z`, energy, light: .28 + energy * .65,
    drift: Math.sin(t * .63) * (settled ? 2 : 5), turn: -12 + Math.sin(t * .42) * 5,
