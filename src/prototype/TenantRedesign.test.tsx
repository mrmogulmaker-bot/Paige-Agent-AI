...
describe("tenant redesign shell",()=>{
  ...
  it("maps View calendar directly to Work Calendar, never Trust Compass",()=>{click("View calendar");expect(host.textContent).toContain("August 18–24");expect(host.querySelector(".drawer-trust")).toBeFalsy()});
  ...
  it("releases Calendar filter and inspector width independently",()=>{click("Work");click("Calendar");click("Hide context");expect(host.querySelector(".tr-calendar-body")?.classList.contains("filters-closed")).toBe(true);click("Hide inspector");expect(host.querySelector(".tr-calendar-body")?.classList.contains("inspector-closed")).toBe(true);click("Show context");click("Show inspector");expect(host.querySelector(".tr-calendar-body")?.classList.contains("filters-open")).toBe(true);expect(host.querySelector(".tr-calendar-body")?.classList.contains("inspector-open")).toBe(true)});
  it("selects distinct operational objects and updates the inspector honestly",()=>{click("Work");click("Calendar");clickContaining("ZION assembles CRM");expect(host.textContent).toContain("Agent currently executing");expect(host.textContent).toContain("no tenant run is queried");clickContaining("Approve delivery");expect(host.textContent).toContain("Named owner · Ask First");expect(host.querySelector('[aria-label*="Approve delivery"][aria-pressed="true"]')).toBeTruthy()});
