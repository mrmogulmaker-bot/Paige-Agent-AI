...
export default function ZapierOAuthCallback(){
 ...
 // NOT during render -- see McpOAuthCallback for the measurement. takeOAuthReturn clears as
 // it reads, and StrictMode's replayed render has fresh hook state, so a read in the body is
 // consumed by the throwaway pass and the committed one gets null. Effects run after commit.
 const[back,setBack]=useState<string|null>(null);const took=useRef(false);
 ...
 useEffect(()=>{if(phase.kind==="done"&&phase.healthy&&back)navigate(back,{replace:true});},[phase,navigate,back]);
