// @ts-nocheck
// Agency pack — PaigeBrain. FAITHFUL port of the Claude Design "CRM agency mode"
// decorative canvas orb ("paige-brain.js"), owner-locked 2026-08-17 (§28/§63:
// "We do not drift off this whatsoever"). Source: a `<paige-brain>` custom element
// that draws a spherical knowledge field in space — lat/long wireframe, six domain
// regions on the shell, a starfield with parallax, and the sub-accounts orbiting as
// satellites. The full draw() body is ported VERBATIM (it is self-contained canvas
// 2D math with no window-globals beyond the owning window, which we derive from the
// canvas).
//
// Web-component → React conversion only: connectedCallback/disconnectedCallback +
// customElements.define become a ref-based canvas + a useEffect that owns the draw
// loop and tears it down. NO unpkg/Babel/new Function/eval (CSP/§34).
//
// Motion-safe (§11/§22): the loop is driven by requestAnimationFrame and torn down
// with cancelAnimationFrame. When useReducedMotion() is true we render ONE static
// frame — no rAF loop is ever scheduled — and redraw only in response to direct
// user interaction (drag / wheel / hover / resize), never on a timer. The design's
// own `prefersReduced`/`motion` toggle is driven from the shared hook so the OS
// preference wins.
...
export const PaigeBrain = ({ style, className }) => {
  ...
  React.useEffect(() => {
    ...
    // `motion` is seeded from the OS preference: reduced-motion → static frame.
    let motion = !reduce;
    ...
    // In reduced-motion mode we never run a loop, so redraw on demand instead —
    // interaction and resize still repaint the static frame, but nothing animates.
    const repaint = () => { if (!motion) draw(); };

    let ro = null;
    try { ro = new win.ResizeObserver(() => { size(); repaint(); }); ro.observe(cv); } catch { /* ResizeObserver unavailable (older engine) — static frame still paints */ }

    const onDown = e => {
      const b = cv.getBoundingClientRect(), px = e.clientX - b.left, py = e.clientY - b.top;
      if (px >= btn.x && px <= btn.x + btn.w && py >= btn.y && py <= btn.y + btn.h) { motion = !motion; repaint(); return; }
    ...
    const onLeave = () => { mx = my = -1e4; drag = null; repaint(); };
    const onWheel = e => { e.preventDefault(); zoom = Math.max(0.7, Math.min(2.4, zoom * (e.deltaY > 0 ? 0.93 : 1.075))); repaint(); };
    ...
    // Motion-safe loop: rAF when animating, a single static frame when reduced.
    let running = true;
    const loop = () => {
      if (!running) return;
      draw();
    ...
    if (motion) {
      ...
      draw(); // one static frame, no rAF loop scheduled
    }
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "block", ...style }} className={className}>
      <canvas
        ref={canvasRef}
