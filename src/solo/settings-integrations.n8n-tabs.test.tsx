...
beforeEach(() => { h.tenant = "workspace-a"; h.user = "user-a"; h.loading = false; h.admin = true; h.apiError = false; h.mcpError = false; h.api = { configured: true, status: "error", workflow_count: 0, last_sync_at: "2026-09-03T12:00:00Z", base_url: "https://example.invalid" }; h.mcp = { configured: true, enabled: true, status: "error", auth_kind: "bearer", tool_count: null, approved_capabilities: [], pinned_count: 0 }; h.rpc.mockImplementation((name: string) => { if (name === "get_tenant_n8n_connection") return Promise.resolve({ data: h.api, error: h.apiError ? { message: "private provider error" } : null }); if (name === "get_tenant_mcp_connections") return Promise.resolve({ data: { n8n: h.mcp }, error: h.mcpError ? { message: "private provider error" } : null }); if (name === "is_current_user_tenant_admin") return Promise.resolve({ data: h.admin, error: null }); return Promise.resolve({ data: null, error: null }); }); h.invoke.mockResolvedValue({ data: { ok: true }, error: null }); });
...
it("does not describe an in-flight save as discardable or canceled by closing", async () => {
  ...
  h.rpc.mockImplementation((name: string, args: unknown) => name === "set_tenant_n8n_connection" ? new Promise(resolve => { finish = resolve; }) : ordinary(name, args));
