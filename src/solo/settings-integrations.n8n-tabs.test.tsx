...
const onlyZapierStatusReads = () => h.invoke.mock.calls.every(call => call[0] === "tenant-zapier-api-connect" && call[1]?.body?.action === "status");
...
describe("approved n8n independent connection tabs", () => {
 ...
 it("refreshes MCP through reads only", async () => { await mount(); await open(); await click(button("Paige tools (MCP)")); const before = h.rpc.mock.calls.length; await click(button("Refresh status")); expect(h.rpc.mock.calls.length).toBeGreaterThan(before); expect(onlyZapierStatusReads()).toBe(true); });
...
it("uses arrow keys for the two connection tabs without changing authority", async () => {
  ...
  expect(onlyZapierStatusReads()).toBe(true);
});
...
it("shows connected OAuth without approving or running workflows",async()=>{
 ...
 expect(host.textContent).toContain("Connected, with no approved workflows or tools");expect(button("Manage access")).toBeTruthy();expect(onlyZapierStatusReads()).toBe(true);
...
it("suggests only the active workspace API address for a new OAuth connection",async()=>{
 ...
 await mount();await open();await click(button("Paige tools (MCP)"));expect(host.querySelector<HTMLInputElement>('input[type="url"]')?.value).toBe("https://another-owner.example/mcp-server/http");expect(onlyZapierStatusReads()).toBe(true);
});
