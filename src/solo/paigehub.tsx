...
const SubAgents=()=>{const[tab,setTab]=React.useState('reg');const[list,setList]=React.useState(PT.subagents);
...
<div className="mono sub" style={{marginTop:7}}>Today: 2/10 proposals · 1 soft shipped · 0 local shipped</div></div>
<div className="row" style={{gap:9}}><button className="btn"><Ic.pulse size={15}/>Refresh</button>
...
{tab==='reg'&&<div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr)',gap:12}}>{list.map((a,i)=>
...
<span className="mono sub">{a.runs} runs</span>{a.ok!=null&&<span className="mono sub">{a.ok}% clean</span>}</div></div>
...
{tab==='prop'&&<div className="g3">{PT.proposals.map((p,i)=><div key={i} className="card" style={{padding:'16px 18px',display:'flex',flexDirection:'column',gap:10}}>
...
{tab==='test'&&<div className="card"><div className="hd"><div><h3>Test console</h3><div className="sub">Run a sub-agent against real data without shipping anything</div></div></div>
<div style={{padding:'16px 20px 20px',display:'grid',gridTemplateColumns:'minmax(0,1fr)',gap:12}}>
<div className="row" style={{gap:9,flexWrap:'wrap'}}><select className="btn" style={{paddingRight:26}}>{PT.subagents.map(a=><option key={a.n}>{a.n}</option>)}</select>
...
{tab==='act'&&<div className="card"><div className="hd"><h3>Activity</h3><button className="btn btn-s">Export log</button></div>
...
const Actions=()=>{const[f,setF]=React.useState('All');
const chips=['All','Filed','Assigned','Drafting','Drafted','Pending approval','Approved','Executing','Done','Blocked','Dismissed'];
const rows=PT.actions.filter(a=>f==='All'||a.st===f);
const tone=s=>s==='Blocked'?'pill-bad':s==='Pending approval'?'pill-warn':s==='Executing'?'pill-v':s==='Done'?'pill-ok':'pill-n';
return <div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr)',gap:16}}>
<div className="g3">{[['Open',PT.actions.filter(a=>!['Done','Dismissed'].includes(a.st)).length,'in flight on the bus','var(--violet)'],
['Needs attention',PT.actions.filter(a=>['Blocked','Pending approval'].includes(a.st)).length,'waiting on you','var(--warn)'],
['Completed today','6','shipped without you','var(--ok)']].map(([k,v,d,c],i)=>
<div key={i} className="card" style={{padding:'16px 18px'}}>
...
<button className="btn btn-s" style={{marginLeft:'auto'}}><Ic.pulse size={13}/>Refresh</button></div>
...
{!rows.length&&<div style={{padding:'40px 20px',textAlign:'center'}}><div className="sub">Nothing in this state.</div></div>}</div></div></div>};

const Skills=()=>{const[q,setQ]=React.useState('');const[tab,setTab]=React.useState('sk');const[list,setList]=React.useState(PT.skills);
const rows=list.filter(s=>s.n.toLowerCase().includes(q.toLowerCase())||s.slug.includes(q.toLowerCase()));
return <div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr)',gap:16}}>
<div className="g4">{[['Skills','117','in the catalog'],['Active','114','live and callable'],['Runs this week','86','92% clean'],['Proposals','2','she wrote them']].map(([k,v,d],i)=>
...
<div className="eyebrow">{k}</div><div className="tile" style={{width:30,height:30,borderRadius:'50%',background:'var(--rail)',color:'var(--gold-bright)'}}><Ic.bolt size={14}/></div></div>
...
{[['sk','Skills (117)'],['runs','Recent runs (86)'],['prop','Proposals (2)'],['forge','Forge new skill']].map(([k,l])=>
...
{tab==='sk'&&<div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr)',gap:12}}>{rows.map((s,i)=>
<div key={i} className="card" style={{padding:'16px 18px'}}>
<div className="row" style={{gap:12,alignItems:'flex-start',flexWrap:'wrap'}}>
...
{s.ro&&<span className="pill pill-ok">read-only</span>}<span className="pill" style={{background:'var(--violet-tint)',color:'var(--violet)'}}>{s.cat}</span></div>
<p style={{fontSize:13,color:'var(--ink-2)',marginTop:7,lineHeight:1.6,maxWidth:760}}>{s.d}</p>
<div className="row" style={{gap:8,marginTop:9,flexWrap:'wrap'}}><span className="mono sub">{s.runs} runs · {s.ok} clean</span>
<span className="sub">triggers:</span>{s.trig.map(t=><span key={t} className="pill pill-n">{t}</span>)}</div></div>
...
{tab==='runs'&&<div className="card">{[['Verify Deployed Surface','clean','2m 14s','12m ago'],['Score Account Risk','clean','48s','7h ago'],
['Chase Failed Charge','blocked — needs approval','1m 02s','11h ago'],['Draft Renewal Note','clean','1m 38s','1d ago']].map(([n,r,d,w],i)=>
<div key={i} className="row" style={{padding:'13px 20px',borderTop:i?'1px solid var(--line-soft)':'0',gap:12}}>
<span className="grow" style={{fontSize:13.2,fontWeight:500}}>{n}</span>
<span className={'pill '+(r==='clean'?'pill-ok':'pill-warn')}>{r}</span><span className="mono sub" style={{width:70,textAlign:'right'}}>{d}</span>
<span className="mono sub" style={{width:70,textAlign:'right'}}>{w}</span></div>)}</div>}
{tab==='prop'&&<div className="g3">{[['Reprice Underwater Account','Ridgeline and Selby both sit under 10% margin on pre-February rates.'],
['Warm Second Domain','Sender reputation dipped once. A warmed backup prevents the next one.']].map(([n,w],i)=>
<div key={i} className="card" style={{padding:'16px 18px',display:'flex',flexDirection:'column',gap:10}}>
<div className="row" style={{gap:8}}><span style={{fontWeight:600,fontSize:14}}>{n}</span><span className="pill pill-v">proposed</span></div>
<p style={{fontSize:12.9,color:'var(--ink-2)',lineHeight:1.55,flex:1}}>{w}</p>
<div className="row" style={{gap:8}}><button className="btn btn-s btn-p">Add to catalog</button><button className="btn btn-s">Dismiss</button></div></div>)}</div>}
{tab==='forge'&&<div className="card"><div className="hd"><div><h3>Forge a new skill</h3><div className="sub">Describe the recipe. Paige writes it, names its triggers, and tests it three times before it runs alone.</div></div></div>
...
const PaigeTeam=()=>{const[team,setTeam]=React.useState(PT.team);
return <div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr)',gap:16}}>
<div className="card" style={{padding:'16px 20px',background:'var(--surface-2)',fontSize:13,color:'var(--ink-2)',lineHeight:1.6}}>
<span style={{fontWeight:600,color:'var(--ink)'}}>One Paige, six departments. </span>She hands work between them the way a staffed team would, and every handoff is on the Action Bus. Set how far each department goes before it needs you.</div>
...
<div><div className="row" style={{justifyContent:'space-between',marginBottom:7}}><span className="eyebrow" style={{fontSize:10}}>Autonomy</span>
<span style={{fontSize:11.5,fontWeight:600,color:d.level>2?'var(--gold)':'var(--ink-3)'}}>{LVL[d.level-1]}</span></div>
<div className="row" style={{gap:4}}>{[1,2,3,4].map(l=><button key={l} title={LVL[l-1]} onClick={()=>setTeam(t=>t.map((x,j)=>j===i?{...x,level:l}:x))}
style={{height:6,flex:1,borderRadius:3,background:l<=d.level?(d.level>2?'var(--gold)':'var(--violet)'):'var(--surface-sunk)',transition:'.15s'}}/>)}</div></div>
