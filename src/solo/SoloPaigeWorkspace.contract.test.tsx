...
describe("Solo PAIGE workspace contract", () => {
  it("ships the approved taxonomy without a second Memory or Code workspace", () => {
    ...
    expect(workspace).not.toContain('label: "Capabilities"');
    expect(workspace).toContain("PaigeWorkingSessionCard");
  ...
  it("clears account-authored UI before the next account hydrates", () => {
    ...
    expect(chat).toContain("const scopeEpoch = `${activeTenantId ?? \"\"}|${clientId ?? \"\"}|${businessMissionId ?? \"\"}|${businessMissionAsk ?? \"\"}`;");
