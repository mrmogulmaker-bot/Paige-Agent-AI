...
function CapabilityGlyph({ item, large = false }: { item: MarketplaceItem; large?: boolean }) {
  ...
  const identity = item.itemType.includes("connector") ? "connector" : item.itemType.includes("workflow") ? "workflow" : "content";
  return <span className={`mk-glyph mk-glyph-${identity}${large ? " mk-glyph-large" : ""}`} aria-hidden="true"><Icon size={large ? 31 : 21} style={{}} /></span>;
}
...
function MarketplaceCard({ item, onOpen }: { item: MarketplaceItem; onOpen: (item: MarketplaceItem, trigger: HTMLButtonElement) => void }) {
  ...
  return <button className="mk-card mk-interactive card" onClick={(event) => onOpen(item, event.currentTarget)} aria-label={`Review ${item.name}`}>
...
function StatePanel({ state, refresh }: { state: "resolving" | "unavailable" | "error"; refresh: () => void }) {
  ...
  return <div className="mk-state card" role="alert"><TruthBadge state="UNAVAILABLE" /><h2>Marketplace catalogue unavailable</h2><p>The safe catalogue read did not complete. No fixture content has been substituted.</p><button className="btn btn-s mk-interactive" onClick={refresh}>Retry safe read</button></div>;
}
...
function MarketplaceDetail({ item, onClose, onOpenPaige }: {
  ...
  return <div className="mk-dialog-layer">
    ...
    <aside ref={dialogRef} className="mk-dialog" role="dialog" aria-modal="true" aria-labelledby="mk-detail-title">
      <header className="mk-dialog-hero"><CapabilityGlyph item={item} large /><div><TruthBadge state={displayCopy.proofState} /><h2 id="mk-detail-title">{item.name}</h2><p>{item.itemType} · {item.category}</p></div><button ref={closeRef} className="mk-close mk-interactive" onClick={onClose} aria-label="Close capability details"><Ic.x size={16} style={{}} /></button></header>
...
export const Marketplace = () => {
  ...
  return <div className="mk-workspace" role="region" aria-label="Marketplace">
    ...
    <div className="mk-tabs"><nav className="mk-tablist" aria-label="Marketplace views">{tabs.map(([key, label, Icon]) => <button className="mk-interactive" key={key} aria-current={tab === key ? "page" : undefined} onClick={() => setTab(key)}><Icon size={15} style={{}} />{label}</button>)}</nav><div className="mk-counts"><span><TruthBadge state={read.state === "ready" ? read.summary.installed.state : "UNAVAILABLE"} />{read.state === "ready" ? `${read.summary.installed.count} visible install observations` : "Install observations unavailable"}</span></div></div>
    <div className="pg-body mk-body" data-marketplace-scroll-owner="catalogue" tabIndex={0} role="region" aria-label="Marketplace catalogue browsing">
      {read.state !== "ready" ? <StatePanel state={read.state} refresh={read.refresh} /> : <>
        {tab === "today" && <div className="mk-flow"><section className="mk-catalogue-intro" data-marketplace-promo-slot="static" aria-label="Marketplace spotlight"><div className="mk-intro-copy"><span className="eyebrow">Marketplace spotlight</span><h1>Review capabilities with their authority in view</h1><p>Explore the caller-scoped catalogue while release, entitlement, and activation claims remain unavailable unless their governing contracts are proven.</p><button className="btn mk-interactive" onClick={() => setTab("browse")}>Browse catalogue records <Ic.arrow size={14} style={{}} /></button></div><div className="mk-lifecycle" aria-label="Marketplace authority lifecycle"><span><b>01</b>Creator submission</span><Ic.chev size={14} style={{}} /><span><b>02</b>Platform authorization</span><Ic.chev size={14} style={{}} /><span><b>03</b>Tenant discovery</span><Ic.chev size={14} style={{}} /><span><b>04</b>Governed entitlement</span></div></section><section><div className="mk-section-head"><div><span className="eyebrow">Catalogue read</span><h2>Records available to review</h2></div><span className="mk-source"><TruthBadge state="LIVE" />Read completed</span></div>{read.items.length ? <div className="mk-grid mk-catalogue-grid">{read.items.map((item) => <MarketplaceCard key={item.slug} item={item} onOpen={openDetail} />)}</div> : <EmptyState state="PARTIAL" title="0 catalogue records returned" copy="The current caller-scoped catalogue returned zero records. This is not proof of complete release or entitlement inventory." />}</section></div>}
        {tab === "browse" && <div className="mk-flow"><div className="mk-tools"><label><Ic.search size={15} style={{}} /><span className="sr-only">Search capabilities</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search catalogue capabilities" /></label><div className="mk-filters" aria-label="Capability categories">{categories.map((value) => <button className="mk-interactive" key={value} aria-pressed={category === value} onClick={() => setCategory(value)}>{value}</button>)}</div></div>{visibleItems.length ? <div className="mk-grid mk-catalogue-grid">{visibleItems.map((item) => <MarketplaceCard key={item.slug} item={item} onOpen={openDetail} />)}</div> : <EmptyState state="PARTIAL" title="No matching catalogue records" copy="The current caller-scoped catalogue has no matching records." />}</div>}
        {tab === "installed" && <div className="mk-flow">{installedItems.length ? <><div className="mk-boundary"><TruthBadge state="PARTIAL" /><p>These are visible legacy active-install observations. The catalogue RPC is not a complete entitlement read, and immutable release binding and activation health are unavailable.</p></div><div className="mk-grid mk-catalogue-grid">{installedItems.map((item) => <MarketplaceCard key={item.slug} item={item} onOpen={openDetail} />)}</div></> : <EmptyState state="PARTIAL" title="0 visible active-install observations" copy="No active install row was joined to the returned catalogue records. This is not proof of a complete entitlement inventory." />}</div>}
