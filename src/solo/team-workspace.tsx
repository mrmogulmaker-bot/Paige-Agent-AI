...
export function InviteDialog({ workspace, onClose, onInvited }: { workspace: TeamWorkspaceRecord; onClose: () => void; onInvited: () => void }) {
  ...
  const openedIn = useRef({ id: workspace.tenant_id, name: workspace.tenant_name });
  const send = async () => {
    const named = openedIn.current;
    if (workspace.tenant_id !== named.id) {
      toast.error(`This invitation was for ${named.name}, but you have since switched workspace. Nothing was sent. Reopen the invitation in the workspace you mean.`);
      return;
    }
    setSending(true);
    const { data, error } = await supabase.functions.invoke("solo-team-invitations", { body: { action: "create", expectedTenantId: named.id, email, permission, jobTitle: title, responsibilities } });
    setSending(false);
...
export function SoloTeamWorkspace({ openPaige }: { openPaige?: () => void } = {}) {
  ...
  const manageInvite = async (action: "resend" | "revoke", invite: TeamInviteRecord) => { if (!workspace) return; const { data, error } = await supabase.functions.invoke("solo-team-invitations", { body: { action, expectedTenantId: workspace.tenant_id, inviteId: invite.id } }); if (error || data?.ok === false) { toast.error(data?.error || error?.message || "Invitation update failed"); return; } if (action === "resend") data?.emailed ? toast.success("Fresh invitation sent; the old link was revoked.") : toast.warning("Fresh invitation created, but email delivery did not complete."); else toast.success("Invitation revoked."); team.refresh(); };
  return <div className="stw-workspace"><div className="stw-tabs" role="tablist" aria-label="Team settings"><button role="tab" aria-selected={view === "team"} onClick={() => setView("team")}>Team</button><button role="tab" aria-selected={view === "roles"} onClick={() => setView("roles")}>Roles &amp; access</button></div>
    ...
    {workspace && selected && <MemberEditor member={selected} workspace={workspace} onClose={() => setSelected(null)} onSaved={team.refresh}/>} {workspace && inviteOpen && <InviteDialog workspace={workspace} onClose={() => setInviteOpen(false)} onInvited={team.refresh}/>}</div>;
}
