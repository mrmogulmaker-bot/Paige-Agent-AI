...
function PipelineSurface({ data, setDetail }) {
  ...
  return <section className="campaigns-surface pipeline-surface"><SurfaceHead truthKey="pipeline" title="Deal workspace" description="Tenant-owned pipelines, custom stages, and contextual work records." action={<div className="pipeline-actions"><label><span className="sr-only">Pipeline</span><select value={selected?.id||""} onChange={(event)=>{setSelectedId(event.target.value);setFocusedStageId("");}}>{activePipelines.map((item)=><option key={item.id} value={item.id}>{item.name} · {item.shortRef}</option>)}</select></label><button className="btn btn-s" disabled={!workspace.canManage} onClick={(event)=>openConfig("config-create",event)}>New deal</button>{selected&&<button className="btn btn-s" onClick={(event)=>openConfig("config-edit",event)}>Manage</button>}</div>}/><StateFrame phase={data.phase} retry={data.retry} noun="pipeline workspace">
