...
function PipelineConfigWorkspace({ mode, pipeline, stages, canManage, run, onBack, newPipeline, setNewPipeline }) {
  ...
  const create=async()=>{const result=await save({type:"create-pipeline",...newPipeline});if(result?.ok){setNewPipeline({name:"",description:""});onBack();}};
  const askPaige=()=>window.dispatchEvent(new CustomEvent("paige:open",{detail:{prompt:`Use pipeline.configure to propose a blank, tenant-owned pipeline for this workflow. Keep every field editable and do not activate anything until I approve. Workflow: ${newPipeline.description||newPipeline.name||"I will describe it in chat."}`}}));
  return <section className="pipeline-config-workspace" aria-labelledby="pipeline-config-title" aria-busy={pending}>
    ...
    {mode==="create"?<div className="pipeline-create-fields"><label><span>Pipeline name</span><input autoFocus disabled={!canManage||pending} value={newPipeline.name} onChange={(event)=>setNewPipeline({...newPipeline,name:event.target.value})}/></label><label><span>Purpose or workflow</span><textarea disabled={!canManage||pending} value={newPipeline.description} onChange={(event)=>setNewPipeline({...newPipeline,description:event.target.value})}/></label><div><button className="btn btn-s" disabled={!canManage||pending} onClick={askPaige}>Ask PAIGE</button><button className="btn btn-s btn-p" disabled={!canManage||pending||!newPipeline.name.trim()} onClick={create}>{pending?"Creating…":"Create blank pipeline"}</button></div></div>:pipeline&&<>
      ...
      <div className="pipeline-lifecycle"><span>Status: <strong>{pipeline.lifecycleStatus}</strong></span><button className="btn btn-s" disabled={!canManage||pending||pipeline.lifecycleStatus==="active"} onClick={()=>save({type:"activate-pipeline",pipelineId:pipeline.id,expectedVersion:pipeline.version})}>Activate</button><button className="btn btn-s" disabled={!canManage||pending||pipeline.lifecycleStatus==="archived"} onClick={()=>save({type:"archive-pipeline",pipelineId:pipeline.id,expectedVersion:pipeline.version})}>Archive pipeline</button><button className="btn btn-s" disabled={!canManage||pending||pipeline.lifecycleStatus!=="archived"} onClick={()=>save({type:"restore-pipeline",pipelineId:pipeline.id,expectedVersion:pipeline.version})}>Restore pipeline</button><button className="btn btn-s pipeline-danger" disabled={!canManage||pending} onClick={()=>save({type:"delete-pipeline",pipelineId:pipeline.id,expectedVersion:pipeline.version})}>Delete pipeline</button></div>
...
function PipelineSurface({ data, setDetail }) {
  ...
  const [newPipeline,setNewPipeline]=React.useState({name:"",description:""});
  ...
  const selected=workspace.pipelines.find((item)=>item.id===selectedId)??activePipelines[0]??workspace.pipelines[0];
  ...
  React.useEffect(()=>{setView("board");setMoving(null);setMovePicker(null);setKeyboardMove(null);setNotice("");setNewPipeline({name:"",description:""});},[data.tenantId]);
  React.useEffect(()=>{if(selected&&selected.id!==selectedId){setSelectedId(selected.id);setFocusedStageId("");}},[selected,selectedId]);
  ...
  const openConfig=(mode,event)=>{openerRef.current=event.currentTarget;setRestoreLabel(event.currentTarget.textContent||"");setView(mode);setNotice("");};
  ...
  if(view!=="board")return <section className="campaigns-surface pipeline-surface"><PipelineConfigWorkspace mode={view==="config-create"?"create":"edit"} pipeline={selected} stages={stages} canManage={workspace.canManage} run={data.pipelineAction} onBack={back} newPipeline={newPipeline} setNewPipeline={setNewPipeline}/></section>;
  return <section className="campaigns-surface pipeline-surface"><SurfaceHead truthKey="pipeline" title="Deal workspace" description="Tenant-owned pipelines, custom stages, and contextual work records." action={<div className="pipeline-actions"><label><span className="sr-only">Pipeline</span><select value={selected?.id||""} onChange={(event)=>{setSelectedId(event.target.value);setFocusedStageId("");}}>{activePipelines.map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><button className="btn btn-s" disabled={!workspace.canManage} onClick={(event)=>openConfig("config-create",event)}>New deal</button>{selected&&<button className="btn btn-s" onClick={(event)=>openConfig("config-edit",event)}>Manage</button>}</div>}/><StateFrame phase={data.phase} retry={data.retry} noun="pipeline workspace">
