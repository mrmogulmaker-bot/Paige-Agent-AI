...
function PipelineFolderOrganizer({ workspace, run, onClose }) {
  ...
  return <><div className="pipeline-folder-scrim" onClick={()=>!pending&&onClose()}/><section className="pipeline-folder-organizer" role="dialog" aria-modal="true" aria-labelledby="pipeline-folder-title" aria-busy={pending}>
    ...
    {archiveFolder&&<section className="pipeline-folder-archive"><h3>Archive {archiveFolder.name}?</h3><p>Its {archiveFolder.pipelineCount} pipeline{archiveFolder.pipelineCount===1?"":"s"} will stay active and move to Unfiled. No pipeline, deal, or history is deleted.</p><label><span>Enter {archiveFolder.name} to confirm</span><input value={confirmedName} onChange={(event)=>setConfirmedName(event.target.value)}/></label><div><button className="btn btn-s" onClick={()=>setArchiveFolder(null)}>Cancel</button><button className="btn btn-s pipeline-danger" disabled={!workspace.canArchiveFolders||pending||confirmedName!==archiveFolder.name} onClick={async()=>{const result=await save({type:"archive-folder",folderId:archiveFolder.id,confirmedName,expectedVersion:archiveFolder.version});if(result?.ok)setArchiveFolder(null);}}>Archive exact folder</button></div></section>}
    {message&&<p className="pipeline-save-message" role={message.toLowerCase().includes("could not")?"alert":"status"}>{message}</p>}
  </section></>;
}
