...
const migrationPath = resolve(process.cwd(), "supabase/migrations/20260902004019_paige_spine_foundation.sql");
...
function spineSqlFindings(sql: string): string[] {
  const required = [
    ...
    "security invoker",
    "e.tenant_id = public.current_user_tenant_id()",
...
describe("Pipeline Spine SQL evidence boundary", () => {
  it("is authenticated, tenant-pinned, allowlisted, retained, and fail-closed", () => {
    expect(spineSqlFindings(migration)).toEqual([]);
    expect(migration).toContain("if auth.uid() is null then return; end if;");
  ...
  it("returns a fixed safe envelope and never raw Rail content", () => {
    ...
    const returnLines = returnsBlock.toLowerCase().split("\n").map((line) => line.trim().replace(/,$/, ""));
    for (const forbidden of ["title text", "summary text", "payload jsonb", "actor_user_id uuid", "ref_id uuid"]) {
      expect(returnLines).not.toContain(forbidden);
    }
  ...
  it("mutation proof detects removal of tenant and allowlist guards", () => {
    const withoutTenantPin = migration.replace("e.tenant_id = public.current_user_tenant_id()", "true");
    expect(spineSqlFindings(withoutTenantPin)).toContain("e.tenant_id = public.current_user_tenant_id()");
    const withoutAllowlist = migration.replace("e.payload->>'policy_result' = 'allowed'", "true");
    expect(spineSqlFindings(withoutAllowlist)).toContain("e.payload->>'policy_result' = 'allowed'");
  });
});
