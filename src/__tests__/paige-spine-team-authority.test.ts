...
const OWNER_ROWS = [
  { fact_key: "viewer_permission", value: "owner", status: "available", source: "team", reason: null, tenant_id: TENANT },
  { fact_key: "viewer_is_legal_owner", value: "true", status: "available", source: "team", reason: null, tenant_id: TENANT },
];
...
const SPLIT_ROWS = [
  { fact_key: "viewer_permission", value: "member", status: "available", source: "team", reason: null, tenant_id: TENANT },
  { fact_key: "viewer_is_legal_owner", value: "false", status: "available", source: "team", reason: null, tenant_id: TENANT },
];

const REFUSED_ROWS = [
  { fact_key: "viewer_permission", value: null, status: "unavailable", source: null, reason: "not permitted for this account", tenant_id: TENANT },
  { fact_key: "viewer_is_legal_owner", value: null, status: "unavailable", source: null, reason: "not permitted for this account", tenant_id: TENANT },
];

describe("team.authority chat evidence", () => {
  it("renders the two facts separately, each in its own line", async () => {
    const evidence = await loadTeamAuthorityForChat(fakeClient(OWNER_ROWS), TENANT);
  ...
  it("keeps a member seat and a false ownership distinct — the collapse this capability exists to prevent", async () => {
    const block = renderTeamAuthorityForChat(await loadTeamAuthorityForChat(fakeClient(SPLIT_ROWS), TENANT));
  ...
  it("renders NOTHING when every row is refused, so a seatless caller learns nothing", async () => {
    const evidence = await loadTeamAuthorityForChat(fakeClient(REFUSED_ROWS), TENANT);
  ...
  it("distinguishes a refusal from a read that failed", async () => {
    ...
    const evidence = await loadTeamAuthorityForChat(fakeClient(rows), TENANT);
    expect(evidence.status).toBe("available");
  ...
  it("degrades honestly on an rpc error, an empty result, and a throw", async () => {
    for (const client of [
      ...
      const block = renderTeamAuthorityForChat(await loadTeamAuthorityForChat(client, TENANT));
  ...
  it("renders NOTHING when the rows name a workspace other than the conversation's", async () => {
    ...
    // conversation scoped to B while this read resolves A. Asserting A's ownership inside B's
    // conversation is a cross-workspace claim, so the whole block is suppressed.
    const foreign = OWNER_ROWS.map((row) => ({ ...row, tenant_id: OTHER_TENANT }));
    const evidence = await loadTeamAuthorityForChat(fakeClient(foreign), TENANT);
  ...
  it("renders NOTHING when the conversation has no workspace to bind against", async () => {
    const evidence = await loadTeamAuthorityForChat(fakeClient(OWNER_ROWS), null);
    expect(evidence.status).toBe("wrong_workspace");
    expect(renderTeamAuthorityForChat(evidence)).toBe("");
  });

  it("suppresses even a PARTIAL workspace mismatch — one foreign row poisons the block", async () => {
    const mixed = [OWNER_ROWS[0], { ...OWNER_ROWS[1], tenant_id: OTHER_TENANT }];
    expect((await loadTeamAuthorityForChat(fakeClient(mixed), TENANT)).status).toBe("wrong_workspace");
  });

  it("builds the block end to end", async () => {
    expect(await buildTeamAuthorityBlock(fakeClient(OWNER_ROWS), TENANT)).toContain("Seat role");
  });
});
